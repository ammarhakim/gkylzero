...
(require "../prover_vector.rkt")
 ...
 (prove-roe-vector2-1d-flux-conservation '#hash((cons-exprs . (Ex phi)) (flux-exprs . ((* e_fact (* (* c c) phi)) (* e_fact Ex))) (max-speed-exprs . ((abs (* c e_fact)) (abs (* c e_fact)))) (name . "maxwell-1d-Ex-phi") (parameters . ((define c 1.0) (define e_fact 1.0) (define b_fact 1.0)))) #:cfl 0.95 #:init-funcs '(0.0 (cond ((< x 0.0) 0.5) (else -0.5))) #:nx 200 #:t-final 1.0 #:x0 -1.5 #:x1 1.5)
