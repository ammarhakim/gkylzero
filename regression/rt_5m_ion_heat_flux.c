...
create_ctx(void)
{
  ...
  double vti = sqrt(T_low/ionMass);
  
  // Box size is chosen to be multiple mean-free-paths long
  double tau = 6.0*sqrt(2.0*M_PI*ionMass*T_low*M_PI*T_low*M_PI*T_low)*epsilon0*epsilon0/(charge*charge*charge*charge*n);
  double lambda = vti*tau;
...
evalElcInit(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double ere = app->n*app->T_high/(app->gasGamma-1);
...
void
evalIonBcLower(double t, int nc, const double *skin, double * GKYL_RESTRICT ghost, void *ctx)
{
  struct heat_flux_ctx *app = ctx;  
  double T = app->T_high;
...
void
evalIonBcUpper(double t, int nc, const double *skin, double * GKYL_RESTRICT ghost, void *ctx)
{
  struct heat_flux_ctx *app = ctx;  
  double T = app->T_low;
