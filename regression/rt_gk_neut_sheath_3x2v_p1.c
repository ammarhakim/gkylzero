...
//    },
  ...
  strcpy(gk->name, "gk_sheath_neut_3x2v_p1");
  gk->cfl_frac = ctx.cfl_frac;

  gk->cdim = 3;
  gk->vdim = 2;
  gk->lower[0] = ctx.R - (0.5 * ctx.Lx);
  gk->upper[0] = ctx.R + (0.5 * ctx.Lx);
  gk->lower[1] = -0.5 * ctx.Ly;
  gk->upper[1] =  0.5 * ctx.Ly; 
  gk->lower[2] = -0.5 * ctx.Lz;
  gk->upper[2] =  0.5 * ctx.Lz;
  gk->cells[0] = NX;
  gk->cells[1] = NY;
  gk->cells[2] = NZ;
