// Wald magnetic equilibrium test in a static (Schwarzschild) black hole geometry, for the general relativistic Maxwell equations.
// Input parameters describe a uniform magnetic field being distorted by a non-rotating black hole.
...
struct maxwell_wald_static_ctx
{
...
create_ctx(void)
{
  ...
  struct maxwell_wald_static_ctx ctx = {
...
evalGRMaxwellInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  struct maxwell_wald_static_ctx *app = ctx;
  ...
  // Set magnetic field.
  fout[3] = 0.0; fout[4] = 0.0; fout[5] = E0;
...
main(int argc, char **argv)
{
  ...
  struct maxwell_wald_static_ctx ctx = create_ctx(); // Context for initialization functions.
