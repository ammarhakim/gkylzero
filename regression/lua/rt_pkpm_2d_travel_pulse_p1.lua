...
B0 = 1.0 / math.sqrt(2.0) -- Reference magnetic field strength.
...
Ly = 2.0 -- Domain size (configuration space: y-direction).
...
train_nn_file = "rt_pkpm_2d_travel_pulse_p1_moms_nn_1" -- File path of neural network to train.
...
test_nn_file = "rt_pkpm_2d_travel_pulse_p1_moms_nn_1" -- File path of neural network to test.
...
pkpmApp = PKPM.App.new {
  ...
  neut = PKPM.Species.new {
    ...
    initDist = function (t, xn)
      local x, y, vx = xn[1], xn[2], xn[3]

      local n_perturb = n0 + (alpha * math.sin(pi * x) * math.sin(pi * y))
    ...
    initFluid = function (t, xn)
      ...
      local mom_y = n_perturb * u0 -- Total momentum density (y-direction).
