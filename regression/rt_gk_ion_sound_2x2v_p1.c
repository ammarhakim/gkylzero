...
struct gk_app_ctx {
  ...
  double wave_amp; // Wave amplitude.
  double wavek; // Wave number of ion acoustic wave.
  double kperp; // Perpendicular wave number.
  ...
  int Nx, Nz, Nvpar, Nmu; // Number of cells in x, z, vpar, mu.
...
void eval_density_ion(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double n0 = app->n0;
  double wave_amp = app->wave_amp;
  double wavek = app->wavek;
  double kperp = app->kperp;

  double perturb = wave_amp*cos(wavek*z)*cos(kperp*x);
...
//  double kperp = app->kperp;
//
//  double perturb = wave_amp*cos(kperp*x);
//
//  fout[0] = n0*(1.+perturb);
}
...
main(int argc, char **argv)
{
  ...
  struct gkyl_gk app_inp = {
    .name = "gk_ion_sound_2x2v_p1",

    .cdim = ctx.cdim, .vdim = ctx.vdim,
    .lower = { -ctx.Lx/2.0, -ctx.Lz/2.0 },
    .upper = {  ctx.Lx/2.0,  ctx.Lz/2.0 },
