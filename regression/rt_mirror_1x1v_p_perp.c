...
evalDistFuncElc(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double magB = sqrt(sq(-16*M_PI*cu(x)*0.5*sin(M_PI*sq(x)*sq(x))) + sq(1 - 0.5*cos(M_PI*sq(x)*sq(x))));
  fout[0] = fv/magB;
}
...
evalMagB(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct mirror_ctx *app = ctx;
  double x = xn[0];
  fout[0] = sqrt(sq(-16*M_PI*cu(x)*0.5*sin(M_PI*sq(x)*sq(x))) + sq(1 - 0.5*cos(M_PI*sq(x)*sq(x))));
}
...
evalGradB(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  struct mirror_ctx *app = ctx;
  double x = xn[0];
  fout[0] = 2.0*cu(x)*sin(M_PI*sq(x)*sq(x))*(2.0*M_PI + (-M_PI + 256.0*cu(M_PI*x)*cu(x))*cos(M_PI*sq(x)*sq(x)) 
    + 192.0*sq(x*M_PI)*sin(M_PI*sq(x)*sq(x)))/sqrt(sq(-2.0 + cos(M_PI*sq(x)*sq(x))) + 256.0*cu(x)*cu(x)*sq(M_PI*sin(M_PI*sq(x)*sq(x))));
}
