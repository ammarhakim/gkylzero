...
struct annulus_sodshock_ctx
{
  ...
  double z_min; // Domain size (configuration space: z-direction, min value).
  double z_max; // Domain size (configuration space: z-direction, max value).
...
create_ctx(void)
{
  ...
  double z_min = -1.0; // Domain size (configuration space: z-direction, min value).
  double z_max = 1.0; // Domain size (configuration space: z-direction, max value).
...
h_ij_inv(double t, const double* xn, double* fout, void* ctx)
{
  ...
  double q_r = xn[0], q_theta = xn[1], q_z = xn[2];
...
hamil(double t, const double* xn, double* fout, void* ctx)
{
  ...
  double q_R = xn[0], q_theta = xn[1], q_z = xn[2], p_R_dot = xn[3], p_theta_dot = xn[4], p_z_dot = xn[5];
...
evalDensityInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  //printf("r: %e, theta: %e, z: %e, rho: %e, det_h_val: %e \n", q_r, xn[1], xn[2], rho, det_h_val);
...
main(int argc, char **argv)
{
  ...
  // VM app
  struct gkyl_vm vm = {
    .name = "can_pb_bgk_surf_cylindrical_sodshock_3x3v_p1",

    .cdim = 3, .vdim = 3,
    .lower = { ctx.rmin, ctx.theta_min, ctx.z_min },
    .upper = { ctx.rmax, ctx.theta_max, ctx.z_max },
    .cells = { NR, NTHETA, NZ },
