// 2D Rayliegh-Taylor instability problem
...
create_ctx(void)
{
  ...
  double gravity = 0.1; // acceleration due to gravity
  ...
  struct euler_rtaylor_2d_ctx ctx = {
    .gas_gamma = gas_gamma,
    .gravity = gravity,
    .Nx = Nx,
    .Ny = Ny,
...
evalEulerInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  struct euler_rtaylor_2d_ctx *app = ctx;
  ...
  fout[4] = (pr+pr0)/(gas_gamma-1);
}

void
applied_acceleration(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  struct euler_rtaylor_2d_ctx *app = ctx;
  double gravity = app->gravity;
  fout[0] = 0.0;
...
main(int argc, char **argv)
{
  ...
  struct euler_rtaylor_2d_ctx ctx = create_ctx(); // Context for initialization functions.
