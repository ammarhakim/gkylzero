...
evalDistFuncElc(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double Lx = app->Lx;
  double n0 = app->n0;
  double vtElcH = app->vtElcH;
  double vtElcC = app->vtElcC;

  double v2H = vx*vx;
  double v2C = (vx + 1.0*vtElcC)*(vx + 1.0*vtElcC);

  double fv = 0.0;
  if (vx >= 0.0)
    fv += n0/(sqrt(M_PI))*(exp(-v2H/(vtElcH*vtElcH))/(vtElcH));
  else 
    fv += n0/(sqrt(M_PI))*(exp(-v2C/(vtElcC*vtElcC))/(vtElcC*(1.0 + erf(1.0))));
    
  fout[0] = fv;
  fout[1] = 0.5*vtElcH*vtElcH*fv;
}
...
create_ctx(void)
{
  ...
  double beta = 64.0;
  double elcTemp = 0.02;
  double tempFac = 1.0/4.0; // Temperature is 4 times colder at right wall
  double vtElcH = sqrt(2.0*elcTemp/massElc);
  double vtElcC = sqrt(2.0*elcTemp*tempFac/massElc);
  double vAe = vtElcH/sqrt(beta);
  double B0 = vAe*sqrt(mu0*n0*massElc);

  // ion cyclotron frequency and gyroradius
  double omegaCe = fabs(chargeElc)*B0/massElc;
  double rhoe = vtElcH/omegaCe;
...
main(int argc, char **argv)
{
  ...
  struct gkyl_vlasov_species elc = {
    ...
    .lower = { -4.0 * ctx.vtElcH},
    .upper = { 4.0 * ctx.vtElcH}, 
    .cells = { VX },
  ...
  // VM app
  struct gkyl_vm vm = {
    .name = "pkpm_high_beta_heat_flux_diff_ics_p1",
