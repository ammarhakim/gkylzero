...
struct sr_bimaxwellian_ctx
{
  ...
  double Tperp; // Perpendicular temperature.
  ...
  double vpar_max; // Domain boundary (velocity space: vpar (vx)-direction).
  double vperp_max; // Domain boundary (velocity space: vperp (vy and vz)-direction).
...
create_ctx(void)
{
  ...
  double n = 1.0; // Number density.
  double Tpar = 1.0; // Parallel temperature.
  double Tperp = 0.01; // Perpendicular temperature.
  ...
  double vpar_max = 10.0; // Domain boundary (velocity space: vpar (vx)-direction).
  double vperp_max = 4.0; // Domain boundary (velocity space: vperp (vy and vz)-direction).
  ...
  struct sr_bimaxwellian_ctx ctx = {
    .mass = mass,
    .charge = charge,
    .n = n,
    .Tpar = Tpar,
...
void
evalTempParInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  struct sr_bimaxwellian_ctx *app = ctx;
  double x = xn[0];

  double Tpar = app->Tpar;
...
void
evalTempPerpInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  struct sr_bimaxwellian_ctx *app = ctx;
  double x = xn[0];

  double Tperp = app->Tperp;
...
static void
mapc2p_vel_par(double t, const double *vc, double* GKYL_RESTRICT vp, void *ctx)
{
  struct sr_bimaxwellian_ctx *app = ctx;
...
static void
mapc2p_vel_perp(double t, const double *vc, double* GKYL_RESTRICT vp, void *ctx)
{
  struct sr_bimaxwellian_ctx *app = ctx;
...
main(int argc, char **argv)
{
  ...
  struct sr_bimaxwellian_ctx ctx = create_ctx(); // Context for initialization functions.
  ...
  struct gkyl_vlasov_species neut = {
    ...
    .mapc2p_vel[0] = {
      .mapc2p_vel_func = mapc2p_vel_par, 
      .mapc2p_vel_ctx = &ctx, 
    },
    .mapc2p_vel[1] = {
      .mapc2p_vel_func = mapc2p_vel_perp, 
      .mapc2p_vel_ctx = &ctx, 
    },
    .mapc2p_vel[2] = {
      .mapc2p_vel_func = mapc2p_vel_perp, 
  ...
  struct gkyl_vm app_inp = {
    .name = "vlasov_sr_nonuniformv_bgk_bimaxwellian_relax_1x3v_p2",
