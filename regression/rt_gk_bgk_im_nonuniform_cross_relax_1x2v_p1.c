...
create_ctx(void)
{
  ...
  double n0 = 7.0e20; //  Reference number density (1 / m^3).
  ...
  double mu_max_elc = mass_elc*(5.0*vte)*(5.0*vte)/(2.0*B0);

  double vpar_max_ion = 5.0*vti;
  double mu_max_ion = mass_ion*(5.0*vti)*(5.0*vti)/(2.0*B0);
...
main(int argc, char **argv)
{
  ...
  struct gkyl_gk app_inp = {
    .name = "gk_bgk_im_nonuniform_cross_relax_1x2v_p1",
