...
double
psi_RZ(double R_in, double Z_in, void *ctx)
{
  struct gk_mirror_ctx *app = ctx;
  double mirror_ratio = app->mirror_ratio;
  double psi = R_in * (pow(sin(Z_in), 2) * (mirror_ratio - 1) + 1) / mirror_ratio;
  return psi;
}

double
R_psiZ(double psi_in, double Z_in, void *ctx)
{
  struct gk_mirror_ctx *app = ctx;
  double mirror_ratio = app->mirror_ratio;
  double R_out = psi_in / ((pow(sin(Z_in), 2) * (mirror_ratio - 1) + 1) / mirror_ratio);
  return R_out;
}

void
Bfield_psiZ(double psi_in, double Z_in, void *ctx, double *BRad, double *BZ, double *Bmag)
{
  struct gk_mirror_ctx *app = ctx;
  double mirror_ratio = app->mirror_ratio;
  double R_in = R_psiZ(psi_in, Z_in, ctx);
  *BRad = R_in * (1/mirror_ratio - 1) * sin(2 * Z_in);
  *BZ = ((mirror_ratio - 1) * pow(sin(Z_in), 2) + 1) / mirror_ratio;
...
double
z_psiZ(double psi_in, double Z_in, void *ctx)
{
  struct gk_mirror_ctx *app = ctx;
  app->psi_in = psi_in;
  double eps = 0.0;
  ...
  if (eps <= Z_in)
  {
    integral = gkyl_dbl_exp(integrand_z_psiZ, ctx, eps, Z_in, 7, 1e-14);
  }
  else
  {
    integral = gkyl_dbl_exp(integrand_z_psiZ, ctx, Z_in, eps, 7, 1e-14); 
...
calculate_mirror_throat_location(void *ctx)
{
  ...
  double interval_right = z_psiZ(psi, app->Z_max, ctx);
  // printf("interval_right: %g\n", interval_right);
  ...
  // printf("Maximum Bmag: %g\n", maximum_Bmag);
  // printf("Maximum Bmag location: %g\n", maximum_Bmag_location);
  // printf("Z at maximum Bmag: %g\n", Z_psiz(psi, maximum_Bmag_location, ctx));
...
{
  struct gk_mirror_ctx *app = ctx;
  double z_min = z_psiZ(psi, app->Z_min, ctx);
  double z_max = z_psiZ(psi, app->Z_max, ctx);
  app->z_min = z_min;
  app->z_max = z_max;
  app->psi_eval = psi;
...
calculate_optimal_mapping(void *ctx)
{
  ...
  while (1)
  {
    ...
    printf("Order %i: maximum dBdCell %g, improvement %g\n", expander_order, max_dB_dCell, improvement);
...
create_ctx(void)
{
  ...
  printf("RatZeq0: %g\n", RatZeq0);
  ctx.psi_eval = psi_RZ(RatZeq0, 0.0, &ctx);
  printf("psi_eval: %g\n", ctx.psi_eval);
  ctx.z_min = z_psiZ(ctx.psi_eval, ctx.Z_min, &ctx);
  ctx.z_max = z_psiZ(ctx.psi_eval, ctx.Z_max, &ctx);
  printf("z_min: %g\n", ctx.z_min);
  printf("z_max: %g\n", ctx.z_max);
  ctx.psi_min = psi_RZ(0.0, 0.0, &ctx);
  ctx.psi_max = psi_RZ(R_max, 0.0, &ctx);
  printf("psi_eval: %g\n", ctx.psi_eval);
  calculate_mirror_throat_location(&ctx);
  printf("psi_eval: %g\n", ctx.psi_eval);
  // Printing
...
int main(int argc, char **argv)
{
  ...
  struct gkyl_gk gk = {  // GK app
    .name = "gk_multimirror_adiabatic_elc_1x2v_p1_nonuniform",
    .cdim = 1,
    .vdim = 2,
    .lower = {ctx.z_min},
    .upper = {ctx.z_max},
