...
struct amr_euler_riem_2d_ctx
{
  ...
  long Nx; // Coarse cell count (x-direction).
  long Ny; // Coarse cell count (y-direction).
  long ref_factor; // Refinement factor.
...
create_ctx(void)
{
  ...
  // Simulation parameters.
  long Nx = 64; // Coarse cell count (x-direction).
  long Ny = 64; // Coarse cell count (y-direction).
  long ref_factor = 4; // Refinement factor.
...
int main(int argc, char **argv)
{
  ...
  struct euler2d_level1_init init = {
    .baseNx = ctx.Nx,
    .baseNy = ctx.Ny,
  ...
  euler2d_run_level1(argc, argv, &init);
}