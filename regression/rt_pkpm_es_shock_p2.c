...
create_ctx(void)
{
  ...
  const char* train_nn_file = "pkpm_es_shock_p2_moms_nn_1.dat"; // File path of neural network to train.
  ...
  const char* test_nn_file = "pkpm_es_shock_p2_moms_nn_1.dat"; // File path of neural network to test.
...
main(int argc, char **argv)
{
  ...
  kann_t *ann_test;
  if (ctx.test_nn) {
    ann_test = kann_load(ctx.test_nn_file);
