...
struct pulsar_atm_ctx 
{
  ...
  double uPos; //drift velocity of electrons
  double nPos; //density of electrons in the magnetosphere
...
create_ctx(void)
{
  ...
  // Simulation parameters.
  int Nx = 576; // Cell count (configuration space: x-direction).
  int Nvx = 4096; // Cell count (velocity space: vx-direction).
  double Lx = 100.0; // Domain size (configuration space: x-direction).
  double vx_max = 4096.0; // Domain boundary (velocity space: vx-direction).  
  double nonuniform_v_pow = 2.0; // How nonuniform is velocity space: grid points are spaced as v^n power
  int poly_order = 2; // Polynomial order.
  double cfl_frac = 1.0; // CFL coefficient.

  double injRate = 0.5;
  double t_end = 400.0; // Final simulation time.
  int num_frames = 160; // Number of output frames.
...
void
evalDensityInitPos(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
...
evalDensityInitElc(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  uP = nP0*uP0/(100.0*exp(-grav*x)+nP0);
  betaE = (J+nP0*uP0)/((1+0.8*x/L)+(100.0*exp(-grav*x)+nP0)*sqrt(1+uP*uP));

  // Set density (no perturbation).
  fout[0] = (J+nP0*uP0)*sqrt(1-betaE*betaE)/betaE;
}
...
void
evalVDriftInitPos(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  struct pulsar_atm_ctx *app = ctx;
  double x = xn[0];
  double grav = app->grav;
  ...
  fout[0] = nP*uP/(100.0*exp(-grav*x)+nP);
  // Set drift (four-) velocity.  
}

void
evalVDriftInitElc(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  struct pulsar_atm_ctx *app = ctx;
  double x = xn[0];
  double grav = app->grav;
  ...
  uP = nP0*uP0/(100.0*exp(-grav*x)+nP0);
  betaE = (J+nP0*uP0)/((1+0.8*x/L)+(100.0*exp(-grav*x)+nP0)*sqrt(1+uP*uP));
  fout[0] = betaE/sqrt(1-betaE*betaE);
}
...
mapc2p_vel(double t, const double *vc, double* GKYL_RESTRICT vp, void *ctx)
{
  ...
  double ncells = app->Nvx/2.0;
  double v1, v2;
  double pmin, pint, pmax, p0, vZero;
  ...
  if (vc[0] < 0.0) {
    v1 = pmin*(-1*vc[0])*ncells+vx_max*pow(vc[0],nonuniform_v_pow);
    vp[0] = -v1;
  }else{
    v1 = pmin*vc[0]*ncells+vx_max*pow(vc[0],nonuniform_v_pow);
    vp[0] = v1;
  }
...
main(int argc, char **argv)
{
  ...
  struct gkyl_vlasov_species pos = {
    ...
    .projection[0] = {
      .proj_id = GKYL_PROJ_VLASOV_LTE,
      .density = evalDensityInitPos,
      .ctx_density = &ctx,
      .temp = evalTempInit,
      .ctx_temp = &ctx,
      .V_drift = evalVDriftInitPos,
      .ctx_V_drift = &ctx,
      .correct_all_moms = true,
    },
