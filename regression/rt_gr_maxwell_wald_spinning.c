// Wald magnetic equilibrium test in a spinning (Kerr) black hole geometry, for the general relativistic Maxwell equations.
// Input parameters describe a uniform magnetic field being distorted by a spinning black hole.
...
struct maxwell_wald_spinning_ctx
{
...
create_ctx(void)
{
  ...
  struct maxwell_wald_spinning_ctx ctx = {
...
evalGRMaxwellInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  struct maxwell_wald_spinning_ctx *app = ctx;
...
main(int argc, char **argv)
{
  ...
  struct maxwell_wald_spinning_ctx ctx = create_ctx(); // Context for initialization functions.
