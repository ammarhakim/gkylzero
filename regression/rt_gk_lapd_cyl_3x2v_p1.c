...
maxwellian(double n, double vpar, double upar, double mu, double bmag, double temp, double mass)
{
  double v2 = (vpar - upar)*(vpar - upar) + 2.0*mu*bmag/mass;
  return n/((2*M_PI*temp/mass)*sqrt(2*M_PI*temp/mass))*exp(-v2/(2.0*temp/mass));
}
...
evalDistFuncElc(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  // Random number generator with non-deterministic seed for completely random ICs
  double perturb = 2e-3*(1.0 - 0.5*gkyl_pcg32_rand_double(&rng));
...
evalDistFuncIon(double t, const double * GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void *ctx)
{
  ...
  double fv = maxwellian(n, vpar, 0.0, mu, bmag, T, massIon);
  fout[0] = fv;
}
...
main(int argc, char **argv)
{
  ...
  gkyl_gyrokinetic_app_write_integrated_L2_f(app);
