...
evalElcInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  double dist = (1.0 / sqrt(2.0 * pi * vte * vte)) * (exp(-(v * v) / (2.0 * vte * vte)));
...
void
evalSourceElcInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  struct sheath_ctx *app = ctx;
  ...
  double n_src = (1.0 / sqrt(2.0 * pi * vte * vte)) * (exp(-(v * v) / (2.0 * vte * vte)));

  double n = 0.0;

  if (fabs(x) < 100.0) {
    n = ((100.0 - fabs(x)) / 100.0) * n_src;
  }
  else {
    n = 0.0;
  }

  // Set electron source number density.
  fout[0] = n;
}
...
evalIonInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  ...
  double dist = (1.0 / sqrt(2.0 * pi * vti * vti)) * (exp(-(v * v) / (2.0 * vti * vti)));
...
void
evalSourceIonInit(double t, const double* GKYL_RESTRICT xn, double* GKYL_RESTRICT fout, void* ctx)
{
  struct sheath_ctx *app = ctx;
  double x = xn[0], v = xn[1];
  ...
  double n_src = (1.0 / sqrt(2.0 * pi * vti * vti)) * (exp(-(v * v) / (2.0 * vti * vti)));
