...
create_ctx(void)
{
  ...
  double beta = 1.0;
  // B0 has 1/sqrt(1 + guide^2) factor because B is subdivided between
  // guide field and in-plane field
  double guide = 1.0;
  double B0 = vAe*sqrt(mu0*n0*massElc)/sqrt(1.0 + guide*guide);
  double tot_B = sqrt(1.0 + guide*guide)*B0;

  double T_e = beta*tot_B*tot_B/(2.0*mu0*n0);
  double T_i = T_e;
  ...
  double Lx = 40.0*M_PI*rhoi;
  double Ly = 20.0*M_PI*rhoi;
...
main(int argc, char **argv)
{
  ...
  struct gkyl_vlasov_species elc = {
    ...
    .lower = { -24.0 * ctx.vtElc},
    .upper = { 24.0 * ctx.vtElc}, 
    .cells = { VX },
