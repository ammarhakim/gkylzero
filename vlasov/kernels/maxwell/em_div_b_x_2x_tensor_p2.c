...
{ 
  ...
  const double *b_surf_lr = &bvar_surf_l[3]; 
  const double *b_surf_cl = &bvar_surf_c[0]; 
  const double *b_surf_cr = &bvar_surf_c[3]; 
  const double *b_surf_rl = &bvar_surf_r[0]; 

  double *max_b_l = &max_b[0]; 
  double *max_b_r = &max_b[3]; 
  ...
  div_b[4] += ((-3.872983346207417*b_c[1])+0.7905694150420947*b_surf_rl[0]-0.7905694150420947*b_surf_lr[0]+0.7905694150420947*b_surf_cr[0]-0.7905694150420947*b_surf_cl[0])*dx1; 
  div_b[5] += (0.3535533905932737*b_surf_rl[2]-0.3535533905932737*b_surf_lr[2]+0.3535533905932737*b_surf_cr[2]-0.3535533905932737*b_surf_cl[2])*dx1; 
  div_b[6] += ((-3.872983346207417*b_c[3])+0.7905694150420948*b_surf_rl[1]-0.7905694150420948*b_surf_lr[1]+0.7905694150420948*b_surf_cr[1]-0.7905694150420948*b_surf_cl[1])*dx1; 
  div_b[7] += (0.6123724356957944*(b_surf_rl[2]+b_surf_lr[2]+b_surf_cr[2]+b_surf_cl[2])-1.732050807568877*b_c[5])*dx1; 
  div_b[8] += ((-3.872983346207417*b_c[7])+0.7905694150420947*b_surf_rl[2]-0.7905694150420947*b_surf_lr[2]+0.7905694150420947*b_surf_cr[2]-0.7905694150420947*b_surf_cl[2])*dx1; 
