...
{ 
  ...
  cflFreq_mid += 5.0*fabs((0.25*p0_over_gamma[0]-0.2795084971874737*p0_over_gamma[5])*dx10); 

  out[1] += 0.8660254037844386*(p0_over_gamma[5]*f[9]+p0_over_gamma[3]*f[6]+p0_over_gamma[2]*f[3]+p0_over_gamma[1]*f[2]+f[0]*p0_over_gamma[0])*dx10; 
  out[4] += (0.8660254037844387*p0_over_gamma[5]*f[16]+0.7745966692414834*p0_over_gamma[3]*f[14]+0.7745966692414833*p0_over_gamma[1]*f[8]+0.8660254037844386*(p0_over_gamma[2]*f[6]+f[3]*p0_over_gamma[3]+p0_over_gamma[0]*f[2]+f[0]*p0_over_gamma[1]))*dx10; 
  out[5] += (0.7745966692414834*p0_over_gamma[3]*f[16]+0.7745966692414833*p0_over_gamma[2]*f[9]+0.8660254037844386*p0_over_gamma[1]*f[6]+0.7745966692414833*f[3]*p0_over_gamma[5]+0.8660254037844386*(f[2]*p0_over_gamma[3]+p0_over_gamma[0]*f[3]+f[0]*p0_over_gamma[2]))*dx10; 
  out[7] += 1.936491673103709*(p0_over_gamma[5]*f[15]+p0_over_gamma[3]*f[10]+p0_over_gamma[2]*f[5]+p0_over_gamma[1]*f[4]+p0_over_gamma[0]*f[1])*dx10; 
  out[10] += (0.7745966692414834*(p0_over_gamma[2]*f[16]+p0_over_gamma[1]*f[14])+0.7745966692414833*(p0_over_gamma[3]*(f[9]+f[8])+p0_over_gamma[5]*f[6])+0.8660254037844386*(p0_over_gamma[0]*f[6]+f[0]*p0_over_gamma[3]+p0_over_gamma[1]*f[3]+f[2]*p0_over_gamma[2]))*dx10; 
  out[11] += (1.936491673103709*p0_over_gamma[5]*f[19]+1.732050807568877*p0_over_gamma[3]*f[18]+1.732050807568877*p0_over_gamma[1]*f[12]+1.936491673103709*(p0_over_gamma[2]*f[10]+p0_over_gamma[3]*f[5]+p0_over_gamma[0]*f[4]+f[1]*p0_over_gamma[1]))*dx10; 
  out[12] += (0.8660254037844386*p0_over_gamma[2]*f[14]+0.8660254037844387*p0_over_gamma[0]*f[8]+0.7745966692414834*(p0_over_gamma[3]*f[6]+p0_over_gamma[1]*f[2]))*dx10; 
  out[13] += (1.732050807568877*p0_over_gamma[3]*f[19]+1.732050807568877*p0_over_gamma[2]*f[15]+1.936491673103709*p0_over_gamma[1]*f[10]+1.732050807568877*f[5]*p0_over_gamma[5]+1.936491673103709*(p0_over_gamma[0]*f[5]+p0_over_gamma[3]*f[4]+f[1]*p0_over_gamma[2]))*dx10; 
  out[15] += (0.8660254037844386*p0_over_gamma[1]*f[16]+(0.5532833351724881*p0_over_gamma[5]+0.8660254037844387*p0_over_gamma[0])*f[9]+0.7745966692414834*p0_over_gamma[3]*f[6]+0.8660254037844387*f[0]*p0_over_gamma[5]+0.7745966692414834*p0_over_gamma[2]*f[3])*dx10; 
  out[17] += (1.732050807568877*(p0_over_gamma[2]*f[19]+p0_over_gamma[1]*f[18])+1.732050807568877*p0_over_gamma[3]*(f[15]+f[12])+1.732050807568877*p0_over_gamma[5]*f[10]+1.936491673103709*(p0_over_gamma[0]*f[10]+p0_over_gamma[1]*f[5]+p0_over_gamma[2]*f[4]+f[1]*p0_over_gamma[3]))*dx10; 
  out[18] += (0.6928203230275508*p0_over_gamma[3]*f[16]+(0.7745966692414834*p0_over_gamma[5]+0.8660254037844387*p0_over_gamma[0])*f[14]+0.8660254037844386*p0_over_gamma[2]*f[8]+0.7745966692414833*(p0_over_gamma[1]*f[6]+f[2]*p0_over_gamma[3]))*dx10; 
  out[19] += ((0.5532833351724881*p0_over_gamma[5]+0.8660254037844387*p0_over_gamma[0])*f[16]+0.6928203230275508*p0_over_gamma[3]*f[14]+0.8660254037844386*p0_over_gamma[1]*f[9]+0.7745966692414833*p0_over_gamma[2]*f[6]+0.8660254037844386*f[2]*p0_over_gamma[5]+0.7745966692414833*f[3]*p0_over_gamma[3])*dx10; 
  ...
  out[2] += 0.6123724356957944*(alpha_vdim[17]*f[17]+alpha_vdim[13]*f[13]+alpha_vdim[12]*f[12]+alpha_vdim[11]*f[11]+alpha_vdim[10]*f[10]+alpha_vdim[8]*f[8]+alpha_vdim[7]*f[7]+alpha_vdim[6]*f[6]+alpha_vdim[5]*f[5]+alpha_vdim[4]*f[4]+alpha_vdim[3]*f[3]+alpha_vdim[2]*f[2]+alpha_vdim[1]*f[1]+alpha_vdim[0]*f[0]); 
  ...
  out[12] += 1.095445115010332*alpha_vdim[17]*f[18]+1.224744871391589*(alpha_vdim[6]*f[18]+alpha_vdim[5]*f[17]+f[5]*alpha_vdim[17]+alpha_vdim[10]*(f[14]+f[13])+f[10]*alpha_vdim[13])+(1.095445115010332*alpha_vdim[11]+1.224744871391589*alpha_vdim[2])*f[12]+1.095445115010332*f[11]*alpha_vdim[12]+1.224744871391589*(f[2]*alpha_vdim[12]+alpha_vdim[1]*f[11]+f[1]*alpha_vdim[11])+1.369306393762915*(alpha_vdim[3]*f[10]+f[3]*alpha_vdim[10])+1.224744871391589*(alpha_vdim[4]*f[8]+f[4]*alpha_vdim[8]+alpha_vdim[4]*f[7]+f[4]*alpha_vdim[7])+1.369306393762915*(alpha_vdim[5]*f[6]+f[5]*alpha_vdim[6]+alpha_vdim[0]*f[4]+f[0]*alpha_vdim[4]+alpha_vdim[1]*f[2]+f[1]*alpha_vdim[2]); 
  out[14] += 1.224744871391589*(alpha_vdim[5]*f[19]+alpha_vdim[4]*f[18])+1.369306393762915*(alpha_vdim[7]*f[17]+f[7]*alpha_vdim[17])+1.224744871391589*(alpha_vdim[3]*f[16]+alpha_vdim[10]*f[15]+alpha_vdim[2]*f[14])+1.369306393762915*(alpha_vdim[11]*f[13]+f[11]*alpha_vdim[13])+1.224744871391589*(alpha_vdim[10]*f[12]+f[10]*alpha_vdim[12])+1.369306393762915*(alpha_vdim[1]*f[10]+f[1]*alpha_vdim[10])+1.224744871391589*(alpha_vdim[6]*(f[9]+f[8])+f[6]*alpha_vdim[8])+1.369306393762915*(alpha_vdim[0]*f[6]+f[0]*alpha_vdim[6]+alpha_vdim[4]*f[5]+f[4]*alpha_vdim[5]+alpha_vdim[2]*f[3]+f[2]*alpha_vdim[3]); 
  out[16] += 0.6123724356957944*alpha_vdim[4]*f[19]+0.5477225575051661*alpha_vdim[17]*f[17]+0.6123724356957944*(alpha_vdim[2]*f[16]+alpha_vdim[1]*f[15])+0.5477225575051661*(alpha_vdim[13]*f[13]+alpha_vdim[10]*f[10])+0.6123724356957944*alpha_vdim[0]*f[9]+0.5477225575051661*(alpha_vdim[6]*f[6]+alpha_vdim[5]*f[5]+alpha_vdim[3]*f[3]); 
  ...
  out[18] += (1.095445115010332*alpha_vdim[13]+1.224744871391589*alpha_vdim[3])*f[19]+(1.095445115010332*alpha_vdim[11]+1.224744871391589*alpha_vdim[2])*f[18]+(1.095445115010332*alpha_vdim[12]+1.224744871391589*alpha_vdim[1])*f[17]+1.095445115010332*(f[15]+f[12])*alpha_vdim[17]+1.224744871391589*(f[1]*alpha_vdim[17]+alpha_vdim[5]*f[16]+alpha_vdim[6]*f[15]+alpha_vdim[4]*(f[14]+f[13])+f[4]*alpha_vdim[13]+alpha_vdim[6]*f[12]+f[6]*alpha_vdim[12]+alpha_vdim[5]*f[11]+f[5]*alpha_vdim[11])+(1.224744871391589*(alpha_vdim[8]+alpha_vdim[7])+1.369306393762915*alpha_vdim[0])*f[10]+1.224744871391589*(f[9]+f[8]+f[7])*alpha_vdim[10]+1.369306393762915*(f[0]*alpha_vdim[10]+alpha_vdim[1]*f[6]+f[1]*alpha_vdim[6]+alpha_vdim[2]*f[5]+f[2]*alpha_vdim[5]+alpha_vdim[3]*f[4]+f[3]*alpha_vdim[4]); 
  ...
  alpha_vdim[7] = (2.0*E1[2]-1.0*p0_over_gamma[0]*B2[2])*dv11; 
  alpha_vdim[8] = 0.0; 
  alpha_vdim[9] = -1.0*B2[0]*p0_over_gamma[5]*dv11; 
  alpha_vdim[10] = -1.0*B2[1]*p0_over_gamma[3]*dv11; 
  alpha_vdim[11] = -1.0*p0_over_gamma[1]*B2[2]*dv11; 
  alpha_vdim[12] = 0.0; 
  alpha_vdim[13] = -1.0*B2[2]*p0_over_gamma[2]*dv11; 
  alpha_vdim[14] = 0.0; 
  alpha_vdim[15] = -1.0*B2[1]*p0_over_gamma[5]*dv11; 
  alpha_vdim[16] = 0.0; 
  alpha_vdim[17] = -1.0*B2[2]*p0_over_gamma[3]*dv11; 
  alpha_vdim[18] = 0.0; 
  alpha_vdim[19] = 0.0; 
  cflFreq_mid += 5.0*fabs(0.1767766952966368*alpha_vdim[0]-0.1976423537605236*(alpha_vdim[9]+alpha_vdim[7])); 

  out[3] += 0.6123724356957944*(alpha_vdim[17]*f[17]+alpha_vdim[15]*f[15]+alpha_vdim[13]*f[13]+alpha_vdim[11]*f[11]+alpha_vdim[10]*f[10]+alpha_vdim[9]*f[9]+alpha_vdim[7]*f[7]+alpha_vdim[6]*f[6]+alpha_vdim[5]*f[5]+alpha_vdim[4]*f[4]+alpha_vdim[3]*f[3]+alpha_vdim[2]*f[2]+alpha_vdim[1]*f[1]+alpha_vdim[0]*f[0]); 
  ...
  out[19] += (1.095445115010332*alpha_vdim[13]+1.224744871391589*alpha_vdim[3])*f[19]+(1.095445115010332*alpha_vdim[11]+1.224744871391589*alpha_vdim[2])*f[18]+(1.095445115010332*alpha_vdim[15]+1.224744871391589*alpha_vdim[1])*f[17]+1.095445115010332*(f[15]+f[12])*alpha_vdim[17]+1.224744871391589*(f[1]*alpha_vdim[17]+alpha_vdim[5]*f[16]+alpha_vdim[6]*f[15]+f[6]*alpha_vdim[15]+alpha_vdim[4]*(f[14]+f[13])+f[4]*alpha_vdim[13]+alpha_vdim[6]*f[12]+alpha_vdim[5]*f[11]+f[5]*alpha_vdim[11])+(1.224744871391589*(alpha_vdim[9]+alpha_vdim[7])+1.369306393762915*alpha_vdim[0])*f[10]+1.224744871391589*(f[9]+f[8]+f[7])*alpha_vdim[10]+1.369306393762915*(f[0]*alpha_vdim[10]+alpha_vdim[1]*f[6]+f[1]*alpha_vdim[6]+alpha_vdim[2]*f[5]+f[2]*alpha_vdim[5]+alpha_vdim[3]*f[4]+f[3]*alpha_vdim[4]); 
