...
test_fem_poisson_perp_consteps(int poly_order, const int *cells, struct gkyl_poisson_bc bcs, bool use_gpu)
{
  ...
  int iters = 102;
  double timeTaken[iters];  // Array to store time taken for each iteration

  for (int ix = 0; ix < iters; ix++){
    // Solve the problem.
    clock_t start = clock(); // start the timer
    clock_t end   = clock(); // define the end timer
    ...
    timeTaken[ix] = (double)(end - start) / CLOCKS_PER_SEC;
  }

  double sum = 0.0;
  for (int i = 2; i < iters; i++) {
    sum += timeTaken[i];
    //printf("Itteration %d took %f seconds\n",i,timeTaken[i]);
  }
  double mean = sum / (iters-2);

  printf("Average time taken during proj_on_basis_new: %f seconds over %d iterations\n", mean, iters-2);
  ...
  for (int ix = 0; ix < iters; ix++){
    ...
    clock_t end   = clock(); // define the end timer
    if (use_gpu) {
        gkyl_fem_poisson_perp_solve(poisson, phi_cu); // Time the solve
        end = clock();  // Stop the timer
  ...
  printf("Average time taken during solve: %f seconds over %d iterations\n", mean, iters);
