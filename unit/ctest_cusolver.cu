 ...
 */
  ...
  int h_csrRowIndA[] = {0, 3, 5, 7, 9};
  int h_csrColIndA[] = {0, 2, 3, 0, 1, 1, 2, 3, 4, 0, 1, 4};

  int rowsA = sizeof(h_csrRowIndA)/sizeof(h_csrRowIndA[0]); // number of rows of A
  ...
  double *d_b = gkyl_cu_malloc(sizeof(double)*rowsA) // a copy of h_b;
...
//    descrA, h_csrRowIndA, h_csrColIndA, h_Qreorder);
  ...
  int *d_csrRowIndA = gkyl_cu_malloc(sizeof(int)*(rowsA+1));
  int *d_csrColIndA = gkyl_cu_malloc(sizeof(int)*nnzA);
  double *d_csrValA = gkyl_cu_malloc(sizeof(double)*nnzA);
  int *d_P = gkyl_cu_malloc(sizeof(int)*rowsA); // P*A*Q^T = L*U
  int *d_Q = gkyl_cu_malloc(sizeof(int)*colsA);
  ...
  // .................. Solve A*x = b .................... ..
  double *d_x = gkyl_cu_malloc(sizeof(double)*colsA);
  gkyl_cu_memcpy(d_x, h_b, sizeof(double)*rowsA, GKYL_CU_MEMCPY_H2D);

  double *d_T = gkyl_cu_malloc(sizeof(double)*rowsA*1); // Working space in cusolverRfSolve, |d_T| = n * nrhs.
