...
// compare to hyper_dg w/o gen geo
void
test_stream_gen_geo_(bool use_gpu)
{
  // initialize grid and ranges
  int cdim = 3, vdim = 3;
  int pdim = cdim+vdim;
  ...
  a0 = mkarr1(use_gpu, confBasis.num_basis, confRange_ext.volume);
  ...
  // call alpha gen geo method
  gkyl_dg_alpha_gen_geo(&confBasis, &basis, &confRange, &phaseRange, &phaseGrid, tv_comp, gij, alpha_geo);
  gkyl_grid_sub_array_write(&phaseGrid, &phaseRange, alpha_geo, "ctest_alpha_geo_fld.gkyl");
  
  // initialize eqn
  struct gkyl_dg_eqn *eqn;
  enum gkyl_field_id field_id = GKYL_FIELD_GEN_GEO;
  eqn = gkyl_dg_vlasov_new(&confBasis, &basis, &confRange, &phaseRange, field_id, use_gpu);
  ...
  cot_vec = mkarr1(use_gpu, 3*confBasis.num_basis, confRange_ext.volume);

  // set cot_vect
  gkyl_array_accumulate_offset(cot_vec, 1.0, a1, 0*a1->ncomp);
  gkyl_array_accumulate_offset(cot_vec, 1.0, a1, 1*a1->ncomp);
  gkyl_array_accumulate_offset(cot_vec, 1.0, a1, 2*a1->ncomp);
  ...
  for(int n=0; n<nrep; n++) {
    ...
    gkyl_vlasov_set_auxfields(eqn,
      (struct gkyl_dg_vlasov_auxfields) { .qmem = qmem, .cot_vec = cot_vec, .alpha_geo = alpha_geo }); // must set fields to use
    if (use_gpu)
  ...
  gkyl_grid_sub_array_write(&phaseGrid, &phaseRange, rhs, "ctest_alpha_geo_rhs.gkyl");
...
test_stream_gen_geo()
{
  test_stream_gen_geo_(false);
}

TEST_LIST = {
  { "test_alpha_gen_geo", test_alpha_gen_geo },
  { "test_stream_gen_geo", test_stream_gen_geo }, 
  { NULL, NULL },
};
