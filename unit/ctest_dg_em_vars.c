...
test(int ndim, int Nx, int poly_order, double eps, bool use_tensor, bool check_analytic, bool use_gpu)
{
  ...
  struct gkyl_array *basis_inv_bvar, *basis_inv_ExB;
  ...
  basis_inv_bvar = mkarr(9*basis.num_basis, local_ext.volume);
  basis_inv_ExB = mkarr(3*basis.num_basis, local_ext.volume);
  ...
  struct gkyl_dg_bin_op_mem *magB2_mem_cu;
  struct gkyl_array *cell_avg_magB2_cu;
  struct gkyl_array *field_cu, *bvar_cu, *ExB_cu;
  struct gkyl_array *basis_inv_bvar_cu, *basis_inv_ExB_cu;
  if (use_gpu) { // Create device copies
    magB2_mem_cu = gkyl_dg_bin_op_mem_cu_dev_new(local.volume, basis.num_basis);
    ...
    basis_inv_bvar_cu = gkyl_array_cu_dev_new(GKYL_DOUBLE, 9*basis.num_basis, local_ext.volume);
    basis_inv_ExB_cu = gkyl_array_cu_dev_new(GKYL_DOUBLE, 3*basis.num_basis, local_ext.volume);
  }
  ...
  if (use_gpu) {
    gkyl_calc_em_vars_bvar(magB2_mem_cu, basis, &local, field_cu, cell_avg_magB2_cu, bvar_cu);
    gkyl_calc_em_vars_ExB(magB2_mem_cu, basis, &local, field_cu, cell_avg_magB2_cu, ExB_cu);
    // Copy host array to device.
    gkyl_array_copy(bvar, bvar_cu);
    gkyl_array_copy(ExB, ExB_cu);
    if (poly_order == 1 || ndim == 1) {
      gkyl_calc_em_vars_bvar_basis_inv(basis, &local, field_cu, basis_inv_bvar_cu);
      gkyl_calc_em_vars_ExB_basis_inv(basis, &local, field_cu, basis_inv_ExB_cu);
      gkyl_array_copy(basis_inv_bvar, basis_inv_bvar_cu);
      gkyl_array_copy(basis_inv_ExB, basis_inv_ExB_cu);
    }
  }
  else {
    gkyl_calc_em_vars_bvar(magB2_mem, basis, &local, field, cell_avg_magB2, bvar);
    gkyl_calc_em_vars_ExB(magB2_mem, basis, &local, field, cell_avg_magB2, ExB);
    if (poly_order == 1 || ndim == 1) {
      gkyl_calc_em_vars_bvar_basis_inv(basis, &local, field, basis_inv_bvar);
      gkyl_calc_em_vars_ExB_basis_inv(basis, &local, field, basis_inv_ExB);
    }
  }
  ...
  if (use_gpu) {
    ...
    for (int i=0; i<6; ++i) {
      gkyl_dg_div_op_range(magB2_mem_cu, basis, 3+i, alt_bibj_cu, i, int_BiBj, 0, magB2, &local);
    }
    ...
    for (int i=0; i<3; ++i) {
      gkyl_dg_div_op_range(magB2_mem_cu, basis, i, alt_ExB_cu, i, int_ExB1, 0, magB2, &local);
    }    
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    const double *basis_inv_bvar_p = gkyl_array_cfetch(basis_inv_bvar, linidx);
    ...
    if (poly_order == 1 || ndim == 1) {
      ...
      // Check b_i b_j and b_i against basis_inv
      for (int m=0; m<9*basis.num_basis; ++m) {
        TEST_CHECK( gkyl_compare(basis_inv_bvar_p[m], bvar_p[m], eps) );
        if (ndim == 1)
          TEST_MSG("Expected: %.13e, coefficient (%d) in cell (%d)", basis_inv_bvar_p[m], m, iter.idx[0]);
        else if (ndim == 2)
          TEST_MSG("Expected: %.13e, coefficient (%d) in cell (%d, %d)", basis_inv_bvar_p[m], m, iter.idx[0], iter.idx[1]);
        else
          TEST_MSG("Expected: %.13e, coefficient (%d) in cell (%d, %d, %d)", basis_inv_bvar_p[m], m, iter.idx[0], iter.idx[1], iter.idx[2]);
        TEST_MSG("Produced: %.13e, coefficient (%d)", bvar_p[m], m);
      }
    }
    if (check_analytic) {
    ...
    TEST_MSG("Cell average B . B/|B|^2 Produced by dg_bin_op: %.13e", basis_inv_bvar_p[3*basis.num_basis] + basis_inv_bvar_p[6*basis.num_basis] + basis_inv_bvar_p[8*basis.num_basis]);
    ...
    const double *basis_inv_ExB_p = gkyl_array_cfetch(basis_inv_ExB, linidx);
    ...
    for (int m=0; m<3*basis.num_basis; ++m) {
      if (poly_order == 1 || ndim == 1) {
        ...
        // Check E x B against basis_inv
        TEST_CHECK( gkyl_compare(basis_inv_ExB_p[m], ExB_p[m], eps) );
        if (ndim == 1)
          TEST_MSG("Expected: %.13e, coefficient (%d) in cell (%d)", basis_inv_ExB_p[m], m, iter.idx[0]);
        else if (ndim == 2)
          TEST_MSG("Expected: %.13e, coefficient (%d) in cell (%d, %d)", basis_inv_ExB_p[m], m, iter.idx[0], iter.idx[1]);
        else
          TEST_MSG("Expected: %.13e, coefficient (%d) in cell (%d, %d, %d)", basis_inv_ExB_p[m], m, iter.idx[0], iter.idx[1], iter.idx[2]);
        TEST_MSG("Produced: %.13e, coefficient (%d)", ExB_p[m], m);
      }
      if (check_analytic) {
        // Check bin_op solution against analytic solution
  ...
  gkyl_array_release(basis_inv_bvar);
  gkyl_array_release(basis_inv_ExB);
  ...
  if (use_gpu) {
    ...
    gkyl_array_release(basis_inv_bvar_cu);
    gkyl_array_release(basis_inv_ExB_cu);

    gkyl_dg_bin_op_mem_release(magB2_mem_cu);
...
// p=2 produces a slightly different solution for bin_op vs. inverse operator 
// *only* in the higher order coefficients. The bin_op version produces smaller
// L^2 errors in certain components, especially the off-diagonal components of bb.
// However, other components there is basically no different. More testing is needed
// to determine if the bin_op version is strictly better.
// NOTE: Production calculations uses the inverse operator because it is much faster (JJ: 07/07/23)
void test_1x_p2() { test(1, 256, 2, 1.0e-2, 0, 0, false); }
...
void test_1x_p2_gpu() { test(1, 256, 2, 1.0e-2, 0, 0, true); }
