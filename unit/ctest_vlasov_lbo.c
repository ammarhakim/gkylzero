...
test_1x1v_p2_cu()
{
  ...
  gkyl_dg_lbo_updater *slvr;
  slvr = gkyl_dg_lbo_updater_cu_dev_new(&phaseGrid, &confBasis, &basis, &confRange);
  ...
  for(int n=0; n<nrep; n++) {
    ...
    gkyl_dg_lbo_updater_advance_cu(slvr, phaseRange, nuSum_cu, nuUSum_cu, nuVtSqSum_cu, fin_cu, cflrate_cu, rhs_cu);
  }
