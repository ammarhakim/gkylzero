...
void test_1x1v(int poly_order, bool use_gpu)
{
  ...
  gkyl_correct_maxwellian *corr_max = gkyl_correct_maxwellian_new(&grid, &confBasis, &basis, &confLocal, &confLocal_ext, bmag, mass, use_gpu);
  gkyl_correct_maxwellian_gyrokinetic(corr_max, fM, m0_corr, m1_corr, m2_corr, jacob_tot, bmag, mass, err_max, iter_max, &confLocal, &local, &confLocal_ext, poly_order, use_gpu);
