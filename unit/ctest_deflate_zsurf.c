...
test_deflate_inflate(bool use_gpu){
  ...
  struct gkyl_array *field_dev =  use_gpu ? gkyl_array_cu_dev_new(GKYL_DOUBLE, basis.num_basis, local_ext.volume) : gkyl_array_acquire(field);
  ...
  struct gkyl_array *deflated_field_dev =  use_gpu ? gkyl_array_cu_dev_new(GKYL_DOUBLE, deflated_basis.num_basis, deflated_local_ext.volume) : gkyl_array_acquire(deflated_field);
