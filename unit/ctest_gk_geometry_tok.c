...
test_3x_p1_straight_cylinder()
{
  ...
  int psi_offset = 2; // Ignore this many cells on the left and right. There are NaNs there
  ...
  gkyl_nodal_ops_m2n(n2m, &basis, &grid, &nrange, &range, 3, dualmag_nodal, gk_geom->dualmag, true);
  ...
  gkyl_nodal_ops_m2n(n2m, &basis, &grid, &nrange, &range, 1, cmag_nodal, gk_geom->cmag, true);
  for (int ia=nrange.lower[AL_IDX]; ia<=nrange.upper[AL_IDX]; ++ia){
    for (int ip=nrange.lower[PSI_IDX]+psi_offset; ip<=nrange.upper[PSI_IDX]-psi_offset; ++ip) {
      for (int it=nrange.lower[TH_IDX]; it<=nrange.upper[TH_IDX]; ++it) {
        cidx[PSI_IDX] = ip;
  ...
  gkyl_nodal_ops_m2n(n2m, &basis, &grid, &nrange, &range, 6, gij_nodal, gk_geom->g_ij, true);
  for (int ia=nrange.lower[AL_IDX]; ia<=nrange.upper[AL_IDX]; ++ia){
    for (int ip=nrange.lower[PSI_IDX]+psi_offset; ip<=nrange.upper[PSI_IDX]-psi_offset; ++ip) {
      for (int it=nrange.lower[TH_IDX]; it<=nrange.upper[TH_IDX]; ++it) {
        cidx[PSI_IDX] = ip;
  ...
  gkyl_nodal_ops_m2n(n2m, &basis, &grid, &nrange, &range, 6, gij_contra_nodal, gk_geom->gij, true);
  for (int ia=nrange.lower[AL_IDX]; ia<=nrange.upper[AL_IDX]; ++ia){
    for (int ip=nrange.lower[PSI_IDX]+psi_offset; ip<=nrange.upper[PSI_IDX]-psi_offset; ++ip) {
      for (int it=nrange.lower[TH_IDX]; it<=nrange.upper[TH_IDX]; ++it) {
        cidx[PSI_IDX] = ip;
  ...
  gkyl_nodal_ops_m2n(n2m, &basis, &grid, &nrange, &range,  9*basis.num_basis, normals_nodal, gk_geom->normals, true);
  for (int ia=nrange.lower[AL_IDX]; ia<=nrange.upper[AL_IDX]; ++ia){
    for (int ip=nrange.lower[PSI_IDX]+psi_offset; ip<=nrange.upper[PSI_IDX]-psi_offset; ++ip) {
      for (int it=nrange.lower[TH_IDX]; it<=nrange.upper[TH_IDX]; ++it) {
        cidx[PSI_IDX] = ip;
        ...
        for (int i=0; i<9; ++i)
        {
          if (i==5)
            tol = 1e-3;
              ...
              tol = 1e-6;
          TEST_CHECK( gkyl_compare( normals_n[i], fout[i], tol) );
        }
      }
    }
  }
