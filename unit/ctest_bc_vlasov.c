...
void test_bc(int cdim, int vdim, int poly_order, char *boundary_type) {
  ...
  for (int bc_dir = 0; bc_dir < cdim; bc_dir++) {
    ...
    gkyl_array_flip_copy_to_buffer_fn(bc_buffer->data, distf, bc_dir,
                                      skin_ghost.lower_skin[bc_dir], bc);
    gkyl_array_copy_from_buffer(distf, bc_buffer->data,
                                skin_ghost.lower_ghost[bc_dir]);

    // Apply BC to the upper ghost cells
    gkyl_array_flip_copy_to_buffer_fn(bc_buffer->data, distf, bc_dir,
                                      skin_ghost.upper_skin[bc_dir], bc);
    gkyl_array_copy_from_buffer(distf, bc_buffer->data,
                                skin_ghost.upper_ghost[bc_dir]);
...
TEST_LIST = {
    ...
    {"test_bc_wall_1x2v_p1", test_bc_wall_1x2v_p1},
    {"test_bc_wall_2x2v_p1", test_bc_wall_2x2v_p1},
    {"test_bc_wall_3x2v_p1", test_bc_wall_3x2v_p1},
    {"test_bc_wall_1x1v_p2", test_bc_wall_1x1v_p2},
    {"test_bc_wall_1x2v_p2", test_bc_wall_1x2v_p2},
    {"test_bc_wall_2x2v_p2", test_bc_wall_2x2v_p2},
    {"test_bc_wall_3x2v_p2", test_bc_wall_3x2v_p2},
