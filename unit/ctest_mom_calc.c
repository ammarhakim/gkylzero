...
test_1x1v_p1()
{
  ...
  m1i = mkarr(confBasis.numBasis, confLocal_ext.volume);
...
test_2x3v_p1()
{
  ...
  m0 = mkarr(vmM0_t->num_mom*confBasis.numBasis, confLocal_ext.volume);
  m1i = mkarr(vmM1i_t->num_mom*confBasis.numBasis, confLocal_ext.volume);
  m2 = mkarr(vmM2_t->num_mom*confBasis.numBasis, confLocal_ext.volume);
  ...
  for (unsigned int i=0; i<cells[0]; ++i) {
    for (unsigned int j=0; j<cells[1]; ++j) {
      ...
      for (unsigned int k=0; k<confBasis.numBasis; ++k) {
...
test_1x1v_p1_cu()
{
  ...
  m1i_cu = mkarr_cu(confBasis.numBasis, confLocal_ext.volume);
