...
vm_species_init(struct gkyl_vm *vm, struct gkyl_vlasov_app *app, struct vm_species *s)
{
  ...
  if (s->model_id  == GKYL_MODEL_SR) {
    ...
    // Write out stuff
    gkyl_grid_sub_array_write(&s->grid, &s->local, 0, s->hamil_host, "check_can_pb_hamil.gkyl");
    gkyl_grid_sub_array_write(&app->grid, &app->local, 0, s->h_ij_host, "check_can_pb_h_ij.gkyl");
    gkyl_grid_sub_array_write(&app->grid, &app->local, 0, s->h_ij_inv_host, "check_can_pb_h_ij_inv.gkyl");
    gkyl_grid_sub_array_write(&app->grid, &app->local, 0, s->det_h_host, "check_can_pb_det_h.gkyl");
