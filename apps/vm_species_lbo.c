...
vm_species_lbo_init(struct gkyl_vlasov_app *app, struct vm_species *s, struct vm_lbo_collisions *lbo)
{
  ...
  if (app->use_gpu) {
    ...
    lbo->cM_mom = gkyl_mom_bcorr_lbo_vlasov_cu_dev_new(&app->confBasis, &app->basis, "f", v_bounds);
    lbo->cM_bcorr = gkyl_mom_calc_bcorr_cu_dev_new(&s->grid, lbo->cM_mom);

    // edge of velocity space energy correction
    lbo->cE_mom = gkyl_mom_bcorr_lbo_vlasov_cu_dev_new(&app->confBasis, &app->basis, "vf",  v_bounds);
    lbo->cE_bcorr = gkyl_mom_calc_bcorr_cu_dev_new(&s->grid, lbo->cE_mom);
    ...
    // edge of velocity space momentum correction 
    lbo->cM_mom = gkyl_mom_bcorr_lbo_vlasov_new(&app->confBasis, &app->basis, "f", v_bounds);
    lbo->cM_bcorr = gkyl_mom_calc_bcorr_new(&s->grid, lbo->cM_mom);

    // edge of velocity space energy correction
    lbo->cE_mom = gkyl_mom_bcorr_lbo_vlasov_new(&app->confBasis, &app->basis, "vf",  v_bounds);
    lbo->cE_bcorr = gkyl_mom_calc_bcorr_new(&s->grid, lbo->cE_mom);
  ...
  struct vm_lbo_collisions *lbo, const struct gkyl_array *fin, struct gkyl_array *rhs)
{
  ...
  if (app->use_gpu) {
    ...
    gkyl_mom_calc_bcorr_advance_cu(lbo->cM_bcorr, species->local, app->local, fin, lbo->cM);
    gkyl_mom_calc_bcorr_advance_cu(lbo->cE_bcorr, species->local, app->local, fin, lbo->cE);
    ...
    // construct boundary corrections
    gkyl_mom_calc_bcorr_advance(lbo->cM_bcorr, species->local, app->local, fin, lbo->cM);
    gkyl_mom_calc_bcorr_advance(lbo->cE_bcorr, species->local, app->local, fin, lbo->cE);
...
vm_species_lbo_release(const struct gkyl_vlasov_app *app, const struct vm_lbo_collisions *lbo)
{
  ...
  gkyl_mom_calc_bcorr_release(lbo->cM_bcorr);
  gkyl_mom_calc_bcorr_release(lbo->cE_bcorr);
