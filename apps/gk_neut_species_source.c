...
void 
gk_neut_species_source_init(struct gkyl_gyrokinetic_oneb *app, struct gk_neut_species *s, struct gk_neut_source *src)
{
...
void
gk_neut_species_source_calc(gkyl_gyrokinetic_app *app, struct gk_neut_species *species, double tm)
{
  if (species->source_id == GKYL_MAXWELLIAN_SOURCE) {
    ...
    gk_neut_species_moment_calc(&species->m0, species->local_ext, app->local_ext, species->src.source); 
    ...
    gkyl_dg_mul_conf_phase_op_range(&app->confBasis, &app->neut_basis, species->src.source, 
...
void
gk_neut_species_source_release(const struct gkyl_gyrokinetic_oneb *app, const struct gk_neut_source *src)
{
