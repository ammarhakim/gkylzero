  ...
  struct gkyl_vlasov_projection inp, struct vm_proj *proj)
{
  ...
  if (proj->proj_id == GKYL_PROJ_FUNC) {
    ...
    struct gkyl_vlasov_lte_proj_on_basis_inp inp_proj = {
      ...
      .jacob_vel_gauss = s->jacob_vel_gauss, 
      .gamma = s->gamma,
    ...
    proj->dens = mkarr(false, app->confBasis.num_basis, app->local_ext.volume);
    proj->V_drift = mkarr(false, vdim*app->confBasis.num_basis, app->local_ext.volume);
    proj->Tpar_over_m = mkarr(false, app->confBasis.num_basis, app->local_ext.volume);
    proj->Tperp_over_m = mkarr(false, app->confBasis.num_basis, app->local_ext.volume);
    proj->vlasov_lte_moms_host = mkarr(false, (vdim+3)*app->confBasis.num_basis, app->local_ext.volume);

    proj->proj_dens = gkyl_proj_on_basis_new(&app->grid, &app->confBasis,
      app->basis.poly_order+1, 1, inp.density, inp.ctx_density);
    proj->proj_V_drift = gkyl_proj_on_basis_new(&app->grid, &app->confBasis,
      app->basis.poly_order+1, vdim, inp.V_drift, inp.ctx_V_drift);
    proj->proj_temp_par = gkyl_proj_on_basis_new(&app->grid, &app->confBasis,
      app->basis.poly_order+1, 1, inp.temp_par, inp.ctx_temp_par);
    proj->proj_temp_perp = gkyl_proj_on_basis_new(&app->grid, &app->confBasis,
      app->basis.poly_order+1, 1, inp.temp_perp, inp.ctx_temp_perp);

    proj->vlasov_lte_moms = mkarr(app->use_gpu, (vdim+3)*app->confBasis.num_basis, app->local_ext.volume);
  ...
  struct vm_proj *proj, struct gkyl_array *f, double tm)
{
  if (proj->proj_id == GKYL_PROJ_FUNC) {
    ...
    gkyl_proj_on_basis_advance(proj->proj_dens, tm, &app->local_ext, proj->dens); 
    gkyl_proj_on_basis_advance(proj->proj_V_drift, tm, &app->local_ext, proj->V_drift);
    gkyl_proj_on_basis_advance(proj->proj_temp_par, tm, &app->local_ext, proj->Tpar_over_m);
    gkyl_proj_on_basis_advance(proj->proj_temp_perp, tm, &app->local_ext, proj->Tperp_over_m);
    gkyl_array_scale(proj->Tpar_over_m, 1.0/s->info.mass);
    gkyl_array_scale(proj->Tperp_over_m, 1.0/s->info.mass);

    // Projection routines expect the LTE moments as a single array.
    gkyl_array_set_offset(proj->vlasov_lte_moms_host, 1.0, proj->dens, 0*app->confBasis.num_basis);
    gkyl_array_set_offset(proj->vlasov_lte_moms_host, 1.0, proj->V_drift, 1*app->confBasis.num_basis);
    gkyl_array_set_offset(proj->vlasov_lte_moms_host, 1.0, proj->Tpar_over_m, (vdim+1)*app->confBasis.num_basis);
    gkyl_array_set_offset(proj->vlasov_lte_moms_host, 1.0, proj->Tperp_over_m, (vdim+2)*app->confBasis.num_basis);

    // Copy the contents into the array we will use (potentially on GPUs).
    gkyl_array_copy(proj->vlasov_lte_moms, proj->vlasov_lte_moms_host);

    // Project a bimaxwellian distribution function.
    // Projection routine also corrects the density of the projected distribution function.
...
vm_species_projection_release(const struct gkyl_vlasov_app *app, const struct vm_proj *proj)
{
  if (proj->proj_id == GKYL_PROJ_FUNC) {
    ...
    if (proj->proj_id == GKYL_PROJ_VLASOV_LTE) {
      gkyl_array_release(proj->T_over_m);
      gkyl_proj_on_basis_release(proj->proj_temp);
    }
    else {
      gkyl_array_release(proj->Tpar_over_m);
      gkyl_proj_on_basis_release(proj->proj_temp_par); 
      gkyl_array_release(proj->Tperp_over_m);
      gkyl_proj_on_basis_release(proj->proj_temp_perp);      
    }
