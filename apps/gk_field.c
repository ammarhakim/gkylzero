...
gk_field_new(struct gkyl_gk *gk, struct gkyl_gyrokinetic_app *app)
{
  ...
  if (f->gkfield_id == GKYL_GK_FIELD_BOLTZMANN) {
    ...
    if (app->cdim == 1) {
      ...
      f->deflated_dg_bin_ops = gkyl_deflated_dg_bin_ops_new(app->grid, app->basis_on_dev.confBasis, app->confBasis, app->global, app->use_gpu);
    }
  }
...
gk_field_rhs(gkyl_gyrokinetic_app *app, struct gk_field *field)
{
  ...
  if (field->gkfield_id == GKYL_GK_FIELD_BOLTZMANN) { 
    ...
    if (app->cdim == 1) {
      ...
      gkyl_deflated_dg_bin_ops_mul(field->deflated_dg_bin_ops, 0, field->rho_c_global_smooth, 0, field->rho_c_global_smooth, 0, field->jacobgeo_global);
...
gk_field_release(const gkyl_gyrokinetic_app* app, struct gk_field *f)
{
  ...
  if (f->gkfield_id == GKYL_GK_FIELD_BOLTZMANN) {
    ...
    if (app->cdim == 1) {
      ...
      gkyl_deflated_dg_bin_ops_release(f->deflated_dg_bin_ops);
    }
  }
