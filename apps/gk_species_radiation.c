...
void 
gk_species_radiation_init(struct gkyl_gyrokinetic_oneb *oneb, struct gk_species *s, struct gk_rad_drag *rad)
{
  // Fitting parameters
  ...
  for (int i=0; i<rad->num_cross_collisions; ++i) {
    ...
    rad->vnu[i] = mkarr(oneb->use_gpu, oneb->basis.num_basis, s->local_ext.volume);
    rad->vsqnu[i] = mkarr(oneb->use_gpu, oneb->basis.num_basis, s->local_ext.volume);

    rad->vnu_host[i] = rad->vnu[i];
    rad->vsqnu_host[i] = rad->vsqnu[i];
    if (oneb->use_gpu) {
      rad->vnu_host[i] = mkarr(false, oneb->basis.num_basis, s->local_ext.volume);
      rad->vsqnu_host[i] = mkarr(false, oneb->basis.num_basis, s->local_ext.volume);
    }

    // Fetch the species we are colliding with and the fitting parameters for that species
    rad->collide_with[i] = gk_find_species(oneb, s->info.radiation.collide_with[i]);
    rad->collide_with_idx[i] = gk_find_species_idx(oneb, s->info.radiation.collide_with[i]);
    ...
    rad->calc_gk_rad_vars[i] = gkyl_dg_calc_gk_rad_vars_new(&s->grid, &oneb->confBasis, &oneb->basis, 
      s->info.charge, s->info.mass, oneb->gk_geom, 
      a, alpha, beta, gamma, v0);

    gkyl_dg_calc_gk_rad_vars_advance(rad->calc_gk_rad_vars[i], &oneb->local, &s->local, rad->vnu_host[i], rad->vsqnu_host[i]);
    if (oneb->use_gpu) {
    ...
    gk_species_moment_init(oneb, rad->collide_with[i], &rad->moms[i], "M0");
  }

  // Total vparallel and mu radiation drag including density scaling
  rad->nvnu_sum = mkarr(oneb->use_gpu, oneb->basis.num_basis, s->local_ext.volume);
  rad->nvsqnu_sum = mkarr(oneb->use_gpu, oneb->basis.num_basis, s->local_ext.volume);
  rad->nvnu_sum_host = rad->nvnu_sum;
  rad->nvsqnu_sum_host = rad->nvsqnu_sum;
  if (oneb->use_gpu) {
    rad->nvnu_sum_host = mkarr(false, oneb->basis.num_basis, s->local_ext.volume);
    rad->nvsqnu_sum_host = mkarr(false, oneb->basis.num_basis, s->local_ext.volume);
  }
  ...
  rad->drag_slvr = gkyl_dg_updater_rad_gyrokinetic_new(&s->grid, 
    &oneb->confBasis, &oneb->basis, &s->local, &drag_inp, oneb->use_gpu);
}

// computes density for computation of total radiation drag
void
gk_species_radiation_moms(struct gkyl_gyrokinetic_oneb *oneb, const struct gk_species *species,
  struct gk_rad_drag *rad, const struct gkyl_array *fin[])
{
  ...
  for (int i=0; i<rad->num_cross_collisions; ++i) {
    ...
    gk_species_moment_calc(&rad->moms[i], species->local, oneb->local, fin[rad->collide_with_idx[i]]);

    // scale drag coefficients by density
    gkyl_dg_mul_conf_phase_op_range(&oneb->confBasis, &oneb->basis, rad->vnu[i], rad->moms[i].marr, rad->vnu[i],
      &oneb->local, &species->local);
    gkyl_dg_mul_conf_phase_op_range(&oneb->confBasis, &oneb->basis, rad->vsqnu[i], rad->moms[i].marr, rad->vsqnu[i],
      &oneb->local, &species->local);
...
void
gk_species_radiation_rhs(struct gkyl_gyrokinetic_oneb *oneb, const struct gk_species *species,
...
void 
gk_species_radiation_release(const struct gkyl_gyrokinetic_oneb *oneb, const struct gk_rad_drag *rad)
{
  for (int i=0; i<rad->num_cross_collisions; ++i) {
    ...
    gk_species_moment_release(oneb, &rad->moms[i]);
  }
