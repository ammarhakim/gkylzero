...
gkyl_vlasov_app_write_species(gkyl_vlasov_app* app, int sidx, double tm, int frame)
{
  ...
  if (vm_s->use_vmap && frame == 0) {
    struct gkyl_msgpack_data *mt_vel = vlasov_array_meta_new( (struct vlasov_output_meta) {
        ...
        .stime = tm,
        .poly_order = app->poly_order,
        .basis_type = app->velBasis.id
      }
    );
    int rank;
    gkyl_comm_get_rank(vm_s->comm, &rank);
    if (rank == 0) { 
      const char *fmt_vmap = "%s-%s_vmap.gkyl";
      int sz_vmap = gkyl_calc_strlen(fmt_vmap, app->name, vm_s->info.name);
      ...
      gkyl_grid_sub_array_write(&vm_s->grid_vel, &vm_s->local_vel, 
        mt_vel, vm_s->vmap_pgkyl, fileNm_vmap);

      const char *fmt_jacob_vel = "%s-%s_jacob_vel.gkyl";
      int sz_jacob_vel = gkyl_calc_strlen(fmt_jacob_vel, app->name, vm_s->info.name);
      ...
      if (vm_s->source_id) {
        if (vm_s->src.write_source) {
          ...
          if (vm_s->source_id == GKYL_PROJ_ADAPT_DENSITY_SOURCE) {
            ...
            int sz_source_M0 = gkyl_calc_strlen(fmt_source_M0, app->name, vm_s->info.name, frame);
