...
void 
vp_species_bgk_init(struct gkyl_vlasov_poisson_app *app, struct vp_species *s, struct vp_bgk_collisions *bgk)
{
  ...
  vp_species_moment_init(app, s, &bgk->moms, "LTEMoments");
...
// computes moments
void
vp_species_bgk_moms(gkyl_vlasov_poisson_app *app, const struct vp_species *species,
  struct vp_bgk_collisions *bgk, const struct gkyl_array *fin)
{
  struct timespec wst = gkyl_wall_clock();

  vp_species_moment_calc(&bgk->moms, species->local, app->local, fin);
...
void
vp_species_bgk_moms_fixed_temp(gkyl_vlasov_poisson_app *app, const struct vp_species *species,
  struct vp_bgk_collisions *bgk, const struct gkyl_array *fin)
{
  vp_species_moment_calc(&bgk->moms, species->local, app->local, fin);
...
void
vp_species_bgk_rhs(gkyl_vlasov_poisson_app *app, const struct vp_species *species,
  struct vp_bgk_collisions *bgk, const struct gkyl_array *fin, struct gkyl_array *rhs)
{
  struct timespec wst = gkyl_wall_clock();
...
void 
vp_species_bgk_release(const struct gkyl_vlasov_poisson_app *app, const struct vp_bgk_collisions *bgk)
{
  ...
  vp_species_moment_release(app, &bgk->moms);
  if (bgk->fixed_temp_relax) {
