...
pkpm_field_new(struct gkyl_pkpm *pkpm, struct gkyl_pkpm_app *app)
{
  ...
  // Duplicate copy of EM data, order p, in case time step fails.
  ...
  // Create updaters for computing EM vars (bvar, em_vars_diag) and limiting EM variables
  f->calc_em_vars = gkyl_dg_calc_em_vars_new(&app->grid, &app->confBasis, &app->confBasis_2p, &app->local_ext, 
    maxwell, app->geom, limiter_fac, app->use_gpu);
...
void
pkpm_field_calc_div_b(gkyl_pkpm_app *app, struct pkpm_field *field)
{
  struct timespec tm = gkyl_wall_clock();
