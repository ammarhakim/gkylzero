...
vm_field_new(struct gkyl_vm *vm, struct gkyl_vlasov_app *app)
{
  ...
  // Bin op memory to compute 1/|B|^2
  if (app->use_gpu) 
    f->magB2_mem = gkyl_dg_bin_op_mem_cu_dev_new(app->local_ext.volume, app->confBasis.num_basis);
  else 
    f->magB2_mem = gkyl_dg_bin_op_mem_new(app->local_ext.volume, app->confBasis.num_basis);
  ...
  const struct gkyl_array *em)
{
  ...
  gkyl_calc_em_vars_bvar(field->magB2_mem, app->confBasis, &app->local_ext, field->tot_em, 
  ...
  const struct gkyl_array *em)
{
  ...
  gkyl_calc_em_vars_ExB(field->magB2_mem, app->confBasis, &app->local_ext, field->tot_em, 
...
vm_field_release(const gkyl_vlasov_app* app, struct vm_field *f)
{
  ...
  gkyl_dg_bin_op_mem_release(f->magB2_mem);
