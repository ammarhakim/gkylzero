#include <gkyl_alloc.h>
#include <gkyl_app.h>
#include <gkyl_array.h>
#include <gkyl_array_ops.h>
#include <gkyl_bc_basic.h>
#include <gkyl_dynvec.h>
#include <gkyl_elem_type.h>
#include <gkyl_eqn_type.h>
#include <gkyl_gyrokinetic_priv.h>
#include <gkyl_proj_on_basis.h>

#include <assert.h>
#include <time.h>

// initialize species object
void
gk_species_init(struct gkyl_gk *gk, struct gkyl_gyrokinetic_oneb *oneb, struct gk_species *s)
{
  int cdim = oneb->cdim, vdim = oneb->vdim;
  int pdim = cdim+vdim;

  int cells[GKYL_MAX_DIM], ghost[GKYL_MAX_DIM];
  double lower[GKYL_MAX_DIM], upper[GKYL_MAX_DIM];

  int cells_vel[GKYL_MAX_DIM], ghost_vel[GKYL_MAX_DIM];
  double lower_vel[GKYL_MAX_DIM], upper_vel[GKYL_MAX_DIM];

  for (int d=0; d<cdim; ++d) {
    cells[d] = gk->cells[d];
    lower[d] = gk->lower[d];
    upper[d] = gk->upper[d];
    ghost[d] = 1;
  }
  for (int d=0; d<vdim; ++d) {
    // full phase space grid
    cells[cdim+d] = s->info.cells[d];
    lower[cdim+d] = s->info.lower[d];
    upper[cdim+d] = s->info.upper[d];
    ghost[cdim+d] = 0; // no ghost-cells in velocity space

    // only velocity space
    cells_vel[d] = s->info.cells[d];
    lower_vel[d] = s->info.lower[d];
    upper_vel[d] = s->info.upper[d];
    ghost_vel[d] = 0; // no ghost-cells in velocity space
  }
  // full phase space grid
  gkyl_rect_grid_init(&s->grid, pdim, lower, upper, cells);
  gkyl_create_grid_ranges(&s->grid, ghost, &s->global_ext, &s->global);
  
  // velocity space grid
  gkyl_rect_grid_init(&s->grid_vel, vdim, lower_vel, upper_vel, cells_vel);
  gkyl_create_grid_ranges(&s->grid_vel, ghost_vel, &s->local_ext_vel, &s->local_vel);

  // phase-space communicator
  s->comm = gkyl_comm_extend_comm(oneb->comm, &s->local_vel);

  // create local and local_ext from app local range
  struct gkyl_range local;
  // local = conf-local X local_vel
  gkyl_range_ten_prod(&local, &oneb->local, &s->local_vel);
  gkyl_create_ranges(&local, ghost, &s->local_ext, &s->local);

  // determine field-type 
  s->gkfield_id = oneb->field->gkfield_id;
  s->gkmodel_id = GKYL_GK_MODEL_GEN_GEO;

  // allocate distribution function arrays
  s->f = mkarr(oneb->use_gpu, oneb->basis.num_basis, s->local_ext.volume);
  s->f1 = mkarr(oneb->use_gpu, oneb->basis.num_basis, s->local_ext.volume);
  s->fnew = mkarr(oneb->use_gpu, oneb->basis.num_basis, s->local_ext.volume);

  s->f_host = s->f;
  if (oneb->use_gpu)
    s->f_host = mkarr(false, oneb->basis.num_basis, s->local_ext.volume);

  // allocate cflrate (scalar array)
  s->cflrate = mkarr(oneb->use_gpu, 1, s->local_ext.volume);

  if (oneb->use_gpu)
    s->omegaCfl_ptr = gkyl_cu_malloc(sizeof(double));
  else 
    s->omegaCfl_ptr = gkyl_malloc(sizeof(double));

  // Need to figure out size of alpha_surf and sgn_alpha_surf by finding size of surface basis set 
  struct gkyl_basis surf_basis, surf_quad_basis;
  if (oneb->poly_order > 1) {
    gkyl_cart_modal_serendip(&surf_basis, pdim-1, oneb->poly_order);
    gkyl_cart_modal_tensor(&surf_quad_basis, pdim-1, oneb->poly_order);
  }
  else {
    gkyl_cart_modal_gkhybrid(&surf_basis, cdim-1, vdim); // p=2 in vparallel
    gkyl_cart_modal_gkhybrid(&surf_quad_basis, cdim-1, vdim); 
  }
  int alpha_surf_sz = (cdim+1)*surf_basis.num_basis;
  int sgn_alpha_surf_sz = (cdim+1)*surf_quad_basis.num_basis; // sign(alpha) is store at quadrature points

  // allocate arrays to store fields: 
  // 1. alpha_surf (surface phase space flux)
  // 2. sgn_alpha_surf (sign(alpha_surf) at quadrature points)
  // 3. const_sgn_alpha (boolean for if sign(alpha_surf) is a constant, either +1 or -1)
  s->alpha_surf = mkarr(oneb->use_gpu, alpha_surf_sz, s->local_ext.volume);
  s->sgn_alpha_surf = mkarr(oneb->use_gpu, sgn_alpha_surf_sz, s->local_ext.volume);
  s->const_sgn_alpha = mk_int_arr(oneb->use_gpu, (cdim+1), s->local_ext.volume);
  // 4. EM fields: phi and (if EM GK) Apar and d/dt Apar  
  s->phi = mkarr(oneb->use_gpu, oneb->confBasis.num_basis, oneb->local_ext.volume);
  s->apar = mkarr(oneb->use_gpu, oneb->confBasis.num_basis, oneb->local_ext.volume);
  s->apardot = mkarr(oneb->use_gpu, oneb->confBasis.num_basis, oneb->local_ext.volume);    

  s->calc_gk_vars = gkyl_dg_calc_gyrokinetic_vars_new(&s->grid, &oneb->confBasis, &oneb->basis, 
    s->info.charge, s->info.mass, s->gkmodel_id, oneb->gk_geom, oneb->use_gpu);

  // by default, we do not have zero-flux boundary conditions in any direction
  bool is_zero_flux[GKYL_MAX_DIM] = {false};

  // determine which directions are not periodic, if any directions are zero-flux, need to set is_zero_flux
  int num_periodic_dir = oneb->num_periodic_dir, is_np[3] = {1, 1, 1};
  for (int d=0; d<num_periodic_dir; ++d)
    is_np[oneb->periodic_dirs[d]] = 0;

  for (int dir=0; dir<oneb->cdim; ++dir) {
    s->lower_bc[dir] = s->upper_bc[dir] = GKYL_SPECIES_COPY;
    if (is_np[dir]) {
      const enum gkyl_species_bc_type *bc;
      if (dir == 0)
        bc = s->info.bcx;
      else if (dir == 1)
        bc = s->info.bcy;
      else
        bc = s->info.bcz;

      s->lower_bc[dir] = bc[0];
      s->upper_bc[dir] = bc[1];
      // Zero flux BCs can only be applied jointly on lower and upper boundary
      if (s->lower_bc[dir] == GKYL_SPECIES_ZERO_FLUX || s->upper_bc[dir] == GKYL_SPECIES_ZERO_FLUX)
        is_zero_flux[dir] = true;
    }
  }

  struct gkyl_dg_gyrokinetic_auxfields aux_inp = { .alpha_surf = s->alpha_surf, 
    .sgn_alpha_surf = s->sgn_alpha_surf, .const_sgn_alpha = s->const_sgn_alpha, 
    .phi = s->phi, .apar = s->apar, .apardot = s->apardot };
  // create solver
  s->slvr = gkyl_dg_updater_gyrokinetic_new(&s->grid, &oneb->confBasis, &oneb->basis, 
    &oneb->local, &s->local, is_zero_flux, 
    s->info.charge, s->info.mass, s->gkmodel_id, 
    oneb->gk_geom, &aux_inp, oneb->use_gpu);

  // acquire equation object
  s->eqn_gyrokinetic = gkyl_dg_updater_gyrokinetic_acquire_eqn(s->slvr);

  // allocate date for density (for use in charge density accumulation and weak division for upar)
  gk_species_moment_init(oneb, s, &s->m0, "M0");
  // allocate data for integrated moments
  gk_species_moment_init(oneb, s, &s->integ_moms, "Integrated");

  // allocate data for diagnostic moments
  int ndm = s->info.num_diag_moments;
  s->moms = gkyl_malloc(sizeof(struct gk_species_moment[ndm]));
  for (int m=0; m<ndm; ++m)
    gk_species_moment_init(oneb, s, &s->moms[m], s->info.diag_moments[m]);

  if (oneb->use_gpu) 
    s->red_integ_diag = gkyl_cu_malloc(sizeof(double[vdim+2]));
  // allocate dynamic-vector to store all-reduced integrated moments 
  s->integ_diag = gkyl_dynvec_new(GKYL_DOUBLE, vdim+2);
  s->is_first_integ_write_call = true;

  // set species source id
  s->source_id = s->info.source.source_id;
  
  // determine collision type to use in gyrokinetic update
  s->collision_id = s->info.collisions.collision_id;
  if (s->collision_id == GKYL_LBO_COLLISIONS) 
    gk_species_lbo_init(oneb, s, &s->lbo);
  else if (s->collision_id == GKYL_BGK_COLLISIONS)
    gk_species_bgk_init(oneb, s, &s->bgk);

  // determine radiation type to use in gyrokinetic update
  s->radiation_id = s->info.radiation.radiation_id;
  if (s->radiation_id == GKYL_GK_RADIATION) 
    gk_species_radiation_init(oneb, s, &s->rad);

  // initialize boundary fluxes for diagnostics and, if present,
  // ambipolar potential solve
  gk_species_bflux_init(oneb, s, &s->bflux); 

  // initialize diffusion if present
  s->has_diffusion = false;  
  if (s->info.diffusion.num_diff_dir) {
    s->has_diffusion = true;
    int diffusion_order = s->info.diffusion.order ? s->info.diffusion.order : 2;

    int szD = cdim;
    s->diffD = mkarr(oneb->use_gpu, szD, 1);
    bool is_zero_flux[GKYL_MAX_CDIM] = {false};
    bool diff_dir[GKYL_MAX_CDIM] = {false};

    int num_diff_dir = s->info.diffusion.num_diff_dir ? s->info.diffusion.num_diff_dir : oneb->cdim;
    for (int d=0; d<num_diff_dir; ++d) {
      int dir = s->info.diffusion.diff_dirs[d]; 
      diff_dir[dir] = 1; 
      gkyl_array_shiftc(s->diffD, s->info.diffusion.D[d], dir);
    }

    s->diff_slvr = gkyl_dg_updater_diffusion_gyrokinetic_new(&s->grid, &oneb->basis, &oneb->confBasis, 
      true, diff_dir, diffusion_order, &s->local, is_zero_flux, oneb->use_gpu);
  }

  // create ranges and allocate buffers for applying periodic and non-periodic BCs
  long buff_sz = 0;
  // compute buffer size needed
  for (int dir=0; dir<cdim; ++dir) {
    // Create local lower skin and ghost ranges for distribution function
    gkyl_skin_ghost_ranges(&s->lower_skin[dir], &s->lower_ghost[dir], dir, GKYL_LOWER_EDGE, &s->local_ext, ghost);
    // Create local upper skin and ghost ranges for distribution function
    gkyl_skin_ghost_ranges(&s->upper_skin[dir], &s->upper_ghost[dir], dir, GKYL_UPPER_EDGE, &s->local_ext, ghost);

    long vol = GKYL_MAX2(s->lower_skin[dir].volume, s->upper_skin[dir].volume);
    buff_sz = buff_sz > vol ? buff_sz : vol;
  }
  s->bc_buffer = mkarr(oneb->use_gpu, oneb->basis.num_basis, buff_sz);
  // buffer arrays for fixed function boundary conditions on distribution function
  s->bc_buffer_lo_fixed = mkarr(oneb->use_gpu, oneb->basis.num_basis, buff_sz);
  s->bc_buffer_up_fixed = mkarr(oneb->use_gpu, oneb->basis.num_basis, buff_sz);

  for (int d=0; d<cdim; ++d) {
    // Copy BCs by default.
    enum gkyl_bc_basic_type bctype = GKYL_BC_COPY;
    if (s->lower_bc[d] == GKYL_SPECIES_GK_SHEATH) {
      s->bc_sheath_lo = gkyl_bc_sheath_gyrokinetic_new(d, GKYL_LOWER_EDGE, oneb->basis_on_dev.basis, 
        &s->lower_skin[d], &s->lower_ghost[d], &s->grid, cdim, 2.0*(s->info.charge/s->info.mass), oneb->use_gpu);
    }
    else { 
      if (s->lower_bc[d] == GKYL_SPECIES_COPY) 
        bctype = GKYL_BC_COPY;
      else if (s->lower_bc[d] == GKYL_SPECIES_ABSORB) 
        bctype = GKYL_BC_ABSORB;
      else if (s->lower_bc[d] == GKYL_SPECIES_REFLECT) 
        bctype = GKYL_BC_REFLECT;
      else if (s->lower_bc[d] == GKYL_SPECIES_FIXED_FUNC) 
        bctype = GKYL_BC_FIXED_FUNC;

      s->bc_lo[d] = gkyl_bc_basic_new(d, GKYL_LOWER_EDGE, bctype, oneb->basis_on_dev.basis,
        &s->lower_skin[d], &s->lower_ghost[d], s->f->ncomp, oneb->cdim, oneb->use_gpu);
    }

    if (s->upper_bc[d] == GKYL_SPECIES_GK_SHEATH) {
      s->bc_sheath_up = gkyl_bc_sheath_gyrokinetic_new(d, GKYL_UPPER_EDGE, oneb->basis_on_dev.basis, 
        &s->upper_skin[d], &s->upper_ghost[d], &s->grid, cdim, 2.0*(s->info.charge/s->info.mass), oneb->use_gpu);
    }
    else {
      // Upper BC updater. Copy BCs by default.
      if (s->upper_bc[d] == GKYL_SPECIES_COPY) 
        bctype = GKYL_BC_COPY;
      else if (s->upper_bc[d] == GKYL_SPECIES_ABSORB) 
        bctype = GKYL_BC_ABSORB;
      else if (s->upper_bc[d] == GKYL_SPECIES_REFLECT) 
        bctype = GKYL_BC_REFLECT;
      else if (s->upper_bc[d] == GKYL_SPECIES_FIXED_FUNC) 
        bctype = GKYL_BC_FIXED_FUNC;

      s->bc_up[d] = gkyl_bc_basic_new(d, GKYL_UPPER_EDGE, bctype, oneb->basis_on_dev.basis,
        &s->upper_skin[d], &s->upper_ghost[d], s->f->ncomp, oneb->cdim, oneb->use_gpu);
    }
  }
}
  
void
gk_species_apply_ic(struct gkyl_gyrokinetic_oneb *oneb, struct gk_species *species, double t0)
{
  int poly_order = oneb->poly_order;
  if (species->info.is_maxwellian) {
    // Project n, udrift, and vt^2 based on input functions
    struct gkyl_array *m0 = mkarr(false, oneb->confBasis.num_basis, oneb->local_ext.volume);
    struct gkyl_array *udrift = mkarr(false, oneb->confBasis.num_basis, oneb->local_ext.volume);
    struct gkyl_array *vtsq = mkarr(false, oneb->confBasis.num_basis, oneb->local_ext.volume);
    gkyl_proj_on_basis *proj_m0 = gkyl_proj_on_basis_new(&oneb->grid, &oneb->confBasis,
      poly_order+1, 1, species->info.init_density, species->info.ctx_density);
    gkyl_proj_on_basis *proj_udrift = gkyl_proj_on_basis_new(&oneb->grid, &oneb->confBasis,
      poly_order+1, 1, species->info.init_upar, species->info.ctx_upar);
    gkyl_proj_on_basis *proj_vtsq = gkyl_proj_on_basis_new(&oneb->grid, &oneb->confBasis,
      poly_order+1, 1, species->info.init_temp, species->info.ctx_temp);

    gkyl_proj_on_basis_advance(proj_m0, 0.0, &oneb->local_ext, m0); 
    gkyl_proj_on_basis_advance(proj_udrift, 0.0, &oneb->local_ext, udrift);
    gkyl_proj_on_basis_advance(proj_vtsq, 0.0, &oneb->local_ext, vtsq);
    gkyl_array_scale(vtsq, 1/species->info.mass);

    // proj_maxwellian expects the primitive moments as a single array.
    struct gkyl_array *prim_moms = mkarr(false, 2*oneb->confBasis.num_basis, oneb->local_ext.volume);
    gkyl_array_set_offset(prim_moms, 1.0, udrift, 0*oneb->confBasis.num_basis);
    gkyl_array_set_offset(prim_moms, 1.0, vtsq  , 1*oneb->confBasis.num_basis);

    // Initialize Maxwellian projection object
    gkyl_proj_maxwellian_on_basis *proj_max = gkyl_proj_maxwellian_on_basis_new(&species->grid,
        &oneb->confBasis, &oneb->basis, poly_order+1, oneb->use_gpu);

    // If on GPUs, need to copy n, udrift, and vt^2 onto device
    struct gkyl_array *prim_moms_dev, *m0_dev;
    if (oneb->use_gpu) {
      prim_moms_dev = mkarr(oneb->use_gpu, 2*oneb->confBasis.num_basis, oneb->local_ext.volume);
      m0_dev = mkarr(oneb->use_gpu, oneb->confBasis.num_basis, oneb->local_ext.volume);

      gkyl_array_copy(prim_moms_dev, prim_moms);
      gkyl_array_copy(m0_dev, m0);
      gkyl_proj_gkmaxwellian_on_basis_prim_mom(proj_max, &species->local_ext, &oneb->local_ext, m0_dev, prim_moms_dev,
          oneb->gk_geom->bmag, oneb->gk_geom->bmag, species->info.mass, species->f);
    }
    else {
      gkyl_proj_gkmaxwellian_on_basis_prim_mom(proj_max, &species->local_ext, &oneb->local_ext, m0, prim_moms,
          oneb->gk_geom->bmag, oneb->gk_geom->bmag, species->info.mass, species->f);
    }
    // Now compute and scale the density to the desired density function based on input density from Maxwellian projection
    gk_species_moment_calc(&species->m0, species->local_ext, oneb->local_ext, species->f); 

    // Rescale projected density to desired input density function
    struct gkyl_array *m0mod = mkarr(oneb->use_gpu, oneb->confBasis.num_basis, oneb->local_ext.volume);
    struct gkyl_dg_bin_op_mem *mem;
    if (oneb->use_gpu) {
      mem = gkyl_dg_bin_op_mem_cu_dev_new(oneb->local.volume, oneb->confBasis.num_basis);
      gkyl_dg_div_op_range(mem, oneb->confBasis, 0, m0mod, 0, m0_dev, 0, species->m0.marr, &oneb->local);
    }
    else {
      mem = gkyl_dg_bin_op_mem_new(oneb->local.volume, oneb->confBasis.num_basis);
      gkyl_dg_div_op_range(mem, oneb->confBasis, 0, m0mod, 0, m0, 0, species->m0.marr, &oneb->local);
    }
    gkyl_dg_mul_conf_phase_op_range(&oneb->confBasis, &oneb->basis, species->f, 
        m0mod, species->f, &oneb->local_ext, &species->local_ext);

    // multiply final distribution function by Jacobian
    gkyl_dg_mul_conf_phase_op_range(&oneb->confBasis, &oneb->basis, species->f, 
        oneb->gk_geom->jacobgeo, species->f, &oneb->local_ext, &species->local_ext);

    // Free temporary variables and projection objects
    gkyl_array_release(m0);
    gkyl_array_release(udrift); 
    gkyl_array_release(vtsq);
    gkyl_array_release(prim_moms);
    if (oneb->use_gpu) {
      gkyl_array_release(m0_dev);
      gkyl_array_release(prim_moms_dev);      
    }
    gkyl_proj_on_basis_release(proj_m0);
    gkyl_proj_on_basis_release(proj_udrift);
    gkyl_proj_on_basis_release(proj_vtsq);
    gkyl_array_release(m0mod); 
    gkyl_dg_bin_op_mem_release(mem);
    gkyl_proj_maxwellian_on_basis_release(proj_max);
  } 
  else if (species->info.is_bimaxwellian) {
    // Project n, udrift, and vtpar^2 and vtperp^2 based on input functions
    struct gkyl_array *m0 = mkarr(false, oneb->confBasis.num_basis, oneb->local_ext.volume);
    struct gkyl_array *udrift = mkarr(false, oneb->confBasis.num_basis, oneb->local_ext.volume);
    struct gkyl_array *vtsqpar = mkarr(false, oneb->confBasis.num_basis, oneb->local_ext.volume);
    struct gkyl_array *vtsqperp = mkarr(false, oneb->confBasis.num_basis, oneb->local_ext.volume);
    gkyl_proj_on_basis *proj_m0 = gkyl_proj_on_basis_new(&oneb->grid, &oneb->confBasis,
      poly_order+1, 1, species->info.init_density, species->info.ctx_density);
    gkyl_proj_on_basis *proj_udrift = gkyl_proj_on_basis_new(&oneb->grid, &oneb->confBasis,
      poly_order+1, 1, species->info.init_upar, species->info.ctx_upar);
    gkyl_proj_on_basis *proj_vtsqpar = gkyl_proj_on_basis_new(&oneb->grid, &oneb->confBasis,
      poly_order+1, 1, species->info.init_temppar, species->info.ctx_temppar);
    gkyl_proj_on_basis *proj_vtsqperp = gkyl_proj_on_basis_new(&oneb->grid, &oneb->confBasis,
      poly_order+1, 1, species->info.init_tempperp, species->info.ctx_tempperp);

    gkyl_proj_on_basis_advance(proj_m0, 0.0, &oneb->local_ext, m0); 
    gkyl_proj_on_basis_advance(proj_udrift, 0.0, &oneb->local_ext, udrift);
    gkyl_proj_on_basis_advance(proj_vtsqpar, 0.0, &oneb->local_ext, vtsqpar);
    gkyl_proj_on_basis_advance(proj_vtsqperp, 0.0, &oneb->local_ext, vtsqperp);
    gkyl_array_scale(vtsqpar, 1.0/species->info.mass);
    gkyl_array_scale(vtsqperp, 1.0/species->info.mass);

    // proj_maxwellian expects the primitive moments as a single array.
    struct gkyl_array *prim_moms = mkarr(false, 4*oneb->confBasis.num_basis, oneb->local_ext.volume);
    gkyl_array_set_offset(prim_moms, 1.0, m0, 0*oneb->confBasis.num_basis);
    gkyl_array_set_offset(prim_moms, 1.0, udrift, 1*oneb->confBasis.num_basis);
    gkyl_array_set_offset(prim_moms, 1.0, vtsqpar , 2*oneb->confBasis.num_basis);
    gkyl_array_set_offset(prim_moms, 1.0, vtsqperp , 3*oneb->confBasis.num_basis);

    // Initialize Maxwellian projection object
    gkyl_proj_bimaxwellian_on_basis *proj_max = gkyl_proj_bimaxwellian_on_basis_new(&species->grid,
        &oneb->confBasis, &oneb->basis, poly_order+1, oneb->use_gpu);

    // If on GPUs, need to copy n, udrift, vtpar^2, vtperp^2 onto device
    struct gkyl_array *prim_moms_dev, *m0_dev;
    if (oneb->use_gpu) {
      prim_moms_dev = mkarr(oneb->use_gpu, 4*oneb->confBasis.num_basis, oneb->local_ext.volume);
      m0_dev = mkarr(oneb->use_gpu, oneb->confBasis.num_basis, oneb->local_ext.volume);

      gkyl_array_copy(prim_moms_dev, prim_moms);
      gkyl_array_copy(m0_dev, m0);

      gkyl_array_copy(prim_moms_dev, prim_moms);
      gkyl_proj_bimaxwellian_on_basis_gyrokinetic_prim_mom(proj_max, &species->local_ext, &oneb->local_ext, prim_moms_dev,
          oneb->gk_geom->bmag, oneb->gk_geom->bmag, species->info.mass, species->f);
    }
    else {
      gkyl_proj_bimaxwellian_on_basis_gyrokinetic_prim_mom(proj_max, &species->local_ext, &oneb->local_ext, prim_moms,
          oneb->gk_geom->bmag, oneb->gk_geom->bmag, species->info.mass, species->f);
    }
    // Now compute and scale the density to the desired density function based on input density from Maxwellian projection
    gk_species_moment_calc(&species->m0, species->local_ext, oneb->local_ext, species->f); 

    // Rescale projected density to desired input density function
    struct gkyl_array *m0mod = mkarr(oneb->use_gpu, oneb->confBasis.num_basis, oneb->local_ext.volume);
    struct gkyl_dg_bin_op_mem *mem;
    if (oneb->use_gpu) {
      mem = gkyl_dg_bin_op_mem_cu_dev_new(oneb->local.volume, oneb->confBasis.num_basis);
      gkyl_dg_div_op_range(mem, oneb->confBasis, 0, m0mod, 0, m0_dev, 0, species->m0.marr, &oneb->local);
    }
    else {
      mem = gkyl_dg_bin_op_mem_new(oneb->local.volume, oneb->confBasis.num_basis);
      gkyl_dg_div_op_range(mem, oneb->confBasis, 0, m0mod, 0, m0, 0, species->m0.marr, &oneb->local);
    }
    gkyl_dg_mul_conf_phase_op_range(&oneb->confBasis, &oneb->basis, species->f, 
        m0mod, species->f, &oneb->local_ext, &species->local_ext);

    // multiply final distribution function by Jacobian
    gkyl_dg_mul_conf_phase_op_range(&oneb->confBasis, &oneb->basis, species->f, 
        oneb->gk_geom->jacobgeo, species->f, &oneb->local_ext, &species->local_ext);

    // Free temporary variables and projection objects
    gkyl_array_release(m0);
    gkyl_array_release(udrift); 
    gkyl_array_release(vtsqpar);
    gkyl_array_release(vtsqperp);
    gkyl_array_release(prim_moms);
    if (oneb->use_gpu) {
      gkyl_array_release(m0_dev);
      gkyl_array_release(prim_moms_dev);      
    }
    gkyl_proj_on_basis_release(proj_m0);
    gkyl_proj_on_basis_release(proj_udrift);
    gkyl_proj_on_basis_release(proj_vtsqpar);
    gkyl_proj_on_basis_release(proj_vtsqperp);
    gkyl_array_release(m0mod); 
    gkyl_dg_bin_op_mem_release(mem);
    gkyl_proj_bimaxwellian_on_basis_release(proj_max);
  }
  else {
    gkyl_proj_on_basis *proj;
    proj = gkyl_proj_on_basis_new(&species->grid, &oneb->basis,
      poly_order+1, 1, species->info.init_dist, species->info.ctx_dist);

    gkyl_proj_on_basis_advance(proj, t0, &species->local, species->f_host);
    gkyl_proj_on_basis_release(proj);    

    if (oneb->use_gpu) // note: f_host is same as f when not on GPUs
      gkyl_array_copy(species->f, species->f_host);

  }

  // we are pre-computing source for now as it is time-independent
  if (species->source_id)
    gk_species_source_calc(oneb, species, t0);

  // copy contents of initial conditions into buffer if specific BCs require them
  // *only works in x dimension for now for cdim > 1*
  if (oneb->cdim>1) {
    gkyl_bc_basic_buffer_fixed_func(species->bc_lo[0], species->bc_buffer_lo_fixed, species->f);
    gkyl_bc_basic_buffer_fixed_func(species->bc_up[0], species->bc_buffer_up_fixed, species->f);
  }
}

// Compute the RHS for species update, returning maximum stable
// time-step.
double
gk_species_rhs(struct gkyl_gyrokinetic_oneb *oneb, struct gk_species *species,
  const struct gkyl_array *fin, struct gkyl_array *rhs)
{
  gkyl_array_set(species->phi, 1.0, oneb->field->phi_smooth);

  gkyl_array_clear(species->cflrate, 0.0);
  gkyl_array_clear(rhs, 0.0);

  // Compute the surface expansion of the phase space flux
  // Note: Each cell stores the *lower* surface expansions of the 
  // phase space flux, so local_ext range needed to index the output
  // values of alpha_surf even though we only loop over local ranges
  // to avoid evaluating quantities such as geometry in ghost cells
  // where they are not defined.
  gkyl_dg_calc_gyrokinetic_vars_alpha_surf(species->calc_gk_vars, 
    &oneb->local, &species->local, &species->local_ext, 
    species->phi, species->alpha_surf, species->sgn_alpha_surf, species->const_sgn_alpha);

  gkyl_dg_updater_gyrokinetic_advance(species->slvr, &species->local, 
    fin, species->cflrate, rhs);

  if (species->collision_id == GKYL_LBO_COLLISIONS)
    gk_species_lbo_rhs(oneb, species, &species->lbo, fin, rhs);
  else if (species->collision_id == GKYL_BGK_COLLISIONS)
    gk_species_bgk_rhs(oneb, species, &species->bgk, fin, rhs);
  
  if (species->has_diffusion)
    gkyl_dg_updater_diffusion_gyrokinetic_advance(species->diff_slvr, &species->local, 
      species->diffD, fin, species->cflrate, rhs);

  if (species->radiation_id == GKYL_GK_RADIATION)
    gk_species_radiation_rhs(oneb, species, &species->rad, fin, rhs);

  oneb->stat.nspecies_omega_cfl +=1;
  struct timespec tm = gkyl_wall_clock();
  gkyl_array_reduce_range(species->omegaCfl_ptr, species->cflrate, GKYL_MAX, &species->local);

  double omegaCfl_ho[1];
  if (oneb->use_gpu)
    gkyl_cu_memcpy(omegaCfl_ho, species->omegaCfl_ptr, sizeof(double), GKYL_CU_MEMCPY_D2H);
  else
    omegaCfl_ho[0] = species->omegaCfl_ptr[0];
  double omegaCfl = omegaCfl_ho[0];

  oneb->stat.species_omega_cfl_tm += gkyl_time_diff_now_sec(tm);
  
  return oneb->cfl/omegaCfl;
}

// Determine which directions are periodic and which directions are not periodic,
// and then apply boundary conditions for distribution function
void
gk_species_apply_bc(struct gkyl_gyrokinetic_oneb *oneb, const struct gk_species *species, struct gkyl_array *f)
{
  struct timespec wst = gkyl_wall_clock();
  
  int num_periodic_dir = oneb->num_periodic_dir, cdim = oneb->cdim;
  gkyl_comm_array_per_sync(species->comm, &species->local, &species->local_ext,
    num_periodic_dir, oneb->periodic_dirs, f); 
  
  int is_np_bc[3] = {1, 1, 1}; // flags to indicate if direction is periodic
  for (int d=0; d<num_periodic_dir; ++d)
    is_np_bc[oneb->periodic_dirs[d]] = 0;

  for (int d=0; d<cdim; ++d) {
    if (is_np_bc[d]) {

      switch (species->lower_bc[d]) {
        case GKYL_SPECIES_GK_SHEATH:
          gkyl_bc_sheath_gyrokinetic_advance(species->bc_sheath_lo, oneb->field->phi_smooth, 
            oneb->field->phi_wall_lo, f, &oneb->local);
          break;
        case GKYL_SPECIES_COPY:
        case GKYL_SPECIES_REFLECT:
        case GKYL_SPECIES_ABSORB:
          gkyl_bc_basic_advance(species->bc_lo[d], species->bc_buffer, f);
          break;
        case GKYL_SPECIES_FIXED_FUNC:
          gkyl_bc_basic_advance(species->bc_lo[d], species->bc_buffer_lo_fixed, f);
          break;
        case GKYL_SPECIES_NO_SLIP:
        case GKYL_SPECIES_WEDGE:
          assert(false);
          break;
        case GKYL_SPECIES_ZERO_FLUX:
          break; // do nothing, BCs already applied in hyper_dg loop by not updating flux
        default:
          break;
      }

      switch (species->upper_bc[d]) {
        case GKYL_SPECIES_GK_SHEATH:
          gkyl_bc_sheath_gyrokinetic_advance(species->bc_sheath_up, oneb->field->phi_smooth, 
            oneb->field->phi_wall_up, f, &oneb->local);
          break;
        case GKYL_SPECIES_COPY:
        case GKYL_SPECIES_REFLECT:
        case GKYL_SPECIES_ABSORB:
          gkyl_bc_basic_advance(species->bc_up[d], species->bc_buffer, f);
          break;
        case GKYL_SPECIES_FIXED_FUNC:
          gkyl_bc_basic_advance(species->bc_up[d], species->bc_buffer_up_fixed, f);
          break;
        case GKYL_SPECIES_NO_SLIP:
        case GKYL_SPECIES_WEDGE:
          assert(false);
          break;
        case GKYL_SPECIES_ZERO_FLUX:
          break; // do nothing, BCs already applied in hyper_dg loop by not updating flux
        default:
          break;
      }      
    }
  }

  gkyl_comm_array_sync(species->comm, &species->local, &species->local_ext, f);

  oneb->stat.species_bc_tm += gkyl_time_diff_now_sec(wst);
}

void
gk_species_coll_tm(struct gkyl_gyrokinetic_oneb *oneb)
{
  for (int i=0; i<oneb->num_species; ++i) {
    if (oneb->species[i].collision_id == GKYL_LBO_COLLISIONS) {
      struct gkyl_dg_updater_lbo_gyrokinetic_tm tm =
        gkyl_dg_updater_lbo_gyrokinetic_get_tm(oneb->species[i].lbo.coll_slvr);
      oneb->stat.species_lbo_coll_diff_tm[i] = tm.diff_tm;
      oneb->stat.species_lbo_coll_drag_tm[i] = tm.drag_tm;
    }
  }
}

void
gk_species_tm(struct gkyl_gyrokinetic_oneb *oneb)
{
  oneb->stat.species_rhs_tm = 0.0;
  for (int i=0; i<oneb->num_species; ++i) {
    struct gkyl_dg_updater_gyrokinetic_tm tm =
      gkyl_dg_updater_gyrokinetic_get_tm(oneb->species[i].slvr);
    oneb->stat.species_rhs_tm += tm.gyrokinetic_tm;
  }
}

// release resources for species
void
gk_species_release(const struct gkyl_gyrokinetic_oneb* oneb, const struct gk_species *s)
{
  // release various arrays
  gkyl_array_release(s->f);
  gkyl_array_release(s->f1);
  gkyl_array_release(s->fnew);
  gkyl_array_release(s->cflrate);
  gkyl_array_release(s->bc_buffer);
  if (oneb->cdim > 1) {
    gkyl_array_release(s->bc_buffer_lo_fixed);
    gkyl_array_release(s->bc_buffer_up_fixed);
  }

  gkyl_comm_release(s->comm);

  if (oneb->use_gpu)
    gkyl_array_release(s->f_host);

  gkyl_array_release(s->alpha_surf);
  gkyl_array_release(s->sgn_alpha_surf);
  gkyl_array_release(s->const_sgn_alpha);
  gkyl_array_release(s->phi);
  gkyl_array_release(s->apar);
  gkyl_array_release(s->apardot);

  gkyl_dg_calc_gyrokinetic_vars_release(s->calc_gk_vars);

  // release equation object and solver
  gkyl_dg_eqn_release(s->eqn_gyrokinetic);
  gkyl_dg_updater_gyrokinetic_release(s->slvr);

  // release moment data
  gk_species_moment_release(oneb, &s->m0);
  for (int i=0; i<s->info.num_diag_moments; ++i)
    gk_species_moment_release(oneb, &s->moms[i]);
  gkyl_free(s->moms);
  gk_species_moment_release(oneb, &s->integ_moms); 
  gkyl_dynvec_release(s->integ_diag);

  if (s->source_id) {
    gk_species_source_release(oneb, &s->src);
  }

  if (s->collision_id == GKYL_LBO_COLLISIONS)
    gk_species_lbo_release(oneb, &s->lbo);
  else if (s->collision_id == GKYL_BGK_COLLISIONS)
    gk_species_bgk_release(oneb, &s->bgk);

  if (s->radiation_id == GKYL_GK_RADIATION) 
    gk_species_radiation_release(oneb, &s->rad);

  // Copy BCs are allocated by default. Need to free.
  for (int d=0; d<oneb->cdim; ++d) {
    if (s->lower_bc[d] == GKYL_SPECIES_GK_SHEATH) 
      gkyl_bc_sheath_gyrokinetic_release(s->bc_sheath_lo);
    else 
      gkyl_bc_basic_release(s->bc_lo[d]);
    
    if (s->upper_bc[d] == GKYL_SPECIES_GK_SHEATH) 
      gkyl_bc_sheath_gyrokinetic_release(s->bc_sheath_up);
    else 
      gkyl_bc_basic_release(s->bc_up[d]);
  }
  
  if (oneb->use_gpu) {
    gkyl_cu_free(s->omegaCfl_ptr);
    gkyl_cu_free(s->red_integ_diag);
  }
  else {
    gkyl_free(s->omegaCfl_ptr);
  }
}
