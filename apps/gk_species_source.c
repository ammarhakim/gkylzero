  ...
  struct gk_source *src)
{
  ...
  gk_species_projection_init(app, s, s->info.source.projection, &src->proj_source);
}
...
{
  gk_species_projection_calc(app, s, &src->proj_source, src->source, tm);
}
...
{
  gk_species_projection_release(app, &src->proj_source);
