...
gkyl_gyrokinetic_app_omegaH_init(gkyl_gyrokinetic_app *app)
{
  ...
  // where
  //   - n_{s0} is either a reference or average density.
  ...
  gkyl_array_accumulate_offset_range(perpfac, pow(kx_min,2), app->field->epsilon, 0*app->confBasis.num_basis, &app->local);
  if (app->cdim > 2) {
