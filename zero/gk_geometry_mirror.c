...
gk_geometry_mirror_init(struct gkyl_gk_geometry_inp *geometry_inp)
{
  ...
  gkyl_mirror_geo_calc(up, &nrange, geo, &ginp, geometry_inp->position_map);
  // calculate mapc2p in cylindrical coords at interior nodes for
  // calculating geo quantity volume expansions 
  gkyl_mirror_geo_calc_interior(up, &nrange_quad, dzc, geo, &ginp, geometry_inp->position_map);
  // calculate mapc2p in cylindrical coords at surfaces
  for (int dir = 0; dir <up->grid.ndim; dir++)
    gkyl_mirror_geo_calc_surface(up, dir, &nrange_quad_surf[dir], dzc, geo, &ginp, geometry_inp->position_map);
  ...
  gkyl_calc_metric_advance_mirror_interior(mcalc, &nrange_quad, up->geo_int.mc2p_nodal_fd, up->geo_int.ddtheta_nodal, up->geo_int.bmag_nodal, dzc, up->geo_int.g_ij, up->geo_int.dxdz, up->geo_int.dzdx, up->geo_int.dualmag, up->geo_int.normals, up->geo_int.jacobgeo, up->geo_int.bcart, &up->local);
  ...
  for (int dir = 0; dir <up->grid.ndim; dir++) {
    gkyl_calc_metric_advance_mirror_surface(mcalc, dir, &nrange_quad_surf[dir], up->geo_surf[dir].mc2p_nodal_fd, up->geo_surf[dir].ddtheta_nodal, up->geo_surf[dir].bmag_nodal, dzc,
