...
gkyl_gk_geometry_mirror_advance(struct gkyl_gk_geometry_inp *geometry_inp)
{
  ...
  ginp.nonuniform_mapping_fraction = geometry_inp->mapc2fa_inp.numerical_mapping_fraction;
  ginp.cgrid = up->grid;
  ...
  gkyl_mirror_geo_calc(up, &nrange, dzc, geo, &ginp, mc2p_nodal_fd, mc2p_nodal, up->mc2p,
      nonuniform_geom, map_c2fa_nodal_fd, map_c2fa_nodal, up->c2fa, &geometry_inp->mapc2fa_inp);
  // calculate bmag
  ...
  gkyl_calc_metric_advance_mirror(mcalc, &nrange, mc2p_nodal_fd, bmag_nodal, dzc, up->g_ij, up->dxdz, up->dzdx, up->normals, up->jacobgeo, up->bcart, &up->local);
