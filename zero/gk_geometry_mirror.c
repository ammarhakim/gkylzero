...
void
gk_geometry_mirror_surf_alloc(struct gk_geometry* gk_geom, int dir, struct gkyl_range nrange)
{
...
void
gk_geometry_mirror_surf_release_nodal(struct gk_geometry* gk_geom, int dir)
{
...
void
gk_geometry_mirror_surf_calc_expansions(struct gk_geometry* gk_geom, int dir, 
...
gk_geometry_mirror_init(struct gkyl_gk_geometry_inp *geometry_inp)
{
  ...
  for (int dir=0; dir<up->grid.ndim; ++dir) {
    gk_geometry_mirror_surf_alloc(up, dir, nrange_quad_surf[dir]);
  }
  ...
  gkyl_mirror_geo_calc(up, &nrange, geo, &ginp, mc2p_nodal, up->mc2p, 
  ...
  gkyl_mirror_geo_calc_interior(up, &nrange_quad, dzc, geo, &ginp, mc2p_nodal_quad, mc2p_quad, 
    mc2p_nodal_fd, ddtheta_nodal, geometry_inp->position_map);
  // calculate mapc2p in cylindrical coords at surfaces
  for (int dir = 0; dir <up->grid.ndim; dir++) {
    gkyl_mirror_geo_calc_surface(up, dir, &nrange_quad_surf[dir], dzc, geo, &ginp, up->geo_surf
  ...
  gkyl_calc_metric_advance_rz_interior(mcalc, &nrange_quad, mc2p_nodal_fd, ddtheta_nodal, 
    bmag_nodal, dzc, up->g_ij, up->dxdz, up->dzdx, up->dualmag, up->normals, up->jacobgeo, 
    up->bcart, &up->local);
  gkyl_array_copy(up->jacobgeo_ghost, up->jacobgeo);
  // Calculate neutral metrics
  gkyl_calc_metric_advance_rz_neut(mcalc, &nrange, mc2p_nodal_fd, ddtheta_nodal, dzc, 
    up->g_ij_neut, up->gij_neut, &up->local);
  ...
  for (int dir = 0; dir <up->grid.ndim; dir++)
    gk_geometry_mirror_surf_calc_expansions(up, dir, nrange_quad_surf[dir]);
  ...
  for (int dir=0; dir<up->grid.ndim; ++dir)
    gk_geometry_mirror_surf_release_nodal(up, dir);
