...
gk_geometry_mirror_init(struct gkyl_gk_geometry_inp *geometry_inp)
{
  ...
  struct gkyl_array* ddalpha_nodal = gkyl_array_new(GKYL_DOUBLE, 3, nrange.volume);
  ...
  // calculate mapc2p
  gkyl_mirror_geo_calc(up, &nrange, dzc, geo, &ginp, mc2p_nodal_fd, mc2p_nodal, up->mc2p, ddalpha_nodal, ddtheta_nodal,
  ...
  gkyl_calc_metric_advance_mirror(mcalc, &nrange, mc2p_nodal_fd, ddalpha_nodal, ddtheta_nodal, bmag_nodal, 
    dzc, up->g_ij, up->dxdz, up->dzdx, up->dualmag, up->normals, up->jacobgeo, up->bcart, &up->local);
  ...
  gkyl_array_release(ddalpha_nodal);
