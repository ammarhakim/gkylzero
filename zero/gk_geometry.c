...
gkyl_gk_geometry* gkyl_gk_geometry_new(const struct gkyl_rect_grid* grid, const struct gkyl_range *range, const struct gkyl_range* range_ext, const struct gkyl_basis* basis, evalf_t mapc2p_func, evalf_t bmag_func){
  ...
  up->grid = grid;
  up->mapc2p_func = mapc2p_func;
  up->bmag_func = bmag_func;
...
void gkyl_gk_geometry_advance(struct gkyl_gk_geometry* up){
  ...
  //First just do bmag
  gkyl_eval_on_nodes *eval_bmag= gkyl_eval_on_nodes_new(up->grid, up->basis, 1, up->bmag_func, 0);
        ...
        for(int ip_delta = 0; ip_delta < ip_delta_max; ip_delta++){
          ...
          for (int it=up->nrange->lower[TH_IDX]; it<=up->nrange->upper[TH_IDX]; ++it) {
            ...
            for(int it_delta = 0; it_delta < it_delta_max; it_delta++){
              ...
              double *mc2p_fd_n = gkyl_array_fetch(up->mc2p_nodal_fd, gkyl_range_idx(up->nrange, cidx));
              double *mc2p_n = gkyl_array_fetch(up->mc2p_nodal, gkyl_range_idx(up->nrange, cidx));

              double xyz[3] = {psi_curr, alpha_curr, theta_curr};
              double XYZ[3] = {0.};
              up->mapc2p_func(0.0, xyz, XYZ, 0);
