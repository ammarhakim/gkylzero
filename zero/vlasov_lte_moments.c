  ...
  const struct gkyl_array *fin, struct gkyl_array *density_out)
{
  ...
  if (lte_moms->model_id == GKYL_MODEL_SR) {
    ...
    while (gkyl_range_iter_next(&biter)) {
      ...
      // Attemp to correct the velocity, if it still fails, pass assertion
      if (V_drift_local[0]/(pow(sqrt(2),vdim)) > 1.0) {
        ...
        V_drift_local[0] = (pow(sqrt(2),vdim))*m1i_local[0]/m0_local[0];

        // If we still cannot correct 
        if (V_drift_local[0] > 0) {
          V_drift_local[0] = 0;
        }
      }
    }
