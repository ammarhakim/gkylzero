...
__global__ void
gkyl_prim_lbo_calc_advance_cu_ker(gkyl_prim_lbo_calc* calc, struct gkyl_nmat *As, struct gkyl_nmat *xs, struct gkyl_basis cbasis, struct gkyl_range conf_rng, const struct gkyl_array* m0, const struct gkyl_array* m1, const struct gkyl_array* m2, const struct gkyl_array* cM, const struct gkyl_array* cE, struct gkyl_array* uout, struct gkyl_array* vtSqout)
{
      ...
      linc2 += gridDim.y*blockDim.y)
  {
    ...
    gkyl_sub_range_inv_idx(&conf_rng, ac1*linc2, idx);
  ...
  struct gkyl_array* uout, struct gkyl_array* vtSqout)
{
  ...
  gkyl_get_array_range_kernel_launch_dims(&dimGrid, &dimBlock, conf_rng, uout->ncomp);
  int nc = conf_rng.ndim;
  int vdim = phase_rng.ndim - nc;
  int N = nc*(vdim + 1);      
  ...
  gkyl_prim_lbo_calc_advance_cu_ker<<<dimGrid, dimBlock>>>(calc, A_d->on_dev, x_d->on_dev, cbasis, conf_rng, m0->on_dev, m1->on_dev, m2->on_dev, cM->on_dev, cE->on_dev, uout->on_dev, vtSqout->on_dev);
