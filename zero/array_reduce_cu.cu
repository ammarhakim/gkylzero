...
template <unsigned int BLOCKSIZE> 
__global__ void
arrayMaxSumComp_blockRedAtomic_cub(const struct gkyl_array* inp, double* out)
{
  unsigned long linc = blockIdx.x*blockDim.x + threadIdx.x;
...
template <unsigned int BLOCKSIZE>
__global__ void
arrayMaxSumComp_range_blockRedAtomic_cub(const struct gkyl_array* inp, const struct gkyl_range range, double* out)
{
  unsigned long linc = blockIdx.x*blockDim.x + threadIdx.x;
