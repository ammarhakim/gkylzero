...
get_prim_id(const char *prim_nm)
{
  ...
  else if (strcmp(prim_nm, "prim") == 0) { // combined (u_par = u_i . b_i, vth_GK^2 = (M2/M0 - upar^2))
    prim_idx = prim;
  }
  else {
    prim_idx = BAD;
...
gkyl_dg_prim_vars_transform_vlasov_gk_set_auxfields_cu_kernel(const struct gkyl_dg_prim_vars_type *pvt, 
...
void
gkyl_dg_prim_vars_transform_vlasov_gk_set_auxfields_cu(const struct gkyl_dg_prim_vars_type *pvt, 
  struct gkyl_dg_prim_vars_auxfields auxin)
{
  gkyl_dg_prim_vars_transform_vlasov_gk_set_auxfields_cu_kernel<<<1,1>>>(pvt, auxin.b_i->on_dev);
}

__global__
static void
set_cu_ptrs(struct dg_prim_vars_type_transform_vlasov_gk* pvt, int prim_id, enum gkyl_basis_type b_type, int vdim,
  int poly_order, int tblidx)
{
  ...
  switch (prim_id) {
    case u_par_i:
      pvt->pvt.kernel = dg_prim_vars_transform_vlasov_gk_u_par_i_kernels[tblidx].kernels[poly_order];
      break;

    case u_par:
      pvt->pvt.kernel = dg_prim_vars_transform_vlasov_gk_u_par_kernels[tblidx].kernels[poly_order];
      break;

    case prim:
      pvt->pvt.kernel = dg_prim_vars_transform_vlasov_gk_kernels[tblidx].kernels[poly_order];
      break;
  ...
  const struct gkyl_basis* pbasis, const struct gkyl_range* conf_range, const char *prim_nm)
{
  ...
  struct dg_prim_vars_type_transform_vlasov_gk *pvt_cu = (struct dg_prim_vars_type_transform_vlasov_gk*)
    gkyl_cu_malloc(sizeof(struct dg_prim_vars_type_transform_vlasov_gk));
  gkyl_cu_memcpy(pvt_cu, pvt, sizeof(struct dg_prim_vars_type_transform_vlasov_gk), GKYL_CU_MEMCPY_H2D);
