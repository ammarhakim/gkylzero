 ...
 const struct gkyl_array *in1, double b, const struct gkyl_array *in2)
{
  switch (op) {
    case GKYL_ABS:
      gkyl_array_comp_op_abs_cu_kernel<<<out->nblocks, out->nthreads>>>(out->on_dev, a, in1->on_dev, b, in2->on_dev);
      return;
    case GKYL_INV:
      gkyl_array_comp_op_inv_cu_kernel<<<out->nblocks, out->nthreads>>>(out->on_dev, a, in1->on_dev, b, in2->on_dev);
      return;
    case GKYL_DIV:
 ...
 const struct gkyl_array *in1, double b, const struct gkyl_array *in2, const struct gkyl_range *range)
{
  ...
  switch (op) {
    case GKYL_ABS:
      gkyl_array_comp_op_range_abs_cu_kernel<<<dimGrid, dimBlock>>>(out->on_dev, a, in1->on_dev, b, in2->on_dev, *range);
      return;
    case GKYL_INV:
      gkyl_array_comp_op_range_inv_cu_kernel<<<dimGrid, dimBlock>>>(out->on_dev, a, in1->on_dev, b, in2->on_dev, *range);
      return;
    case GKYL_DIV:
      assert(out->ncomp == in2->ncomp);
      assert(out->size == in2->size);
