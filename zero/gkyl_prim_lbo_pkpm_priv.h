...
static const gkyl_prim_lbo_pkpm_self_kern_list ten_self_prim_kernels[] = {
  // 1x kernels
  { NULL, NULL, pkpm_self_prim_moments_1x1v_tensor_p2 }, // 0
  // 2x kernels
  { NULL, NULL, pkpm_self_prim_moments_2x1v_tensor_p2 }, // 1
  // 3x kernels
  { NULL, NULL, NULL }, // 2
};
  ...
  const int* idx, const double *moms, const double *boundary_corrections)
{
  ...
  return prim_pkpm->self_prim(A, rhs, moms, boundary_corrections);
}
