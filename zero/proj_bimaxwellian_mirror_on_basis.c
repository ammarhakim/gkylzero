...
#include <gkyl_proj_bimaxwellian_mirror_on_basis_priv.h>
...
gkyl_proj_bimaxwellian_mirror_on_basis*
gkyl_proj_bimaxwellian_mirror_on_basis_new(
  const struct gkyl_rect_grid *grid,
  const struct gkyl_basis *conf_basis, const struct gkyl_basis *phase_basis,
  int num_quad, bool use_gpu)
{
  gkyl_proj_bimaxwellian_mirror_on_basis *up = gkyl_malloc(sizeof(gkyl_proj_bimaxwellian_mirror_on_basis));
...
static void
proj_on_basis(const gkyl_proj_bimaxwellian_mirror_on_basis *up, const struct gkyl_array *fun_at_ords, double* f)
{
...
void
gkyl_proj_bimaxwellian_mirror_on_basis_gyrokinetic_lab_mom(const gkyl_proj_bimaxwellian_mirror_on_basis *up,
  const struct gkyl_range *phase_rng, const struct gkyl_range *conf_rng,
  const struct gkyl_array *moms, const struct gk_geometry *gk_geom,
  double mass, struct gkyl_array *fmax)
{

#ifdef GKYL_HAVE_CUDA
  if (up->use_gpu)
    return gkyl_proj_bimaxwellian_mirror_on_basis_gyrokinetic_lab_mom_cu(up, phase_rng, conf_rng, moms, 
  ...
  double exp_amp[tot_conf_quad], upar[tot_conf_quad], vtparsq[tot_conf_quad], vtperpsq[tot_conf_quad];
  ...
  while (gkyl_range_iter_next(&conf_iter)) {
    ...
    for (int n=0; n<tot_conf_quad; ++n) {
      ...
      double m2par_n = 0.0; // parallel kinetic energy density.
      double m2perp_n = 0.0; // perpendicular kinetic energy density.
    ...
    while (gkyl_range_iter_next(&vel_iter)) {
      ...
      while (gkyl_range_iter_next(&qiter)) {
        ...
        fq[0] = fJacB_floor+exp_amp[cqidx]*exp(-efact);
      }
  ...
  double mass, struct gkyl_array *fmax)
{
...
#ifdef GKYL_HAVE_CUDA
  if (up->use_gpu)
    return gkyl_proj_bimaxwellian_mirror_on_basis_gyrokinetic_prim_mom_cu(up, phase_rng, conf_rng, prim_moms,
                                                                   bmag, jacob_tot, mass, fmax);
#endif
  struct gkyl_array *bmag = gk_geom->bmag;
  struct gkyl_array *jacob_tot = gk_geom->jacobtot;
  ...
  bmag_ctx->range = &gk_geom->prange;
  bmag_ctx->grid = &gk_geom->pgrid;
  ...
  double xc[GKYL_MAX_DIM], xmu[GKYL_MAX_DIM] = {0.};
  double expamp_o[tot_conf_quad], upar_o[tot_conf_quad], vtparsq_o[tot_conf_quad], vtperpsq_o[tot_conf_quad];
...
void
gkyl_proj_bimaxwellian_mirror_on_basis_release(gkyl_proj_bimaxwellian_mirror_on_basis* up)
{
#ifdef GKYL_HAVE_CUDA
  if (up->use_gpu)
