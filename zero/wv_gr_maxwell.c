...
gkyl_gr_maxwell_max_abs_speed(double light_speed, const double q[14])
{
  double spatial_det = q[8];
  double lapse = q[9];

  bool in_excision_region = false;
  if (q[13] < pow(10.0, -8.0)) {
...
static void
gkyl_gr_maxwell_flux(double light_speed, double e_fact, double b_fact, const double q[14], double flux[14])
{
  ...
  if (!in_excision_region) {
    ...
    flux[1] = (1.0 / ((lapse * lapse) * spatial_det)) * (light_speed * light_speed) * (Bz - (shift_z / lapse));
    flux[2] = -(1.0 / ((lapse * lapse) * spatial_det)) * (light_speed * light_speed) * (By - (shift_y / lapse));
    flux[3] = b_fact * psi;
    flux[4] = -(Ez - (shift_z / lapse));
    flux[5] = (Ey - (shift_y / lapse));
    flux[6] = e_fact * (Ex - (shift_x / lapse));
    flux[7] = (1.0 / ((lapse * lapse) * spatial_det)) * b_fact * (light_speed * light_speed) * (Bx - (shift_x / lapse));
...
{
  const double *w0 = &waves[0 * 14], *w1 = &waves[1 * 14], *w2 = &waves[2 * 14];
  const double *w3 = &waves[3 * 14], *w4 = &waves[4 * 14], *w5 = &waves[5 * 14];
...
gkyl_wv_gr_maxwell_inew(const struct gkyl_wv_gr_maxwell_inp* inp)
{
  ...
  gr_maxwell->eqn.num_equations = 14;
