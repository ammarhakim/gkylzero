...
gkyl_pghost_cdm_set_cu_ker_ptrs(struct gkyl_phase_ghost_conf_div_mul_kernels *kernels,
...
void
pghost_cdm_choose_shift_kernel_cu(struct gkyl_phase_ghost_conf_div_mul_kernels *kernels,
  struct gkyl_basis *cbasis, struct gkyl_basis *pbasis)
{
...
__global__ static void
gkyl_phase_ghost_conf_div_mul_advance_cu_ker(struct gkyl_phase_ghost_conf_div_mul_kernels *kers
  ...
  const struct gkyl_range phase_ghost_r, struct gkyl_array *jac, struct gkyl_array *jf)
{
  ...
  int cdim = skin_r.ndim; // Problem dimensionality.
  ...
  for (unsigned long linc = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    gkyl_copy_int_arr(dim, gidx, sidx);
    ...
    kernels->conf_inv_op(jacghost_c, jacghost_inv_c);
    ...
    up->conf_basis->flip_odd_sign(up->dir, jacskin_c, jacskin_flipped_c);
    ...
    kernels->conf_phase_mul_op(jacghost_inv_c, jf_c, jf_c);
    kernels->conf_phase_mul_op(jacskin_flipped_c, jf_c, jf_c);
  }
}
  ...
  const struct gkyl_range *phase_ghost_r, struct gkyl_array *GKYL_RESTRICT jac, struct gkyl_array *GKYL_RESTRICT jf)
{
  ...
  int nblocks = up->phase_ghost_r->nblocks, nthreads = up->phase_ghost_r->nthreads; // CUDA grid configuration.

  // Launch the CUDA kernel to advance the ghost-to-skin update.
  phase_ghost_conf_div_mul_advance_cu_ker<<<nblocks, nthreads>>>(up->kernels, dir, edge,
    conf_basis, *up->conf_skin_r, *up->conf_ghost_r, *up->phase_ghost_r, jac->on_dev, jf->on_dev);
}
