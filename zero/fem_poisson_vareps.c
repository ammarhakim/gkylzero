...
#include <gkyl_fem_poisson_vareps_priv.h>

gkyl_fem_poisson_vareps*
gkyl_fem_poisson_vareps_new(const struct gkyl_rect_grid *grid, const struct gkyl_basis basis,
  struct gkyl_poisson_bc *bcs, struct gkyl_array *epsilon, bool use_gpu)
{

  gkyl_fem_poisson_vareps *up = gkyl_malloc(sizeof(gkyl_fem_poisson_vareps));

  up->kernels = gkyl_malloc(sizeof(struct gkyl_fem_poisson_vareps_kernels));
#ifdef GKYL_HAVE_CUDA
  if (use_gpu)
    up->kernels_cu = gkyl_cu_malloc(sizeof(struct gkyl_fem_poisson_vareps_kernels));
  else
  ...
  up->numnodes_global = fem_poisson_vareps_global_num_nodes(up->ndim, up->poly_order, basis.b_type, up->num_cells, up->isdirperiodic);
  ...
  fem_poisson_vareps_choose_local2global_kernels(&basis, up->isdirperiodic, up->kernels->l2g);

  // Select lhs kernels:
  fem_poisson_vareps_choose_lhs_kernels(&basis, bcs, up->kernels->lhsker);

  // Select rhs src kernels:
  fem_poisson_vareps_choose_src_kernels(&basis, bcs, up->kernels->srcker);

  // Select sol kernel:
  up->kernels->solker = fem_poisson_vareps_choose_sol_kernels(&basis);

#ifdef GKYL_HAVE_CUDA
  if (up->use_gpu)
    fem_poisson_vareps_choose_kernels_cu(&basis, bcs,  up->isdirperiodic, up->kernels_cu);
#endif
  ...
  while (gkyl_range_iter_next(&up->solve_iter)) {
    ...
    double *epsilon_p = gkyl_array_fetch(epsilon, linidx);
    ...
    up->kernels->lhsker[keri](epsilon_p, up->dx, up->bcvals, up->globalidx, tri);
  }
#ifdef GKYL_HAVE_CUDA
  if (up->use_gpu) {
...
void
gkyl_fem_poisson_vareps_set_rhs(gkyl_fem_poisson_vareps* up, struct gkyl_array *rhsin)
{
  ...
  if (up->use_gpu) {
    ...
    gkyl_fem_poisson_vareps_set_rhs_cu(up, rhsin);
    return;
  }
#endif
  ...
  while (gkyl_range_iter_next(&up->solve_iter)) {
    ...
    double *epsilon_p = gkyl_array_fetch(up->epsilon, linidx);
    ...
    keri = idx_to_inloup_ker(up->ndim, up->num_cells, up->solve_iter.idx);
    up->kernels->srcker[keri](epsilon_p, up->dx, rhsin_p, up->bcvals, up->globalidx, brhs_p);
  }
...
void
gkyl_fem_poisson_vareps_solve(gkyl_fem_poisson_vareps* up, struct gkyl_array *phiout) {
#ifdef GKYL_HAVE_CUDA
  if (up->use_gpu) {
    assert(gkyl_array_is_cu_dev(phiout));
    gkyl_fem_poisson_vareps_solve_cu(up, phiout);
    return;
  }
#endif
...
void gkyl_fem_poisson_vareps_release(gkyl_fem_poisson_vareps *up)
{
  if (up->isdomperiodic) {
