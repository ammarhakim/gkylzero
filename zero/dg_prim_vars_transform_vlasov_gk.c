  ...
  const struct gkyl_range* conf_range, const char *prim_nm, bool use_gpu)
{
  ...
  else if (strcmp(prim_nm, "u_par") == 0) { // projection of Vlasov u_i, vth^2 to upar (u_i . b_i) and vth_GK^2 (M2/M0 - upar^2)
    if (vdim == 2 ) vdim += 1; // convert vdim_gk to vdim_vlasov
    pvt->pvt.vdim = vdim;
    pvt->pvt.kernel = dg_prim_vars_transform_vlasov_gk_u_par_kernels[cv_index[cdim].vdim[vdim]].kernels[poly_order];
    pvt->pvt.num_mom = 2; 
  }
