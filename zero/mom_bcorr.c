  ...
  const struct gkyl_array *fIn, struct gkyl_array *out)
{
  ...
  // config-space cell the inner loop walks over the edges of velocity space
  ...
  while (gkyl_range_iter_next(&conf_iter)) {
    ...
    for (int d=0; d<phase_rng.ndim - conf_rng.ndim; ++d) {
      ...
      while (gkyl_range_iter_next(&vel_iter)) {
        ...
        // pidx used to store identifiers for which dimension and edge of velocity space are being used for the kernel
