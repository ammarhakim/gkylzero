...
gk_geometry_tok_init(struct gkyl_gk_geometry_inp *geometry_inp)
{
  ...
  gkyl_tok_geo_calc(up, &nrange, dzc, geo, &ginp, mc2p_nodal_fd, mc2p_nodal, up->mc2p, 
    ddalpha_nodal, ddtheta_nodal, map_mc2nu_nodal, up->mc2nu_pos, geometry_inp->position_map);
  // calculate bmag
  ...
  gkyl_calc_metric_advance_rz(mcalc, &nrange, mc2p_nodal_fd, ddalpha_nodal, ddtheta_nodal, bmag_nodal, dzc, 
    up->g_ij, up->dxdz, up->dzdx, up->dualmag, up->normals, up->jacobgeo, up->bcart, &up->local);
