        ...
        for(int ip_delta = 0; ip_delta < ip_delta_max; ip_delta++){
          for (int it=nrange->lower[TH_IDX]; it<=nrange->upper[TH_IDX]; ++it) {
            ...
            for(int it_delta = 0; it_delta < it_delta_max; it_delta++){
              ...
              double dxdz[3][3];
              dxdz[0][0] = -(mc2p_xyz_n[3 +X_IDX] -   mc2p_xyz_n[6+X_IDX])/dzc[0];
              //printf("dxdz[0][0] = %g\n",dxdz[0][0]);
              dxdz[0][1] = -(mc2p_xyz_n[9 +X_IDX] -  mc2p_xyz_n[12+X_IDX])/dzc[1];
              dxdz[0][2] = -(mc2p_xyz_n[15 +X_IDX] - mc2p_xyz_n[18+X_IDX])/dzc[2];
              dxdz[1][0] = -(mc2p_xyz_n[3 +Y_IDX] -   mc2p_xyz_n[6+Y_IDX])/dzc[0];
              dxdz[1][1] = -(mc2p_xyz_n[9 +Y_IDX] -  mc2p_xyz_n[12+Y_IDX])/dzc[1];
              dxdz[1][2] = -(mc2p_xyz_n[15 +Y_IDX] - mc2p_xyz_n[18+Y_IDX])/dzc[2];
              dxdz[2][0] = -(mc2p_xyz_n[3 +Zc_IDX] -   mc2p_xyz_n[6+Zc_IDX])/dzc[0];
              dxdz[2][1] = -(mc2p_xyz_n[9 +Zc_IDX] -  mc2p_xyz_n[12+Zc_IDX])/dzc[1];
              dxdz[2][2] = -(mc2p_xyz_n[15 +Zc_IDX] - mc2p_xyz_n[18+Zc_IDX])/dzc[2];
