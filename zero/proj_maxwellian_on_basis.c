  ...
  int num_quad)
{
  ...
  up->basis_at_ords = gkyl_array_new(GKYL_DOUBLE, phase_basis->num_basis, tot_quad);
  for (int n=0; n<tot_quad; ++n)
    phase_basis->eval(gkyl_array_fetch(up->ordinates, n),
...
void
gkyl_proj_maxwellian_on_basis_advance(const gkyl_proj_maxwellian_on_basis *up,
  const struct gkyl_range *update_range, struct gkyl_array *arr)
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    for (int i=0; i<tot_quad; ++i) {
      ...
      // compute distribution function at ordinates
    }
