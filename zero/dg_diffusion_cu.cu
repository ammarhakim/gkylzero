...
dg_diffusion_set_cu_dev_ptrs(struct dg_diffusion *diffusion, enum gkyl_basis_type b_type, int cdim, int poly_order, enum gkyl_diffusion_id diffusion_id, int diff_order, int diffdirs_linidx)
{
  ...
  const struct gkyl_range *conf_range)
{
  ...
  dg_diffusion_set_cu_dev_ptrs<<<1,1>>>(diffusion_cu, cbasis->b_type, cdim, poly_order, diffusion_id, diff_order, dirs_linidx);
