...
gkyl_pos_shift_gk_set_cu_ker_ptrs(struct gkyl_positivity_shift_gyrokinetic_kernels *kers,
...
pos_shift_gk_choose_shift_kernel_cu(struct gkyl_positivity_shift_gyrokinetic_kernels *kernels,
  int cdim, struct gkyl_basis pbasis)
{
  gkyl_pos_shift_gk_set_cu_ker_ptrs<<<1,1>>>(kernels, cdim, pbasis);
}

// Function borrowed from array_reduce_cu.cu.
__device__ static __forceinline__ double
atomicMax_double_posshift(double *address, double val)
{
...
gkyl_positivity_shift_gyrokinetic_advance_cu_ker(
  struct gkyl_positivity_shift_gyrokinetic_kernels *kers, const struct gkyl_rect_cart grid,
  const struct gkyl_range phase_range, const struct gkyl_range conf_range,
  double *ffloor, double ffloor_fac, double cellav_fac, double mass, const struct gkyl_range* GKYL_RESTRICT bmag, 
  struct gkyl_array* GKYL_RESTRICT distf, struct gkyl_array* GKYL_RESTRICT mom)
{
  ...
  for(unsigned long tid = threadIdx.x + blockIdx.x*blockDim.x;
    ...
    int max_num_basis = 80; // hard-coded to 4 * max confBasis.num_basis (3x p=2 Ser) for now.
  ...
  atomicMax_double_posshift(&ffloor, ffloor_fac * distf_max * cellav_fac);
}
  ...
  struct gkyl_array *GKYL_RESTRICT distf, struct gkyl_array *GKYL_RESTRICT mom)
{
  ...
  gkyl_positivity_shift_gyrokinetic_advance_cu_ker<<<nblocks, nthreads>>>
    (up->grid, phase_rng, conf_rng, up->ffloor, up->ffloor_fac, up->cellav_fac,
