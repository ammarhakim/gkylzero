...
gkyl_gk_geometry_cu_dev_new(struct gk_geometry* geo_host, struct gkyl_gk_geometry_inp *geometry_inp)
{
  ...
  struct gkyl_array *mu2nu_pos_dev = gkyl_array_cu_dev_new(geo_host->mu2nu_pos->type, geo_host->mu2nu_pos->ncomp, geo_host->mu2nu_pos->size);
  ...
  struct gkyl_array *bmag_mid_dev = gkyl_array_cu_dev_new(geo_host->bmag_mid->type, geo_host->bmag_mid->ncomp, geo_host->bmag_mid->size);

  gkyl_array_copy(mc2p_dev, geo_host->mc2p);
  gkyl_array_copy(mu2nu_pos_dev, geo_host->mu2nu_pos);
  ...
  gkyl_array_copy(bmag_mid_dev, geo_host->bmag_mid);

  // this is for the memcpy below
  up->mc2p  = mc2p_dev->on_dev;
  up->mu2nu_pos  = mu2nu_pos_dev->on_dev;
  ...
  up->bmag_mid  = bmag_mid_dev->on_dev;
