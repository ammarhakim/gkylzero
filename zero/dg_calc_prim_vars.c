...
// Probably a better way to do this (JJ: 11/16/22)
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    pkpm_prim_vars(bvar_d, vlasov_pkpm_moms_d, euler_pkpm_d, 
      u_i_d, p_ij_d, T_ij_d, T_perp_over_m_d);
  }
}
  ...
  struct gkyl_array* g_dist_source, struct gkyl_array* F_k_m_1)
{
  ...
  if (gkyl_array_is_cu_dev(g_dist_source)) {
    return gkyl_calc_prim_vars_pkpm_dist_mirror_force_cu(basis, conf_range, phase_range, 
      T_perp_over_m, fIn, F_k_p_1, g_dist_source, F_k_m_1);
  }
#endif  
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    pkpm_dist_mirror_force(T_perp_over_m_d, fIn_d, F_k_p_1_d, g_dist_source_d, F_k_m_1_d);
  }  
}
...
// Probably a better way to do this (JJ: 11/16/22)
  ...
  if (gkyl_array_is_cu_dev(div_p)) {
    return gkyl_calc_prim_vars_pkpm_recovery_cu(grid, basis, range, nuHyp, 
      ...
      T_perp_over_m, nu, nu_vthsq, 
      div_b, bb_grad_u, div_p, p_force, p_perp_source, p_perp_div_b);
  }
#endif
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    double *p_perp_div_b_d = gkyl_array_fetch(p_perp_div_b, linc);
    ...
    for (int dir=0; dir<cdim; ++dir) {
      ...
      pkpm_recovery[dir](grid->dx, nuHyp, 
        ...
        T_perp_over_m_d, nu_d, nu_vthsq_d, 
        div_b_d, bb_grad_u_d, div_p_d, 
        p_force_d, p_perp_source_d, p_perp_div_b_d);
    }
  }
}
