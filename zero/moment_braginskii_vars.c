  ...
  double current_par[][3], double current_cross[][3])
{
  ...
  for (int j = start; j <= end; ++j) {
    for (int n = 0; n < nfluids; ++n) {
      ...
      u[j][n][0] = fluid_d[j][n][MX] / rho[n];
      u[j][n][1] = fluid_d[j][n][MY] / rho[n];
      u[j][n][2] = fluid_d[j][n][MZ] / rho[n];
  ...
  double *cflrate[GKYL_MAX_SPECIES], double *brag_d[GKYL_MAX_SPECIES])
{
  ...
  double b_avg[3] = {0.0};
  double eta_par_avg[2] = {0.0}, eta_perp_avg[2] = {0.0}, eta_cross_avg[2] = {0.0};
  double w[2][6] = {0.0};
  ...
  double kappa_par_avg[2] = {0.0}, kappa_perp_avg[2] = {0.0}, kappa_cross_avg[2] = {0.0};
  double current_par_avg[3] = {0.0};   // Parallel current multiplied by parallel thermal force coefficient at cell edges (using arithmetic average)
  double current_cross_avg[3] = {0.0}; // Cross current multiplied by perpendicular thermal force coefficient at cell edges (using arithmetic average)
  ...
  double bbgradT[2][3] = {0.0};     // Parallel temperature gradient (b_hat b_hat dot grad T)
  double perp_gradT[2][3] = {0.0};  // Perpendicular temperature gradient (grad T - b_hat b_hat dot grad T)
  double cross_gradT[2][3] = {0.0}; // Cross temperature gradient (b x grad T)
  ...
  if (ndim == 1) {
    ...
    for (int n = 0; n < nfluids; ++n) {
      ...
      if (bes->param[n].type_eqn == GKYL_EQN_EULER) {
        ...
        bbgradT[n][0] = b_avg[0] * b_avg[0] * gradxT[n];
        bbgradT[n][1] = b_avg[1] * b_avg[0] * gradxT[n];
        bbgradT[n][2] = b_avg[2] * b_avg[0] * gradxT[n];
        ...
        u_avg[n][0] = calc_arithm_avg_1D(u[L_1D][n][0], u[U_1D][n][0]);
        u_avg[n][1] = calc_arithm_avg_1D(u[L_1D][n][1], u[U_1D][n][1]);
        u_avg[n][2] = calc_arithm_avg_1D(u[L_1D][n][2], u[U_1D][n][2]);
      }
    }
    ...
    for (int n = 0; n < nfluids; ++n) {
      ...
      if (bes->param[n].type_eqn == GKYL_EQN_EULER) {
        ...
        bbgradT[n][0] = b_avg[0] * (b_avg[0]*gradxT[n] + b_avg[1]*gradyT[n]);
        bbgradT[n][1] = b_avg[1] * (b_avg[0]*gradxT[n] + b_avg[1]*gradyT[n]);
        bbgradT[n][2] = b_avg[2] * (b_avg[0]*gradxT[n] + b_avg[1]*gradyT[n]);
        ...
        // Average velocity at cell edges (using arithmetic average)
        u_avg[n][0] = calc_arithm_avg_2D(u[LL_2D][n][0], u[LU_2D][n][0], u[UL_2D][n][0], u[UU_2D][n][0]);
        u_avg[n][1] = calc_arithm_avg_2D(u[LL_2D][n][1], u[LU_2D][n][1], u[UL_2D][n][1], u[UU_2D][n][1]);
        u_avg[n][2] = calc_arithm_avg_2D(u[LL_2D][n][2], u[LU_2D][n][2], u[UL_2D][n][2], u[UU_2D][n][2]);
      }
    }
  ...
  for (int n = 0; n < nfluids; ++n) {
    ...
    brag_d[n][PIXX] = pi_par[n][0] + pi_perp[n][0] + pi_cross[n][0];
    brag_d[n][PIXY] = pi_par[n][1] + pi_perp[n][1] + pi_cross[n][1];
    brag_d[n][PIXZ] = pi_par[n][2] + pi_perp[n][2] + pi_cross[n][2];
    brag_d[n][PIYY] = pi_par[n][3] + pi_perp[n][3] + pi_cross[n][3];
    brag_d[n][PIYZ] = pi_par[n][4] + pi_perp[n][4] + pi_cross[n][4];
    brag_d[n][PIZZ] = pi_par[n][5] + pi_perp[n][5] + pi_cross[n][5];
    ...
    brag_d[n][QX] = -kappa_par_avg[n]*bbgradT[n][0] - kappa_perp_avg[n]*perp_gradT[n][0] + kappa_cross_avg[n]*cross_gradT[n][0] + Piu[0];
    brag_d[n][QY] = -kappa_par_avg[n]*bbgradT[n][1] - kappa_perp_avg[n]*perp_gradT[n][1] + kappa_cross_avg[n]*cross_gradT[n][1] + Piu[1];
    brag_d[n][QZ] = -kappa_par_avg[n]*bbgradT[n][2] - kappa_perp_avg[n]*perp_gradT[n][2] + kappa_cross_avg[n]*cross_gradT[n][2] + Piu[2];
  }

  brag_d[ELC][QX] += current_par_avg[0] + current_cross_avg[0];
  brag_d[ELC][QY] += current_par_avg[1] + current_cross_avg[1];
  brag_d[ELC][QZ] += current_par_avg[2] + current_cross_avg[2];
}
  ...
  double *cflrate[GKYL_MAX_SPECIES], double *brag_d[GKYL_MAX_SPECIES])
{
  ...
  for (int n = 0; n < nfluids; ++n) {
    // Total viscous stress tensor
    brag_d[n][PIXX] = -eta_avg[n] * w[n][0];
    brag_d[n][PIXY] = -eta_avg[n] * w[n][1];
    brag_d[n][PIXZ] = -eta_avg[n] * w[n][2];
    brag_d[n][PIYY] = -eta_avg[n] * w[n][3];
    brag_d[n][PIYZ] = -eta_avg[n] * w[n][4];
    brag_d[n][PIZZ] = -eta_avg[n] * w[n][5];
    ...
    // Total heat flux + viscous heating
    brag_d[n][QX] = -kappa_avg[n]*gradT[0] + Piu[0];
    brag_d[n][QY] = -kappa_avg[n]*gradT[1] + Piu[1];
    brag_d[n][QZ] = -kappa_avg[n]*gradT[2] + Piu[2];
  }

  brag_d[ELC][QX] += current_avg[0];
  brag_d[ELC][QY] += current_avg[1];
  brag_d[ELC][QZ] += current_avg[2];
}
