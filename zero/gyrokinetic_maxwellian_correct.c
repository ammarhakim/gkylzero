...
gkyl_gyrokinetic_maxwellian_correct_inew(const struct gkyl_gyrokinetic_maxwellian_correct_inp *inp)
{
  ...
  up->num_moms_correct = 3; // correct n, u_par, T/m
  up->correct_bimaxwellian = inp->correct_bimaxwellian;
  if (up->correct_bimaxwellian) {
    up->num_moms_correct = 4; // correct n, u_par, T_par/m, T_perp/m
  }
  ...
  if (up->use_gpu) {
    up->moms_iter = gkyl_array_cu_dev_new(GKYL_DOUBLE, up->num_moms_correct*inp->conf_basis->num_basis, conf_range_ext_ncells);
    up->d_moms = gkyl_array_cu_dev_new(GKYL_DOUBLE, up->num_moms_correct*inp->conf_basis->num_basis, conf_range_ext_ncells);
    up->dd_moms = gkyl_array_cu_dev_new(GKYL_DOUBLE, up->num_moms_correct*inp->conf_basis->num_basis, conf_range_ext_ncells);
    ...
    up->abs_diff_moms = gkyl_array_cu_dev_new(GKYL_DOUBLE, up->num_moms_correct, conf_range_ext_ncells);
    up->error_cu = gkyl_cu_malloc(sizeof(double[up->num_moms_correct]));
    ...
    up->moms_iter = gkyl_array_new(GKYL_DOUBLE, up->num_moms_correct*inp->conf_basis->num_basis, conf_range_ext_ncells);
    up->d_moms = gkyl_array_new(GKYL_DOUBLE, up->num_moms_correct*inp->conf_basis->num_basis, conf_range_ext_ncells);
    up->dd_moms = gkyl_array_new(GKYL_DOUBLE, up->num_moms_correct*inp->conf_basis->num_basis, conf_range_ext_ncells);
    // Arrays for correcting the density 
  ...
  up->error = gkyl_malloc(sizeof(double[up->num_moms_correct]));
  ...
  const struct gkyl_range *phase_range, const struct gkyl_range *conf_range)
{
  ...
  for (int i=0; i<up->num_moms_correct; ++i) {
    up->error[i] = 1.0; 
  }
  // Copy the initial max error to GPU so initial error is set correctly (no uninitialized values).
  if (up->use_gpu) {
    gkyl_cu_memcpy(up->error_cu, up->error, sizeof(double[up->num_moms_correct]), GKYL_CU_MEMCPY_H2D);
  }
    ...
    (fabs(up->error[2]) > tol))))
  {
    ...
    if ((niter % 1) == 0) {
      if (up->use_gpu) {
        ...
        gkyl_cu_memcpy(up->error, up->error_cu, sizeof(double[up->num_moms_correct]), GKYL_CU_MEMCPY_D2H);
      }
      else {
    ...
    (fabs(up->error[2]) > tol) || (fabs(up->error[3]) > tol))))
  {
    ...
    if ((niter % 1) == 0) {
      if (up->use_gpu) {
        ...
        // by first calling a specialized kernel for computing the absolute value 
        // of the difference of the cell averages, then calling reduce_range.
        gkyl_gyrokinetic_bimaxwellian_correct_all_moments_abs_diff_cu(conf_range, 
          nc, moms_target, up->moms_iter, up->abs_diff_moms);
        gkyl_array_reduce_range(up->error_cu, up->abs_diff_moms, GKYL_MAX, conf_range);
        gkyl_cu_memcpy(up->error, up->error_cu, sizeof(double[up->num_moms_correct]), GKYL_CU_MEMCPY_D2H);
      }
        ...
        while (gkyl_range_iter_next(&biter)){
          ...
          const double *moms_target_local = gkyl_array_cfetch(moms_target, midx);
          // Check the error in the absolute value of the cell average
          // Note: for density, T_par/m, and T_perp/m this error is a relative error compared to the target moment value
          // so that we can converge to the correct target moments in SI units and minimize finite precision issues.
          up->error[0] = fmax(fabs(moms_local[0*nc] - moms_target_local[0*nc])/moms_target_local[0*nc],fabs(up->error[0]));
          up->error[2] = fmax(fabs(moms_local[2*nc] - moms_target_local[2*nc])/moms_target_local[2*nc],fabs(up->error[2]));
          ...
          // Check if density, T_par/m, and T_perp/m are positive, if they aren't we will break out of the iteration
          ispositive_f_max = (moms_local[0*nc]>0.0) && ispositive_f_max;
          ispositive_f_max = (moms_local[2*nc]>0.0) && ispositive_f_max;
          ispositive_f_max = (moms_local[3*nc]>0.0) && ispositive_f_max;
        }
      }
    }
    ...
    // 2. Update the gyrokinetic BiMaxwellian distribution function using the corrected moments.
...
gkyl_gyrokinetic_maxwellian_correct_release(gkyl_gyrokinetic_maxwellian_correct *up)
{
  ...
  if (up->correct_bimaxwellian) {
    gkyl_proj_bimaxwellian_on_basis_release(up->proj_bimax_prim);
  }
  else {
