  ...
  const struct gkyl_basis* pbasis, const char *mom)
{
  ...
  struct mom_type_vlasov *momt = gkyl_malloc(sizeof(struct mom_type_vlasov));
  ...
  if (strcmp(mom, "M0") == 0) { // density
    ...
    momt->kernel = five_moments_kernels[cv_index[cdim].vdim[vdim]].kernels[poly_order];
  ...
  momt->momt.flag = 0;
  GKYL_CLEAR_CU_ALLOC(momt->momt.flag);
  momt->momt.ref_count = gkyl_ref_count_init(gkyl_mom_free);
  
  momt->momt.on_dev = &momt->momt; // on host, self-reference
