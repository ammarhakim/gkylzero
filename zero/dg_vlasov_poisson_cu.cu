...
gkyl_dg_vlasov_poisson_set_auxfields_cu_kernel(const struct gkyl_dg_eqn *eqn, 
...
void
gkyl_dg_vlasov_poisson_set_auxfields_cu(const struct gkyl_dg_eqn *eqn, struct gkyl_dg_vlasov_poisson_auxfields auxin)
{
  gkyl_dg_vlasov_poisson_set_auxfields_cu_kernel<<<1,1>>>(eqn, auxin.field->on_dev);
}
  ...
  enum gkyl_model_id model_id, enum gkyl_field_id field_id)
{
  ...
  // Streaming terms.
  vlasov->stream_surf[0] = surf_x_kernels[cv_index].kernels[poly_order];
  if (cdim>1)
    vlasov->stream_surf[1] = surf_y_kernels[cv_index].kernels[poly_order];
  if (cdim>2)
    vlasov->stream_surf[2] = surf_z_kernels[cv_index].kernels[poly_order];

  vlasov->stream_boundary_surf[0] = boundary_surf_x_kernels[cv_index].kernels[poly_order];
  if (cdim>1)
    vlasov->stream_boundary_surf[1] = boundary_surf_y_kernels[cv_index].kernels[poly_order];
  if (cdim>2)
    vlasov->stream_boundary_surf[2] = boundary_surf_z_kernels[cv_index].kernels[poly_order];
     
  // Acceleration terms.
  vlasov->accel_surf[0] = surf_vx_kernels[cv_index].kernels[poly_order];
  if (vdim>1)
    vlasov->accel_surf[1] = surf_vy_kernels[cv_index].kernels[poly_order];
  if (vdim>2)
    vlasov->accel_surf[2] = surf_vz_kernels[cv_index].kernels[poly_order];

  vlasov->accel_boundary_surf[0] = boundary_surf_vx_kernels[cv_index].kernels[poly_order];
  if (vdim>1)
    vlasov->accel_boundary_surf[1] = boundary_surf_vy_kernels[cv_index].kernels[poly_order];
  if (vdim>2)
    vlasov->accel_boundary_surf[2] = boundary_surf_vz_kernels[cv_index].kernels[poly_order];
...
gkyl_dg_vlasov_poisson_cu_dev_new(const struct gkyl_basis* cbasis, const struct gkyl_basis* pbasis,
  ...
  const struct gkyl_array *vmap, enum gkyl_field_id field_id)
{
  ...
  gkyl_cu_memcpy(vlasov_poisson_cu, vlasov, sizeof(struct dg_vlasov_poisson), GKYL_CU_MEMCPY_H2D);
  ...
  vlasov->eqn.on_dev = &vlasov_poisson_cu->eqn;
