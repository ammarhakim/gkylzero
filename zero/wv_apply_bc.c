  ...
  const struct gkyl_range *update_rng, struct gkyl_array *out)
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    bc->eqn->rotate_to_local_func(wg->tau1[dir], wg->tau2[dir], wg->norm[dir],
    ...
    bc->eqn->rotate_to_global_func(wg->tau1[dir], wg->tau2[dir], wg->norm[dir],
  ...
  const struct gkyl_range *update_rng, const struct gkyl_array *inp, double *buffer)
{
  ...
  while (gkyl_range_iter_next(&iter)) {
    ...
    bc->eqn->rotate_to_local_func(wgs->tau1[dir], wgs->tau2[dir], wgs->norm[dir],
    ...
    bc->eqn->rotate_to_global_func(wgg->tau1[dir], wgg->tau2[dir], wgg->norm[dir],
