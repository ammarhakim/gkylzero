...
gkyl_vlasov_lbo_drag_set_nuSum_cu_kernel(const struct gkyl_dg_eqn *eqn, const struct gkyl_array *nuSum)
{
...
void
gkyl_vlasov_lbo_drag_set_nuSum_cu(const struct gkyl_dg_eqn *eqn, const struct gkyl_array *nuSum)
{
  gkyl_vlasov_lbo_drag_set_nuSum_cu_kernel<<<1,1>>>(eqn, nuSum->on_dev);
}
...
// and so its members cannot be modified without a full __global__ kernel on device.
__global__ static void
gkyl_vlasov_lbo_drag_set_nuUSum_cu_kernel(const struct gkyl_dg_eqn *eqn, const struct gkyl_array *nuUSum)
{
  struct dg_vlasov_lbo_drag *vlasov_lbo_drag = container_of(eqn, struct dg_vlasov_lbo_drag, eqn);
...
void
gkyl_vlasov_lbo_drag_set_nuUSum_cu(const struct gkyl_dg_eqn *eqn, const struct gkyl_array *nuUSum)
{
  gkyl_vlasov_lbo_drag_set_nuUSum_cu_kernel<<<1,1>>>(eqn, nuUSum->on_dev);
}
...
// and so its members cannot be modified without a full __global__ kernel on device.
__global__ static void
gkyl_vlasov_lbo_drag_set_nuVtSqSum_cu_kernel(const struct gkyl_dg_eqn *eqn, const struct gkyl_array *nuVtSqSum)
{
  struct dg_vlasov_lbo_drag *vlasov_lbo_drag = container_of(eqn, struct dg_vlasov_lbo_drag, eqn);
...
void
gkyl_vlasov_lbo_drag_set_nuVtSqSum_cu(const struct gkyl_dg_eqn *eqn, const struct gkyl_array *nuVtSqSum)
{
  gkyl_vlasov_lbo_drag_set_nuVtSqSum_cu_kernel<<<1,1>>>(eqn, nuVtSqSum->on_dev);
}

// CUDA kernel to set device pointers to range object and vlasov LBO kernel function
// Doing function pointer stuff in here avoids troublesome cudaMemcpyFromSymbol
__global__ static void
dg_vlasov_lbo_drag_set_cu_dev_ptrs(struct dg_vlasov_lbo_drag *vlasov_lbo_drag, enum gkyl_basis_type b_type,
  int cv_index, int cdim, int vdim, int poly_order)
{
  ...
  vlasov_lbo_drag->vol = vol_kernels[cv_index].kernels[poly_order];

  vlasov_lbo_drag->surf[0] = surf_vx_kernels[cv_index].kernels[poly_order];
  if (vdim>1)
    vlasov_lbo_drag->surf[1] = surf_vy_kernels[cv_index].kernels[poly_order];
  if (vdim>2)
    vlasov_lbo_drag->surf[2] = surf_vz_kernels[cv_index].kernels[poly_order];

  vlasov_lbo_drag->boundary_surf[0] = boundary_surf_vx_kernels[cv_index].kernels[poly_order];
  if (vdim>1)
    vlasov_lbo_drag->boundary_surf[1] = boundary_surf_vy_kernels[cv_index].kernels[poly_order];
  if (vdim>2)
    vlasov_lbo_drag->boundary_surf[2] = boundary_surf_vz_kernels[cv_index].kernels[poly_order];
}
  ...
  const struct gkyl_range* conf_range)
{
  ...
  GKYL_SET_CU_ALLOC(vlasov_lbo_drag->eqn.flags);
