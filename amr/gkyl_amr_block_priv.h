...
// Context for copying job pool information.
struct copy_job_ctx {
  int bidx;
...
* @param btopo Topology/connectivity informations for the entire block hierarchy.
* @param bdata Block-structured data for the Euler equations.
...
// Block-structured data (including grid, range and boundary condition information) for the 10-moment equations.
struct ten_moment_block_data {
  struct gkyl_rect_grid grid;
  gkyl_fv_proj *fv_proj;
...
// Job pool information context for updating block-structured data for the 10-moment equations using threads.
struct ten_moment_update_block_ctx {
  const struct ten_moment_block_data *bdata;
  int bidx;
  int dir;
  double t_curr;
  double dt;
...
/**
* Boundary condition function for applying transmissive boundary conditions for the 10-moment equations.
*
* @param t Current simulation time.
* @param nc Number of boundary cells to apply transmissive conditions to.
* @param skin Skin cells in the boundary region (from which values are copied).
...
* @param ctx Context to pass to the function.
*/
static void ten_moment_transmissive_bc(double t, int nc, const double* skin, double* GKYL_RESTRICT ghost, void* ctx);

/**
* Initialize updaters for both physical (outer-block) and non-physical (inter-block) boundary conditions for the 10-moment equations.
*
* @param bdata Block-structured data for the 10-moment equations.
* @param conn Topology/connectivity data for the block hierarchy.
*/
void ten_moment_block_bc_updaters_init(struct ten_moment_block_data* bdata, const struct gkyl_block_connections* conn);

/**
* Release updaters for both physical (outer-block) and non-physical (inter-block) boundary conditions for the 10-moment equations.
*
* @param bdata Block-structured data for the 10-moment equations.
*/
void ten_moment_block_bc_updaters_release(struct ten_moment_block_data* bdata);

/**
* Apply both physical (outer-block) and non-physical (inter-block) boundary conditions for the 10-moment equations.
*
* @param bdata Block-structured data for the 10-moment equations.
* @param tm Simulation time at which the boundary conditions are applied.
* @param fld Output array.
*/
void ten_moment_block_bc_updaters_apply(const struct ten_moment_block_data* bdata, double tm, struct gkyl_array* fld);

/**
* Synchronize all blocks in the block hierarchy by applying all appropriate physical (outer-block) and non-physical (inter-block) boundary conditions for the 10-moment equations.
*
* @param btopo Topology/connectivity information for the entire block hierarchy.
* @param bdata Block-structured data for the 10-moment equations.
* @param fld Output array.
*/
void ten_moment_sync_blocks(const struct gkyl_block_topo* btopo, const struct ten_moment_block_data bdata[], struct gkyl_array* fld[]);

/**
* Write block-structured simulation data for the 10-moment equations onto disk.
*
* @param fileNm File name schema to use for the simulation output.
* @param bdata Block-structured data for the 10-moment equations.
*/
void ten_moment_block_data_write(const char* fileNm, const struct ten_moment_block_data* bdata);

/**
* Calculate the maximum stable time-step for the block-structured 10-moment equations.
*
* @param bdata Block-structured data for the 10-moment equations.
*/
double ten_moment_block_data_max_dt(const struct ten_moment_block_data* bdata);

/**
* Update the block-structured 10-moment equations simulation data using the thread-based job pool.
*
* @param ctx Context to pass to the function.
*/
void ten_moment_update_block_job_func(void *ctx);

/**
* Update all blocks in the block hierarchy by using the thread-based job pool for the 10-moment equations.
*
* @param job_pool Job pool for updating block-structured data for the 10-moment equations using threads.
* @param btopo Topology/connectivity informations for the entire block hierarchy.
* @param bdata Block-structured data for the 10-moment equations.
* @param t_curr Current simulation time.
* @param dt Current stable time-step for the simulation.
*/
struct gkyl_update_status ten_moment_update_all_blocks(const struct gkyl_job_pool* job_pool, const struct gkyl_block_topo* btopo,
  const struct ten_moment_block_data bdata[], double t_curr, double dt);

/**
* Initialize a new job in the thread-based job pool for updating the block-structured 10-moment equations simulation data.
*
* @param ctx Context to pass to the function.
*/
void ten_moment_init_job_func(void* ctx);
...
/**
* Take a single time step across the entire block hierarchy for the 10-moment equations.
*
* @param job_pool Job pool for updating block-structured data for the 10-moment equations using threads.
* @param btopo Topology/connectivity information for the entire block hierarchy.
* @param bdata Block-structured data for the 10-moment equations.
...
* @param stats Simulation statistics (allowing for tracking of the number of failed time-steps).
*/
struct gkyl_update_status ten_moment_update(const struct gkyl_job_pool* job_pool, const struct gkyl_block_topo* btopo,
  const struct ten_moment_block_data bdata[], double t_curr, double dt0, struct sim_stats* stats);

/**
* Write the complete simulation output for the entire block hierarchy for the 10-moment equations onto disk.
*
* @param fbase Base file name schema to use for the simulation output.
* @param num_blocks Number of blocks in the block hierarchy.
* @param bdata Array of block-structured data for the 10-moment equations.
*/
void ten_moment_write_sol(const char* fbase, int num_blocks, const struct ten_moment_block_data bdata[]);

/**
* Calculate the maximum stable time-step across all blocks in the block hierarchy for the 10-moment equations.
*
* @param num_blocks Number of blocks in the block hierarchy.
* @param bdata Array of block-structured data for the 10-moment equations.
*/
double ten_moment_max_dt(int num_blocks, const struct ten_moment_block_data bdata[]);