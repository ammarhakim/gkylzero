...
ten_moment_1d_run_single(int argc, char **argv, struct ten_moment_1d_single_init* init)
{
  ...
  for (int i = 0; i < num_patches; i++) {
    mesh_pdata[i].euler_elc = gkyl_wv_ten_moment_new(k0_elc, false, NULL, app_args.use_gpu);
    mesh_pdata[i].euler_ion = gkyl_wv_ten_moment_new(k0_ion, false, NULL, app_args.use_gpu);
...
ten_moment_2d_run_single(int argc, char **argv, struct ten_moment_2d_single_init* init)
{
  ...
  for (int i = 0; i < num_blocks; i++) {
    mesh_bdata[i].euler_elc = gkyl_wv_ten_moment_new(k0_elc, false, NULL, app_args.use_gpu);
    mesh_bdata[i].euler_ion = gkyl_wv_ten_moment_new(k0_ion, false, NULL, app_args.use_gpu);
