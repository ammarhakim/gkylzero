...
five_moment_2d_run_double(int argc, char **argv, struct five_moment_2d_double_init* init)
{
...
#ifdef AMR_DEBUG
    intermediate_bdata[i].fv_proj_elc = gkyl_fv_proj_new(&intermediate_bdata[i].grid, 2, 5, eval_elc, 0);
    intermediate_bdata[i].fv_proj_ion = gkyl_fv_proj_new(&intermediate_bdata[i].grid, 2, 5, eval_ion, 0);
  ...
  for (int i = 0; i < num_blocks; i++) {
    ...
    if (low_order_flux) {
      ...
      intermediate_bdata[i].euler_elc = gkyl_wv_euler_inew(&inp);
      intermediate_bdata[i].euler_ion = gkyl_wv_euler_inew(&inp);

      fine_bdata[i].euler_elc = gkyl_wv_euler_inew(&inp);
      fine_bdata[i].euler_ion = gkyl_wv_euler_inew(&inp);
    }
    else {
      intermediate_bdata[i].euler_elc = gkyl_wv_euler_new(gas_gamma, app_args.use_gpu);
      intermediate_bdata[i].euler_ion = gkyl_wv_euler_new(gas_gamma, app_args.use_gpu);
  ...
  for (int i = 0; i < num_blocks; i++) {
    five_moment_nested_block_bc_updaters_init(&coarse_bdata[i], &btopo->conn[i]);

#ifdef AMR_DEBUG
    five_moment_nested_block_bc_updaters_init(&intermediate_bdata[i], &btopo->conn[i]);
    five_moment_nested_block_bc_updaters_init(&fine_bdata[i], &btopo->conn[i]);
#endif
  }
  ...
  for (int i = 0; i < num_blocks; i++) {
...
#ifdef AMR_DEBUG
    intermediate_bdata[i].fdup_elc = gkyl_array_new(GKYL_DOUBLE, 5, intermediate_bdata[i].ext_range.volume);
    intermediate_bdata[i].fdup_ion = gkyl_array_new(GKYL_DOUBLE, 5, intermediate_bdata[i].ext_range.volume);
    intermediate_bdata[i].fdup_maxwell = gkyl_array_new(GKYL_DOUBLE, 8, intermediate_bdata[i].ext_range.volume);

    for (int d = 0; d < ndim + 1; d++) {
      intermediate_bdata[i].f_elc[d] = gkyl_array_new(GKYL_DOUBLE, 5, intermediate_bdata[i].ext_range.volume);
      intermediate_bdata[i].f_ion[d] = gkyl_array_new(GKYL_DOUBLE, 5, intermediate_bdata[i].ext_range.volume);
    ...
    intermediate_bdata[i].rhs_source_elc = gkyl_array_new(GKYL_DOUBLE, 5, intermediate_bdata[i].ext_range.volume);
    intermediate_bdata[i].rhs_source_ion = gkyl_array_new(GKYL_DOUBLE, 5, intermediate_bdata[i].ext_range.volume);
  ...
  snprintf(intermediate0_elc_b0, 64, "%s_intermediate_0_elc_b0.gkyl", five_moment_output);
  snprintf(intermediate0_ion_b0, 64, "%s_intermediate_0_ion_b0.gkyl", five_moment_output);
  snprintf(intermediate0_field_b0, 64, "%s_intermediate_0_field_b0.gkyl", five_moment_output);
  ...
  for (int i = 1; i < 9; i++) {
    ...
    snprintf(fine0_elc_bi, 64, "%s_fine_0_elc_b%d.gkyl", five_moment_output, i);
    ...
    snprintf(coarse0_field_bi, 64, "%s_coarse_0_field_b%d.gkyl", five_moment_output, i);

    snprintf(elc_bi, 64, "%s_0_elc_b%d.gkyl", five_moment_output, i);
    snprintf(ion_bi, 64, "%s_0_ion_b%d.gkyl", five_moment_output, i);
    snprintf(field_bi, 64, "%s_0_field_b%d.gkyl", five_moment_output, i);
  ...
  for (int i = 9; i < 25; i++) {
    ...
    snprintf(buf_del1_elc, 64, "%s_intermediate_0_elc_b%d.gkyl", five_moment_output, i);
    snprintf(buf_del1_ion, 64, "%s_intermediate_0_ion_b%d.gkyl", five_moment_output, i);
    snprintf(buf_del1_field, 64, "%s_intermediate_0_field_b%d.gkyl", five_moment_output, i);
    
    snprintf(buf_del2_elc, 64, "%s_fine_0_elc_b%d.gkyl", five_moment_output, i);
    snprintf(buf_del2_ion, 64, "%s_fine_0_ion_b%d.gkyl", five_moment_output, i);
    snprintf(buf_del2_field, 64, "%s_fine_0_field_b%d.gkyl", five_moment_output, i);
