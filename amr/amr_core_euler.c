...
euler2d_run_double(int argc, char **argv, struct euler2d_double_init* init)
{
  ...
  for (int i = 0; i < num_blocks; i++) {
    ...
    if (low_order_flux) {
      ...
      intermediate_bdata[i].euler = gkyl_wv_euler_inew(&inp);
      fine_bdata[i].euler = gkyl_wv_euler_inew(&inp);
    }
    else {
      intermediate_bdata[i].euler = gkyl_wv_euler_new(gas_gamma, app_args.use_gpu);
      fine_bdata[i].euler = gkyl_wv_euler_new(gas_gamma, app_args.use_gpu);
    }
  ...
  for (int i = 0; i < num_blocks; i++) {
    euler_nested_block_bc_updaters_init(&coarse_bdata[i], &btopo->conn[i]);

#ifdef AMR_DEBUG
    euler_nested_block_bc_updaters_init(&intermediate_bdata[i], &btopo->conn[i]);
    euler_nested_block_bc_updaters_init(&fine_bdata[i], &btopo->conn[i]);
#endif
  }
...
#ifdef AMR_DEBUG
    intermediate_bdata[i].fdup = gkyl_array_new(GKYL_DOUBLE, 5, intermediate_bdata[i].ext_range.volume);
    fine_bdata[i].fdup = gkyl_array_new(GKYL_DOUBLE, 5, fine_bdata[i].ext_range.volume);

    for (int d = 0; d < ndim + 1; d++) {
      intermediate_bdata[i].f[d] = gkyl_array_new(GKYL_DOUBLE, 5, intermediate_bdata[i].ext_range.volume);
      fine_bdata[i].f[d] = gkyl_array_new(GKYL_DOUBLE, 5, fine_bdata[i].ext_range.volume);
    }
#endif
  }
  ...
  snprintf(intermediate0, 64, "%s_intermediate_0", euler_output);
  ...
  char b0[64];
  snprintf(fine0b0, 64, "%s_fine_0_b0.gkyl", euler_output);
  snprintf(intermediate0b0, 64, "%s_intermediate_0_b0.gkyl", euler_output);
  snprintf(coarse0b0, 64, "%s_coarse_0_b0.gkyl", euler_output);
  snprintf(b0, 64, "%s_0_b0.gkyl", euler_output);
  rename(fine0b0, b0);
  ...
  for (int i = 1; i < 9; i++) {
    ...
    char coarse0bi[64];
    char bi[64];
    snprintf(fine0bi, 64, "%s_fine_0_b%d.gkyl", euler_output, i);
    snprintf(intermediate0bi, 64, "%s_intermediate_0_b%d.gkyl", euler_output, i);
    snprintf(coarse0bi, 64, "%s_coarse_0_b%d.gkyl", euler_output, i);
    snprintf(bi, 64, "%s_0_b%d.gkyl", euler_output, i);
  ...
  for (int i = 9; i < 25; i++) {
    ...
    snprintf(buf_old, 64, "%s_coarse_0_b%d.gkyl", euler_output, i);
    snprintf(buf_new, 64, "%s_0_b%d.gkyl", euler_output, i);
    snprintf(buf_del1, 64, "%s_intermediate_0_b%d.gkyl", euler_output, i);
    snprintf(buf_del2, 64, "%s_fine_0_b%d.gkyl", euler_output, i);
  ...
  while ((coarse_t_curr < t_end) && (coarse_step <= num_steps)) {
    ...
    for (int i = 1; i < num_frames; i++) {
      if (coarse_t_curr < (i * io_trigger) && (coarse_t_curr + coarse_status.dt_actual) > (i * io_trigger)) {
        ...
        snprintf(buf_coarse, 64, "%s_coarse_%d", euler_output, i);
        snprintf(buf_intermediate, 64, "%s_intermediate_%d", euler_output, i);
        snprintf(buf_fine, 64, "%s_fine_%d", euler_output, i);
        ...
        snprintf(buf_fine_old, 64, "%s_fine_%d_b0.gkyl", euler_output, i);
        snprintf(buf_fine_new, 64, "%s_%d_b0.gkyl", euler_output, i);
        snprintf(buf_intermediate_old, 64, "%s_intermediate_%d_b0.gkyl", euler_output, i);
        snprintf(buf_coarse_old, 64, "%s_coarse_%d_b0.gkyl", euler_output, i);
        ...
        for (int j = 1; j < 9; j++) {
          ...
          char coarse_bi[64];
          char bi[64];
          snprintf(fine_bi, 64, "%s_fine_%d_b%d.gkyl", euler_output, i, j);
          snprintf(intermediate_bi, 64, "%s_intermediate_%d_b%d.gkyl", euler_output, i, j);
          snprintf(coarse_bi, 64, "%s_coarse_%d_b%d.gkyl", euler_output, i, j);
          snprintf(bi, 64, "%s_%d_b%d.gkyl", euler_output, i, j);
        ...
        for (int j = 9; j < 25; j++) {
          ...
          snprintf(buf_old, 64, "%s_coarse_%d_b%d.gkyl", euler_output, i, j);
          snprintf(buf_new, 64, "%s_%d_b%d.gkyl", euler_output, i, j);
          snprintf(buf_del1, 64, "%s_intermediate_%d_b%d.gkyl", euler_output, i, j);
          snprintf(buf_del2, 64, "%s_fine_%d_b%d.gkyl", euler_output, i, j);
  ...
  snprintf(buf_coarse, 64, "%s_coarse_%d", euler_output, num_frames);
  snprintf(buf_intermediate, 64, "%s_intermediate_%d", euler_output, num_frames);
  snprintf(buf_fine, 64, "%s_fine_%d", euler_output, num_frames);
  ...
  snprintf(buf_fine_old, 64, "%s_fine_%d_b0.gkyl", euler_output, num_frames);
  snprintf(buf_fine_new, 64, "%s_%d_b0.gkyl", euler_output, num_frames);
  snprintf(buf_intermediate_old, 64, "%s_intermediate_%d_b0.gkyl", euler_output, num_frames);
  snprintf(buf_coarse_old, 64, "%s_coarse_%d_b0.gkyl", euler_output,num_frames);
  ...
  for (int i = 1; i < 9; i++) {
    ...
    char coarse_bi[64];
    char bi[64];
    snprintf(fine_bi, 64, "%s_fine_%d_b%d.gkyl", euler_output, num_frames, i);
    snprintf(intermediate_bi, 64, "%s_intermediate_%d_b%d.gkyl", euler_output, num_frames, i);
    snprintf(coarse_bi, 64, "%s_coarse_%d_b%d.gkyl", euler_output, num_frames, i);
    snprintf(bi, 64, "%s_%d_b%d.gkyl", euler_output, num_frames, i);
    rename(intermediate_bi, bi);
    remove(fine_bi);
    remove(coarse_bi);
  }
  ...
  for (int i = 9; i < 25; i++) {
    ...
    snprintf(buf_old, 64, "%s_coarse_%d_b%d.gkyl", euler_output, num_frames, i);
    snprintf(buf_new, 64, "%s_%d_b%d.gkyl", euler_output, num_frames, i);
    snprintf(buf_del1, 64, "%s_intermediate_%d_b%d.gkyl", euler_output, num_frames, i);
    snprintf(buf_del2, 64, "%s_fine_%d_b%d.gkyl", euler_output, num_frames, i);
