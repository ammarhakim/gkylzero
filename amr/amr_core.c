...
void
ten_moment_2d_run_level1(int argc, char **argv, struct ten_moment_2d_level1_init* init)
{
  struct gkyl_app_args app_args = parse_app_args(argc, argv);
  ...
  struct ten_moment_block_data coarse_bdata[num_blocks];
  struct ten_moment_block_data fine_bdata[num_blocks];
  ...
  for (int i = 0; i < num_blocks; i++)
  {
    coarse_bdata[i].fv_proj = gkyl_fv_proj_new(&coarse_bdata[i].grid, 2, 10, eval, 0);
    fine_bdata[i].fv_proj = gkyl_fv_proj_new(&fine_bdata[i].grid, 2, 10, eval, 0);
  }
  ...
  for (int i = 0; i < num_blocks; i++)
  {
    coarse_bdata[i].ten_moment = gkyl_wv_ten_moment_new(k0);
    fine_bdata[i].ten_moment = gkyl_wv_ten_moment_new(k0);

    for (int d = 0; d < ndim; d++)
    {
      coarse_bdata[i].slvr[d] = gkyl_wave_prop_new(& (struct gkyl_wave_prop_inp) {
          .grid = &coarse_bdata[i].grid,
          .equation = coarse_bdata[i].ten_moment,
          .limiter = GKYL_MONOTONIZED_CENTERED,
          .num_up_dirs = 1,
          .update_dirs = { d },
          .cfl = cfl_frac,
      ...
      fine_bdata[i].slvr[d] = gkyl_wave_prop_new(& (struct gkyl_wave_prop_inp) {
          .grid = &fine_bdata[i].grid,
          .equation = fine_bdata[i].ten_moment,
          .limiter = GKYL_MONOTONIZED_CENTERED,
          .num_up_dirs = 1,
          .update_dirs = { d },
          .cfl = cfl_frac,
  ...
  for (int i = 0; i < num_blocks; i++)
  {
    coarse_bdata[i].fdup = gkyl_array_new(GKYL_DOUBLE, 10, coarse_bdata[i].ext_range.volume);
    fine_bdata[i].fdup = gkyl_array_new(GKYL_DOUBLE, 10, fine_bdata[i].ext_range.volume);

    for (int d = 0; d < ndim + 1; d++)
    {
      coarse_bdata[i].f[d] = gkyl_array_new(GKYL_DOUBLE, 10, coarse_bdata[i].ext_range.volume);
      fine_bdata[i].f[d] = gkyl_array_new(GKYL_DOUBLE, 10, fine_bdata[i].ext_range.volume);
    }
  }

  for (int i = 0; i < num_blocks; i++)
  {
    gkyl_job_pool_add_work(coarse_job_pool, ten_moment_init_job_func, &coarse_bdata[i]);
    gkyl_job_pool_add_work(fine_job_pool, ten_moment_init_job_func, &fine_bdata[i]);
  }
  gkyl_job_pool_wait(coarse_job_pool);
  gkyl_job_pool_wait(fine_job_pool);

  ten_moment_write_sol("10m_amr_coarse_0", num_blocks, coarse_bdata);
  ten_moment_write_sol("10m_amr_fine_0", num_blocks, fine_bdata);

  double coarse_t_curr = 0.0;
  double fine_t_curr = 0.0;
  double coarse_dt = ten_moment_max_dt(num_blocks, coarse_bdata);
  double fine_dt = ten_moment_max_dt(num_blocks, fine_bdata);
  ...
  {
    printf("Taking coarse (level 0) time-step %ld at t = %g; ", coarse_step, coarse_t_curr);
    struct gkyl_update_status coarse_status = ten_moment_update(coarse_job_pool, btopo, coarse_bdata, coarse_t_curr, coarse_dt, &stats);
    printf(" dt = %g\n", coarse_status.dt_actual);
    ...
    {
      printf("   Taking fine (level 1) time-step %ld at t = %g; ", fine_step, fine_t_curr);
      struct gkyl_update_status fine_status = ten_moment_update(fine_job_pool, btopo, fine_bdata, fine_t_curr, fine_dt, &stats);
      printf(" dt = %g\n", fine_status.dt_actual);
  ...
  ten_moment_write_sol("10m_amr_coarse_1", num_blocks, coarse_bdata);
  ten_moment_write_sol("10m_amr_fine_1", num_blocks, fine_bdata);
  ...
  for (int i = 0; i < num_blocks; i++)
  {
    ...
    gkyl_wv_eqn_release(coarse_bdata[i].ten_moment);
    gkyl_wv_eqn_release(fine_bdata[i].ten_moment);

    ten_moment_block_bc_updaters_release(&coarse_bdata[i]);
    ten_moment_block_bc_updaters_release(&fine_bdata[i]);
