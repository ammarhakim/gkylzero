...
GKYL_CU_DH void fpo_vlasov_diff_surfvxvx_1x3v_ser_p1(const double* dxv, const double* diff_coeff_stencil[9], const double* f_stencil[9], double* GKYL_RESTRICT out) { 
  ...
  const double* DCL = &diff_coeff_stencil[0][0]; 
  const double* DCC = &diff_coeff_stencil[1][0]; 
  const double* DCR = &diff_coeff_stencil[2][0]; 
  ...
  df_rec_lo[0] = -(0.7654655446197428*fCL[2])-0.7654655446197428*fCC[2]-0.7954951288348656*fCL[0]+0.7954951288348656*fCC[0]; 
  df_rec_lo[1] = -(0.7654655446197428*fCL[5])-0.7654655446197428*fCC[5]-0.7954951288348656*fCL[1]+0.7954951288348656*fCC[1]; 
  df_rec_lo[2] = -(0.7654655446197428*fCL[7])-0.7654655446197428*fCC[7]-0.7954951288348656*fCL[3]+0.7954951288348656*fCC[3]; 
  df_rec_lo[3] = -(0.7654655446197428*fCL[9])-0.7654655446197428*fCC[9]-0.7954951288348656*fCL[4]+0.7954951288348656*fCC[4]; 
  df_rec_lo[4] = -(0.7654655446197428*fCL[11])-0.7654655446197428*fCC[11]-0.7954951288348656*fCL[6]+0.7954951288348656*fCC[6]; 
  df_rec_lo[5] = -(0.7654655446197428*fCL[12])-0.7654655446197428*fCC[12]-0.7954951288348656*fCL[8]+0.7954951288348656*fCC[8]; 
  df_rec_lo[6] = -(0.7654655446197428*fCL[14])-0.7654655446197428*fCC[14]-0.7954951288348656*fCL[10]+0.7954951288348656*fCC[10]; 
  df_rec_lo[7] = -(0.7654655446197428*fCL[15])-0.7654655446197428*fCC[15]-0.7954951288348656*fCL[13]+0.7954951288348656*fCC[13]; 
  df_rec_up[0] = -(0.7654655446197428*fCR[2])-0.7654655446197428*fCC[2]+0.7954951288348656*fCR[0]-0.7954951288348656*fCC[0]; 
  df_rec_up[1] = -(0.7654655446197428*fCR[5])-0.7654655446197428*fCC[5]+0.7954951288348656*fCR[1]-0.7954951288348656*fCC[1]; 
  df_rec_up[2] = -(0.7654655446197428*fCR[7])-0.7654655446197428*fCC[7]+0.7954951288348656*fCR[3]-0.7954951288348656*fCC[3]; 
  df_rec_up[3] = -(0.7654655446197428*fCR[9])-0.7654655446197428*fCC[9]+0.7954951288348656*fCR[4]-0.7954951288348656*fCC[4]; 
  df_rec_up[4] = -(0.7654655446197428*fCR[11])-0.7654655446197428*fCC[11]+0.7954951288348656*fCR[6]-0.7954951288348656*fCC[6]; 
  df_rec_up[5] = -(0.7654655446197428*fCR[12])-0.7654655446197428*fCC[12]+0.7954951288348656*fCR[8]-0.7954951288348656*fCC[8]; 
  df_rec_up[6] = -(0.7654655446197428*fCR[14])-0.7654655446197428*fCC[14]+0.7954951288348656*fCR[10]-0.7954951288348656*fCC[10]; 
  df_rec_up[7] = -(0.7654655446197428*fCR[15])-0.7654655446197428*fCC[15]+0.7954951288348656*fCR[13]-0.7954951288348656*fCC[13]; 
  ...
  out[2] += (0.6123724356957945*(surft2_lo[0]+surft1_up[0]+surft1_lo[0])-0.6123724356957945*surft2_up[0])*dv1_sq; 
  out[3] += (0.35355339059327373*surft1_up[2]-0.35355339059327373*surft1_lo[2])*dv1_sq; 
  out[4] += (0.35355339059327373*surft1_up[3]-0.35355339059327373*surft1_lo[3])*dv1_sq; 
  out[5] += (0.6123724356957945*(surft2_lo[1]+surft1_up[1]+surft1_lo[1])-0.6123724356957945*surft2_up[1])*dv1_sq; 
  out[6] += (0.35355339059327373*surft1_up[4]-0.35355339059327373*surft1_lo[4])*dv1_sq; 
  ...
  out[9] += (0.6123724356957945*(surft2_lo[3]+surft1_up[3]+surft1_lo[3])-0.6123724356957945*surft2_up[3])*dv1_sq; 
  out[10] += (0.35355339059327373*surft1_up[6]-0.35355339059327373*surft1_lo[6])*dv1_sq; 
  out[11] += (0.6123724356957945*(surft2_lo[4]+surft1_up[4]+surft1_lo[4])-0.6123724356957945*surft2_up[4])*dv1_sq; 
  out[12] += (0.6123724356957945*(surft2_lo[5]+surft1_up[5]+surft1_lo[5])-0.6123724356957945*surft2_up[5])*dv1_sq; 
  out[13] += (0.35355339059327373*surft1_up[7]-0.35355339059327373*surft1_lo[7])*dv1_sq; 
  ...
  out[16] += (-(2.3717082451262845*(surft2_up[0]+surft2_lo[0]))+0.7905694150420948*surft1_up[0]-0.7905694150420948*surft1_lo[0])*dv1_sq; 
  out[17] += (-(2.3717082451262845*(surft2_up[1]+surft2_lo[1]))+0.7905694150420949*surft1_up[1]-0.7905694150420949*surft1_lo[1])*dv1_sq; 
  out[18] += (-(2.3717082451262845*(surft2_up[2]+surft2_lo[2]))+0.7905694150420949*surft1_up[2]-0.7905694150420949*surft1_lo[2])*dv1_sq; 
  out[19] += (-(2.3717082451262845*(surft2_up[3]+surft2_lo[3]))+0.7905694150420949*surft1_up[3]-0.7905694150420949*surft1_lo[3])*dv1_sq; 
  out[20] += (-(2.3717082451262845*(surft2_up[4]+surft2_lo[4]))+0.7905694150420948*surft1_up[4]-0.7905694150420948*surft1_lo[4])*dv1_sq; 
  out[21] += (-(2.3717082451262845*(surft2_up[5]+surft2_lo[5]))+0.7905694150420948*surft1_up[5]-0.7905694150420948*surft1_lo[5])*dv1_sq; 
  out[22] += (-(2.3717082451262845*(surft2_up[6]+surft2_lo[6]))+0.7905694150420948*surft1_up[6]-0.7905694150420948*surft1_lo[6])*dv1_sq; 
  out[23] += (-(2.3717082451262845*(surft2_up[7]+surft2_lo[7]))+0.7905694150420949*surft1_up[7]-0.7905694150420949*surft1_lo[7])*dv1_sq; 
} 
