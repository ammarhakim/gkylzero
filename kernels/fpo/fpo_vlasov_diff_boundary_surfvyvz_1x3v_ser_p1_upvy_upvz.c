...
GKYL_CU_DH void fpo_vlasov_diff_boundary_surfvyvz_1x3v_ser_p1_upvy_upvz(const double *dxv, const double* diff_coeff_stencil[9], const double* f_stencil[9], double* out) { 
  ...
  const double* DBL = &diff_coeff_stencil[0][280]; 
  const double* fBL = f_stencil[0]; 
  const double* DCL = &diff_coeff_stencil[1][280]; 
  const double* fCL = f_stencil[1]; 
  const double* DBC = &diff_coeff_stencil[2][280]; 
  const double* fBC = f_stencil[2]; 
  const double* DCC = &diff_coeff_stencil[3][280]; 
