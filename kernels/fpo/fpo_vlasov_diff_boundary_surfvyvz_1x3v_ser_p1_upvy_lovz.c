...
GKYL_CU_DH void fpo_vlasov_diff_boundary_surfvyvz_1x3v_ser_p1_upvy_lovz(const double *dxv, const double* diff_coeff_stencil[9], const double* f_stencil[9], double* out) { 
  ...
  const double* DCL = &diff_coeff_stencil[0][200]; 
  const double* fCL = f_stencil[0]; 
  const double* DTL = &diff_coeff_stencil[1][200]; 
  const double* fTL = f_stencil[1]; 
  const double* DCC = &diff_coeff_stencil[2][200]; 
  const double* fCC = f_stencil[2]; 
  const double* DTC = &diff_coeff_stencil[3][200]; 
