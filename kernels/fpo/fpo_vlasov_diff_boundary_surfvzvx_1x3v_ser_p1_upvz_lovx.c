...
GKYL_CU_DH void fpo_vlasov_diff_boundary_surfvzvx_1x3v_ser_p1_upvz_lovx(const double *dxv, const double* diff_coeff_stencil[9], const double* f_stencil[9], double* out) { 
  ...
  const double* DCL = &diff_coeff_stencil[0][240]; 
  const double* fCL = f_stencil[0]; 
  const double* DCC = &diff_coeff_stencil[1][240]; 
  const double* fCC = f_stencil[1]; 
  const double* DTL = &diff_coeff_stencil[2][240]; 
  const double* fTL = f_stencil[2]; 
  const double* DTC = &diff_coeff_stencil[3][240]; 
