...
{ 
  ...
  double alphaR[3]; 
  alphaR[0] = -(0.3535533905932737*((2.23606797749979*(3.0*BstarZdBmag[3]+1.732050807568877*BstarZdBmag[1])*hamil[4]+hamil[1]*(3.0*BstarZdBmag[2]+1.732050807568877*BstarZdBmag[0]))*rdx2+2.828427124746191*apardot[0]*q_))/m_; 
  alphaR[1] = -(0.07071067811865474*((30.0*hamil[4]*BstarZdBmag[6]+1.732050807568877*(10.0*BstarZdBmag[4]*hamil[4]+5.0*BstarZdBmag[1]*hamil[1])+2.23606797749979*(15.0*BstarZdBmag[2]+8.660254037844386*BstarZdBmag[0])*hamil[4]+15.0*hamil[1]*BstarZdBmag[3])*rdx2+14.14213562373095*apardot[1]*q_))/m_; 
  alphaR[2] = -(0.07071067811865474*((15.0*hamil[1]*BstarZdBmag[6]+1.732050807568877*(10.0*BstarZdBmag[1]*hamil[4]+5.0*hamil[1]*BstarZdBmag[4])+30.0*BstarZdBmag[3]*hamil[4])*rdx2+14.14213562373095*apardot[2]*q_))/m_; 

  double fUpOrdR[3];
  if (alphaR[0]-1.118033988749896*alphaR[2] > 0.) {
    fUpOrdR[0] = (-1.25*fskin[8])-0.9682458365518543*fskin[6]+1.118033988749895*fskin[5]-0.5590169943749475*fskin[4]+0.8660254037844386*fskin[2]+0.5*fskin[0]; 
  } else {
    fUpOrdR[0] = (-1.25*fedge[8])+0.9682458365518543*fedge[6]+1.118033988749895*fedge[5]-0.5590169943749475*fedge[4]-0.8660254037844386*fedge[2]+0.5*fedge[0]; 
  }
  if (alphaR[2]-1.499999999999997*alphaR[1]+1.118033988749892*alphaR[0] > 0.) {
    fUpOrdR[1] = 1.0*fskin[8]-1.5*fskin[7]+0.7745966692414834*fskin[6]+1.118033988749895*fskin[5]+0.447213595499958*fskin[4]-1.161895003862225*fskin[3]+0.8660254037844386*fskin[2]-0.6708203932499369*fskin[1]+0.5*fskin[0]; 
  } else {
    fUpOrdR[1] = 1.0*fedge[8]-1.5*fedge[7]-0.7745966692414834*fedge[6]+1.118033988749895*fedge[5]+0.447213595499958*fedge[4]+1.161895003862225*fedge[3]-0.8660254037844386*fedge[2]-0.6708203932499369*fedge[1]+0.5*fedge[0]; 
  }
  if (alphaR[2]+1.499999999999997*alphaR[1]+1.118033988749892*alphaR[0] > 0.) {
    fUpOrdR[2] = 1.0*fskin[8]+1.5*fskin[7]+0.7745966692414834*fskin[6]+1.118033988749895*fskin[5]+0.447213595499958*fskin[4]+1.161895003862225*fskin[3]+0.8660254037844386*fskin[2]+0.6708203932499369*fskin[1]+0.5*fskin[0]; 
  } else {
    fUpOrdR[2] = 1.0*fedge[8]+1.5*fedge[7]-0.7745966692414834*fedge[6]+1.118033988749895*fedge[5]+0.447213595499958*fedge[4]-1.161895003862225*fedge[3]-0.8660254037844386*fedge[2]+0.6708203932499369*fedge[1]+0.5*fedge[0]; 
  }
  ...
  out[8] += -1.58113883008419*GhatR[2]*rdvpar2; 

  } else { 

  double alphaL[3]; 
  alphaL[0] = -(0.3535533905932737*((2.23606797749979*(1.732050807568877*BstarZdBmag[1]-3.0*BstarZdBmag[3])*hamil[4]+hamil[1]*(1.732050807568877*BstarZdBmag[0]-3.0*BstarZdBmag[2]))*rdx2+2.828427124746191*apardot[0]*q_))/m_; 
  alphaL[1] = (0.07071067811865474*((30.0*hamil[4]*BstarZdBmag[6]+1.732050807568877*((-10.0*BstarZdBmag[4]*hamil[4])-5.0*BstarZdBmag[1]*hamil[1])+2.23606797749979*(15.0*BstarZdBmag[2]-8.660254037844386*BstarZdBmag[0])*hamil[4]+15.0*hamil[1]*BstarZdBmag[3])*rdx2-14.14213562373095*apardot[1]*q_))/m_; 
  alphaL[2] = (0.07071067811865474*((15.0*hamil[1]*BstarZdBmag[6]+1.732050807568877*((-10.0*BstarZdBmag[1]*hamil[4])-5.0*hamil[1]*BstarZdBmag[4])+30.0*BstarZdBmag[3]*hamil[4])*rdx2-14.14213562373095*apardot[2]*q_))/m_; 

  double fUpOrdL[3];
  if (alphaL[0]-1.118033988749896*alphaL[2] > 0.) {
    fUpOrdL[0] = (-1.25*fedge[8])-0.9682458365518543*fedge[6]+1.118033988749895*fedge[5]-0.5590169943749475*fedge[4]+0.8660254037844386*fedge[2]+0.5*fedge[0]; 
  } else {
    fUpOrdL[0] = (-1.25*fskin[8])+0.9682458365518543*fskin[6]+1.118033988749895*fskin[5]-0.5590169943749475*fskin[4]-0.8660254037844386*fskin[2]+0.5*fskin[0]; 
  }
  if (alphaL[2]-1.499999999999997*alphaL[1]+1.118033988749892*alphaL[0] > 0.) {
    fUpOrdL[1] = 1.0*fedge[8]-1.5*fedge[7]+0.7745966692414834*fedge[6]+1.118033988749895*fedge[5]+0.447213595499958*fedge[4]-1.161895003862225*fedge[3]+0.8660254037844386*fedge[2]-0.6708203932499369*fedge[1]+0.5*fedge[0]; 
  } else {
    fUpOrdL[1] = 1.0*fskin[8]-1.5*fskin[7]-0.7745966692414834*fskin[6]+1.118033988749895*fskin[5]+0.447213595499958*fskin[4]+1.161895003862225*fskin[3]-0.8660254037844386*fskin[2]-0.6708203932499369*fskin[1]+0.5*fskin[0]; 
  }
  if (alphaL[2]+1.499999999999997*alphaL[1]+1.118033988749892*alphaL[0] > 0.) {
    fUpOrdL[2] = 1.0*fedge[8]+1.5*fedge[7]+0.7745966692414834*fedge[6]+1.118033988749895*fedge[5]+0.447213595499958*fedge[4]+1.161895003862225*fedge[3]+0.8660254037844386*fedge[2]+0.6708203932499369*fedge[1]+0.5*fedge[0]; 
  } else {
    fUpOrdL[2] = 1.0*fskin[8]+1.5*fskin[7]-0.7745966692414834*fskin[6]+1.118033988749895*fskin[5]+0.447213595499958*fskin[4]-1.161895003862225*fskin[3]-0.8660254037844386*fskin[2]+0.6708203932499369*fskin[1]+0.5*fskin[0]; 
  }
  ...
  out[8] += 1.58113883008419*GhatL[2]*rdvpar2; 
