...
GKYL_CU_DH double gyrokinetic_boundary_surfvpar_2x2v_ser_p2(const double *w, const double *dxv, const double q_, const double m_, const double *bmag, const double *jacobtot_inv, const double *cmag, const double *b_i, const double *phi, const double *apar, const double *apardot, const int edge, const double *fedge, const double *fskin, double* GKYL_RESTRICT out) 
{ 
  // w[NDIM]: cell-center.
  ...
  if (alphaR_n > 0.) {
    fUpOrdR[0] = ser_4x_p2_surfx3_eval_quad_node_0_r(fskin); 
  } else { 
    fUpOrdR[0] = ser_4x_p2_surfx3_eval_quad_node_0_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  ...
  if (alphaR_n > 0.) {
    fUpOrdR[1] = ser_4x_p2_surfx3_eval_quad_node_1_r(fskin); 
  } else { 
    fUpOrdR[1] = ser_4x_p2_surfx3_eval_quad_node_1_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.5692099788303081*alphaR[17])+0.4242640687119286*alphaR[13]-0.4242640687119282*alphaR[12]-0.4242640687119286*alphaR[11]+0.8538149682454614*alphaR[10]+0.3162277660168378*alphaR[8]+0.3162277660168378*alphaR[7]-0.6363961030678927*alphaR[6]-0.6363961030678927*alphaR[5]+0.6363961030678927*alphaR[4]+0.4743416490252568*alphaR[3]-0.4743416490252568*alphaR[2]-0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[2] = ser_4x_p2_surfx3_eval_quad_node_2_r(fskin); 
  } else { 
    fUpOrdR[2] = ser_4x_p2_surfx3_eval_quad_node_2_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  ...
  if (alphaR_n > 0.) {
    fUpOrdR[3] = ser_4x_p2_surfx3_eval_quad_node_3_r(fskin); 
  } else { 
    fUpOrdR[3] = ser_4x_p2_surfx3_eval_quad_node_3_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  ...
  if (alphaR_n > 0.) {
    fUpOrdR[4] = ser_4x_p2_surfx3_eval_quad_node_4_r(fskin); 
  } else { 
    fUpOrdR[4] = ser_4x_p2_surfx3_eval_quad_node_4_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = 0.4242640687119286*alphaR[13]+0.5303300858899102*alphaR[12]-0.3952847075210471*alphaR[8]+0.3162277660168378*alphaR[7]-0.6363961030678927*alphaR[5]+0.4743416490252568*alphaR[3]-0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[5] = ser_4x_p2_surfx3_eval_quad_node_5_r(fskin); 
  } else { 
    fUpOrdR[5] = ser_4x_p2_surfx3_eval_quad_node_5_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.5692099788303081*alphaR[17])-0.4242640687119286*alphaR[13]-0.4242640687119282*alphaR[12]+0.4242640687119286*alphaR[11]+0.8538149682454614*alphaR[10]+0.3162277660168378*alphaR[8]+0.3162277660168378*alphaR[7]-0.6363961030678927*alphaR[6]+0.6363961030678927*alphaR[5]-0.6363961030678927*alphaR[4]-0.4743416490252568*alphaR[3]+0.4743416490252568*alphaR[2]-0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[6] = ser_4x_p2_surfx3_eval_quad_node_6_r(fskin); 
  } else { 
    fUpOrdR[6] = ser_4x_p2_surfx3_eval_quad_node_6_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.4242640687119282*alphaR[12])+0.4242640687119286*alphaR[11]+0.3162277660168378*alphaR[8]+0.3162277660168378*alphaR[7]-0.6363961030678927*alphaR[4]+0.4743416490252568*alphaR[2]-0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[7] = ser_4x_p2_surfx3_eval_quad_node_7_r(fskin); 
  } else { 
    fUpOrdR[7] = ser_4x_p2_surfx3_eval_quad_node_7_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = 0.5692099788303081*alphaR[17]+0.4242640687119286*alphaR[13]-0.4242640687119282*alphaR[12]+0.4242640687119286*alphaR[11]-0.8538149682454614*alphaR[10]+0.3162277660168378*alphaR[8]+0.3162277660168378*alphaR[7]+0.6363961030678927*alphaR[6]-0.6363961030678927*alphaR[5]-0.6363961030678927*alphaR[4]+0.4743416490252568*alphaR[3]+0.4743416490252568*alphaR[2]-0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[8] = ser_4x_p2_surfx3_eval_quad_node_8_r(fskin); 
  } else { 
    fUpOrdR[8] = ser_4x_p2_surfx3_eval_quad_node_8_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.7115124735378848*alphaR[17])+0.5303300858899102*alphaR[13]+0.5303300858899102*alphaR[11]+0.3162277660168378*alphaR[8]-0.3952847075210471*alphaR[7]+0.6363961030678927*alphaR[6]-0.4743416490252568*alphaR[3]-0.4743416490252568*alphaR[2]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[9] = ser_4x_p2_surfx3_eval_quad_node_9_r(fskin); 
  } else { 
    fUpOrdR[9] = ser_4x_p2_surfx3_eval_quad_node_9_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  ...
  if (alphaR_n > 0.) {
    fUpOrdR[10] = ser_4x_p2_surfx3_eval_quad_node_10_r(fskin); 
  } else { 
    fUpOrdR[10] = ser_4x_p2_surfx3_eval_quad_node_10_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = 0.7115124735378848*alphaR[17]-0.5303300858899102*alphaR[13]+0.5303300858899102*alphaR[11]+0.3162277660168378*alphaR[8]-0.3952847075210471*alphaR[7]-0.6363961030678927*alphaR[6]+0.4743416490252568*alphaR[3]-0.4743416490252568*alphaR[2]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[11] = ser_4x_p2_surfx3_eval_quad_node_11_r(fskin); 
  } else { 
    fUpOrdR[11] = ser_4x_p2_surfx3_eval_quad_node_11_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  ...
  if (alphaR_n > 0.) {
    fUpOrdR[12] = ser_4x_p2_surfx3_eval_quad_node_12_r(fskin); 
  } else { 
    fUpOrdR[12] = ser_4x_p2_surfx3_eval_quad_node_12_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.3952847075210471*alphaR[8])-0.3952847075210471*alphaR[7]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[13] = ser_4x_p2_surfx3_eval_quad_node_13_r(fskin); 
  } else { 
    fUpOrdR[13] = ser_4x_p2_surfx3_eval_quad_node_13_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.5303300858899102*alphaR[13])-0.3952847075210471*alphaR[8]-0.3952847075210471*alphaR[7]+0.4743416490252568*alphaR[3]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[14] = ser_4x_p2_surfx3_eval_quad_node_14_r(fskin); 
  } else { 
    fUpOrdR[14] = ser_4x_p2_surfx3_eval_quad_node_14_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = 0.7115124735378848*alphaR[17]+0.5303300858899102*alphaR[13]-0.5303300858899102*alphaR[11]+0.3162277660168378*alphaR[8]-0.3952847075210471*alphaR[7]-0.6363961030678927*alphaR[6]-0.4743416490252568*alphaR[3]+0.4743416490252568*alphaR[2]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[15] = ser_4x_p2_surfx3_eval_quad_node_15_r(fskin); 
  } else { 
    fUpOrdR[15] = ser_4x_p2_surfx3_eval_quad_node_15_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.5303300858899102*alphaR[11])+0.3162277660168378*alphaR[8]-0.3952847075210471*alphaR[7]+0.4743416490252568*alphaR[2]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[16] = ser_4x_p2_surfx3_eval_quad_node_16_r(fskin); 
  } else { 
    fUpOrdR[16] = ser_4x_p2_surfx3_eval_quad_node_16_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.7115124735378848*alphaR[17])-0.5303300858899102*alphaR[13]-0.5303300858899102*alphaR[11]+0.3162277660168378*alphaR[8]-0.3952847075210471*alphaR[7]+0.6363961030678927*alphaR[6]+0.4743416490252568*alphaR[3]+0.4743416490252568*alphaR[2]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[17] = ser_4x_p2_surfx3_eval_quad_node_17_r(fskin); 
  } else { 
    fUpOrdR[17] = ser_4x_p2_surfx3_eval_quad_node_17_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = 0.5692099788303081*alphaR[17]-0.4242640687119286*alphaR[13]+0.4242640687119282*alphaR[12]-0.4242640687119286*alphaR[11]+0.8538149682454614*alphaR[10]+0.3162277660168378*alphaR[8]+0.3162277660168378*alphaR[7]+0.6363961030678927*alphaR[6]-0.6363961030678927*alphaR[5]-0.6363961030678927*alphaR[4]-0.4743416490252568*alphaR[3]-0.4743416490252568*alphaR[2]+0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[18] = ser_4x_p2_surfx3_eval_quad_node_18_r(fskin); 
  } else { 
    fUpOrdR[18] = ser_4x_p2_surfx3_eval_quad_node_18_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = 0.4242640687119282*alphaR[12]-0.4242640687119286*alphaR[11]+0.3162277660168378*alphaR[8]+0.3162277660168378*alphaR[7]-0.6363961030678927*alphaR[4]-0.4743416490252568*alphaR[2]+0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[19] = ser_4x_p2_surfx3_eval_quad_node_19_r(fskin); 
  } else { 
    fUpOrdR[19] = ser_4x_p2_surfx3_eval_quad_node_19_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.5692099788303081*alphaR[17])+0.4242640687119286*alphaR[13]+0.4242640687119282*alphaR[12]-0.4242640687119286*alphaR[11]-0.8538149682454614*alphaR[10]+0.3162277660168378*alphaR[8]+0.3162277660168378*alphaR[7]-0.6363961030678927*alphaR[6]+0.6363961030678927*alphaR[5]-0.6363961030678927*alphaR[4]+0.4743416490252568*alphaR[3]-0.4743416490252568*alphaR[2]+0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[20] = ser_4x_p2_surfx3_eval_quad_node_20_r(fskin); 
  } else { 
    fUpOrdR[20] = ser_4x_p2_surfx3_eval_quad_node_20_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.4242640687119286*alphaR[13])-0.5303300858899102*alphaR[12]-0.3952847075210471*alphaR[8]+0.3162277660168378*alphaR[7]-0.6363961030678927*alphaR[5]-0.4743416490252568*alphaR[3]+0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[21] = ser_4x_p2_surfx3_eval_quad_node_21_r(fskin); 
  } else { 
    fUpOrdR[21] = ser_4x_p2_surfx3_eval_quad_node_21_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.5303300858899102*alphaR[12])-0.3952847075210471*alphaR[8]+0.3162277660168378*alphaR[7]+0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[22] = ser_4x_p2_surfx3_eval_quad_node_22_r(fskin); 
  } else { 
    fUpOrdR[22] = ser_4x_p2_surfx3_eval_quad_node_22_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = 0.4242640687119286*alphaR[13]-0.5303300858899102*alphaR[12]-0.3952847075210471*alphaR[8]+0.3162277660168378*alphaR[7]+0.6363961030678927*alphaR[5]+0.4743416490252568*alphaR[3]+0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[23] = ser_4x_p2_surfx3_eval_quad_node_23_r(fskin); 
  } else { 
    fUpOrdR[23] = ser_4x_p2_surfx3_eval_quad_node_23_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = (-0.5692099788303081*alphaR[17])-0.4242640687119286*alphaR[13]+0.4242640687119282*alphaR[12]+0.4242640687119286*alphaR[11]-0.8538149682454614*alphaR[10]+0.3162277660168378*alphaR[8]+0.3162277660168378*alphaR[7]-0.6363961030678927*alphaR[6]-0.6363961030678927*alphaR[5]+0.6363961030678927*alphaR[4]-0.4743416490252568*alphaR[3]+0.4743416490252568*alphaR[2]+0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[24] = ser_4x_p2_surfx3_eval_quad_node_24_r(fskin); 
  } else { 
    fUpOrdR[24] = ser_4x_p2_surfx3_eval_quad_node_24_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = 0.4242640687119282*alphaR[12]+0.4242640687119286*alphaR[11]+0.3162277660168378*alphaR[8]+0.3162277660168378*alphaR[7]+0.6363961030678927*alphaR[4]+0.4743416490252568*alphaR[2]+0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[25] = ser_4x_p2_surfx3_eval_quad_node_25_r(fskin); 
  } else { 
    fUpOrdR[25] = ser_4x_p2_surfx3_eval_quad_node_25_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  alphaR_n = 0.5692099788303081*alphaR[17]+0.4242640687119286*alphaR[13]+0.4242640687119282*alphaR[12]+0.4242640687119286*alphaR[11]+0.8538149682454614*alphaR[10]+0.3162277660168378*alphaR[8]+0.3162277660168378*alphaR[7]+0.6363961030678927*alphaR[6]+0.6363961030678927*alphaR[5]+0.6363961030678927*alphaR[4]+0.4743416490252568*alphaR[3]+0.4743416490252568*alphaR[2]+0.4743416490252568*alphaR[1]+0.3535533905932734*alphaR[0];
  if (alphaR_n > 0.) {
    fUpOrdR[26] = ser_4x_p2_surfx3_eval_quad_node_26_r(fskin); 
  } else { 
    fUpOrdR[26] = ser_4x_p2_surfx3_eval_quad_node_26_l(fedge); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaR_n-fabs(alphaR_n)); 
  ...
  out[11] += -0.7071067811865475*GhatR[7]*rdvpar2; 
  out[12] += -0.7071067811865475*GhatR[8]*rdvpar2; 
  out[13] += -1.58113883008419*GhatR[0]*rdvpar2; 
  out[14] += -0.7071067811865475*GhatR[9]*rdvpar2; 
  out[15] += -1.224744871391589*GhatR[4]*rdvpar2; 
  out[16] += -0.7071067811865475*GhatR[10]*rdvpar2; 
  out[17] += -1.224744871391589*GhatR[5]*rdvpar2; 
  out[18] += -1.224744871391589*GhatR[6]*rdvpar2; 
  out[19] += -0.7071067811865475*GhatR[11]*rdvpar2; 
  out[20] += -0.7071067811865475*GhatR[12]*rdvpar2; 
  out[21] += -1.224744871391589*GhatR[7]*rdvpar2; 
  out[22] += -1.224744871391589*GhatR[8]*rdvpar2; 
  out[23] += -1.58113883008419*GhatR[1]*rdvpar2; 
  out[24] += -1.58113883008419*GhatR[2]*rdvpar2; 
  out[25] += -0.7071067811865475*GhatR[13]*rdvpar2; 
  out[26] += -0.7071067811865475*GhatR[14]*rdvpar2; 
  out[27] += -1.58113883008419*GhatR[3]*rdvpar2; 
  out[28] += -0.7071067811865475*GhatR[15]*rdvpar2; 
  out[29] += -0.7071067811865475*GhatR[16]*rdvpar2; 
  out[30] += -1.224744871391589*GhatR[9]*rdvpar2; 
  out[31] += -1.224744871391589*GhatR[10]*rdvpar2; 
  out[32] += -1.224744871391589*GhatR[11]*rdvpar2; 
  out[33] += -1.224744871391589*GhatR[12]*rdvpar2; 
  out[34] += -1.58113883008419*GhatR[4]*rdvpar2; 
  out[35] += -0.7071067811865475*GhatR[17]*rdvpar2; 
  out[36] += -0.7071067811865475*GhatR[18]*rdvpar2; 
  out[37] += -1.224744871391589*GhatR[13]*rdvpar2; 
  out[38] += -1.224744871391589*GhatR[14]*rdvpar2; 
  out[39] += -1.58113883008419*GhatR[5]*rdvpar2; 
  out[40] += -1.58113883008419*GhatR[6]*rdvpar2; 
  out[41] += -0.7071067811865475*GhatR[19]*rdvpar2; 
  out[42] += -1.224744871391589*GhatR[15]*rdvpar2; 
  out[43] += -1.224744871391589*GhatR[16]*rdvpar2; 
  out[44] += -1.224744871391589*GhatR[17]*rdvpar2; 
  out[45] += -1.224744871391589*GhatR[18]*rdvpar2; 
  out[46] += -1.58113883008419*GhatR[10]*rdvpar2; 
  out[47] += -1.224744871391589*GhatR[19]*rdvpar2; 
  ...
  if (alphaL_n > 0.) {
    fUpOrdL[1] = ser_4x_p2_surfx3_eval_quad_node_1_r(fedge); 
  } else { 
    fUpOrdL[1] = ser_4x_p2_surfx3_eval_quad_node_1_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.5692099788303081*alphaL[17])+0.4242640687119286*alphaL[13]-0.4242640687119282*alphaL[12]-0.4242640687119286*alphaL[11]+0.8538149682454614*alphaL[10]+0.3162277660168378*alphaL[8]+0.3162277660168378*alphaL[7]-0.6363961030678927*alphaL[6]-0.6363961030678927*alphaL[5]+0.6363961030678927*alphaL[4]+0.4743416490252568*alphaL[3]-0.4743416490252568*alphaL[2]-0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[2] = ser_4x_p2_surfx3_eval_quad_node_2_r(fedge); 
  } else { 
    fUpOrdL[2] = ser_4x_p2_surfx3_eval_quad_node_2_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  ...
  if (alphaL_n > 0.) {
    fUpOrdL[3] = ser_4x_p2_surfx3_eval_quad_node_3_r(fedge); 
  } else { 
    fUpOrdL[3] = ser_4x_p2_surfx3_eval_quad_node_3_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  ...
  if (alphaL_n > 0.) {
    fUpOrdL[4] = ser_4x_p2_surfx3_eval_quad_node_4_r(fedge); 
  } else { 
    fUpOrdL[4] = ser_4x_p2_surfx3_eval_quad_node_4_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = 0.4242640687119286*alphaL[13]+0.5303300858899102*alphaL[12]-0.3952847075210471*alphaL[8]+0.3162277660168378*alphaL[7]-0.6363961030678927*alphaL[5]+0.4743416490252568*alphaL[3]-0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[5] = ser_4x_p2_surfx3_eval_quad_node_5_r(fedge); 
  } else { 
    fUpOrdL[5] = ser_4x_p2_surfx3_eval_quad_node_5_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.5692099788303081*alphaL[17])-0.4242640687119286*alphaL[13]-0.4242640687119282*alphaL[12]+0.4242640687119286*alphaL[11]+0.8538149682454614*alphaL[10]+0.3162277660168378*alphaL[8]+0.3162277660168378*alphaL[7]-0.6363961030678927*alphaL[6]+0.6363961030678927*alphaL[5]-0.6363961030678927*alphaL[4]-0.4743416490252568*alphaL[3]+0.4743416490252568*alphaL[2]-0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[6] = ser_4x_p2_surfx3_eval_quad_node_6_r(fedge); 
  } else { 
    fUpOrdL[6] = ser_4x_p2_surfx3_eval_quad_node_6_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.4242640687119282*alphaL[12])+0.4242640687119286*alphaL[11]+0.3162277660168378*alphaL[8]+0.3162277660168378*alphaL[7]-0.6363961030678927*alphaL[4]+0.4743416490252568*alphaL[2]-0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[7] = ser_4x_p2_surfx3_eval_quad_node_7_r(fedge); 
  } else { 
    fUpOrdL[7] = ser_4x_p2_surfx3_eval_quad_node_7_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = 0.5692099788303081*alphaL[17]+0.4242640687119286*alphaL[13]-0.4242640687119282*alphaL[12]+0.4242640687119286*alphaL[11]-0.8538149682454614*alphaL[10]+0.3162277660168378*alphaL[8]+0.3162277660168378*alphaL[7]+0.6363961030678927*alphaL[6]-0.6363961030678927*alphaL[5]-0.6363961030678927*alphaL[4]+0.4743416490252568*alphaL[3]+0.4743416490252568*alphaL[2]-0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[8] = ser_4x_p2_surfx3_eval_quad_node_8_r(fedge); 
  } else { 
    fUpOrdL[8] = ser_4x_p2_surfx3_eval_quad_node_8_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.7115124735378848*alphaL[17])+0.5303300858899102*alphaL[13]+0.5303300858899102*alphaL[11]+0.3162277660168378*alphaL[8]-0.3952847075210471*alphaL[7]+0.6363961030678927*alphaL[6]-0.4743416490252568*alphaL[3]-0.4743416490252568*alphaL[2]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[9] = ser_4x_p2_surfx3_eval_quad_node_9_r(fedge); 
  } else { 
    fUpOrdL[9] = ser_4x_p2_surfx3_eval_quad_node_9_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  ...
  if (alphaL_n > 0.) {
    fUpOrdL[10] = ser_4x_p2_surfx3_eval_quad_node_10_r(fedge); 
  } else { 
    fUpOrdL[10] = ser_4x_p2_surfx3_eval_quad_node_10_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = 0.7115124735378848*alphaL[17]-0.5303300858899102*alphaL[13]+0.5303300858899102*alphaL[11]+0.3162277660168378*alphaL[8]-0.3952847075210471*alphaL[7]-0.6363961030678927*alphaL[6]+0.4743416490252568*alphaL[3]-0.4743416490252568*alphaL[2]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[11] = ser_4x_p2_surfx3_eval_quad_node_11_r(fedge); 
  } else { 
    fUpOrdL[11] = ser_4x_p2_surfx3_eval_quad_node_11_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  ...
  if (alphaL_n > 0.) {
    fUpOrdL[12] = ser_4x_p2_surfx3_eval_quad_node_12_r(fedge); 
  } else { 
    fUpOrdL[12] = ser_4x_p2_surfx3_eval_quad_node_12_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.3952847075210471*alphaL[8])-0.3952847075210471*alphaL[7]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[13] = ser_4x_p2_surfx3_eval_quad_node_13_r(fedge); 
  } else { 
    fUpOrdL[13] = ser_4x_p2_surfx3_eval_quad_node_13_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.5303300858899102*alphaL[13])-0.3952847075210471*alphaL[8]-0.3952847075210471*alphaL[7]+0.4743416490252568*alphaL[3]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[14] = ser_4x_p2_surfx3_eval_quad_node_14_r(fedge); 
  } else { 
    fUpOrdL[14] = ser_4x_p2_surfx3_eval_quad_node_14_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = 0.7115124735378848*alphaL[17]+0.5303300858899102*alphaL[13]-0.5303300858899102*alphaL[11]+0.3162277660168378*alphaL[8]-0.3952847075210471*alphaL[7]-0.6363961030678927*alphaL[6]-0.4743416490252568*alphaL[3]+0.4743416490252568*alphaL[2]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[15] = ser_4x_p2_surfx3_eval_quad_node_15_r(fedge); 
  } else { 
    fUpOrdL[15] = ser_4x_p2_surfx3_eval_quad_node_15_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.5303300858899102*alphaL[11])+0.3162277660168378*alphaL[8]-0.3952847075210471*alphaL[7]+0.4743416490252568*alphaL[2]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[16] = ser_4x_p2_surfx3_eval_quad_node_16_r(fedge); 
  } else { 
    fUpOrdL[16] = ser_4x_p2_surfx3_eval_quad_node_16_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.7115124735378848*alphaL[17])-0.5303300858899102*alphaL[13]-0.5303300858899102*alphaL[11]+0.3162277660168378*alphaL[8]-0.3952847075210471*alphaL[7]+0.6363961030678927*alphaL[6]+0.4743416490252568*alphaL[3]+0.4743416490252568*alphaL[2]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[17] = ser_4x_p2_surfx3_eval_quad_node_17_r(fedge); 
  } else { 
    fUpOrdL[17] = ser_4x_p2_surfx3_eval_quad_node_17_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = 0.5692099788303081*alphaL[17]-0.4242640687119286*alphaL[13]+0.4242640687119282*alphaL[12]-0.4242640687119286*alphaL[11]+0.8538149682454614*alphaL[10]+0.3162277660168378*alphaL[8]+0.3162277660168378*alphaL[7]+0.6363961030678927*alphaL[6]-0.6363961030678927*alphaL[5]-0.6363961030678927*alphaL[4]-0.4743416490252568*alphaL[3]-0.4743416490252568*alphaL[2]+0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[18] = ser_4x_p2_surfx3_eval_quad_node_18_r(fedge); 
  } else { 
    fUpOrdL[18] = ser_4x_p2_surfx3_eval_quad_node_18_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = 0.4242640687119282*alphaL[12]-0.4242640687119286*alphaL[11]+0.3162277660168378*alphaL[8]+0.3162277660168378*alphaL[7]-0.6363961030678927*alphaL[4]-0.4743416490252568*alphaL[2]+0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[19] = ser_4x_p2_surfx3_eval_quad_node_19_r(fedge); 
  } else { 
    fUpOrdL[19] = ser_4x_p2_surfx3_eval_quad_node_19_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.5692099788303081*alphaL[17])+0.4242640687119286*alphaL[13]+0.4242640687119282*alphaL[12]-0.4242640687119286*alphaL[11]-0.8538149682454614*alphaL[10]+0.3162277660168378*alphaL[8]+0.3162277660168378*alphaL[7]-0.6363961030678927*alphaL[6]+0.6363961030678927*alphaL[5]-0.6363961030678927*alphaL[4]+0.4743416490252568*alphaL[3]-0.4743416490252568*alphaL[2]+0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[20] = ser_4x_p2_surfx3_eval_quad_node_20_r(fedge); 
  } else { 
    fUpOrdL[20] = ser_4x_p2_surfx3_eval_quad_node_20_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.4242640687119286*alphaL[13])-0.5303300858899102*alphaL[12]-0.3952847075210471*alphaL[8]+0.3162277660168378*alphaL[7]-0.6363961030678927*alphaL[5]-0.4743416490252568*alphaL[3]+0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[21] = ser_4x_p2_surfx3_eval_quad_node_21_r(fedge); 
  } else { 
    fUpOrdL[21] = ser_4x_p2_surfx3_eval_quad_node_21_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.5303300858899102*alphaL[12])-0.3952847075210471*alphaL[8]+0.3162277660168378*alphaL[7]+0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[22] = ser_4x_p2_surfx3_eval_quad_node_22_r(fedge); 
  } else { 
    fUpOrdL[22] = ser_4x_p2_surfx3_eval_quad_node_22_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = 0.4242640687119286*alphaL[13]-0.5303300858899102*alphaL[12]-0.3952847075210471*alphaL[8]+0.3162277660168378*alphaL[7]+0.6363961030678927*alphaL[5]+0.4743416490252568*alphaL[3]+0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[23] = ser_4x_p2_surfx3_eval_quad_node_23_r(fedge); 
  } else { 
    fUpOrdL[23] = ser_4x_p2_surfx3_eval_quad_node_23_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = (-0.5692099788303081*alphaL[17])-0.4242640687119286*alphaL[13]+0.4242640687119282*alphaL[12]+0.4242640687119286*alphaL[11]-0.8538149682454614*alphaL[10]+0.3162277660168378*alphaL[8]+0.3162277660168378*alphaL[7]-0.6363961030678927*alphaL[6]-0.6363961030678927*alphaL[5]+0.6363961030678927*alphaL[4]-0.4743416490252568*alphaL[3]+0.4743416490252568*alphaL[2]+0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[24] = ser_4x_p2_surfx3_eval_quad_node_24_r(fedge); 
  } else { 
    fUpOrdL[24] = ser_4x_p2_surfx3_eval_quad_node_24_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = 0.4242640687119282*alphaL[12]+0.4242640687119286*alphaL[11]+0.3162277660168378*alphaL[8]+0.3162277660168378*alphaL[7]+0.6363961030678927*alphaL[4]+0.4743416490252568*alphaL[2]+0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[25] = ser_4x_p2_surfx3_eval_quad_node_25_r(fedge); 
  } else { 
    fUpOrdL[25] = ser_4x_p2_surfx3_eval_quad_node_25_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  alphaL_n = 0.5692099788303081*alphaL[17]+0.4242640687119286*alphaL[13]+0.4242640687119282*alphaL[12]+0.4242640687119286*alphaL[11]+0.8538149682454614*alphaL[10]+0.3162277660168378*alphaL[8]+0.3162277660168378*alphaL[7]+0.6363961030678927*alphaL[6]+0.6363961030678927*alphaL[5]+0.6363961030678927*alphaL[4]+0.4743416490252568*alphaL[3]+0.4743416490252568*alphaL[2]+0.4743416490252568*alphaL[1]+0.3535533905932734*alphaL[0];
  if (alphaL_n > 0.) {
    fUpOrdL[26] = ser_4x_p2_surfx3_eval_quad_node_26_r(fedge); 
  } else { 
    fUpOrdL[26] = ser_4x_p2_surfx3_eval_quad_node_26_l(fskin); 
  } 
  cflFreq += -0.15625*rdvpar2*(alphaL_n-fabs(alphaL_n)); 
  ...
  out[11] += 0.7071067811865475*GhatL[7]*rdvpar2; 
  out[12] += 0.7071067811865475*GhatL[8]*rdvpar2; 
  out[13] += 1.58113883008419*GhatL[0]*rdvpar2; 
  out[14] += 0.7071067811865475*GhatL[9]*rdvpar2; 
  out[15] += -1.224744871391589*GhatL[4]*rdvpar2; 
  out[16] += 0.7071067811865475*GhatL[10]*rdvpar2; 
  out[17] += -1.224744871391589*GhatL[5]*rdvpar2; 
  out[18] += -1.224744871391589*GhatL[6]*rdvpar2; 
  out[19] += 0.7071067811865475*GhatL[11]*rdvpar2; 
  out[20] += 0.7071067811865475*GhatL[12]*rdvpar2; 
  out[21] += -1.224744871391589*GhatL[7]*rdvpar2; 
  out[22] += -1.224744871391589*GhatL[8]*rdvpar2; 
  out[23] += 1.58113883008419*GhatL[1]*rdvpar2; 
  out[24] += 1.58113883008419*GhatL[2]*rdvpar2; 
  out[25] += 0.7071067811865475*GhatL[13]*rdvpar2; 
  out[26] += 0.7071067811865475*GhatL[14]*rdvpar2; 
  out[27] += 1.58113883008419*GhatL[3]*rdvpar2; 
  out[28] += 0.7071067811865475*GhatL[15]*rdvpar2; 
  out[29] += 0.7071067811865475*GhatL[16]*rdvpar2; 
  out[30] += -1.224744871391589*GhatL[9]*rdvpar2; 
  out[31] += -1.224744871391589*GhatL[10]*rdvpar2; 
  out[32] += -1.224744871391589*GhatL[11]*rdvpar2; 
  out[33] += -1.224744871391589*GhatL[12]*rdvpar2; 
  out[34] += 1.58113883008419*GhatL[4]*rdvpar2; 
  out[35] += 0.7071067811865475*GhatL[17]*rdvpar2; 
  out[36] += 0.7071067811865475*GhatL[18]*rdvpar2; 
  out[37] += -1.224744871391589*GhatL[13]*rdvpar2; 
  out[38] += -1.224744871391589*GhatL[14]*rdvpar2; 
  out[39] += 1.58113883008419*GhatL[5]*rdvpar2; 
  out[40] += 1.58113883008419*GhatL[6]*rdvpar2; 
  out[41] += 0.7071067811865475*GhatL[19]*rdvpar2; 
  out[42] += -1.224744871391589*GhatL[15]*rdvpar2; 
  out[43] += -1.224744871391589*GhatL[16]*rdvpar2; 
  out[44] += -1.224744871391589*GhatL[17]*rdvpar2; 
  out[45] += -1.224744871391589*GhatL[18]*rdvpar2; 
  out[46] += 1.58113883008419*GhatL[10]*rdvpar2; 
  out[47] += -1.224744871391589*GhatL[19]*rdvpar2; 
