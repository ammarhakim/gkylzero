...
GKYL_CU_DH double gyrokinetic_vol_1x1v_tensor_p2(const double *w, const double *dxv, const double q_, const double m_, const double *bmag, const double *jacobtot_inv, const double *cmag, const double *b_i, const double *phi, const double *apar, const double* apardot, const double *fin, double* GKYL_RESTRICT out) 
{ 
  // w[NDIM]: cell-center.
  ...
  out[1] += 0.8660254037844386*(alphax[8]*fin[8]+alphax[7]*fin[7]+alphax[6]*fin[6]+alphax[5]*fin[5]+alphax[4]*fin[4]+alphax[3]*fin[3]+alphax[2]*fin[2]+alphax[1]*fin[1]+alphax[0]*fin[0]); 
  ...
  out[3] += 0.1*(7.745966692414834*(alphax[6]*fin[8]+fin[6]*alphax[8]+alphax[3]*fin[7]+fin[3]*alphax[7])+(8.660254037844387*alphax[4]+7.745966692414834*alphavpar[3])*fin[6]+8.660254037844387*fin[4]*alphax[6]+7.745966692414834*(fin[3]*alphavpar[6]+alphax[2]*fin[5]+fin[2]*alphax[5]+alphavpar[1]*fin[4]+fin[1]*alphavpar[4])+8.660254037844386*((alphavpar[2]+alphax[1])*fin[3]+fin[1]*alphax[3]+fin[2]*(alphavpar[3]+alphax[0])+fin[0]*alphax[2]+alphavpar[0]*fin[1]+fin[0]*alphavpar[1])); 
  out[4] += 0.1*(17.32050807568877*(alphax[7]*fin[8]+fin[7]*alphax[8])+19.36491673103708*(alphax[5]*fin[7]+fin[5]*alphax[7])+17.32050807568877*(alphax[3]*fin[6]+fin[3]*alphax[6])+17.32050807568877*(alphax[1]*fin[4]+fin[1]*alphax[4])+19.36491673103709*(alphax[2]*fin[3]+fin[2]*alphax[3]+alphax[0]*fin[1]+fin[0]*alphax[1])); 
  out[5] += 0.1*(17.32050807568877*(alphavpar[6]*fin[8]+alphavpar[3]*fin[7])+19.36491673103708*(alphavpar[4]*fin[6]+fin[4]*alphavpar[6])+17.32050807568877*alphavpar[2]*fin[5]+19.36491673103709*(alphavpar[1]*fin[3]+fin[1]*alphavpar[3]+alphavpar[0]*fin[2]+fin[0]*alphavpar[2])); 
  out[6] += 0.01428571428571429*(108.4435336938077*(alphax[3]*fin[8]+fin[3]*alphax[8])+(108.4435336938077*alphax[6]+121.2435565298214*alphax[2])*fin[7]+(108.4435336938077*fin[6]+121.2435565298214*fin[2])*alphax[7]+(38.72983346207417*alphavpar[6]+60.6217782649107*alphavpar[2])*fin[6]+121.2435565298214*(alphax[1]*fin[6]+fin[1]*alphax[6])+60.6217782649107*fin[2]*alphavpar[6]+121.2435565298214*(alphax[3]*fin[5]+fin[3]*alphax[5])+(38.72983346207417*alphavpar[4]+121.2435565298214*alphax[3]+60.62177826491071*alphavpar[0])*fin[4]+121.2435565298214*fin[3]*alphax[4]+60.62177826491071*fin[0]*alphavpar[4]+54.22176684690384*alphavpar[3]*fin[3]+135.5544171172596*(alphax[0]*fin[3]+fin[0]*alphax[3]+alphax[1]*fin[2])+fin[1]*(135.5544171172596*alphax[2]+54.22176684690384*alphavpar[1])); 
  out[7] += 0.01428571428571429*((38.72983346207417*alphax[8]+60.62177826491071*alphax[4]+108.4435336938077*alphavpar[3])*fin[8]+60.62177826491071*fin[4]*alphax[8]+(38.72983346207417*alphax[7]+108.4435336938077*alphavpar[6]+121.2435565298214*alphavpar[2])*fin[7]+60.6217782649107*(alphax[1]*fin[7]+fin[1]*alphax[7])+54.22176684690384*alphax[6]*fin[6]+121.2435565298214*(alphavpar[1]*fin[6]+fin[1]*alphavpar[6])+(38.72983346207417*alphax[5]+121.2435565298214*alphavpar[3])*fin[5]+60.62177826491071*(alphax[0]*fin[5]+fin[0]*alphax[5])+121.2435565298214*alphavpar[3]*fin[4]+fin[3]*(121.2435565298214*alphavpar[4]+54.22176684690384*alphax[3])+135.5544171172596*(alphavpar[0]*fin[3]+fin[0]*alphavpar[3])+54.22176684690384*alphax[2]*fin[2]+135.5544171172596*(alphavpar[1]*fin[2]+fin[1]*alphavpar[2])); 
  out[8] += 0.01428571428571429*((77.45966692414834*(alphax[7]+alphavpar[6])+121.2435565298214*(alphavpar[2]+alphax[1]))*fin[8]+(77.45966692414834*fin[7]+121.2435565298214*fin[1])*alphax[8]+(86.60254037844389*alphax[5]+121.2435565298214*alphax[4]+108.4435336938077*alphavpar[3]+135.5544171172596*alphax[0])*fin[7]+(86.60254037844389*fin[5]+121.2435565298214*fin[4]+135.5544171172596*fin[0])*alphax[7]+(86.60254037844389*alphavpar[4]+108.4435336938077*alphax[3]+135.5544171172596*alphavpar[0])*fin[6]+108.4435336938077*fin[3]*alphax[6]+(121.2435565298214*fin[5]+86.60254037844389*fin[4]+135.5544171172596*fin[0])*alphavpar[6]+135.5544171172596*(alphax[1]*fin[5]+fin[1]*alphax[5]+alphavpar[2]*fin[4]+fin[2]*alphavpar[4])+121.2435565298214*((alphax[2]+alphavpar[1])*fin[3]+fin[2]*alphax[3]+fin[1]*alphavpar[3])); 
...
{ 
  ...
  out[8] += -(0.1010152544552211*(17.32050807568877*apardot[2]*fin[6]+27.11088342345192*apardot[0]*fin[6]+24.24871130596428*apardot[1]*fin[3]+27.11088342345192*apardot[2]*fin[2])*q_*rdvpar2)/m_; 
  double cflFreq = 0.0; 
  double alphaL = 0.0; 
  double alphaR = 0.0; 
  // Evaluate alpha at left surface quadrature points.
  ...
  cflFreq += -0.5*(alphaL-fabs(alphaL)); 
  alphaL = -(0.25*(0.6324555320336758*apardot[2]-0.9486832980505137*apardot[1]+0.7071067811865475*apardot[0])*q_*rdvpar2)/m_; 
  cflFreq += -0.5*(alphaL-fabs(alphaL)); 
  alphaL = -(0.25*(0.6324555320336758*apardot[2]+0.9486832980505137*apardot[1]+0.7071067811865475*apardot[0])*q_*rdvpar2)/m_; 
  cflFreq += -0.5*(alphaL-fabs(alphaL)); 
  // Evaluate alpha at right surface quadrature points.
  ...
  cflFreq += 0.5*(alphaR+fabs(alphaR)); 
  alphaR = -(0.25*(0.6324555320336758*apardot[2]-0.9486832980505137*apardot[1]+0.7071067811865475*apardot[0])*q_*rdvpar2)/m_; 
  cflFreq += 0.5*(alphaR+fabs(alphaR)); 
  alphaR = -(0.25*(0.6324555320336758*apardot[2]+0.9486832980505137*apardot[1]+0.7071067811865475*apardot[0])*q_*rdvpar2)/m_; 
  cflFreq += 0.5*(alphaR+fabs(alphaR)); 
