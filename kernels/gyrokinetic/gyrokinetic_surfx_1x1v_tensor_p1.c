...
GKYL_CU_DH void gyrokinetic_surfx_1x1v_tensor_p1(const double *w, const double *dxv, const double q_, const double m_, const double *bmag, const double *jacobtot_inv, const double *cmag, const double *b_i, const double *phi, const double *apar, const double *apardot, const double *fl, const double *fc, const double *fr, double* GKYL_RESTRICT out) 
{ 
  // w[NDIM]: cell-center.
  ...
  if (alphaL[0]-1.0*alphaL[1] > 0.) {
    fUpOrdL[0] = (-0.8660254037844386*fl[3])-0.5*fl[2]+0.8660254037844386*fl[1]+0.5*fl[0]; 
  } else {
    fUpOrdL[0] = 0.8660254037844386*fc[3]-0.5*fc[2]-0.8660254037844386*fc[1]+0.5*fc[0]; 
  }
  if (alphaL[1]+alphaL[0] > 0.) {
    fUpOrdL[1] = 0.8660254037844386*fl[3]+0.5*fl[2]+0.8660254037844386*fl[1]+0.5*fl[0]; 
  } else {
    fUpOrdL[1] = (-0.8660254037844386*fc[3])+0.5*fc[2]-0.8660254037844386*fc[1]+0.5*fc[0]; 
  }
  ...
  if (alphaR[0]-1.0*alphaR[1] > 0.) {
    fUpOrdR[0] = (-0.8660254037844386*fc[3])-0.5*fc[2]+0.8660254037844386*fc[1]+0.5*fc[0]; 
  } else {
    fUpOrdR[0] = 0.8660254037844386*fr[3]-0.5*fr[2]-0.8660254037844386*fr[1]+0.5*fr[0]; 
  }
  if (alphaR[1]+alphaR[0] > 0.) {
    fUpOrdR[1] = 0.8660254037844386*fc[3]+0.5*fc[2]+0.8660254037844386*fc[1]+0.5*fc[0]; 
  } else {
    fUpOrdR[1] = (-0.8660254037844386*fr[3])+0.5*fr[2]-0.8660254037844386*fr[1]+0.5*fr[0]; 
  }
