...
{ 
  ...
  out[0] += (0.4472135954999579*vmap1R2*f[8]*m_+vmap[0]*vmap[1]*f[2]*m_+0.5*f[0]*vmap1R2*m_+0.5*f[0]*vmap0R2*m_)*volFact; 
  out[1] += (0.44721359549995804*vmap1R2*f[12]*m_+vmap[0]*vmap[1]*f[4]*m_+0.5*f[1]*vmap1R2*m_+0.5*vmap0R2*f[1]*m_)*volFact; 
  out[2] += (1.0000000000000002*vmap[0]*vmap[1]*f[11]*m_+0.5*vmap1R2*f[7]*m_+0.5*vmap0R2*f[7]*m_)*volFact; 
  double tmp[3]; 
  tmp[0] = 1.4142135623730951*f[3]*vmap[3]+1.4142135623730951*f[0]*vmap[2]; 
  tmp[1] = 1.4142135623730951*vmap[3]*f[5]+1.4142135623730951*f[1]*vmap[2]; 
  tmp[2] = 1.4142135623730951*vmap[3]*f[13]+1.4142135623730951*vmap[2]*f[7]; 
  out[0] += (0.7071067811865475*bmag[2]*tmp[2]+0.7071067811865475*bmag[1]*tmp[1]+0.7071067811865475*bmag[0]*tmp[0])*volFact; 
  out[1] += (0.6324555320336759*bmag[1]*tmp[2]+0.6324555320336759*tmp[1]*bmag[2]+0.7071067811865475*bmag[0]*tmp[1]+0.7071067811865475*tmp[0]*bmag[1])*volFact; 
  out[2] += (0.45175395145262565*bmag[2]*tmp[2]+0.7071067811865475*bmag[0]*tmp[2]+0.7071067811865475*tmp[0]*bmag[2]+0.6324555320336759*bmag[1]*tmp[1])*volFact; 
  out[0] += (1.4142135623730951*phi[2]*f[7]*q_+1.4142135623730951*f[1]*phi[1]*q_+1.4142135623730951*f[0]*phi[0]*q_)*volFact; 
  out[1] += (1.264911064067352*phi[1]*f[7]*q_+1.264911064067352*f[1]*phi[2]*q_+1.4142135623730951*f[0]*phi[1]*q_+1.4142135623730951*phi[0]*f[1]*q_)*volFact; 
  out[2] += (0.9035079029052515*phi[2]*f[7]*q_+1.4142135623730951*phi[0]*f[7]*q_+1.4142135623730951*f[0]*phi[2]*q_+1.264911064067352*f[1]*phi[1]*q_)*volFact; 
} 
