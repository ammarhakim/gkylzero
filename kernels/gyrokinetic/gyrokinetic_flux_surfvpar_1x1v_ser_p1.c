...
{ 
  ...
  alpha_quad = -(0.8660254037844386*hamil[1]*rdx2)/m_/bmag_quad * B3_quad -(0.8660254037844386*hamil[1]*rdx2)/m_/bmag_quad * 1/q_*dualcurlbhat_quad[0]*((0.8164965809277261*(0.8660254037844386*hamil[2]-3.354101966249685*hamil[4]))/vmap[1]);

  cfl = fmax(fabs(alpha_quad), fabs(cfl)) ;
  if (alpha_quad > 0) {
    Jf_quad =  ((-1.118033988749895*JfL[5])+1.118033988749895*JfL[4]-0.8660254037844386*JfL[3]+0.8660254037844386*JfL[2]-0.5*JfL[1]+0.5*JfL[0])/vmap_prime_l[0];
    flux_surf_nodal[0] = alpha_quad*Jf_quad;
  }
  else {
    Jf_quad =  ((-1.118033988749895*JfR[5])+1.118033988749895*JfR[4]+0.8660254037844386*JfR[3]-0.8660254037844386*JfR[2]-0.5*JfR[1]+0.5*JfR[0])/vmap_prime_r[0];
    flux_surf_nodal[0] = alpha_quad*Jf_quad;
  }
