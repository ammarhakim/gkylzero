...
{ 
  ...
  if (alphaL[2]-1.499999999999997*alphaL[1]+1.118033988749892*alphaL[0] > 0.) {
    fUpOrdL[0] = fl[8]-1.5*fl[7]+0.7745966692414833*fl[6]+1.118033988749895*fl[5]+0.4472135954999579*fl[4]-1.161895003862225*fl[3]+0.8660254037844386*fl[2]-0.6708203932499369*fl[1]+0.5*fl[0]; 
  } else {
    fUpOrdL[0] = fc[8]-1.5*fc[7]-0.7745966692414833*fc[6]+1.118033988749895*fc[5]+0.4472135954999579*fc[4]+1.161895003862225*fc[3]-0.8660254037844386*fc[2]-0.6708203932499369*fc[1]+0.5*fc[0]; 
  }
  if (alphaL[0]-1.118033988749896*alphaL[2] > 0.) {
    fUpOrdL[1] = (-1.25*fl[8])-0.9682458365518543*fl[6]+1.118033988749895*fl[5]-0.5590169943749475*fl[4]+0.8660254037844386*fl[2]+0.5*fl[0]; 
  } else {
    fUpOrdL[1] = (-1.25*fc[8])+0.9682458365518543*fc[6]+1.118033988749895*fc[5]-0.5590169943749475*fc[4]-0.8660254037844386*fc[2]+0.5*fc[0]; 
  }
  if (alphaL[2]+1.499999999999997*alphaL[1]+1.118033988749892*alphaL[0] > 0.) {
    fUpOrdL[2] = fl[8]+1.5*fl[7]+0.7745966692414833*fl[6]+1.118033988749895*fl[5]+0.4472135954999579*fl[4]+1.161895003862225*fl[3]+0.8660254037844386*fl[2]+0.6708203932499369*fl[1]+0.5*fl[0]; 
  } else {
    fUpOrdL[2] = fc[8]+1.5*fc[7]-0.7745966692414833*fc[6]+1.118033988749895*fc[5]+0.4472135954999579*fc[4]-1.161895003862225*fc[3]-0.8660254037844386*fc[2]+0.6708203932499369*fc[1]+0.5*fc[0]; 
  }
  ...
  if (alphaR[2]-1.499999999999997*alphaR[1]+1.118033988749892*alphaR[0] > 0.) {
    fUpOrdR[0] = fc[8]-1.5*fc[7]+0.7745966692414833*fc[6]+1.118033988749895*fc[5]+0.4472135954999579*fc[4]-1.161895003862225*fc[3]+0.8660254037844386*fc[2]-0.6708203932499369*fc[1]+0.5*fc[0]; 
  } else {
    fUpOrdR[0] = fr[8]-1.5*fr[7]-0.7745966692414833*fr[6]+1.118033988749895*fr[5]+0.4472135954999579*fr[4]+1.161895003862225*fr[3]-0.8660254037844386*fr[2]-0.6708203932499369*fr[1]+0.5*fr[0]; 
  }
  if (alphaR[0]-1.118033988749896*alphaR[2] > 0.) {
    fUpOrdR[1] = (-1.25*fc[8])-0.9682458365518543*fc[6]+1.118033988749895*fc[5]-0.5590169943749475*fc[4]+0.8660254037844386*fc[2]+0.5*fc[0]; 
  } else {
    fUpOrdR[1] = (-1.25*fr[8])+0.9682458365518543*fr[6]+1.118033988749895*fr[5]-0.5590169943749475*fr[4]-0.8660254037844386*fr[2]+0.5*fr[0]; 
  }
  if (alphaR[2]+1.499999999999997*alphaR[1]+1.118033988749892*alphaR[0] > 0.) {
    fUpOrdR[2] = fc[8]+1.5*fc[7]+0.7745966692414833*fc[6]+1.118033988749895*fc[5]+0.4472135954999579*fc[4]+1.161895003862225*fc[3]+0.8660254037844386*fc[2]+0.6708203932499369*fc[1]+0.5*fc[0]; 
  } else {
    fUpOrdR[2] = fr[8]+1.5*fr[7]-0.7745966692414833*fr[6]+1.118033988749895*fr[5]+0.4472135954999579*fr[4]-1.161895003862225*fr[3]-0.8660254037844386*fr[2]+0.6708203932499369*fr[1]+0.5*fr[0]; 
  }
