...
GKYL_CU_DH void gyrokinetic_surfvpar_1x1v_tensor_p2(const double *w, const double *dxv, const double q_, const double m_, const double *bmag, const double *jacobtot_inv, const double *cmag, const double *b_i, const double *phi, const double *apar, const double *apardot, const double *fl, const double *fc, const double *fr, double* GKYL_RESTRICT out) 
{ 
  // w[NDIM]: cell-center.
  ...
  if (alphaL[0]-1.118033988749896*alphaL[2] > 0.) {
    fUpOrdL[0] = (-1.25*fl[8])-0.9682458365518543*fl[6]+1.118033988749895*fl[5]-0.5590169943749475*fl[4]+0.8660254037844386*fl[2]+0.5*fl[0]; 
  } else {
    fUpOrdL[0] = (-1.25*fc[8])+0.9682458365518543*fc[6]+1.118033988749895*fc[5]-0.5590169943749475*fc[4]-0.8660254037844386*fc[2]+0.5*fc[0]; 
  }
  if (alphaL[2]-1.499999999999997*alphaL[1]+1.118033988749892*alphaL[0] > 0.) {
    fUpOrdL[1] = 1.0*fl[8]-1.5*fl[7]+0.7745966692414834*fl[6]+1.118033988749895*fl[5]+0.447213595499958*fl[4]-1.161895003862225*fl[3]+0.8660254037844386*fl[2]-0.6708203932499369*fl[1]+0.5*fl[0]; 
  } else {
    fUpOrdL[1] = 1.0*fc[8]-1.5*fc[7]-0.7745966692414834*fc[6]+1.118033988749895*fc[5]+0.447213595499958*fc[4]+1.161895003862225*fc[3]-0.8660254037844386*fc[2]-0.6708203932499369*fc[1]+0.5*fc[0]; 
  }
  if (alphaL[2]+1.499999999999997*alphaL[1]+1.118033988749892*alphaL[0] > 0.) {
    fUpOrdL[2] = 1.0*fl[8]+1.5*fl[7]+0.7745966692414834*fl[6]+1.118033988749895*fl[5]+0.447213595499958*fl[4]+1.161895003862225*fl[3]+0.8660254037844386*fl[2]+0.6708203932499369*fl[1]+0.5*fl[0]; 
  } else {
    fUpOrdL[2] = 1.0*fc[8]+1.5*fc[7]-0.7745966692414834*fc[6]+1.118033988749895*fc[5]+0.447213595499958*fc[4]-1.161895003862225*fc[3]-0.8660254037844386*fc[2]+0.6708203932499369*fc[1]+0.5*fc[0]; 
  }
  ...
  if (alphaR[0]-1.118033988749896*alphaR[2] > 0.) {
    fUpOrdR[0] = (-1.25*fc[8])-0.9682458365518543*fc[6]+1.118033988749895*fc[5]-0.5590169943749475*fc[4]+0.8660254037844386*fc[2]+0.5*fc[0]; 
  } else {
    fUpOrdR[0] = (-1.25*fr[8])+0.9682458365518543*fr[6]+1.118033988749895*fr[5]-0.5590169943749475*fr[4]-0.8660254037844386*fr[2]+0.5*fr[0]; 
  }
  if (alphaR[2]-1.499999999999997*alphaR[1]+1.118033988749892*alphaR[0] > 0.) {
    fUpOrdR[1] = 1.0*fc[8]-1.5*fc[7]+0.7745966692414834*fc[6]+1.118033988749895*fc[5]+0.447213595499958*fc[4]-1.161895003862225*fc[3]+0.8660254037844386*fc[2]-0.6708203932499369*fc[1]+0.5*fc[0]; 
  } else {
    fUpOrdR[1] = 1.0*fr[8]-1.5*fr[7]-0.7745966692414834*fr[6]+1.118033988749895*fr[5]+0.447213595499958*fr[4]+1.161895003862225*fr[3]-0.8660254037844386*fr[2]-0.6708203932499369*fr[1]+0.5*fr[0]; 
  }
  if (alphaR[2]+1.499999999999997*alphaR[1]+1.118033988749892*alphaR[0] > 0.) {
    fUpOrdR[2] = 1.0*fc[8]+1.5*fc[7]+0.7745966692414834*fc[6]+1.118033988749895*fc[5]+0.447213595499958*fc[4]+1.161895003862225*fc[3]+0.8660254037844386*fc[2]+0.6708203932499369*fc[1]+0.5*fc[0]; 
  } else {
    fUpOrdR[2] = 1.0*fr[8]+1.5*fr[7]-0.7745966692414834*fr[6]+1.118033988749895*fr[5]+0.447213595499958*fr[4]-1.161895003862225*fr[3]-0.8660254037844386*fr[2]+0.6708203932499369*fr[1]+0.5*fr[0]; 
  }
  ...
  out[8] += (1.58113883008419*GhatL[2]-1.58113883008419*GhatR[2])*rdvpar2; 
