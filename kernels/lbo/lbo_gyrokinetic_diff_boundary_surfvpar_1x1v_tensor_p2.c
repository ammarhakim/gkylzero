...
GKYL_CU_DH void lbo_gyrokinetic_diff_boundary_surfvpar_1x1v_tensor_p2(const double *w, const double *dxv, const double m_, const double *bmag_inv, const double *nuSum, const double *nuUSum, const double *nuVtSqSum, const int edge, const double *fedge, const double *fskin, double* GKYL_RESTRICT out) 
{ 
  ...
  vol_incr[5] = 4.743416490252569*nuVtSqSum[2]*fskin[4]*rdvSq4+4.743416490252569*fskin[1]*nuVtSqSum[1]*rdvSq4+4.743416490252569*fskin[0]*nuVtSqSum[0]*rdvSq4; 
  vol_incr[7] = 4.242640687119286*nuVtSqSum[1]*fskin[4]*rdvSq4+4.242640687119286*fskin[1]*nuVtSqSum[2]*rdvSq4+4.743416490252569*fskin[0]*nuVtSqSum[1]*rdvSq4+4.743416490252569*nuVtSqSum[0]*fskin[1]*rdvSq4; 
  vol_incr[8] = 3.030457633656632*nuVtSqSum[2]*fskin[4]*rdvSq4+4.743416490252569*nuVtSqSum[0]*fskin[4]*rdvSq4+4.743416490252569*fskin[0]*nuVtSqSum[2]*rdvSq4+4.242640687119286*fskin[1]*nuVtSqSum[1]*rdvSq4; 

  double temp_diff[9] = {0.0}; 
  double temp_edge[9] = {0.0}; 
  double diff_incr[9] = {0.0}; 
  double edge_incr[9] = {0.0}; 
  ...
  temp_diff[4] = (-0.6708203932499369*fskin[8])+0.6708203932499369*fedge[8]-1.190784930203603*fskin[6]-1.190784930203603*fedge[6]-0.9375*fskin[4]+0.9375*fedge[4]; 
  ...
  temp_diff[6] = (-1.585502557353661*fskin[8])+0.7382874503707888*fedge[8]-2.671875*fskin[6]-1.453125*fedge[6]-2.056810333988042*fskin[4]+1.190784930203603*fedge[4]; 
  ...
  temp_diff[8] = (-3.140625*fskin[8])-0.140625*fedge[8]-5.022775277112744*fskin[6]-0.3025768239224544*fedge[6]-3.773364712030896*fskin[4]+0.4192627457812106*fedge[4]; 
  ...
  temp_edge[6] = 1.936491673103709*fskin[8]-1.5*fskin[6]+0.8660254037844387*fskin[4]; 
  temp_edge[7] = (-7.5*fskin[7])+5.809475019311126*fskin[3]-3.354101966249684*fskin[1]; 
  temp_edge[8] = (-7.5*fskin[8])+5.809475019311126*fskin[6]-3.354101966249685*fskin[4]; 
  ...
  diff_incr[5] = 0.7071067811865475*nuVtSqSum[2]*temp_diff[8]+0.7071067811865475*nuVtSqSum[1]*temp_diff[7]+0.7071067811865475*nuVtSqSum[0]*temp_diff[5]; 
  ...
  diff_incr[7] = 0.632455532033676*nuVtSqSum[1]*temp_diff[8]+0.6324555320336759*nuVtSqSum[2]*temp_diff[7]+0.7071067811865475*nuVtSqSum[0]*temp_diff[7]+0.7071067811865475*nuVtSqSum[1]*temp_diff[5]; 
  diff_incr[8] = 0.4517539514526256*nuVtSqSum[2]*temp_diff[8]+0.7071067811865475*nuVtSqSum[0]*temp_diff[8]+0.632455532033676*nuVtSqSum[1]*temp_diff[7]+0.7071067811865475*nuVtSqSum[2]*temp_diff[5]; 
  ...
  edge_incr[5] = 0.7071067811865475*nuVtSqSum[2]*temp_edge[8]+0.7071067811865475*nuVtSqSum[1]*temp_edge[7]+0.7071067811865475*nuVtSqSum[0]*temp_edge[5]; 
  ...
  edge_incr[7] = 0.632455532033676*nuVtSqSum[1]*temp_edge[8]+0.6324555320336759*nuVtSqSum[2]*temp_edge[7]+0.7071067811865475*nuVtSqSum[0]*temp_edge[7]+0.7071067811865475*nuVtSqSum[1]*temp_edge[5]; 
  edge_incr[8] = 0.4517539514526256*nuVtSqSum[2]*temp_edge[8]+0.7071067811865475*nuVtSqSum[0]*temp_edge[8]+0.632455532033676*nuVtSqSum[1]*temp_edge[7]+0.7071067811865475*nuVtSqSum[2]*temp_edge[5]; 
