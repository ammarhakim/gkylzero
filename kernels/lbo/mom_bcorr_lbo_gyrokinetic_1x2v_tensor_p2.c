...
{ 
  ...
  if (edge == GKYL_VX_LOWER) {
    ...
    out[3] += vBoundary[0]*((-2.23606797749979*fIn[8])+1.732050807568877*fIn[2]-1.0*fIn[0])*dS; 
    out[4] += vBoundary[0]*((-2.23606797749979*fIn[12])+1.732050807568877*fIn[4]-1.0*fIn[1])*dS; 
    out[5] += vBoundary[0]*((-2.23606797749979*fIn[20])+1.732050807568877*fIn[11]-1.0*fIn[7])*dS; 
    ...
    out[3] += vBoundary[2]*(2.23606797749979*fIn[8]+1.732050807568877*fIn[2]+fIn[0])*dS; 
    out[4] += vBoundary[2]*(2.23606797749979*fIn[12]+1.732050807568877*fIn[4]+fIn[1])*dS; 
    out[5] += vBoundary[2]*(2.23606797749979*fIn[20]+1.732050807568877*fIn[11]+fIn[7])*dS; 
  }
  ...
  if (edge == GKYL_VY_LOWER) {
    ...
    out[5] += vBoundary[1]*((-2.23606797749979*fIn[21])+1.732050807568877*fIn[13]-1.0*fIn[7])*dS; 
    ...
    out[5] += vBoundary[3]*(2.23606797749979*fIn[21]+1.732050807568877*fIn[13]+fIn[7])*dS; 
  }
