...
GKYL_CU_DH void vlasov_lbo_constNu_surfvy_2x2v_ser_p1(const double *w, const double *dxv, const double nuSum, const double *nuUSum_l, const double *nuUSum_r, const double *nuVtSqSum_l, const double *nuVtSqSum_r, const double *fl, const double *fc, const double *fr, double* GKYL_RESTRICT out) 
{ 
  ...
  const double *sumNuUy_l = &nuUSum_l[4]; 
  const double *sumNuUy_r = &nuUSum_r[4]; 
  ...
  Gdiff2_l[0] = 0.05892556509887892*(3.464101615137754*nuVtSqSum_l[3]*fl[12]-3.464101615137754*nuVtSqSum_l[3]*fc[12]+3.464101615137754*nuVtSqSum_l[2]*fl[9]-3.464101615137754*nuVtSqSum_l[2]*fc[9]+3.464101615137754*nuVtSqSum_l[1]*fl[8]-3.464101615137754*nuVtSqSum_l[1]*fc[8]+3.0*nuVtSqSum_l[3]*fl[5]+3.0*nuVtSqSum_l[3]*fc[5]+3.464101615137754*nuVtSqSum_l[0]*fl[4]-3.464101615137754*nuVtSqSum_l[0]*fc[4]+(3.0*fl[2]+3.0*fc[2])*nuVtSqSum_l[2]+(3.0*fl[1]+3.0*fc[1])*nuVtSqSum_l[1]+(3.0*fl[0]+3.0*fc[0])*nuVtSqSum_l[0]); 
  Gdiff2_l[1] = 0.05892556509887892*(3.464101615137754*nuVtSqSum_l[2]*fl[12]-3.464101615137754*nuVtSqSum_l[2]*fc[12]+3.464101615137754*nuVtSqSum_l[3]*fl[9]-3.464101615137754*nuVtSqSum_l[3]*fc[9]+3.464101615137754*nuVtSqSum_l[0]*fl[8]-3.464101615137754*nuVtSqSum_l[0]*fc[8]+3.0*nuVtSqSum_l[2]*fl[5]+3.0*nuVtSqSum_l[2]*fc[5]+3.464101615137754*nuVtSqSum_l[1]*fl[4]-3.464101615137754*nuVtSqSum_l[1]*fc[4]+(3.0*fl[2]+3.0*fc[2])*nuVtSqSum_l[3]+(3.0*fl[0]+3.0*fc[0])*nuVtSqSum_l[1]+3.0*nuVtSqSum_l[0]*fl[1]+3.0*nuVtSqSum_l[0]*fc[1]); 
  Gdiff2_l[2] = 0.05892556509887892*(3.464101615137754*nuVtSqSum_l[1]*fl[12]-3.464101615137754*nuVtSqSum_l[1]*fc[12]+3.464101615137754*nuVtSqSum_l[0]*fl[9]-3.464101615137754*nuVtSqSum_l[0]*fc[9]+3.464101615137754*nuVtSqSum_l[3]*fl[8]-3.464101615137754*nuVtSqSum_l[3]*fc[8]+3.0*nuVtSqSum_l[1]*fl[5]+3.0*nuVtSqSum_l[1]*fc[5]+3.464101615137754*nuVtSqSum_l[2]*fl[4]-3.464101615137754*nuVtSqSum_l[2]*fc[4]+(3.0*fl[1]+3.0*fc[1])*nuVtSqSum_l[3]+(3.0*fl[0]+3.0*fc[0])*nuVtSqSum_l[2]+3.0*nuVtSqSum_l[0]*fl[2]+3.0*nuVtSqSum_l[0]*fc[2]); 
  Gdiff2_l[3] = 0.05892556509887892*(3.464101615137754*nuVtSqSum_l[3]*fl[15]-3.464101615137754*nuVtSqSum_l[3]*fc[15]+3.464101615137754*nuVtSqSum_l[2]*fl[14]-3.464101615137754*nuVtSqSum_l[2]*fc[14]+3.464101615137754*nuVtSqSum_l[1]*fl[13]-3.464101615137754*nuVtSqSum_l[1]*fc[13]+3.0*nuVtSqSum_l[3]*fl[11]+3.0*nuVtSqSum_l[3]*fc[11]+3.464101615137754*nuVtSqSum_l[0]*fl[10]-3.464101615137754*nuVtSqSum_l[0]*fc[10]+3.0*nuVtSqSum_l[2]*fl[7]+3.0*nuVtSqSum_l[2]*fc[7]+3.0*nuVtSqSum_l[1]*fl[6]+3.0*nuVtSqSum_l[1]*fc[6]+3.0*nuVtSqSum_l[0]*fl[3]+3.0*nuVtSqSum_l[0]*fc[3]); 
  Gdiff2_l[4] = 0.05892556509887892*(3.464101615137754*nuVtSqSum_l[0]*fl[12]-3.464101615137754*nuVtSqSum_l[0]*fc[12]+3.464101615137754*nuVtSqSum_l[1]*fl[9]-3.464101615137754*nuVtSqSum_l[1]*fc[9]+3.464101615137754*nuVtSqSum_l[2]*fl[8]-3.464101615137754*nuVtSqSum_l[2]*fc[8]+3.0*nuVtSqSum_l[0]*fl[5]+3.0*nuVtSqSum_l[0]*fc[5]+3.464101615137754*nuVtSqSum_l[3]*fl[4]-3.464101615137754*nuVtSqSum_l[3]*fc[4]+(3.0*fl[0]+3.0*fc[0])*nuVtSqSum_l[3]+(3.0*fl[1]+3.0*fc[1])*nuVtSqSum_l[2]+3.0*nuVtSqSum_l[1]*fl[2]+3.0*nuVtSqSum_l[1]*fc[2]); 
  Gdiff2_l[5] = 0.05892556509887892*(3.464101615137754*nuVtSqSum_l[2]*fl[15]-3.464101615137754*nuVtSqSum_l[2]*fc[15]+3.464101615137754*nuVtSqSum_l[3]*fl[14]-3.464101615137754*nuVtSqSum_l[3]*fc[14]+3.464101615137754*nuVtSqSum_l[0]*fl[13]-3.464101615137754*nuVtSqSum_l[0]*fc[13]+3.0*nuVtSqSum_l[2]*fl[11]+3.0*nuVtSqSum_l[2]*fc[11]+3.464101615137754*nuVtSqSum_l[1]*fl[10]-3.464101615137754*nuVtSqSum_l[1]*fc[10]+3.0*nuVtSqSum_l[3]*fl[7]+3.0*nuVtSqSum_l[3]*fc[7]+3.0*nuVtSqSum_l[0]*fl[6]+3.0*nuVtSqSum_l[0]*fc[6]+3.0*nuVtSqSum_l[1]*fl[3]+3.0*nuVtSqSum_l[1]*fc[3]); 
  Gdiff2_l[6] = 0.05892556509887892*(3.464101615137754*nuVtSqSum_l[1]*fl[15]-3.464101615137754*nuVtSqSum_l[1]*fc[15]+3.464101615137754*nuVtSqSum_l[0]*fl[14]-3.464101615137754*nuVtSqSum_l[0]*fc[14]+3.464101615137754*nuVtSqSum_l[3]*fl[13]-3.464101615137754*nuVtSqSum_l[3]*fc[13]+3.0*nuVtSqSum_l[1]*fl[11]+3.0*nuVtSqSum_l[1]*fc[11]+3.464101615137754*nuVtSqSum_l[2]*fl[10]-3.464101615137754*nuVtSqSum_l[2]*fc[10]+3.0*nuVtSqSum_l[0]*fl[7]+3.0*nuVtSqSum_l[0]*fc[7]+3.0*nuVtSqSum_l[3]*fl[6]+3.0*nuVtSqSum_l[3]*fc[6]+3.0*nuVtSqSum_l[2]*fl[3]+3.0*nuVtSqSum_l[2]*fc[3]); 
  Gdiff2_l[7] = 0.05892556509887892*(3.464101615137754*nuVtSqSum_l[0]*fl[15]-3.464101615137754*nuVtSqSum_l[0]*fc[15]+3.464101615137754*nuVtSqSum_l[1]*fl[14]-3.464101615137754*nuVtSqSum_l[1]*fc[14]+3.464101615137754*nuVtSqSum_l[2]*fl[13]-3.464101615137754*nuVtSqSum_l[2]*fc[13]+3.0*nuVtSqSum_l[0]*fl[11]+3.0*nuVtSqSum_l[0]*fc[11]+3.464101615137754*nuVtSqSum_l[3]*fl[10]-3.464101615137754*nuVtSqSum_l[3]*fc[10]+3.0*nuVtSqSum_l[1]*fl[7]+3.0*nuVtSqSum_l[1]*fc[7]+3.0*nuVtSqSum_l[2]*fl[6]+3.0*nuVtSqSum_l[2]*fc[6]+(3.0*fl[3]+3.0*fc[3])*nuVtSqSum_l[3]); 


  Gdiff2_r[0] = -0.05892556509887892*(3.464101615137754*nuVtSqSum_r[3]*fr[12]-3.464101615137754*nuVtSqSum_r[3]*fc[12]+3.464101615137754*nuVtSqSum_r[2]*fr[9]-3.464101615137754*nuVtSqSum_r[2]*fc[9]+3.464101615137754*nuVtSqSum_r[1]*fr[8]-3.464101615137754*nuVtSqSum_r[1]*fc[8]-3.0*nuVtSqSum_r[3]*fr[5]-3.0*nuVtSqSum_r[3]*fc[5]+3.464101615137754*nuVtSqSum_r[0]*fr[4]-3.464101615137754*nuVtSqSum_r[0]*fc[4]+((-3.0*fr[2])-3.0*fc[2])*nuVtSqSum_r[2]+((-3.0*fr[1])-3.0*fc[1])*nuVtSqSum_r[1]+((-3.0*fr[0])-3.0*fc[0])*nuVtSqSum_r[0]); 
  Gdiff2_r[1] = -0.05892556509887892*(3.464101615137754*nuVtSqSum_r[2]*fr[12]-3.464101615137754*nuVtSqSum_r[2]*fc[12]+3.464101615137754*nuVtSqSum_r[3]*fr[9]-3.464101615137754*nuVtSqSum_r[3]*fc[9]+3.464101615137754*nuVtSqSum_r[0]*fr[8]-3.464101615137754*nuVtSqSum_r[0]*fc[8]-3.0*nuVtSqSum_r[2]*fr[5]-3.0*nuVtSqSum_r[2]*fc[5]+3.464101615137754*nuVtSqSum_r[1]*fr[4]-3.464101615137754*nuVtSqSum_r[1]*fc[4]+((-3.0*fr[2])-3.0*fc[2])*nuVtSqSum_r[3]+((-3.0*fr[0])-3.0*fc[0])*nuVtSqSum_r[1]-3.0*nuVtSqSum_r[0]*fr[1]-3.0*nuVtSqSum_r[0]*fc[1]); 
  Gdiff2_r[2] = -0.05892556509887892*(3.464101615137754*nuVtSqSum_r[1]*fr[12]-3.464101615137754*nuVtSqSum_r[1]*fc[12]+3.464101615137754*nuVtSqSum_r[0]*fr[9]-3.464101615137754*nuVtSqSum_r[0]*fc[9]+3.464101615137754*nuVtSqSum_r[3]*fr[8]-3.464101615137754*nuVtSqSum_r[3]*fc[8]-3.0*nuVtSqSum_r[1]*fr[5]-3.0*nuVtSqSum_r[1]*fc[5]+3.464101615137754*nuVtSqSum_r[2]*fr[4]-3.464101615137754*nuVtSqSum_r[2]*fc[4]+((-3.0*fr[1])-3.0*fc[1])*nuVtSqSum_r[3]+((-3.0*fr[0])-3.0*fc[0])*nuVtSqSum_r[2]-3.0*nuVtSqSum_r[0]*fr[2]-3.0*nuVtSqSum_r[0]*fc[2]); 
  Gdiff2_r[3] = -0.05892556509887892*(3.464101615137754*nuVtSqSum_r[3]*fr[15]-3.464101615137754*nuVtSqSum_r[3]*fc[15]+3.464101615137754*nuVtSqSum_r[2]*fr[14]-3.464101615137754*nuVtSqSum_r[2]*fc[14]+3.464101615137754*nuVtSqSum_r[1]*fr[13]-3.464101615137754*nuVtSqSum_r[1]*fc[13]-3.0*nuVtSqSum_r[3]*fr[11]-3.0*nuVtSqSum_r[3]*fc[11]+3.464101615137754*nuVtSqSum_r[0]*fr[10]-3.464101615137754*nuVtSqSum_r[0]*fc[10]-3.0*nuVtSqSum_r[2]*fr[7]-3.0*nuVtSqSum_r[2]*fc[7]-3.0*nuVtSqSum_r[1]*fr[6]-3.0*nuVtSqSum_r[1]*fc[6]-3.0*nuVtSqSum_r[0]*fr[3]-3.0*nuVtSqSum_r[0]*fc[3]); 
  Gdiff2_r[4] = -0.05892556509887892*(3.464101615137754*nuVtSqSum_r[0]*fr[12]-3.464101615137754*nuVtSqSum_r[0]*fc[12]+3.464101615137754*nuVtSqSum_r[1]*fr[9]-3.464101615137754*nuVtSqSum_r[1]*fc[9]+3.464101615137754*nuVtSqSum_r[2]*fr[8]-3.464101615137754*nuVtSqSum_r[2]*fc[8]-3.0*nuVtSqSum_r[0]*fr[5]-3.0*nuVtSqSum_r[0]*fc[5]+3.464101615137754*nuVtSqSum_r[3]*fr[4]-3.464101615137754*nuVtSqSum_r[3]*fc[4]+((-3.0*fr[0])-3.0*fc[0])*nuVtSqSum_r[3]+((-3.0*fr[1])-3.0*fc[1])*nuVtSqSum_r[2]-3.0*nuVtSqSum_r[1]*fr[2]-3.0*nuVtSqSum_r[1]*fc[2]); 
  Gdiff2_r[5] = -0.05892556509887892*(3.464101615137754*nuVtSqSum_r[2]*fr[15]-3.464101615137754*nuVtSqSum_r[2]*fc[15]+3.464101615137754*nuVtSqSum_r[3]*fr[14]-3.464101615137754*nuVtSqSum_r[3]*fc[14]+3.464101615137754*nuVtSqSum_r[0]*fr[13]-3.464101615137754*nuVtSqSum_r[0]*fc[13]-3.0*nuVtSqSum_r[2]*fr[11]-3.0*nuVtSqSum_r[2]*fc[11]+3.464101615137754*nuVtSqSum_r[1]*fr[10]-3.464101615137754*nuVtSqSum_r[1]*fc[10]-3.0*nuVtSqSum_r[3]*fr[7]-3.0*nuVtSqSum_r[3]*fc[7]-3.0*nuVtSqSum_r[0]*fr[6]-3.0*nuVtSqSum_r[0]*fc[6]-3.0*nuVtSqSum_r[1]*fr[3]-3.0*nuVtSqSum_r[1]*fc[3]); 
  Gdiff2_r[6] = -0.05892556509887892*(3.464101615137754*nuVtSqSum_r[1]*fr[15]-3.464101615137754*nuVtSqSum_r[1]*fc[15]+3.464101615137754*nuVtSqSum_r[0]*fr[14]-3.464101615137754*nuVtSqSum_r[0]*fc[14]+3.464101615137754*nuVtSqSum_r[3]*fr[13]-3.464101615137754*nuVtSqSum_r[3]*fc[13]-3.0*nuVtSqSum_r[1]*fr[11]-3.0*nuVtSqSum_r[1]*fc[11]+3.464101615137754*nuVtSqSum_r[2]*fr[10]-3.464101615137754*nuVtSqSum_r[2]*fc[10]-3.0*nuVtSqSum_r[0]*fr[7]-3.0*nuVtSqSum_r[0]*fc[7]-3.0*nuVtSqSum_r[3]*fr[6]-3.0*nuVtSqSum_r[3]*fc[6]-3.0*nuVtSqSum_r[2]*fr[3]-3.0*nuVtSqSum_r[2]*fc[3]); 
  Gdiff2_r[7] = -0.05892556509887892*(3.464101615137754*nuVtSqSum_r[0]*fr[15]-3.464101615137754*nuVtSqSum_r[0]*fc[15]+3.464101615137754*nuVtSqSum_r[1]*fr[14]-3.464101615137754*nuVtSqSum_r[1]*fc[14]+3.464101615137754*nuVtSqSum_r[2]*fr[13]-3.464101615137754*nuVtSqSum_r[2]*fc[13]-3.0*nuVtSqSum_r[0]*fr[11]-3.0*nuVtSqSum_r[0]*fc[11]+3.464101615137754*nuVtSqSum_r[3]*fr[10]-3.464101615137754*nuVtSqSum_r[3]*fc[10]-3.0*nuVtSqSum_r[1]*fr[7]-3.0*nuVtSqSum_r[1]*fc[7]-3.0*nuVtSqSum_r[2]*fr[6]-3.0*nuVtSqSum_r[2]*fc[6]+((-3.0*fr[3])-3.0*fc[3])*nuVtSqSum_r[3]); 


  Gdiff_l[0] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_l[3]*fl[12]+5.0*nuVtSqSum_l[3]*fc[12]+5.0*nuVtSqSum_l[2]*fl[9]+5.0*nuVtSqSum_l[2]*fc[9]+5.0*nuVtSqSum_l[1]*fl[8]+5.0*nuVtSqSum_l[1]*fc[8]+5.0*nuVtSqSum_l[0]*fl[4]+5.0*nuVtSqSum_l[0]*fc[4])+9.0*nuVtSqSum_l[3]*fl[5]-9.0*nuVtSqSum_l[3]*fc[5]+(9.0*fl[2]-9.0*fc[2])*nuVtSqSum_l[2]+(9.0*fl[1]-9.0*fc[1])*nuVtSqSum_l[1]+(9.0*fl[0]-9.0*fc[0])*nuVtSqSum_l[0]); 
  Gdiff_l[1] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_l[2]*fl[12]+5.0*nuVtSqSum_l[2]*fc[12]+5.0*nuVtSqSum_l[3]*fl[9]+5.0*nuVtSqSum_l[3]*fc[9]+5.0*nuVtSqSum_l[0]*fl[8]+5.0*nuVtSqSum_l[0]*fc[8]+5.0*nuVtSqSum_l[1]*fl[4]+5.0*nuVtSqSum_l[1]*fc[4])+9.0*nuVtSqSum_l[2]*fl[5]-9.0*nuVtSqSum_l[2]*fc[5]+(9.0*fl[2]-9.0*fc[2])*nuVtSqSum_l[3]+(9.0*fl[0]-9.0*fc[0])*nuVtSqSum_l[1]+9.0*nuVtSqSum_l[0]*fl[1]-9.0*nuVtSqSum_l[0]*fc[1]); 
  Gdiff_l[2] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_l[1]*fl[12]+5.0*nuVtSqSum_l[1]*fc[12]+5.0*nuVtSqSum_l[0]*fl[9]+5.0*nuVtSqSum_l[0]*fc[9]+5.0*nuVtSqSum_l[3]*fl[8]+5.0*nuVtSqSum_l[3]*fc[8]+5.0*nuVtSqSum_l[2]*fl[4]+5.0*nuVtSqSum_l[2]*fc[4])+9.0*nuVtSqSum_l[1]*fl[5]-9.0*nuVtSqSum_l[1]*fc[5]+(9.0*fl[1]-9.0*fc[1])*nuVtSqSum_l[3]+(9.0*fl[0]-9.0*fc[0])*nuVtSqSum_l[2]+9.0*nuVtSqSum_l[0]*fl[2]-9.0*nuVtSqSum_l[0]*fc[2]); 
  Gdiff_l[3] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_l[3]*fl[15]+5.0*nuVtSqSum_l[3]*fc[15]+5.0*nuVtSqSum_l[2]*fl[14]+5.0*nuVtSqSum_l[2]*fc[14]+5.0*nuVtSqSum_l[1]*fl[13]+5.0*nuVtSqSum_l[1]*fc[13]+5.0*nuVtSqSum_l[0]*fl[10]+5.0*nuVtSqSum_l[0]*fc[10])+9.0*nuVtSqSum_l[3]*fl[11]-9.0*nuVtSqSum_l[3]*fc[11]+9.0*nuVtSqSum_l[2]*fl[7]-9.0*nuVtSqSum_l[2]*fc[7]+9.0*nuVtSqSum_l[1]*fl[6]-9.0*nuVtSqSum_l[1]*fc[6]+9.0*nuVtSqSum_l[0]*fl[3]-9.0*nuVtSqSum_l[0]*fc[3]); 
  Gdiff_l[4] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_l[0]*fl[12]+5.0*nuVtSqSum_l[0]*fc[12]+5.0*nuVtSqSum_l[1]*fl[9]+5.0*nuVtSqSum_l[1]*fc[9]+5.0*nuVtSqSum_l[2]*fl[8]+5.0*nuVtSqSum_l[2]*fc[8]+5.0*nuVtSqSum_l[3]*fl[4]+5.0*nuVtSqSum_l[3]*fc[4])+9.0*nuVtSqSum_l[0]*fl[5]-9.0*nuVtSqSum_l[0]*fc[5]+(9.0*fl[0]-9.0*fc[0])*nuVtSqSum_l[3]+(9.0*fl[1]-9.0*fc[1])*nuVtSqSum_l[2]+9.0*nuVtSqSum_l[1]*fl[2]-9.0*nuVtSqSum_l[1]*fc[2]); 
  Gdiff_l[5] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_l[2]*fl[15]+5.0*nuVtSqSum_l[2]*fc[15]+5.0*nuVtSqSum_l[3]*fl[14]+5.0*nuVtSqSum_l[3]*fc[14]+5.0*nuVtSqSum_l[0]*fl[13]+5.0*nuVtSqSum_l[0]*fc[13]+5.0*nuVtSqSum_l[1]*fl[10]+5.0*nuVtSqSum_l[1]*fc[10])+9.0*nuVtSqSum_l[2]*fl[11]-9.0*nuVtSqSum_l[2]*fc[11]+9.0*nuVtSqSum_l[3]*fl[7]-9.0*nuVtSqSum_l[3]*fc[7]+9.0*nuVtSqSum_l[0]*fl[6]-9.0*nuVtSqSum_l[0]*fc[6]+9.0*nuVtSqSum_l[1]*fl[3]-9.0*nuVtSqSum_l[1]*fc[3]); 
  Gdiff_l[6] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_l[1]*fl[15]+5.0*nuVtSqSum_l[1]*fc[15]+5.0*nuVtSqSum_l[0]*fl[14]+5.0*nuVtSqSum_l[0]*fc[14]+5.0*nuVtSqSum_l[3]*fl[13]+5.0*nuVtSqSum_l[3]*fc[13]+5.0*nuVtSqSum_l[2]*fl[10]+5.0*nuVtSqSum_l[2]*fc[10])+9.0*nuVtSqSum_l[1]*fl[11]-9.0*nuVtSqSum_l[1]*fc[11]+9.0*nuVtSqSum_l[0]*fl[7]-9.0*nuVtSqSum_l[0]*fc[7]+9.0*nuVtSqSum_l[3]*fl[6]-9.0*nuVtSqSum_l[3]*fc[6]+9.0*nuVtSqSum_l[2]*fl[3]-9.0*nuVtSqSum_l[2]*fc[3]); 
  Gdiff_l[7] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_l[0]*fl[15]+5.0*nuVtSqSum_l[0]*fc[15]+5.0*nuVtSqSum_l[1]*fl[14]+5.0*nuVtSqSum_l[1]*fc[14]+5.0*nuVtSqSum_l[2]*fl[13]+5.0*nuVtSqSum_l[2]*fc[13]+5.0*nuVtSqSum_l[3]*fl[10]+5.0*nuVtSqSum_l[3]*fc[10])+9.0*nuVtSqSum_l[0]*fl[11]-9.0*nuVtSqSum_l[0]*fc[11]+9.0*nuVtSqSum_l[1]*fl[7]-9.0*nuVtSqSum_l[1]*fc[7]+9.0*nuVtSqSum_l[2]*fl[6]-9.0*nuVtSqSum_l[2]*fc[6]+(9.0*fl[3]-9.0*fc[3])*nuVtSqSum_l[3]); 


  Gdiff_r[0] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_r[3]*fr[12]+5.0*nuVtSqSum_r[3]*fc[12]+5.0*nuVtSqSum_r[2]*fr[9]+5.0*nuVtSqSum_r[2]*fc[9]+5.0*nuVtSqSum_r[1]*fr[8]+5.0*nuVtSqSum_r[1]*fc[8]+5.0*nuVtSqSum_r[0]*fr[4]+5.0*nuVtSqSum_r[0]*fc[4])-9.0*nuVtSqSum_r[3]*fr[5]+9.0*nuVtSqSum_r[3]*fc[5]+(9.0*fc[2]-9.0*fr[2])*nuVtSqSum_r[2]+(9.0*fc[1]-9.0*fr[1])*nuVtSqSum_r[1]+(9.0*fc[0]-9.0*fr[0])*nuVtSqSum_r[0]); 
  Gdiff_r[1] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_r[2]*fr[12]+5.0*nuVtSqSum_r[2]*fc[12]+5.0*nuVtSqSum_r[3]*fr[9]+5.0*nuVtSqSum_r[3]*fc[9]+5.0*nuVtSqSum_r[0]*fr[8]+5.0*nuVtSqSum_r[0]*fc[8]+5.0*nuVtSqSum_r[1]*fr[4]+5.0*nuVtSqSum_r[1]*fc[4])-9.0*nuVtSqSum_r[2]*fr[5]+9.0*nuVtSqSum_r[2]*fc[5]+(9.0*fc[2]-9.0*fr[2])*nuVtSqSum_r[3]+(9.0*fc[0]-9.0*fr[0])*nuVtSqSum_r[1]-9.0*nuVtSqSum_r[0]*fr[1]+9.0*nuVtSqSum_r[0]*fc[1]); 
  Gdiff_r[2] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_r[1]*fr[12]+5.0*nuVtSqSum_r[1]*fc[12]+5.0*nuVtSqSum_r[0]*fr[9]+5.0*nuVtSqSum_r[0]*fc[9]+5.0*nuVtSqSum_r[3]*fr[8]+5.0*nuVtSqSum_r[3]*fc[8]+5.0*nuVtSqSum_r[2]*fr[4]+5.0*nuVtSqSum_r[2]*fc[4])-9.0*nuVtSqSum_r[1]*fr[5]+9.0*nuVtSqSum_r[1]*fc[5]+(9.0*fc[1]-9.0*fr[1])*nuVtSqSum_r[3]+(9.0*fc[0]-9.0*fr[0])*nuVtSqSum_r[2]-9.0*nuVtSqSum_r[0]*fr[2]+9.0*nuVtSqSum_r[0]*fc[2]); 
  Gdiff_r[3] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_r[3]*fr[15]+5.0*nuVtSqSum_r[3]*fc[15]+5.0*nuVtSqSum_r[2]*fr[14]+5.0*nuVtSqSum_r[2]*fc[14]+5.0*nuVtSqSum_r[1]*fr[13]+5.0*nuVtSqSum_r[1]*fc[13]+5.0*nuVtSqSum_r[0]*fr[10]+5.0*nuVtSqSum_r[0]*fc[10])-9.0*nuVtSqSum_r[3]*fr[11]+9.0*nuVtSqSum_r[3]*fc[11]-9.0*nuVtSqSum_r[2]*fr[7]+9.0*nuVtSqSum_r[2]*fc[7]-9.0*nuVtSqSum_r[1]*fr[6]+9.0*nuVtSqSum_r[1]*fc[6]-9.0*nuVtSqSum_r[0]*fr[3]+9.0*nuVtSqSum_r[0]*fc[3]); 
  Gdiff_r[4] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_r[0]*fr[12]+5.0*nuVtSqSum_r[0]*fc[12]+5.0*nuVtSqSum_r[1]*fr[9]+5.0*nuVtSqSum_r[1]*fc[9]+5.0*nuVtSqSum_r[2]*fr[8]+5.0*nuVtSqSum_r[2]*fc[8]+5.0*nuVtSqSum_r[3]*fr[4]+5.0*nuVtSqSum_r[3]*fc[4])-9.0*nuVtSqSum_r[0]*fr[5]+9.0*nuVtSqSum_r[0]*fc[5]+(9.0*fc[0]-9.0*fr[0])*nuVtSqSum_r[3]+(9.0*fc[1]-9.0*fr[1])*nuVtSqSum_r[2]-9.0*nuVtSqSum_r[1]*fr[2]+9.0*nuVtSqSum_r[1]*fc[2]); 
  Gdiff_r[5] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_r[2]*fr[15]+5.0*nuVtSqSum_r[2]*fc[15]+5.0*nuVtSqSum_r[3]*fr[14]+5.0*nuVtSqSum_r[3]*fc[14]+5.0*nuVtSqSum_r[0]*fr[13]+5.0*nuVtSqSum_r[0]*fc[13]+5.0*nuVtSqSum_r[1]*fr[10]+5.0*nuVtSqSum_r[1]*fc[10])-9.0*nuVtSqSum_r[2]*fr[11]+9.0*nuVtSqSum_r[2]*fc[11]-9.0*nuVtSqSum_r[3]*fr[7]+9.0*nuVtSqSum_r[3]*fc[7]-9.0*nuVtSqSum_r[0]*fr[6]+9.0*nuVtSqSum_r[0]*fc[6]-9.0*nuVtSqSum_r[1]*fr[3]+9.0*nuVtSqSum_r[1]*fc[3]); 
  Gdiff_r[6] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_r[1]*fr[15]+5.0*nuVtSqSum_r[1]*fc[15]+5.0*nuVtSqSum_r[0]*fr[14]+5.0*nuVtSqSum_r[0]*fc[14]+5.0*nuVtSqSum_r[3]*fr[13]+5.0*nuVtSqSum_r[3]*fc[13]+5.0*nuVtSqSum_r[2]*fr[10]+5.0*nuVtSqSum_r[2]*fc[10])-9.0*nuVtSqSum_r[1]*fr[11]+9.0*nuVtSqSum_r[1]*fc[11]-9.0*nuVtSqSum_r[0]*fr[7]+9.0*nuVtSqSum_r[0]*fc[7]-9.0*nuVtSqSum_r[3]*fr[6]+9.0*nuVtSqSum_r[3]*fc[6]-9.0*nuVtSqSum_r[2]*fr[3]+9.0*nuVtSqSum_r[2]*fc[3]); 
  Gdiff_r[7] = -0.04419417382415917*(1.732050807568877*(5.0*nuVtSqSum_r[0]*fr[15]+5.0*nuVtSqSum_r[0]*fc[15]+5.0*nuVtSqSum_r[1]*fr[14]+5.0*nuVtSqSum_r[1]*fc[14]+5.0*nuVtSqSum_r[2]*fr[13]+5.0*nuVtSqSum_r[2]*fc[13]+5.0*nuVtSqSum_r[3]*fr[10]+5.0*nuVtSqSum_r[3]*fc[10])-9.0*nuVtSqSum_r[0]*fr[11]+9.0*nuVtSqSum_r[0]*fc[11]-9.0*nuVtSqSum_r[1]*fr[7]+9.0*nuVtSqSum_r[1]*fc[7]-9.0*nuVtSqSum_r[2]*fr[6]+9.0*nuVtSqSum_r[2]*fc[6]+(9.0*fc[3]-9.0*fr[3])*nuVtSqSum_r[3]); 
