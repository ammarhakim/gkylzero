...
{ 
  ...
  gkyl_mat_set(A,0,4,-1.414213562373095*cMr[0]); 
  gkyl_mat_set(A,0,5,-1.414213562373095*cMr[1]); 
  gkyl_mat_set(A,1,4,-1.414213562373095*cMr[1]); 
  gkyl_mat_set(A,1,5,-1.414213562373095*cMr[0]); 
 
  // ... Block from weak multiply of m_self, nu, m1X and uCrossX ... // 
  gkyl_mat_set(A,4,0,(-0.5*m0r[1]*u_self[1])-0.5*m0r[1]*u_other[1]-0.5*m0r[0]*u_self[0]-0.5*m0r[0]*u_other[0]+1.414213562373095*m1r[0]); 
  gkyl_mat_set(A,4,1,(-0.5*m0r[0]*u_self[1])-0.5*m0r[0]*u_other[1]+1.414213562373095*m1r[1]-0.5*u_self[0]*m0r[1]-0.5*u_other[0]*m0r[1]); 
  gkyl_mat_set(A,5,0,(-0.5*m0r[0]*u_self[1])-0.5*m0r[0]*u_other[1]+1.414213562373095*m1r[1]-0.5*u_self[0]*m0r[1]-0.5*u_other[0]*m0r[1]); 
  gkyl_mat_set(A,5,1,(-0.9*m0r[1]*u_self[1])-0.9*m0r[1]*u_other[1]-0.5*m0r[0]*u_self[0]-0.5*m0r[0]*u_other[0]+1.414213562373095*m1r[0]); 
  ...
  // ... Block from correction to momentum conservation (self) ... // 
  gkyl_mat_set(A,2,4,-1.414213562373095*cMr[2]); 
  gkyl_mat_set(A,2,5,-1.414213562373095*cMr[3]); 
  gkyl_mat_set(A,3,4,-1.414213562373095*cMr[3]); 
  gkyl_mat_set(A,3,5,-1.414213562373095*cMr[2]); 
 
  // ... Block from weak multiply of m_self, nu, m1Y and uCrossY ... // 
  gkyl_mat_set(A,4,2,(-0.5*m0r[1]*u_self[3])-0.5*m0r[1]*u_other[3]-0.5*m0r[0]*u_self[2]-0.5*m0r[0]*u_other[2]+1.414213562373095*m1r[2]); 
  gkyl_mat_set(A,4,3,(-0.5*m0r[0]*u_self[3])-0.5*m0r[0]*u_other[3]+1.414213562373095*m1r[3]-0.5*m0r[1]*u_self[2]-0.5*m0r[1]*u_other[2]); 
  gkyl_mat_set(A,5,2,(-0.5*m0r[0]*u_self[3])-0.5*m0r[0]*u_other[3]+1.414213562373095*m1r[3]-0.5*m0r[1]*u_self[2]-0.5*m0r[1]*u_other[2]); 
  gkyl_mat_set(A,5,3,(-0.9*m0r[1]*u_self[3])-0.9*m0r[1]*u_other[3]-0.5*m0r[0]*u_self[2]-0.5*m0r[0]*u_other[2]+1.414213562373095*m1r[2]); 
  ...
  gkyl_mat_set(A,4,4,0.7071067811865475*ucMSelf[0]+0.7071067811865475*ucMOther[0]+2.828427124746191*m0r[0]-1.414213562373095*cEr[0]); 
  gkyl_mat_set(A,4,5,0.7071067811865475*ucMSelf[1]+0.7071067811865475*ucMOther[1]+2.828427124746191*m0r[1]-1.414213562373095*cEr[1]); 
  gkyl_mat_set(A,5,4,0.7071067811865475*ucMSelf[1]+0.7071067811865475*ucMOther[1]+2.828427124746191*m0r[1]-1.414213562373095*cEr[1]); 
  gkyl_mat_set(A,5,5,0.7071067811865475*ucMSelf[0]+0.7071067811865475*ucMOther[0]+2.828427124746191*m0r[0]-1.414213562373095*cEr[0]); 
  ...
  double enRHS[2] = {0.0}; 
  enRHS[0] = (-(4.0*greene[1]*vtsq_self[1]*m_self)/(2.828427124746191*m_self+2.828427124746191*m_other))-(1.0*greene[1]*uSumSq[1]*m_self)/(2.828427124746191*m_self+2.828427124746191*m_other)-(4.0*greene[0]*vtsq_self[0]*m_self)/(2.828427124746191*m_self+2.828427124746191*m_other)-(1.0*greene[0]*uSumSq[0]*m_self)/(2.828427124746191*m_self+2.828427124746191*m_other)+(4.0*greene[1]*vtsq_other[1]*m_other)/(2.828427124746191*m_self+2.828427124746191*m_other)+(greene[1]*uSumSq[1]*m_other)/(2.828427124746191*m_self+2.828427124746191*m_other)+(4.0*greene[0]*vtsq_other[0]*m_other)/(2.828427124746191*m_self+2.828427124746191*m_other)+(greene[0]*uSumSq[0]*m_other)/(2.828427124746191*m_self+2.828427124746191*m_other)-1.0*uM1Self[0]-1.0*uM1Other[0]+2.0*m2r[0]; 
  enRHS[1] = (-(4.0*greene[0]*vtsq_self[1]*m_self)/(2.828427124746191*m_self+2.828427124746191*m_other))-(1.0*greene[0]*uSumSq[1]*m_self)/(2.828427124746191*m_self+2.828427124746191*m_other)-(4.0*vtsq_self[0]*greene[1]*m_self)/(2.828427124746191*m_self+2.828427124746191*m_other)-(1.0*uSumSq[0]*greene[1]*m_self)/(2.828427124746191*m_self+2.828427124746191*m_other)+(4.0*greene[0]*vtsq_other[1]*m_other)/(2.828427124746191*m_self+2.828427124746191*m_other)+(greene[0]*uSumSq[1]*m_other)/(2.828427124746191*m_self+2.828427124746191*m_other)+(4.0*vtsq_other[0]*greene[1]*m_other)/(2.828427124746191*m_self+2.828427124746191*m_other)+(uSumSq[0]*greene[1]*m_other)/(2.828427124746191*m_self+2.828427124746191*m_other)-1.0*uM1Self[1]-1.0*uM1Other[1]+2.0*m2r[1]; 
