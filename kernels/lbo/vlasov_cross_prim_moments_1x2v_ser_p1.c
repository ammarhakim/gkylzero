...
{ 
  ...
  if (notCellAvg && (0.5*(2.4494897427831783*moms_self[1]+1.4142135623730951*moms_self[0]) < 0)) notCellAvg = false; 
  if (notCellAvg && (-(0.5*(2.4494897427831783*moms_self[7]-1.4142135623730951*moms_self[6])) < 0)) notCellAvg = false; 
  if (notCellAvg && (0.5*(2.4494897427831783*moms_self[7]+1.4142135623730951*moms_self[6]) < 0)) notCellAvg = false; 
  ...
  if (notCellAvg && (0.5*(2.4494897427831783*moms_other[1]+1.4142135623730951*moms_other[0]) < 0)) notCellAvg = false; 
  if (notCellAvg && (-(0.5*(2.4494897427831783*moms_other[7]-1.4142135623730951*moms_other[6])) < 0)) notCellAvg = false; 
  if (notCellAvg && (0.5*(2.4494897427831783*moms_other[7]+1.4142135623730951*moms_other[6]) < 0)) notCellAvg = false; 
  ...
  gkyl_mat_set(A,1,5,-(1.4142135623730951*cMr[0])); 
 
  // ... Block from weak multiply of m_self, nu, m1X and uCrossX ... // 
  gkyl_mat_set(A,4,0,-(0.5*m0r[1]*u_selfr[1])-0.5*m0r[1]*u_otherr[1]-0.5*m0r[0]*u_selfr[0]-0.5*m0r[0]*u_otherr[0]+1.4142135623730951*m1r[0]); 
  gkyl_mat_set(A,4,1,-(0.5*m0r[0]*u_selfr[1])-0.5*m0r[0]*u_otherr[1]+1.4142135623730951*m1r[1]-0.5*u_selfr[0]*m0r[1]-0.5*u_otherr[0]*m0r[1]); 
  gkyl_mat_set(A,5,0,-(0.5*m0r[0]*u_selfr[1])-0.5*m0r[0]*u_otherr[1]+1.4142135623730951*m1r[1]-0.5*u_selfr[0]*m0r[1]-0.5*u_otherr[0]*m0r[1]); 
  gkyl_mat_set(A,5,1,-(0.9*m0r[1]*u_selfr[1])-0.9*m0r[1]*u_otherr[1]-0.5*m0r[0]*u_selfr[0]-0.5*m0r[0]*u_otherr[0]+1.4142135623730951*m1r[0]); 
  ...
  gkyl_mat_set(A,2,5,-(1.4142135623730951*cMr[3])); 
  gkyl_mat_set(A,3,4,-(1.4142135623730951*cMr[3])); 
  gkyl_mat_set(A,3,5,-(1.4142135623730951*cMr[2])); 
 
  // ... Block from weak multiply of m_self, nu, m1Y and uCrossY ... // 
  gkyl_mat_set(A,4,2,-(0.5*m0r[1]*u_selfr[3])-0.5*m0r[1]*u_otherr[3]-0.5*m0r[0]*u_selfr[2]-0.5*m0r[0]*u_otherr[2]+1.4142135623730951*m1r[2]); 
  gkyl_mat_set(A,4,3,-(0.5*m0r[0]*u_selfr[3])-0.5*m0r[0]*u_otherr[3]+1.4142135623730951*m1r[3]-0.5*m0r[1]*u_selfr[2]-0.5*m0r[1]*u_otherr[2]); 
  gkyl_mat_set(A,5,2,-(0.5*m0r[0]*u_selfr[3])-0.5*m0r[0]*u_otherr[3]+1.4142135623730951*m1r[3]-0.5*m0r[1]*u_selfr[2]-0.5*m0r[1]*u_otherr[2]); 
  gkyl_mat_set(A,5,3,-(0.9*m0r[1]*u_selfr[3])-0.9*m0r[1]*u_otherr[3]-0.5*m0r[0]*u_selfr[2]-0.5*m0r[0]*u_otherr[2]+1.4142135623730951*m1r[2]); 
  ...
  gkyl_mat_set(A,4,4,0.7071067811865475*ucMSelf[0]+0.7071067811865475*ucMOther[0]+2.8284271247461907*m0r[0]-1.4142135623730951*cEr[0]); 
  gkyl_mat_set(A,4,5,0.7071067811865475*ucMSelf[1]+0.7071067811865475*ucMOther[1]+2.8284271247461907*m0r[1]-1.4142135623730951*cEr[1]); 
  gkyl_mat_set(A,5,4,0.7071067811865475*ucMSelf[1]+0.7071067811865475*ucMOther[1]+2.8284271247461907*m0r[1]-1.4142135623730951*cEr[1]); 
  gkyl_mat_set(A,5,5,0.7071067811865475*ucMSelf[0]+0.7071067811865475*ucMOther[0]+2.8284271247461907*m0r[0]-1.4142135623730951*cEr[0]); 
  ...
  double enRHS[2] = {0.0}; 
  enRHS[0] = -((1.4142135623730951*greene[1]*vtsq_self[1]*m_self)/m_sum)-(1.4142135623730951*greene[0]*vtsq_self[0]*m_self)/m_sum+(1.4142135623730951*greene[1]*vtsq_other[1]*m_other)/m_sum+(1.4142135623730951*greene[0]*vtsq_other[0]*m_other)/m_sum+(0.3535533905932737*greene[1]*uSumSq[1]*m_diff)/m_sum+(0.3535533905932737*greene[0]*uSumSq[0]*m_diff)/m_sum-1.0*uM1Self[0]-1.0*uM1Other[0]+2.0*m2r[0]; 
  enRHS[1] = -((1.4142135623730951*greene[0]*vtsq_self[1]*m_self)/m_sum)-(1.4142135623730951*vtsq_self[0]*greene[1]*m_self)/m_sum+(1.4142135623730951*greene[0]*vtsq_other[1]*m_other)/m_sum+(1.4142135623730951*vtsq_other[0]*greene[1]*m_other)/m_sum+(0.3535533905932737*greene[0]*uSumSq[1]*m_diff)/m_sum+(0.3535533905932737*uSumSq[0]*greene[1]*m_diff)/m_sum-1.0*uM1Self[1]-1.0*uM1Other[1]+2.0*m2r[1]; 
