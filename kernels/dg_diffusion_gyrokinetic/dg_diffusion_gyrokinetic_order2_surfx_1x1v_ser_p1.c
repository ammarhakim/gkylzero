...
GKYL_CU_DH double dg_diffusion_gyrokinetic_order2_surfx_1x1v_ser_p1_constcoeff(const double *w, const double *dx, const double *coeff, const double *jacobgeo_inv, const double *ql, const double *qc, const double *qr, double* GKYL_RESTRICT out) 
{
  ...
  out[2] += -0.0625*(8.660254037844386*coeff[0]*fr[3]-8.660254037844386*coeff[0]*fl[3]-9.0*coeff[0]*fr[2]-9.0*coeff[0]*fl[2]+18.0*coeff[0]*fc[2])*rdx2Sq; 
  out[3] += -0.0625*(7.0*coeff[0]*fr[3]+7.0*coeff[0]*fl[3]+46.0*coeff[0]*fc[3]-8.660254037844386*coeff[0]*fr[2]+8.660254037844386*coeff[0]*fl[2])*rdx2Sq; 
  out[4] += -0.0625*(8.660254037844387*coeff[0]*fr[5]-8.660254037844387*coeff[0]*fl[5]-9.0*coeff[0]*fr[4]-9.0*coeff[0]*fl[4]+18.0*coeff[0]*fc[4])*rdx2Sq; 
  out[5] += -0.0625*(7.0*coeff[0]*fr[5]+7.0*coeff[0]*fl[5]+46.0*coeff[0]*fc[5]-8.660254037844387*coeff[0]*fr[4]+8.660254037844387*coeff[0]*fl[4])*rdx2Sq; 
