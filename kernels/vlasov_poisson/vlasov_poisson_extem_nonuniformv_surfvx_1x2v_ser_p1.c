...
GKYL_CU_DH double vlasov_poisson_extem_nonuniformv_surfvx_1x2v_ser_p1(const double *w, const double *dxv, const double *vcoord, const double *field, const double *fl, const double *fc, const double *fr, double* GKYL_RESTRICT out) 
{ 
  ...
  alpha[4] = 1.224744871391589*A1[1]*vcoord[13]*rdx2; 
  ...
  if (0.4472135954999579*alpha[4]-0.6708203932499369*alpha[2]+0.5*alpha[0] > 0) { 
  ...
  Ghat_l[0] = 0.5*alpha[4]*fUpwind_l[4]+0.5*alpha[2]*fUpwind_l[2]+0.5*alpha[0]*fUpwind_l[0]; 
  Ghat_l[1] = 0.5000000000000001*alpha[4]*fUpwind_l[5]+0.5*alpha[2]*fUpwind_l[3]+0.5*alpha[0]*fUpwind_l[1]; 
  Ghat_l[2] = 0.4472135954999579*alpha[2]*fUpwind_l[4]+0.4472135954999579*fUpwind_l[2]*alpha[4]+0.5*alpha[0]*fUpwind_l[2]+0.5*fUpwind_l[0]*alpha[2]; 
  Ghat_l[3] = 0.447213595499958*alpha[2]*fUpwind_l[5]+0.4472135954999579*fUpwind_l[3]*alpha[4]+0.5*alpha[0]*fUpwind_l[3]+0.5*fUpwind_l[1]*alpha[2]; 
  Ghat_l[4] = 0.31943828249997*alpha[4]*fUpwind_l[4]+0.5*alpha[0]*fUpwind_l[4]+0.5*fUpwind_l[0]*alpha[4]+0.4472135954999579*alpha[2]*fUpwind_l[2]; 
  Ghat_l[5] = 0.31943828249997*alpha[4]*fUpwind_l[5]+0.5*alpha[0]*fUpwind_l[5]+0.5000000000000001*fUpwind_l[1]*alpha[4]+0.447213595499958*alpha[2]*fUpwind_l[3]; 

  Ghat_r[0] = 0.5*alpha[4]*fUpwind_r[4]+0.5*alpha[2]*fUpwind_r[2]+0.5*alpha[0]*fUpwind_r[0]; 
  Ghat_r[1] = 0.5000000000000001*alpha[4]*fUpwind_r[5]+0.5*alpha[2]*fUpwind_r[3]+0.5*alpha[0]*fUpwind_r[1]; 
  Ghat_r[2] = 0.4472135954999579*alpha[2]*fUpwind_r[4]+0.4472135954999579*fUpwind_r[2]*alpha[4]+0.5*alpha[0]*fUpwind_r[2]+0.5*fUpwind_r[0]*alpha[2]; 
  Ghat_r[3] = 0.447213595499958*alpha[2]*fUpwind_r[5]+0.4472135954999579*fUpwind_r[3]*alpha[4]+0.5*alpha[0]*fUpwind_r[3]+0.5*fUpwind_r[1]*alpha[2]; 
  Ghat_r[4] = 0.31943828249997*alpha[4]*fUpwind_r[4]+0.5*alpha[0]*fUpwind_r[4]+0.5*fUpwind_r[0]*alpha[4]+0.4472135954999579*alpha[2]*fUpwind_r[2]; 
  Ghat_r[5] = 0.31943828249997*alpha[4]*fUpwind_r[5]+0.5*alpha[0]*fUpwind_r[5]+0.5000000000000001*fUpwind_r[1]*alpha[4]+0.447213595499958*alpha[2]*fUpwind_r[3]; 
  ...
  out[9] += (1.58113883008419*Ghat_l[1]-1.58113883008419*Ghat_r[1])*rdv2; 
  out[10] += (1.58113883008419*Ghat_l[2]-1.58113883008419*Ghat_r[2])*rdv2; 
  out[11] += (1.58113883008419*Ghat_l[3]-1.58113883008419*Ghat_r[3])*rdv2; 
  out[12] += (0.7071067811865475*Ghat_l[4]-0.7071067811865475*Ghat_r[4])*rdv2; 
  out[13] += (0.7071067811865475*Ghat_l[5]-0.7071067811865475*Ghat_r[5])*rdv2; 
  out[14] += -1.224744871391589*(Ghat_r[4]+Ghat_l[4])*rdv2; 
  out[15] += -1.224744871391589*(Ghat_r[5]+Ghat_l[5])*rdv2; 
