...
{ 
  ...
  const double *Tii_surf_lr = &prim_surf_l[7]; 
  const double *Tii_surf_cl = &prim_surf_c[6]; 
  const double *Tii_surf_cr = &prim_surf_c[7]; 
  const double *Tii_surf_rl = &prim_surf_r[6]; 

  const double *pkpm_div_ppar = &prim_c[9]; 
  const double *T_perp_over_m = &prim_c[12]; 
  ...
  div_b_comp[0] = (0.2445699350390395*b_r[2]-0.2445699350390395*b_l[2]-0.3518228202874282*(b_r[1]+b_l[1])+0.7036456405748563*b_c[1]+0.25*b_r[0]-0.25*b_l[0])*dx1; 
  div_b_comp[1] = (0.4236075534914363*(b_r[2]+b_l[2])+0.8472151069828725*b_c[2]-0.609375*b_r[1]+0.609375*b_l[1]+0.4330127018922193*(b_r[0]+b_l[0])-0.8660254037844386*b_c[0])*dx1; 
  div_b_comp[2] = (0.546875*b_r[2]-0.546875*b_l[2]-0.7866997421983816*(b_r[1]+b_l[1])-2.299583861810654*b_c[1]+0.5590169943749475*b_r[0]-0.5590169943749475*b_l[0])*dx1; 
