...
{ 
  ...
  const double *pkpm_div_ppar = &prim[18]; 
  const double *T_perp_over_m = &prim[21]; 
  const double *T_perp_over_m_inv = &prim[24]; 
  ...
  double *pkpm_vars_io_uy = &pkpm_vars_io[3]; 
  ...
  binop_mul_1d_ser_p2(rhoux, uy, rhouxuy); 
 
  double rhouxuz[3] = {0.0}; 
  binop_mul_1d_ser_p2(rhoux, uz, rhouxuz); 
 
  double rhouyuy[3] = {0.0}; 
  binop_mul_1d_ser_p2(rhouy, uy, rhouyuy); 
 
  double rhouyuz[3] = {0.0}; 
  binop_mul_1d_ser_p2(rhouy, uz, rhouyuz); 
 
  double rhouzuz[3] = {0.0}; 
  binop_mul_1d_ser_p2(rhouz, uz, rhouzuz); 
