...
GKYL_CU_DH void pkpm_vars_pressure_1x_tensor_p2(const double *bb, const double *vlasov_pkpm_moms, 
...
{ 
  ...
  const double *bxby = &bb[3]; 
  const double *bxbz = &bb[6]; 
  const double *byby = &bb[9]; 
  const double *bybz = &bb[12]; 
  const double *bzbz = &bb[15]; 
  ...
  binop_mul_1d_tensor_p2(DP, bxbx, DP_bxbx); 
 
  double DP_bxby[3] = {0.0}; 
  binop_mul_1d_tensor_p2(DP, bxby, DP_bxby); 
 
  double DP_bxbz[3] = {0.0}; 
  binop_mul_1d_tensor_p2(DP, bxbz, DP_bxbz); 
 
  double DP_byby[3] = {0.0}; 
  binop_mul_1d_tensor_p2(DP, byby, DP_byby); 
 
  double DP_bybz[3] = {0.0}; 
  binop_mul_1d_tensor_p2(DP, bybz, DP_bybz); 
 
  double DP_bzbz[3] = {0.0}; 
  binop_mul_1d_tensor_p2(DP, bzbz, DP_bzbz); 
