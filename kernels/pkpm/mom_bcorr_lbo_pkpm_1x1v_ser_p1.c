...
{ 
  ...
  if (edge == GKYL_VX_LOWER) {
    out[0] += ((-1.58113883008419*fIn[4])+1.224744871391589*fIn[2]-0.7071067811865475*fIn[0])*mass; 
    out[1] += ((-1.58113883008419*fIn[5])+1.224744871391589*fIn[3]-0.7071067811865475*fIn[1])*mass; 
    out[2] += out[0]*vBoundary[0]; 
    out[3] += vBoundary[0]*out[1]; 
  } 
  else if (edge == GKYL_VX_UPPER) {
    out[0] += (1.58113883008419*fIn[4]+1.224744871391589*fIn[2]+0.7071067811865475*fIn[0])*mass; 
    out[1] += (1.58113883008419*fIn[5]+1.224744871391589*fIn[3]+0.7071067811865475*fIn[1])*mass; 
    out[2] += out[0]*vBoundary[1]; 
    out[3] += out[1]*vBoundary[1]; 
  }
