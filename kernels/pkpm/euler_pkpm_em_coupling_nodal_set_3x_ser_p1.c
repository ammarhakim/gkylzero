...
{ 
  ...
  for (int s = 0; s < num_species; ++s) { 
    // Set RHS for flow velocity equations, including solution at known time-step and external forces. 
    gkyl_mat_set(&rhs4, 0 + s*3, 0, qbym[s]*rhoux_nodal[s][4] + 0.5*dt*qbym[s]*rho_nodal[s][4]*(qbym[s]*ext_Ex_nodal[4] + app_accel_x_nodal[s][4])); 
    gkyl_mat_set(&rhs4, 1 + s*3, 0, qbym[s]*rhouy_nodal[s][4] + 0.5*dt*qbym[s]*rho_nodal[s][4]*(qbym[s]*ext_Ey_nodal[4] + app_accel_y_nodal[s][4])); 
    gkyl_mat_set(&rhs4, 2 + s*3, 0, qbym[s]*rhouz_nodal[s][4] + 0.5*dt*qbym[s]*rho_nodal[s][4]*(qbym[s]*ext_Ez_nodal[4] + app_accel_z_nodal[s][4])); 
 
    // Set LHS matrix for flow velocity equation: J_s^{n+1} - 0.5*dt*((q_s/m_s)^2 rho_s^n)/epsilon0*(epsilon0*E)^{n+1} + q_s/m_s*J_s^{n+1} x B^n). 
    double E_field_fac = -0.5*dt*qbym[s]*qbym[s]*rho_nodal[s][4]/epsilon0; 
    double B_field_fac = -0.5*dt*qbym[s]; 
    ...
    // For Ampere's Law LHS: epsilon0*E^{n+1} + 0.5*dt*sum_s J_s^{n+1}. 
    if (!pkpm_field_static) { 
      gkyl_mat_set(&lhs4, 0 + num_species*3, 0 + s*3, 0.5*dt); 
      gkyl_mat_set(&lhs4, 1 + num_species*3, 1 + s*3, 0.5*dt); 
      gkyl_mat_set(&lhs4, 2 + num_species*3, 2 + s*3, 0.5*dt); 
    } 
  } 
  // Set RHS for Ampere's Law, including solution at known time-step and external currents. 
  ...
  gkyl_mat_set(&lhs4, 1 + num_species*3, 1 + num_species*3, 1.0); 
  gkyl_mat_set(&lhs4, 2 + num_species*3, 2 + num_species*3, 1.0); 
  ...
  for (int s = 0; s < num_species; ++s) { 
    // Set RHS for flow velocity equations, including solution at known time-step and external forces. 
    gkyl_mat_set(&rhs5, 0 + s*3, 0, qbym[s]*rhoux_nodal[s][5] + 0.5*dt*qbym[s]*rho_nodal[s][5]*(qbym[s]*ext_Ex_nodal[5] + app_accel_x_nodal[s][5])); 
    gkyl_mat_set(&rhs5, 1 + s*3, 0, qbym[s]*rhouy_nodal[s][5] + 0.5*dt*qbym[s]*rho_nodal[s][5]*(qbym[s]*ext_Ey_nodal[5] + app_accel_y_nodal[s][5])); 
    gkyl_mat_set(&rhs5, 2 + s*3, 0, qbym[s]*rhouz_nodal[s][5] + 0.5*dt*qbym[s]*rho_nodal[s][5]*(qbym[s]*ext_Ez_nodal[5] + app_accel_z_nodal[s][5])); 
 
    // Set LHS matrix for flow velocity equation: J_s^{n+1} - 0.5*dt*((q_s/m_s)^2 rho_s^n)/epsilon0*(epsilon0*E)^{n+1} + q_s/m_s*J_s^{n+1} x B^n). 
    double E_field_fac = -0.5*dt*qbym[s]*qbym[s]*rho_nodal[s][5]/epsilon0; 
    double B_field_fac = -0.5*dt*qbym[s]; 
    ...
    // For Ampere's Law LHS: epsilon0*E^{n+1} + 0.5*dt*sum_s J_s^{n+1}. 
    if (!pkpm_field_static) { 
      gkyl_mat_set(&lhs5, 0 + num_species*3, 0 + s*3, 0.5*dt); 
      gkyl_mat_set(&lhs5, 1 + num_species*3, 1 + s*3, 0.5*dt); 
      gkyl_mat_set(&lhs5, 2 + num_species*3, 2 + s*3, 0.5*dt); 
    } 
  } 
  // Set RHS for Ampere's Law, including solution at known time-step and external currents. 
  ...
  gkyl_mat_set(&lhs5, 1 + num_species*3, 1 + num_species*3, 1.0); 
  gkyl_mat_set(&lhs5, 2 + num_species*3, 2 + num_species*3, 1.0); 
  ...
  for (int s = 0; s < num_species; ++s) { 
    // Set RHS for flow velocity equations, including solution at known time-step and external forces. 
    gkyl_mat_set(&rhs6, 0 + s*3, 0, qbym[s]*rhoux_nodal[s][6] + 0.5*dt*qbym[s]*rho_nodal[s][6]*(qbym[s]*ext_Ex_nodal[6] + app_accel_x_nodal[s][6])); 
    gkyl_mat_set(&rhs6, 1 + s*3, 0, qbym[s]*rhouy_nodal[s][6] + 0.5*dt*qbym[s]*rho_nodal[s][6]*(qbym[s]*ext_Ey_nodal[6] + app_accel_y_nodal[s][6])); 
    gkyl_mat_set(&rhs6, 2 + s*3, 0, qbym[s]*rhouz_nodal[s][6] + 0.5*dt*qbym[s]*rho_nodal[s][6]*(qbym[s]*ext_Ez_nodal[6] + app_accel_z_nodal[s][6])); 
 
    // Set LHS matrix for flow velocity equation: J_s^{n+1} - 0.5*dt*((q_s/m_s)^2 rho_s^n)/epsilon0*(epsilon0*E)^{n+1} + q_s/m_s*J_s^{n+1} x B^n). 
    double E_field_fac = -0.5*dt*qbym[s]*qbym[s]*rho_nodal[s][6]/epsilon0; 
    double B_field_fac = -0.5*dt*qbym[s]; 
    ...
    // For Ampere's Law LHS: epsilon0*E^{n+1} + 0.5*dt*sum_s J_s^{n+1}. 
    if (!pkpm_field_static) { 
      gkyl_mat_set(&lhs6, 0 + num_species*3, 0 + s*3, 0.5*dt); 
      gkyl_mat_set(&lhs6, 1 + num_species*3, 1 + s*3, 0.5*dt); 
      gkyl_mat_set(&lhs6, 2 + num_species*3, 2 + s*3, 0.5*dt); 
    } 
  } 
  // Set RHS for Ampere's Law, including solution at known time-step and external currents. 
  ...
  gkyl_mat_set(&lhs6, 1 + num_species*3, 1 + num_species*3, 1.0); 
  gkyl_mat_set(&lhs6, 2 + num_species*3, 2 + num_species*3, 1.0); 
  ...
  for (int s = 0; s < num_species; ++s) { 
    // Set RHS for flow velocity equations, including solution at known time-step and external forces. 
    gkyl_mat_set(&rhs7, 0 + s*3, 0, qbym[s]*rhoux_nodal[s][7] + 0.5*dt*qbym[s]*rho_nodal[s][7]*(qbym[s]*ext_Ex_nodal[7] + app_accel_x_nodal[s][7])); 
    gkyl_mat_set(&rhs7, 1 + s*3, 0, qbym[s]*rhouy_nodal[s][7] + 0.5*dt*qbym[s]*rho_nodal[s][7]*(qbym[s]*ext_Ey_nodal[7] + app_accel_y_nodal[s][7])); 
    gkyl_mat_set(&rhs7, 2 + s*3, 0, qbym[s]*rhouz_nodal[s][7] + 0.5*dt*qbym[s]*rho_nodal[s][7]*(qbym[s]*ext_Ez_nodal[7] + app_accel_z_nodal[s][7])); 
 
    // Set LHS matrix for flow velocity equation: J_s^{n+1} - 0.5*dt*((q_s/m_s)^2 rho_s^n)/epsilon0*(epsilon0*E)^{n+1} + q_s/m_s*J_s^{n+1} x B^n). 
    double E_field_fac = -0.5*dt*qbym[s]*qbym[s]*rho_nodal[s][7]/epsilon0; 
    double B_field_fac = -0.5*dt*qbym[s]; 
    ...
    // For Ampere's Law LHS: epsilon0*E^{n+1} + 0.5*dt*sum_s J_s^{n+1}. 
    if (!pkpm_field_static) { 
      gkyl_mat_set(&lhs7, 0 + num_species*3, 0 + s*3, 0.5*dt); 
      gkyl_mat_set(&lhs7, 1 + num_species*3, 1 + s*3, 0.5*dt); 
      gkyl_mat_set(&lhs7, 2 + num_species*3, 2 + s*3, 0.5*dt); 
    } 
  } 
  // Set RHS for Ampere's Law, including solution at known time-step and external currents. 
  ...
  gkyl_mat_set(&lhs7, 1 + num_species*3, 1 + num_species*3, 1.0); 
  gkyl_mat_set(&lhs7, 2 + num_species*3, 2 + num_species*3, 1.0); 
