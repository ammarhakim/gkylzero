...
{ 
  ...
  struct gkyl_mat A_pkpm_div_ppar = gkyl_nmat_get(A, count+6); 
  struct gkyl_mat A_T_perp_over_m = gkyl_nmat_get(A, count+7); 
  struct gkyl_mat A_T_perp_over_m_inv = gkyl_nmat_get(A, count+8); 
  ...
  struct gkyl_mat rhs_pkpm_div_ppar = gkyl_nmat_get(rhs, count+6); 
  struct gkyl_mat rhs_T_perp_over_m = gkyl_nmat_get(rhs, count+7); 
  struct gkyl_mat rhs_T_perp_over_m_inv = gkyl_nmat_get(rhs, count+8); 
  ...
  if (2.5*p_par[8]+1.936491673103709*p_par[7]+1.936491673103709*p_par[6]+1.118033988749895*p_par[5]+1.118033988749895*p_par[4]+1.5*p_par[3]+0.8660254037844386*p_par[2]+0.8660254037844386*p_par[1]+0.5*p_par[0] < 0.0) cell_avg = 1; 
  if (2.5*p_perp[8]+1.936491673103709*p_perp[7]+1.936491673103709*p_perp[6]+1.118033988749895*p_perp[5]+1.118033988749895*p_perp[4]+1.5*p_perp[3]+0.8660254037844386*p_perp[2]+0.8660254037844386*p_perp[1]+0.5*p_perp[0] < 0.0) cell_avg = 1; 
  double rho_temp[9] = {0.0}; 
  double p_perp_temp[9] = {0.0}; 
  if (cell_avg) { 
    rho_temp[0] = rho[0]; 
    p_perp_temp[0] = p_perp[0]; 
  } else { 
    rho_temp[0] = rho[0]; 
    p_perp_temp[0] = p_perp[0]; 
    rho_temp[1] = rho[1]; 
    p_perp_temp[1] = p_perp[1]; 
    rho_temp[2] = rho[2]; 
    p_perp_temp[2] = p_perp[2]; 
    rho_temp[3] = rho[3]; 
    p_perp_temp[3] = p_perp[3]; 
    rho_temp[4] = rho[4]; 
    p_perp_temp[4] = p_perp[4]; 
    rho_temp[5] = rho[5]; 
    p_perp_temp[5] = p_perp[5]; 
    rho_temp[6] = rho[6]; 
    p_perp_temp[6] = p_perp[6]; 
    rho_temp[7] = rho[7]; 
    p_perp_temp[7] = p_perp[7]; 
    rho_temp[8] = rho[8]; 
    p_perp_temp[8] = p_perp[8]; 
  } 
  ...
  if (cell_avg) { 
    ...
    gkyl_mat_set(&rhs_Txx,1,0,0.0); 
    gkyl_mat_set(&rhs_Tyy,1,0,0.0); 
    gkyl_mat_set(&rhs_Tzz,1,0,0.0); 
    gkyl_mat_set(&rhs_pkpm_div_ppar,1,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m,1,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m_inv,1,0,0.0); 
    gkyl_mat_set(&rhs_ux,2,0,0.0); 
    gkyl_mat_set(&rhs_uy,2,0,0.0); 
    gkyl_mat_set(&rhs_uz,2,0,0.0); 
    gkyl_mat_set(&rhs_Txx,2,0,0.0); 
    gkyl_mat_set(&rhs_Tyy,2,0,0.0); 
    gkyl_mat_set(&rhs_Tzz,2,0,0.0); 
    gkyl_mat_set(&rhs_pkpm_div_ppar,2,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m,2,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m_inv,2,0,0.0); 
    gkyl_mat_set(&rhs_ux,3,0,0.0); 
    gkyl_mat_set(&rhs_uy,3,0,0.0); 
    gkyl_mat_set(&rhs_uz,3,0,0.0); 
    gkyl_mat_set(&rhs_Txx,3,0,0.0); 
    gkyl_mat_set(&rhs_Tyy,3,0,0.0); 
    gkyl_mat_set(&rhs_Tzz,3,0,0.0); 
    gkyl_mat_set(&rhs_pkpm_div_ppar,3,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m,3,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m_inv,3,0,0.0); 
    gkyl_mat_set(&rhs_ux,4,0,0.0); 
    gkyl_mat_set(&rhs_uy,4,0,0.0); 
    gkyl_mat_set(&rhs_uz,4,0,0.0); 
    gkyl_mat_set(&rhs_Txx,4,0,0.0); 
    gkyl_mat_set(&rhs_Tyy,4,0,0.0); 
    gkyl_mat_set(&rhs_Tzz,4,0,0.0); 
    gkyl_mat_set(&rhs_pkpm_div_ppar,4,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m,4,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m_inv,4,0,0.0); 
    gkyl_mat_set(&rhs_ux,5,0,0.0); 
    gkyl_mat_set(&rhs_uy,5,0,0.0); 
    gkyl_mat_set(&rhs_uz,5,0,0.0); 
    gkyl_mat_set(&rhs_Txx,5,0,0.0); 
    gkyl_mat_set(&rhs_Tyy,5,0,0.0); 
    gkyl_mat_set(&rhs_Tzz,5,0,0.0); 
    gkyl_mat_set(&rhs_pkpm_div_ppar,5,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m,5,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m_inv,5,0,0.0); 
    gkyl_mat_set(&rhs_ux,6,0,0.0); 
    gkyl_mat_set(&rhs_uy,6,0,0.0); 
    gkyl_mat_set(&rhs_uz,6,0,0.0); 
    gkyl_mat_set(&rhs_Txx,6,0,0.0); 
    gkyl_mat_set(&rhs_Tyy,6,0,0.0); 
    gkyl_mat_set(&rhs_Tzz,6,0,0.0); 
    gkyl_mat_set(&rhs_pkpm_div_ppar,6,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m,6,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m_inv,6,0,0.0); 
    gkyl_mat_set(&rhs_ux,7,0,0.0); 
    gkyl_mat_set(&rhs_uy,7,0,0.0); 
    gkyl_mat_set(&rhs_uz,7,0,0.0); 
    gkyl_mat_set(&rhs_Txx,7,0,0.0); 
    gkyl_mat_set(&rhs_Tyy,7,0,0.0); 
    gkyl_mat_set(&rhs_Tzz,7,0,0.0); 
    gkyl_mat_set(&rhs_pkpm_div_ppar,7,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m,7,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m_inv,7,0,0.0); 
    gkyl_mat_set(&rhs_ux,8,0,0.0); 
    gkyl_mat_set(&rhs_uy,8,0,0.0); 
    gkyl_mat_set(&rhs_uz,8,0,0.0); 
    gkyl_mat_set(&rhs_Txx,8,0,0.0); 
    gkyl_mat_set(&rhs_Tyy,8,0,0.0); 
    gkyl_mat_set(&rhs_Tzz,8,0,0.0); 
    gkyl_mat_set(&rhs_pkpm_div_ppar,8,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m,8,0,0.0); 
    gkyl_mat_set(&rhs_T_perp_over_m_inv,8,0,0.0); 
  } else { 
    gkyl_mat_set(&rhs_ux,0,0,rhoux[0]); 
    gkyl_mat_set(&rhs_uy,0,0,rhouy[0]); 
    gkyl_mat_set(&rhs_uz,0,0,rhouz[0]); 
  ...
  temp_p_perp = 0.5*p_perp_temp[0]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,0,0,temp_p_perp); 

  temp_rho = 0.5*rho_temp[1]; 
  ...
  temp_p_perp = 0.5*p_perp_temp[1]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,0,1,temp_p_perp); 

  temp_rho = 0.5*rho_temp[2]; 
  ...
  temp_p_perp = 0.5*p_perp_temp[2]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,0,2,temp_p_perp); 

  temp_rho = 0.5*rho_temp[3]; 
  ...
  temp_p_perp = 0.5*p_perp_temp[3]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,0,3,temp_p_perp); 

  temp_rho = 0.5*rho_temp[4]; 
  ...
  temp_p_perp = 0.5*p_perp_temp[4]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,0,4,temp_p_perp); 

  temp_rho = 0.5*rho_temp[5]; 
  ...
  temp_p_perp = 0.5*p_perp_temp[5]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,0,5,temp_p_perp); 

  temp_rho = 0.5*rho_temp[6]; 
  ...
  temp_p_perp = 0.5*p_perp_temp[6]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,0,6,temp_p_perp); 

  temp_rho = 0.5*rho_temp[7]; 
  ...
  temp_p_perp = 0.5*p_perp_temp[7]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,0,7,temp_p_perp); 

  temp_rho = 0.5*rho_temp[8]; 
  ...
  temp_rho = 0.4472135954999579*rho_temp[4]+0.5*rho_temp[0]; 
  ...
  temp_p_perp = 0.4472135954999579*p_perp_temp[4]+0.5*p_perp_temp[0]; 
  ...
  temp_rho = 0.447213595499958*rho_temp[6]+0.5*rho_temp[2]; 
  ...
  temp_p_perp = 0.447213595499958*p_perp_temp[6]+0.5*p_perp_temp[2]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,1,3,temp_p_perp); 

  temp_rho = 0.4472135954999579*rho_temp[1]; 
  ...
  temp_p_perp = 0.4472135954999579*p_perp_temp[1]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,1,4,temp_p_perp); 

  temp_rho = 0.5000000000000001*rho_temp[7]; 
  ...
  temp_p_perp = 0.5000000000000001*p_perp_temp[7]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,1,5,temp_p_perp); 

  temp_rho = 0.447213595499958*rho_temp[3]; 
  ...
  temp_p_perp = 0.447213595499958*p_perp_temp[3]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,1,6,temp_p_perp); 

  temp_rho = 0.447213595499958*rho_temp[8]+0.5000000000000001*rho_temp[5]; 
  ...
  temp_p_perp = 0.447213595499958*p_perp_temp[8]+0.5000000000000001*p_perp_temp[5]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,1,7,temp_p_perp); 

  temp_rho = 0.447213595499958*rho_temp[7]; 
  ...
  temp_p_perp = 0.447213595499958*p_perp_temp[7]; 
  ...
  temp_rho = 0.4472135954999579*rho_temp[5]+0.5*rho_temp[0]; 
  ...
  temp_p_perp = 0.4472135954999579*p_perp_temp[5]+0.5*p_perp_temp[0]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,2,2,temp_p_perp); 

  temp_rho = 0.447213595499958*rho_temp[7]+0.5*rho_temp[1]; 
  ...
  temp_p_perp = 0.447213595499958*p_perp_temp[7]+0.5*p_perp_temp[1]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,2,3,temp_p_perp); 

  temp_rho = 0.5000000000000001*rho_temp[6]; 
  ...
  temp_p_perp = 0.5000000000000001*p_perp_temp[6]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,2,4,temp_p_perp); 

  temp_rho = 0.4472135954999579*rho_temp[2]; 
  ...
  temp_p_perp = 0.4472135954999579*p_perp_temp[2]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,2,5,temp_p_perp); 

  temp_rho = 0.447213595499958*rho_temp[8]+0.5000000000000001*rho_temp[4]; 
  ...
  temp_p_perp = 0.447213595499958*p_perp_temp[8]+0.5000000000000001*p_perp_temp[4]; 
  ...
  temp_rho = 0.447213595499958*rho_temp[6]; 
  ...
  temp_p_perp = 0.447213595499958*p_perp_temp[6]; 
  ...
  temp_rho = 0.4*rho_temp[8]+0.4472135954999579*rho_temp[5]+0.4472135954999579*rho_temp[4]+0.5*rho_temp[0]; 
  ...
  temp_p_perp = 0.4*p_perp_temp[8]+0.4472135954999579*p_perp_temp[5]+0.4472135954999579*p_perp_temp[4]+0.5*p_perp_temp[0]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,3,3,temp_p_perp); 

  temp_rho = 0.4472135954999579*rho_temp[3]; 
  ...
  temp_p_perp = 0.4472135954999579*p_perp_temp[3]; 
  ...
  temp_rho = 0.4*rho_temp[7]+0.447213595499958*rho_temp[1]; 
  ...
  temp_p_perp = 0.4*p_perp_temp[7]+0.447213595499958*p_perp_temp[1]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,3,6,temp_p_perp); 

  temp_rho = 0.4*rho_temp[6]+0.447213595499958*rho_temp[2]; 
  ...
  temp_p_perp = 0.4*p_perp_temp[6]+0.447213595499958*p_perp_temp[2]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,3,7,temp_p_perp); 

  temp_rho = 0.4*rho_temp[3]; 
  ...
  temp_rho = 0.31943828249997*rho_temp[4]+0.5*rho_temp[0]; 
  ...
  temp_p_perp = 0.31943828249997*p_perp_temp[4]+0.5*p_perp_temp[0]; 
  ...
  temp_rho = 0.31943828249997*rho_temp[6]+0.5000000000000001*rho_temp[2]; 
  ...
  temp_p_perp = 0.31943828249997*p_perp_temp[6]+0.5000000000000001*p_perp_temp[2]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,4,6,temp_p_perp); 

  temp_rho = 0.4472135954999579*rho_temp[7]; 
  ...
  temp_p_perp = 0.4472135954999579*p_perp_temp[7]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,4,7,temp_p_perp); 

  temp_rho = 0.31943828249997*rho_temp[8]+0.5*rho_temp[5]; 
  ...
  temp_p_perp = 0.31943828249997*p_perp_temp[8]+0.5*p_perp_temp[5]; 
  ...
  temp_rho = 0.31943828249997*rho_temp[5]+0.5*rho_temp[0]; 
  ...
  temp_p_perp = 0.31943828249997*p_perp_temp[5]+0.5*p_perp_temp[0]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,5,5,temp_p_perp); 

  temp_rho = 0.4472135954999579*rho_temp[6]; 
  ...
  temp_p_perp = 0.4472135954999579*p_perp_temp[6]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,5,6,temp_p_perp); 

  temp_rho = 0.31943828249997*rho_temp[7]+0.5000000000000001*rho_temp[1]; 
  ...
  temp_p_perp = 0.31943828249997*p_perp_temp[7]+0.5000000000000001*p_perp_temp[1]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,5,7,temp_p_perp); 

  temp_rho = 0.31943828249997*rho_temp[8]+0.5*rho_temp[4]; 
  ...
  temp_p_perp = 0.31943828249997*p_perp_temp[8]+0.5*p_perp_temp[4]; 
  ...
  temp_rho = 0.2857142857142857*rho_temp[8]+0.4472135954999579*rho_temp[5]+0.31943828249997*rho_temp[4]+0.5*rho_temp[0]; 
  ...
  temp_p_perp = 0.2857142857142857*p_perp_temp[8]+0.4472135954999579*p_perp_temp[5]+0.31943828249997*p_perp_temp[4]+0.5*p_perp_temp[0]; 
  ...
  temp_rho = 0.2857142857142857*rho_temp[6]+0.447213595499958*rho_temp[2]; 
  ...
  temp_p_perp = 0.2857142857142857*p_perp_temp[6]+0.447213595499958*p_perp_temp[2]; 
  ...
  temp_rho = 0.2857142857142857*rho_temp[8]+0.31943828249997*rho_temp[5]+0.4472135954999579*rho_temp[4]+0.5*rho_temp[0]; 
  ...
  temp_p_perp = 0.2857142857142857*p_perp_temp[8]+0.31943828249997*p_perp_temp[5]+0.4472135954999579*p_perp_temp[4]+0.5*p_perp_temp[0]; 
  gkyl_mat_set(&A_T_perp_over_m_inv,7,7,temp_p_perp); 

  temp_rho = 0.2857142857142857*rho_temp[7]+0.447213595499958*rho_temp[1]; 
  ...
  temp_p_perp = 0.2857142857142857*p_perp_temp[7]+0.447213595499958*p_perp_temp[1]; 
  ...
  temp_rho = 0.2040816326530612*rho_temp[8]+0.31943828249997*rho_temp[5]+0.31943828249997*rho_temp[4]+0.5*rho_temp[0]; 
  ...
  temp_p_perp = 0.2040816326530612*p_perp_temp[8]+0.31943828249997*p_perp_temp[5]+0.31943828249997*p_perp_temp[4]+0.5*p_perp_temp[0]; 
