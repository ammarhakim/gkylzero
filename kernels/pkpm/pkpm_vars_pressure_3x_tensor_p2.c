...
GKYL_CU_DH void pkpm_vars_pressure_3x_tensor_p2(const double *bb, const double *vlasov_pkpm_moms, 
...
{ 
  ...
  const double *bxby = &bb[27]; 
  const double *bxbz = &bb[54]; 
  const double *byby = &bb[81]; 
  const double *bybz = &bb[108]; 
  const double *bzbz = &bb[135]; 
