...
{ 
  ...
  const double *Pyy_l = &p_ij_l[6]; 
  const double *Pyz_l = &p_ij_l[8]; 
  const double *Pzz_l = &p_ij_l[10]; 

  const double *Pxx_c = &p_ij_c[0]; 
  const double *Pxy_c = &p_ij_c[2]; 
  const double *Pxz_c = &p_ij_c[4]; 
  const double *Pyy_c = &p_ij_c[6]; 
  const double *Pyz_c = &p_ij_c[8]; 
  const double *Pzz_c = &p_ij_c[10]; 

  const double *Pxx_r = &p_ij_r[0]; 
  const double *Pxy_r = &p_ij_r[2]; 
  const double *Pxz_r = &p_ij_r[4]; 
  const double *Pyy_r = &p_ij_r[6]; 
  const double *Pyz_r = &p_ij_r[8]; 
  const double *Pzz_r = &p_ij_r[10]; 
  ...
  q_lr[2] = q_lr[0]*uyl_r; 
  q_lr[3] = q_lr[0]*uzl_r; 
  q_lr[4] = 1.224744871391589*Pxx_l[1]+0.7071067811865475*Pxx_l[0] + q_lr[1]*q_lr[1]/q_lr[0]; 
  q_lr[5] = 1.224744871391589*Pxy_l[1]+0.7071067811865475*Pxy_l[0] + q_lr[1]*q_lr[2]/q_lr[0]; 
  q_lr[6] = 1.224744871391589*Pxz_l[1]+0.7071067811865475*Pxz_l[0] + q_lr[1]*q_lr[3]/q_lr[0]; 
  q_lr[7] = 1.224744871391589*Pyy_l[1]+0.7071067811865475*Pyy_l[0] + q_lr[2]*q_lr[2]/q_lr[0]; 
  q_lr[8] = 1.224744871391589*Pyz_l[1]+0.7071067811865475*Pyz_l[0] + q_lr[2]*q_lr[3]/q_lr[0]; 
  q_lr[9] = 1.224744871391589*Pzz_l[1]+0.7071067811865475*Pzz_l[0] + q_lr[3]*q_lr[3]/q_lr[0]; 
  q_cl[0] = 0.7071067811865475*rho_c[0]-1.224744871391589*rho_c[1]; 
  q_cl[1] = q_cl[0]*uxc_l; 
  q_cl[2] = q_cl[0]*uyc_l; 
  q_cl[3] = q_cl[0]*uzc_l; 
  q_cl[4] = 0.7071067811865475*Pxx_c[0]-1.224744871391589*Pxx_c[1] + q_cl[1]*q_cl[1]/q_cl[0]; 
  q_cl[5] = 0.7071067811865475*Pxy_c[0]-1.224744871391589*Pxy_c[1] + q_cl[1]*q_cl[2]/q_cl[0]; 
  q_cl[6] = 0.7071067811865475*Pxz_c[0]-1.224744871391589*Pxz_c[1] + q_cl[1]*q_cl[3]/q_cl[0]; 
  q_cl[7] = 0.7071067811865475*Pyy_c[0]-1.224744871391589*Pyy_c[1] + q_cl[2]*q_cl[2]/q_cl[0]; 
  q_cl[8] = 0.7071067811865475*Pyz_c[0]-1.224744871391589*Pyz_c[1] + q_cl[2]*q_cl[3]/q_cl[0]; 
  q_cl[9] = 0.7071067811865475*Pzz_c[0]-1.224744871391589*Pzz_c[1] + q_cl[3]*q_cl[3]/q_cl[0]; 
  q_cr[0] = 1.224744871391589*rho_c[1]+0.7071067811865475*rho_c[0]; 
  q_cr[1] = q_cr[0]*uxc_r; 
  q_cr[2] = q_cr[0]*uyc_r; 
  q_cr[3] = q_cr[0]*uzc_r; 
  q_cr[4] = 1.224744871391589*Pxx_c[1]+0.7071067811865475*Pxx_c[0] + q_cr[1]*q_cr[1]/q_cr[0]; 
  q_cr[5] = 1.224744871391589*Pxy_c[1]+0.7071067811865475*Pxy_c[0] + q_cr[1]*q_cr[2]/q_cr[0]; 
  q_cr[6] = 1.224744871391589*Pxz_c[1]+0.7071067811865475*Pxz_c[0] + q_cr[1]*q_cr[3]/q_cr[0]; 
  q_cr[7] = 1.224744871391589*Pyy_c[1]+0.7071067811865475*Pyy_c[0] + q_cr[2]*q_cr[2]/q_cr[0]; 
  q_cr[8] = 1.224744871391589*Pyz_c[1]+0.7071067811865475*Pyz_c[0] + q_cr[2]*q_cr[3]/q_cr[0]; 
  q_cr[9] = 1.224744871391589*Pzz_c[1]+0.7071067811865475*Pzz_c[0] + q_cr[3]*q_cr[3]/q_cr[0]; 
  q_rl[0] = 0.7071067811865475*rho_r[0]-1.224744871391589*rho_r[1]; 
  q_rl[1] = q_rl[0]*uxr_l; 
  q_rl[2] = q_rl[0]*uyr_l; 
  q_rl[3] = q_rl[0]*uzr_l; 
  q_rl[4] = 0.7071067811865475*Pxx_r[0]-1.224744871391589*Pxx_r[1] + q_rl[1]*q_rl[1]/q_rl[0]; 
  q_rl[5] = 0.7071067811865475*Pxy_r[0]-1.224744871391589*Pxy_r[1] + q_rl[1]*q_rl[2]/q_rl[0]; 
  q_rl[6] = 0.7071067811865475*Pxz_r[0]-1.224744871391589*Pxz_r[1] + q_rl[1]*q_rl[3]/q_rl[0]; 
  q_rl[7] = 0.7071067811865475*Pyy_r[0]-1.224744871391589*Pyy_r[1] + q_rl[2]*q_rl[2]/q_rl[0]; 
  q_rl[8] = 0.7071067811865475*Pyz_r[0]-1.224744871391589*Pyz_r[1] + q_rl[2]*q_rl[3]/q_rl[0]; 
  q_rl[9] = 0.7071067811865475*Pzz_r[0]-1.224744871391589*Pzz_r[1] + q_rl[3]*q_rl[3]/q_rl[0]; 
  ...
  Ghat_rhoux_l = 0.1530931089239486*rho_l[1]*uxl_r_sq-0.1530931089239486*rho_c[1]*uxl_r_sq+0.0883883476483184*rho_l[0]*uxl_r_sq+0.0883883476483184*rho_c[0]*uxl_r_sq+0.3061862178478971*rho_l[1]*uxc_l*uxl_r-0.3061862178478971*rho_c[1]*uxc_l*uxl_r+0.1767766952966368*rho_l[0]*uxc_l*uxl_r+0.1767766952966368*rho_c[0]*uxc_l*uxl_r+0.3061862178478971*rho_l[1]*max_speed_l*uxl_r+0.3061862178478971*rho_c[1]*max_speed_l*uxl_r+0.1767766952966368*rho_l[0]*max_speed_l*uxl_r-0.1767766952966368*rho_c[0]*max_speed_l*uxl_r+0.1530931089239486*rho_l[1]*uxc_l_sq-0.1530931089239486*rho_c[1]*uxc_l_sq+0.0883883476483184*rho_l[0]*uxc_l_sq+0.0883883476483184*rho_c[0]*uxc_l_sq+0.3061862178478971*rho_l[1]*max_speed_l*uxc_l+0.3061862178478971*rho_c[1]*max_speed_l*uxc_l+0.1767766952966368*rho_l[0]*max_speed_l*uxc_l-0.1767766952966368*rho_c[0]*max_speed_l*uxc_l-0.5*apdq_l[1]+0.5*amdq_l[1]+0.6123724356957944*Pxx_l[1]-0.6123724356957944*Pxx_c[1]+0.3535533905932737*Pxx_l[0]+0.3535533905932737*Pxx_c[0]; 
  Ghat_rhouy_l = 0.1530931089239486*rho_l[1]*uxl_r*uyl_r-0.1530931089239486*rho_c[1]*uxl_r*uyl_r+0.08838834764831843*rho_l[0]*uxl_r*uyl_r+0.08838834764831843*rho_c[0]*uxl_r*uyl_r+0.1530931089239486*rho_l[1]*uxc_l*uyl_r-0.1530931089239486*rho_c[1]*uxc_l*uyl_r+0.08838834764831843*rho_l[0]*uxc_l*uyl_r+0.08838834764831843*rho_c[0]*uxc_l*uyl_r+0.3061862178478972*rho_l[1]*max_speed_l*uyl_r+0.3061862178478972*rho_c[1]*max_speed_l*uyl_r+0.1767766952966369*rho_l[0]*max_speed_l*uyl_r-0.1767766952966369*rho_c[0]*max_speed_l*uyl_r+0.1530931089239486*rho_l[1]*uxl_r*uyc_l-0.1530931089239486*rho_c[1]*uxl_r*uyc_l+0.08838834764831843*rho_l[0]*uxl_r*uyc_l+0.08838834764831843*rho_c[0]*uxl_r*uyc_l+0.1530931089239486*rho_l[1]*uxc_l*uyc_l-0.1530931089239486*rho_c[1]*uxc_l*uyc_l+0.08838834764831843*rho_l[0]*uxc_l*uyc_l+0.08838834764831843*rho_c[0]*uxc_l*uyc_l+0.3061862178478972*rho_l[1]*max_speed_l*uyc_l+0.3061862178478972*rho_c[1]*max_speed_l*uyc_l+0.1767766952966369*rho_l[0]*max_speed_l*uyc_l-0.1767766952966369*rho_c[0]*max_speed_l*uyc_l-0.5*apdq_l[2]+0.5*amdq_l[2]+0.6123724356957945*Pxy_l[1]-0.6123724356957945*Pxy_c[1]+0.3535533905932737*Pxy_l[0]+0.3535533905932737*Pxy_c[0]; 
  Ghat_rhouz_l = 0.1530931089239486*rho_l[1]*uxl_r*uzl_r-0.1530931089239486*rho_c[1]*uxl_r*uzl_r+0.08838834764831843*rho_l[0]*uxl_r*uzl_r+0.08838834764831843*rho_c[0]*uxl_r*uzl_r+0.1530931089239486*rho_l[1]*uxc_l*uzl_r-0.1530931089239486*rho_c[1]*uxc_l*uzl_r+0.08838834764831843*rho_l[0]*uxc_l*uzl_r+0.08838834764831843*rho_c[0]*uxc_l*uzl_r+0.3061862178478972*rho_l[1]*max_speed_l*uzl_r+0.3061862178478972*rho_c[1]*max_speed_l*uzl_r+0.1767766952966369*rho_l[0]*max_speed_l*uzl_r-0.1767766952966369*rho_c[0]*max_speed_l*uzl_r+0.1530931089239486*rho_l[1]*uxl_r*uzc_l-0.1530931089239486*rho_c[1]*uxl_r*uzc_l+0.08838834764831843*rho_l[0]*uxl_r*uzc_l+0.08838834764831843*rho_c[0]*uxl_r*uzc_l+0.1530931089239486*rho_l[1]*uxc_l*uzc_l-0.1530931089239486*rho_c[1]*uxc_l*uzc_l+0.08838834764831843*rho_l[0]*uxc_l*uzc_l+0.08838834764831843*rho_c[0]*uxc_l*uzc_l+0.3061862178478972*rho_l[1]*max_speed_l*uzc_l+0.3061862178478972*rho_c[1]*max_speed_l*uzc_l+0.1767766952966369*rho_l[0]*max_speed_l*uzc_l-0.1767766952966369*rho_c[0]*max_speed_l*uzc_l-0.5*apdq_l[3]+0.5*amdq_l[3]+0.6123724356957945*Pxz_l[1]-0.6123724356957945*Pxz_c[1]+0.3535533905932737*Pxz_l[0]+0.3535533905932737*Pxz_c[0]; 
  Ghat_rhoux_r = (-0.1530931089239486*rho_r[1]*uxr_l_sq)+0.1530931089239486*rho_c[1]*uxr_l_sq+0.0883883476483184*rho_r[0]*uxr_l_sq+0.0883883476483184*rho_c[0]*uxr_l_sq-0.3061862178478971*rho_r[1]*uxc_r*uxr_l+0.3061862178478971*rho_c[1]*uxc_r*uxr_l+0.1767766952966368*rho_r[0]*uxc_r*uxr_l+0.1767766952966368*rho_c[0]*uxc_r*uxr_l+0.3061862178478971*rho_r[1]*max_speed_r*uxr_l+0.3061862178478971*rho_c[1]*max_speed_r*uxr_l-0.1767766952966368*rho_r[0]*max_speed_r*uxr_l+0.1767766952966368*rho_c[0]*max_speed_r*uxr_l-0.1530931089239486*rho_r[1]*uxc_r_sq+0.1530931089239486*rho_c[1]*uxc_r_sq+0.0883883476483184*rho_r[0]*uxc_r_sq+0.0883883476483184*rho_c[0]*uxc_r_sq+0.3061862178478971*rho_r[1]*max_speed_r*uxc_r+0.3061862178478971*rho_c[1]*max_speed_r*uxc_r-0.1767766952966368*rho_r[0]*max_speed_r*uxc_r+0.1767766952966368*rho_c[0]*max_speed_r*uxc_r-0.5*apdq_r[1]+0.5*amdq_r[1]-0.6123724356957944*Pxx_r[1]+0.6123724356957944*Pxx_c[1]+0.3535533905932737*Pxx_r[0]+0.3535533905932737*Pxx_c[0]; 
  Ghat_rhouy_r = (-0.1530931089239486*rho_r[1]*uxr_l*uyr_l)+0.1530931089239486*rho_c[1]*uxr_l*uyr_l+0.08838834764831843*rho_r[0]*uxr_l*uyr_l+0.08838834764831843*rho_c[0]*uxr_l*uyr_l-0.1530931089239486*rho_r[1]*uxc_r*uyr_l+0.1530931089239486*rho_c[1]*uxc_r*uyr_l+0.08838834764831843*rho_r[0]*uxc_r*uyr_l+0.08838834764831843*rho_c[0]*uxc_r*uyr_l+0.3061862178478972*rho_r[1]*max_speed_r*uyr_l+0.3061862178478972*rho_c[1]*max_speed_r*uyr_l-0.1767766952966369*rho_r[0]*max_speed_r*uyr_l+0.1767766952966369*rho_c[0]*max_speed_r*uyr_l-0.1530931089239486*rho_r[1]*uxr_l*uyc_r+0.1530931089239486*rho_c[1]*uxr_l*uyc_r+0.08838834764831843*rho_r[0]*uxr_l*uyc_r+0.08838834764831843*rho_c[0]*uxr_l*uyc_r-0.1530931089239486*rho_r[1]*uxc_r*uyc_r+0.1530931089239486*rho_c[1]*uxc_r*uyc_r+0.08838834764831843*rho_r[0]*uxc_r*uyc_r+0.08838834764831843*rho_c[0]*uxc_r*uyc_r+0.3061862178478972*rho_r[1]*max_speed_r*uyc_r+0.3061862178478972*rho_c[1]*max_speed_r*uyc_r-0.1767766952966369*rho_r[0]*max_speed_r*uyc_r+0.1767766952966369*rho_c[0]*max_speed_r*uyc_r-0.5*apdq_r[2]+0.5*amdq_r[2]-0.6123724356957945*Pxy_r[1]+0.6123724356957945*Pxy_c[1]+0.3535533905932737*Pxy_r[0]+0.3535533905932737*Pxy_c[0]; 
  Ghat_rhouz_r = (-0.1530931089239486*rho_r[1]*uxr_l*uzr_l)+0.1530931089239486*rho_c[1]*uxr_l*uzr_l+0.08838834764831843*rho_r[0]*uxr_l*uzr_l+0.08838834764831843*rho_c[0]*uxr_l*uzr_l-0.1530931089239486*rho_r[1]*uxc_r*uzr_l+0.1530931089239486*rho_c[1]*uxc_r*uzr_l+0.08838834764831843*rho_r[0]*uxc_r*uzr_l+0.08838834764831843*rho_c[0]*uxc_r*uzr_l+0.3061862178478972*rho_r[1]*max_speed_r*uzr_l+0.3061862178478972*rho_c[1]*max_speed_r*uzr_l-0.1767766952966369*rho_r[0]*max_speed_r*uzr_l+0.1767766952966369*rho_c[0]*max_speed_r*uzr_l-0.1530931089239486*rho_r[1]*uxr_l*uzc_r+0.1530931089239486*rho_c[1]*uxr_l*uzc_r+0.08838834764831843*rho_r[0]*uxr_l*uzc_r+0.08838834764831843*rho_c[0]*uxr_l*uzc_r-0.1530931089239486*rho_r[1]*uxc_r*uzc_r+0.1530931089239486*rho_c[1]*uxc_r*uzc_r+0.08838834764831843*rho_r[0]*uxc_r*uzc_r+0.08838834764831843*rho_c[0]*uxc_r*uzc_r+0.3061862178478972*rho_r[1]*max_speed_r*uzc_r+0.3061862178478972*rho_c[1]*max_speed_r*uzc_r-0.1767766952966369*rho_r[0]*max_speed_r*uzc_r+0.1767766952966369*rho_c[0]*max_speed_r*uzc_r-0.5*apdq_r[3]+0.5*amdq_r[3]-0.6123724356957945*Pxz_r[1]+0.6123724356957945*Pxz_c[1]+0.3535533905932737*Pxz_r[0]+0.3535533905932737*Pxz_c[0]; 
