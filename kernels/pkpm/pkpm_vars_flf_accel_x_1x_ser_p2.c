...
{ 
  ...
  bb_grad_u_comp[2] = grad_u_x[2]; 

  bb_grad_u[0] += bb_grad_u_comp[0]; 
  bb_grad_u[1] += bb_grad_u_comp[1]; 
  bb_grad_u[2] += bb_grad_u_comp[2]; 

  p_perp_source[0] += (-0.7071067811865475*(div_b[2]*ux_c[2]+div_b[1]*ux_c[1]+div_b[0]*ux_c[0]))-2.0*nu_c[0]; 
  p_perp_source[1] += (-0.6324555320336759*div_b[1]*ux_c[2])+ux_c[1]*((-0.6324555320336759*div_b[2])-0.7071067811865475*div_b[0])-2.0*nu_c[1]-0.7071067811865475*ux_c[0]*div_b[1]; 
  p_perp_source[2] += ((-0.4517539514526256*div_b[2])-0.7071067811865475*div_b[0])*ux_c[2]-2.0*nu_c[2]-0.7071067811865475*ux_c[0]*div_b[2]-0.6324555320336759*div_b[1]*ux_c[1]; 
