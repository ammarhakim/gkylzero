...
{ 
  ...
  double F_0_cr[3] = {0.0}; 
  double F_0_rl[3] = {0.0}; 
  double Ghat_F_0_vpar_l[2] = {0.0}; 
  double Ghat_F_0_vpar_r[2] = {0.0}; 
  ...
  pkpm_div_ppar[1] += dx1*volFact*((1.732050807568877*(Ghat_F_0_vpar_r[0]+Ghat_F_0_vpar_l[0])-1.224744871391589*(F_0c[6]*alpha_c[6]+F_0c[4]*alpha_c[4]+F_0c[3]*alpha_c[3]+F_0c[2]*alpha_c[2]+F_0c[1]*alpha_c[1]+F_0c[0]*alpha_c[0]))*wvpar+((-0.3162277660168379*alpha_c[3]*F_0c[7])-0.3535533905932737*(F_0c[4]*alpha_c[6]+alpha_c[4]*F_0c[6])-0.3162277660168379*alpha_c[2]*F_0c[5]-0.3535533905932737*(F_0c[1]*alpha_c[3]+alpha_c[1]*F_0c[3]+F_0c[0]*alpha_c[2]+alpha_c[0]*F_0c[2])+0.5*(Ghat_F_0_vpar_r[1]+Ghat_F_0_vpar_l[1]))*dvpar); 
  pkpm_div_ppar[2] += dx1*volFact*(((-2.449489742783178*(F_0c[3]*alpha_c[6]+alpha_c[3]*F_0c[6]+F_0c[1]*alpha_c[4]+alpha_c[1]*F_0c[4]))-2.738612787525831*(F_0c[2]*alpha_c[3]+alpha_c[2]*F_0c[3]+F_0c[0]*alpha_c[1]+alpha_c[0]*F_0c[1])+2.23606797749979*Ghat_F_0_vpar_r[0]-2.23606797749979*Ghat_F_0_vpar_l[0])*wvpar+((-0.7071067811865475*(alpha_c[2]*F_0c[7]+F_0c[1]*alpha_c[6]+alpha_c[1]*F_0c[6]+alpha_c[3]*F_0c[5]+F_0c[3]*alpha_c[4]+alpha_c[3]*F_0c[4]))-0.6324555320336759*alpha_c[6]*F_0c[7]-0.7905694150420947*(F_0c[0]*alpha_c[3]+alpha_c[0]*F_0c[3]+F_0c[1]*alpha_c[2]+alpha_c[1]*F_0c[2])+0.6454972243679029*Ghat_F_0_vpar_r[1]-0.6454972243679029*Ghat_F_0_vpar_l[1])*dvpar); 
