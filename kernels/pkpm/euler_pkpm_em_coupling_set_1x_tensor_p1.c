...
GKYL_CU_DH void euler_pkpm_em_coupling_set_1x_tensor_p1(int count, 
...
{ 
  ...
  for (int s = 0; s < num_species; ++s) { 
    ...
    if (!pkpm_field_static) { 
      gkyl_mat_set(&lhs, 1 + num_species*(6), 1 + s*(6), 0.5*dt*qbym[s]*(0.6324555320336759*rho[s][2]+0.7071067811865475*rho[s][0])); 
      gkyl_mat_set(&lhs, 3 + num_species*(6), 3 + s*(6), 0.5*dt*qbym[s]*(0.6324555320336759*rho[s][2]+0.7071067811865475*rho[s][0])); 
      gkyl_mat_set(&lhs, 5 + num_species*(6), 5 + s*(6), 0.5*dt*qbym[s]*(0.6324555320336759*rho[s][2]+0.7071067811865475*rho[s][0])); 
    } 
