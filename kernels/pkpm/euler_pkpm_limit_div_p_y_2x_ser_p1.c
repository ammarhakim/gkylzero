...
{ 
  ...
  double *div_p_x = &div_p_cell_avg[3]; 
  double *div_p_y = &div_p_cell_avg[4]; 
  double *div_p_z = &div_p_cell_avg[5]; 

  double *grad_u_x = &div_p_cell_avg[12]; 
  double *grad_u_y = &div_p_cell_avg[13]; 
  double *grad_u_z = &div_p_cell_avg[14]; 

  div_p_x[0] = ((-0.2886751345948129*Pxy_r[2])-0.2886751345948129*Pxy_l[2]+0.5773502691896258*Pxy_c[2]+0.25*Pxy_r[0]-0.25*Pxy_l[0])*dx1; 
  div_p_y[0] = ((-0.2886751345948129*Pyy_r[2])-0.2886751345948129*Pyy_l[2]+0.5773502691896258*Pyy_c[2]+0.25*Pyy_r[0]-0.25*Pyy_l[0])*dx1; 
  div_p_z[0] = ((-0.2886751345948129*Pyz_r[2])-0.2886751345948129*Pyz_l[2]+0.5773502691896258*Pyz_c[2]+0.25*Pyz_r[0]-0.25*Pyz_l[0])*dx1; 
  grad_u_x[0] = ((-0.2886751345948129*ux_r[2])-0.2886751345948129*ux_l[2]+0.5773502691896258*ux_c[2]+0.25*ux_r[0]-0.25*ux_l[0])*dx1; 
  grad_u_y[0] = ((-0.2886751345948129*uy_r[2])-0.2886751345948129*uy_l[2]+0.5773502691896258*uy_c[2]+0.25*uy_r[0]-0.25*uy_l[0])*dx1; 
  grad_u_z[0] = ((-0.2886751345948129*uz_r[2])-0.2886751345948129*uz_l[2]+0.5773502691896258*uz_c[2]+0.25*uz_r[0]-0.25*uz_l[0])*dx1; 
} 
