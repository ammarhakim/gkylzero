...
{ 
  ...
  const double *pkpm_div_ppar = &prim[48]; 
  const double *T_perp_over_m = &prim[56]; 
  const double *T_perp_over_m_inv = &prim[64]; 

  const double *div_b = &pkpm_accel[0]; 
  const double *bb_grad_u = &pkpm_accel[8]; 
  const double *p_perp_source = &pkpm_accel[24]; 
  const double *p_perp_div_b = &pkpm_accel[32]; 
  ...
  double *pkpm_vars_io_div_b = &pkpm_vars_io[40]; 
  double *pkpm_vars_io_pkpm_div_ppar = &pkpm_vars_io[48]; 
  double *pkpm_vars_io_p_perp_div_b = &pkpm_vars_io[56]; 
  double *pkpm_vars_io_bb_grad_u = &pkpm_vars_io[64]; 
  double *pkpm_vars_io_p_perp_source = &pkpm_vars_io[72]; 
