...
{ 
  ...
  bool cellAvg = false;
  if (0.5*(5.0*rho[8]-3.872983346207417*(rho[7]+rho[6])+2.23606797749979*(rho[5]+rho[4])+3.0*rho[3]-1.732050807568877*(rho[2]+rho[1])+rho[0]) < 0) cellAvg = true; 
  if (0.5*(5.0*p_parallel[8]-3.872983346207417*(p_parallel[7]+p_parallel[6])+2.23606797749979*(p_parallel[5]+p_parallel[4])+3.0*p_parallel[3]-1.732050807568877*(p_parallel[2]+p_parallel[1])+p_parallel[0]) < 0) cellAvg = true; 
  if (0.5*(5.0*p_perp[8]-3.872983346207417*(p_perp[7]+p_perp[6])+2.23606797749979*(p_perp[5]+p_perp[4])+3.0*p_perp[3]-1.732050807568877*(p_perp[2]+p_perp[1])+p_perp[0]) < 0) cellAvg = true; 
  if (0.5*(5.0*rho[8]+3.872983346207417*rho[7]-3.872983346207417*rho[6]+2.23606797749979*(rho[5]+rho[4])-3.0*rho[3]-1.732050807568877*rho[2]+1.732050807568877*rho[1]+rho[0]) < 0) cellAvg = true; 
  if (0.5*(5.0*p_parallel[8]+3.872983346207417*p_parallel[7]-3.872983346207417*p_parallel[6]+2.23606797749979*(p_parallel[5]+p_parallel[4])-3.0*p_parallel[3]-1.732050807568877*p_parallel[2]+1.732050807568877*p_parallel[1]+p_parallel[0]) < 0) cellAvg = true; 
  if (0.5*(5.0*p_perp[8]+3.872983346207417*p_perp[7]-3.872983346207417*p_perp[6]+2.23606797749979*(p_perp[5]+p_perp[4])-3.0*p_perp[3]-1.732050807568877*p_perp[2]+1.732050807568877*p_perp[1]+p_perp[0]) < 0) cellAvg = true; 
  if (0.5*(5.0*rho[8]-3.872983346207417*rho[7]+3.872983346207417*rho[6]+2.23606797749979*(rho[5]+rho[4])-3.0*rho[3]+1.732050807568877*rho[2]-1.732050807568877*rho[1]+rho[0]) < 0) cellAvg = true; 
  if (0.5*(5.0*p_parallel[8]-3.872983346207417*p_parallel[7]+3.872983346207417*p_parallel[6]+2.23606797749979*(p_parallel[5]+p_parallel[4])-3.0*p_parallel[3]+1.732050807568877*p_parallel[2]-1.732050807568877*p_parallel[1]+p_parallel[0]) < 0) cellAvg = true; 
  if (0.5*(5.0*p_perp[8]-3.872983346207417*p_perp[7]+3.872983346207417*p_perp[6]+2.23606797749979*(p_perp[5]+p_perp[4])-3.0*p_perp[3]+1.732050807568877*p_perp[2]-1.732050807568877*p_perp[1]+p_perp[0]) < 0) cellAvg = true; 
  if (0.5*(5.0*rho[8]+3.872983346207417*(rho[7]+rho[6])+2.23606797749979*(rho[5]+rho[4])+3.0*rho[3]+1.732050807568877*(rho[2]+rho[1])+rho[0]) < 0) cellAvg = true; 
  ...
  if (cellAvg) { 
    ...
    gkyl_mat_set(rhs,8,0,2.0*p_perp[8]+p_parallel[8]); 
  } 
  ...
  gkyl_mat_set(A,0,8,1.5*m0r[8]-0.5*cEr[8]); 
  ...
  gkyl_mat_set(A,1,7,1.341640786499874*m0r[8]-0.447213595499958*cEr[8]+1.5*m0r[5]-0.5000000000000001*cEr[5]); 
  gkyl_mat_set(A,1,8,1.341640786499874*m0r[7]-0.447213595499958*cEr[7]); 
  ...
  gkyl_mat_set(A,2,6,1.341640786499874*m0r[8]-0.447213595499958*cEr[8]+1.5*m0r[4]-0.5000000000000001*cEr[4]); 
  gkyl_mat_set(A,2,7,1.341640786499874*m0r[3]-0.447213595499958*cEr[3]); 
  gkyl_mat_set(A,2,8,1.341640786499874*m0r[6]-0.447213595499958*cEr[6]); 
  ...
  gkyl_mat_set(A,3,3,1.2*m0r[8]-0.4*cEr[8]+1.341640786499874*m0r[5]-0.4472135954999579*cEr[5]+1.341640786499874*m0r[4]-0.4472135954999579*cEr[4]+1.5*m0r[0]-0.5*cEr[0]); 
  ...
  gkyl_mat_set(A,3,8,1.2*m0r[3]-0.4*cEr[3]); 
  ...
  gkyl_mat_set(A,4,5,1.5*m0r[8]-0.5*cEr[8]); 
  ...
  gkyl_mat_set(A,4,8,0.9583148474999099*m0r[8]-0.31943828249997*cEr[8]+1.5*m0r[5]-0.5*cEr[5]); 
  ...
  gkyl_mat_set(A,5,4,1.5*m0r[8]-0.5*cEr[8]); 
  ...
  gkyl_mat_set(A,5,8,0.9583148474999099*m0r[8]-0.31943828249997*cEr[8]+1.5*m0r[4]-0.5*cEr[4]); 
  gkyl_mat_set(A,6,0,1.5*m0r[6]-0.5*cEr[6]); 
  gkyl_mat_set(A,6,1,1.341640786499874*m0r[3]-0.447213595499958*cEr[3]); 
  gkyl_mat_set(A,6,2,1.341640786499874*m0r[8]-0.447213595499958*cEr[8]+1.5*m0r[4]-0.5000000000000001*cEr[4]); 
  ...
  gkyl_mat_set(A,6,6,0.8571428571428571*m0r[8]-0.2857142857142857*cEr[8]+1.341640786499874*m0r[5]-0.4472135954999579*cEr[5]+0.9583148474999099*m0r[4]-0.31943828249997*cEr[4]+1.5*m0r[0]-0.5*cEr[0]); 
  gkyl_mat_set(A,6,7,1.2*m0r[3]-0.4*cEr[3]); 
  gkyl_mat_set(A,6,8,0.8571428571428571*m0r[6]-0.2857142857142857*cEr[6]+1.341640786499874*m0r[2]-0.447213595499958*cEr[2]); 
  gkyl_mat_set(A,7,0,1.5*m0r[7]-0.5*cEr[7]); 
  gkyl_mat_set(A,7,1,1.341640786499874*m0r[8]-0.447213595499958*cEr[8]+1.5*m0r[5]-0.5000000000000001*cEr[5]); 
  ...
  gkyl_mat_set(A,7,7,0.8571428571428571*m0r[8]-0.2857142857142857*cEr[8]+0.9583148474999099*m0r[5]-0.31943828249997*cEr[5]+1.341640786499874*m0r[4]-0.4472135954999579*cEr[4]+1.5*m0r[0]-0.5*cEr[0]); 
  gkyl_mat_set(A,7,8,0.8571428571428571*m0r[7]-0.2857142857142857*cEr[7]+1.341640786499874*m0r[1]-0.447213595499958*cEr[1]); 
  gkyl_mat_set(A,8,0,1.5*m0r[8]-0.5*cEr[8]); 
  gkyl_mat_set(A,8,1,1.341640786499874*m0r[7]-0.447213595499958*cEr[7]); 
  gkyl_mat_set(A,8,2,1.341640786499874*m0r[6]-0.447213595499958*cEr[6]); 
  gkyl_mat_set(A,8,3,1.2*m0r[3]-0.4*cEr[3]); 
  gkyl_mat_set(A,8,4,0.9583148474999099*m0r[8]-0.31943828249997*cEr[8]+1.5*m0r[5]-0.5*cEr[5]); 
  gkyl_mat_set(A,8,5,0.9583148474999099*m0r[8]-0.31943828249997*cEr[8]+1.5*m0r[4]-0.5*cEr[4]); 
  gkyl_mat_set(A,8,6,0.8571428571428571*m0r[6]-0.2857142857142857*cEr[6]+1.341640786499874*m0r[2]-0.447213595499958*cEr[2]); 
  gkyl_mat_set(A,8,7,0.8571428571428571*m0r[7]-0.2857142857142857*cEr[7]+1.341640786499874*m0r[1]-0.447213595499958*cEr[1]); 
  gkyl_mat_set(A,8,8,0.6122448979591837*m0r[8]-0.2040816326530612*cEr[8]+0.9583148474999099*m0r[5]-0.31943828249997*cEr[5]+0.9583148474999099*m0r[4]-0.31943828249997*cEr[4]+1.5*m0r[0]-0.5*cEr[0]); 
