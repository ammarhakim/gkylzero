...
{ 
  ...
  const double *T_perp_over_m = &pkpm_prim[28]; 
  const double *T_perp_over_m_inv = &pkpm_prim[32]; 

  const double *div_b = &pkpm_accel[0]; 
  const double *p_perp_source = &pkpm_accel[12]; 
