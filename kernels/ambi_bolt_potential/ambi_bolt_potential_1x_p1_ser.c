...
{ 
  ...
  out[0] = 0.5*((1.414213562373095*jacInv[1]-2.449489742783178*jacInv[0])*m0JacIon[1]+m0JacIon[0]*(1.414213562373095*jacInv[0]-2.449489742783178*jacInv[1])); 
  double phiS_qp[1];
...
{ 
  ...
  out[4] = 0.5*((1.414213562373095*jacInv[1]+2.449489742783178*jacInv[0])*m0JacIon[1]+m0JacIon[0]*(2.449489742783178*jacInv[1]+1.414213562373095*jacInv[0])); 
  double phiS_qp[1];
  if ((isfinite(GammaJacIonB[0]/2)) && (GammaJacIonB[0]/2>0.) && (m0JacIonB[0]/2>0.)) {
    phiS_qp[0] = (T_e*log((2.506628274631001*GammaJacIonB[0])/(m0JacIonB[0]*sqrt(T_e/m_e))))/q_e;
  } else {
  ...
  out[6] = 1.414213562373095*phiS_qp[0]; 
...
{ 
  ...
  phi_qp[1] = (-(1.0*log((1.0*m0Ion[1])/(1.0*sheathvals[1]+sheathvals[0])+m0Ion[0]/(1.0*sheathvals[1]+sheathvals[0]))*T_e)/q_e)+0.7071067811865475*sheathvals[3]+0.7071067811865475*sheathvals[2]; 
