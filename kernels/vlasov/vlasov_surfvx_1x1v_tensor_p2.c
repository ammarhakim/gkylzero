...
{ 
  ...
  if (0.6324555320336759*alpha[2]-0.9486832980505137*alpha[1]+0.7071067811865475*alpha[0] > 0) { 
    fUpwindQuad_l[0] = tensor_2x_p2_surfx2_quad_0_r(fl); 
    fUpwindQuad_r[0] = tensor_2x_p2_surfx2_quad_0_r(fc); 
  } else { 
    fUpwindQuad_l[0] = tensor_2x_p2_surfx2_quad_0_l(fc); 
    fUpwindQuad_r[0] = tensor_2x_p2_surfx2_quad_0_l(fr); 
  } 
  if (0.7071067811865475*alpha[0]-0.7905694150420947*alpha[2] > 0) { 
    fUpwindQuad_l[1] = tensor_2x_p2_surfx2_quad_1_r(fl); 
    fUpwindQuad_r[1] = tensor_2x_p2_surfx2_quad_1_r(fc); 
  } else { 
    fUpwindQuad_l[1] = tensor_2x_p2_surfx2_quad_1_l(fc); 
    fUpwindQuad_r[1] = tensor_2x_p2_surfx2_quad_1_l(fr); 
  } 
  if (0.6324555320336759*alpha[2]+0.9486832980505137*alpha[1]+0.7071067811865475*alpha[0] > 0) { 
    fUpwindQuad_l[2] = tensor_2x_p2_surfx2_quad_2_r(fl); 
    fUpwindQuad_r[2] = tensor_2x_p2_surfx2_quad_2_r(fc); 
  } else { 
    fUpwindQuad_l[2] = tensor_2x_p2_surfx2_quad_2_l(fc); 
    fUpwindQuad_r[2] = tensor_2x_p2_surfx2_quad_2_l(fr); 
  } 

  // Project nodal basis back onto modal basis. 
  tensor_2x_p2_upwind(fUpwindQuad_l, fUpwind_l); 
  tensor_2x_p2_upwind(fUpwindQuad_r, fUpwind_r); 
