...
{ 
  ...
  if (0.6324555320336759*alpha[2]-0.9486832980505137*alpha[1]+0.7071067811865475*alpha[0] > 0) { 
    ...
    fUpwindQuad[0] = (-1.5*fSkin[7])+0.7745966692414833*fSkin[6]+1.118033988749895*fSkin[5]+0.4472135954999579*fSkin[4]-1.161895003862225*fSkin[3]+0.8660254037844386*fSkin[2]-0.6708203932499369*fSkin[1]+0.5*fSkin[0]; 
  } else { 

    fUpwindQuad[0] = (-1.5*fEdge[7])-0.7745966692414833*fEdge[6]+1.118033988749895*fEdge[5]+0.4472135954999579*fEdge[4]+1.161895003862225*fEdge[3]-0.8660254037844386*fEdge[2]-0.6708203932499369*fEdge[1]+0.5*fEdge[0]; 
  } 
  if (0.7071067811865475*alpha[0]-0.7905694150420947*alpha[2] > 0) { 

    fUpwindQuad[1] = (-0.9682458365518543*fSkin[6])+1.118033988749895*fSkin[5]-0.5590169943749475*fSkin[4]+0.8660254037844386*fSkin[2]+0.5*fSkin[0]; 
  } else { 

    fUpwindQuad[1] = 0.9682458365518543*fEdge[6]+1.118033988749895*fEdge[5]-0.5590169943749475*fEdge[4]-0.8660254037844386*fEdge[2]+0.5*fEdge[0]; 
  } 
  if (0.6324555320336759*alpha[2]+0.9486832980505137*alpha[1]+0.7071067811865475*alpha[0] > 0) { 

    fUpwindQuad[2] = 1.5*fSkin[7]+0.7745966692414833*fSkin[6]+1.118033988749895*fSkin[5]+0.4472135954999579*fSkin[4]+1.161895003862225*fSkin[3]+0.8660254037844386*fSkin[2]+0.6708203932499369*fSkin[1]+0.5*fSkin[0]; 
  } else { 

    fUpwindQuad[2] = 1.5*fEdge[7]-0.7745966692414833*fEdge[6]+1.118033988749895*fEdge[5]+0.4472135954999579*fEdge[4]-1.161895003862225*fEdge[3]-0.8660254037844386*fEdge[2]+0.6708203932499369*fEdge[1]+0.5*fEdge[0]; 
  } 
