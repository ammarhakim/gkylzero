...
GKYL_CU_DH void vlasov_poisson_extem_boundary_surfvy_2x2v_tensor_p2(const double *w, const double *dxv, const double *fac_phi, const double *vecA, const int edge, const double *fEdge, const double *fSkin, double* GKYL_RESTRICT out) 
{ 
  ...
  alpha[11] = 5.477225575051662*A0[8]*dx11*wv1-5.477225575051662*phi[8]*dx11; 
  alpha[12] = -5.477225575051662*A1[8]*dx10*wv1; 
  alpha[13] = 0.7071067811865475*A0[6]*dv1*dx11; 
  alpha[14] = -0.7071067811865475*A1[7]*dv1*dx10; 
  alpha[17] = 1.58113883008419*A0[8]*dv1*dx11; 
  alpha[18] = -1.58113883008419*A1[8]*dv1*dx10; 
  ...
  if (0.5692099788303082*(alpha[18]+alpha[17])-0.4242640687119285*(alpha[14]+alpha[13])-0.4242640687119281*alpha[12]-0.4242640687119285*alpha[11]-0.853814968245462*alpha[10]+0.3162277660168379*(alpha[8]+alpha[7])+0.6363961030678926*(alpha[6]+alpha[5]+alpha[4])-0.4743416490252568*(alpha[3]+alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[0] = tensor_2x2v_p2_surfvy_quad_0(1, fSkin); 
  } else { 

    fUpwindQuad[0] = tensor_2x2v_p2_surfvy_quad_0(-1, fEdge); 
  } 
  if ((-0.711512473537885*alpha[17])-0.4242640687119285*alpha[14]+0.5303300858899104*(alpha[13]+alpha[11])+0.3162277660168379*alpha[8]-0.3952847075210473*alpha[7]+0.6363961030678926*alpha[6]-0.4743416490252568*(alpha[3]+alpha[2])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[1] = tensor_2x2v_p2_surfvy_quad_1(1, fSkin); 
  } else { 

    fUpwindQuad[1] = tensor_2x2v_p2_surfvy_quad_1(-1, fEdge); 
  } 
  if ((-0.5692099788303082*alpha[18])+0.5692099788303082*alpha[17]-0.4242640687119285*(alpha[14]+alpha[13])+0.4242640687119281*alpha[12]-0.4242640687119285*alpha[11]+0.853814968245462*alpha[10]+0.3162277660168379*(alpha[8]+alpha[7])+0.6363961030678926*alpha[6]-0.6363961030678926*(alpha[5]+alpha[4])-0.4743416490252568*(alpha[3]+alpha[2])+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[2] = tensor_2x2v_p2_surfvy_quad_2(1, fSkin); 
  } else { 

    fUpwindQuad[2] = tensor_2x2v_p2_surfvy_quad_2(-1, fEdge); 
  } 
  if ((-0.711512473537885*alpha[18])+0.5303300858899104*alpha[14]-0.4242640687119285*alpha[13]+0.5303300858899104*alpha[12]-0.3952847075210473*alpha[8]+0.3162277660168379*alpha[7]+0.6363961030678926*alpha[5]-0.4743416490252568*(alpha[3]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[3] = tensor_2x2v_p2_surfvy_quad_3(1, fSkin); 
  } else { 

    fUpwindQuad[3] = tensor_2x2v_p2_surfvy_quad_3(-1, fEdge); 
  } 
  if (0.5303300858899104*(alpha[14]+alpha[13])-0.3952847075210473*(alpha[8]+alpha[7])-0.4743416490252568*alpha[3]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[4] = tensor_2x2v_p2_surfvy_quad_4(1, fSkin); 
  } else { 

    fUpwindQuad[4] = tensor_2x2v_p2_surfvy_quad_4(-1, fEdge); 
  } 
  if (0.711512473537885*alpha[18]+0.5303300858899104*alpha[14]-0.4242640687119285*alpha[13]-0.5303300858899104*alpha[12]-0.3952847075210473*alpha[8]+0.3162277660168379*alpha[7]-0.6363961030678926*alpha[5]-0.4743416490252568*alpha[3]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[5] = tensor_2x2v_p2_surfvy_quad_5(1, fSkin); 
  } else { 

    fUpwindQuad[5] = tensor_2x2v_p2_surfvy_quad_5(-1, fEdge); 
  } 
  if (0.5692099788303082*alpha[18]-0.5692099788303082*alpha[17]-0.4242640687119285*(alpha[14]+alpha[13])-0.4242640687119281*alpha[12]+0.4242640687119285*alpha[11]+0.853814968245462*alpha[10]+0.3162277660168379*(alpha[8]+alpha[7])-0.6363961030678926*alpha[6]+0.6363961030678926*alpha[5]-0.6363961030678926*alpha[4]-0.4743416490252568*alpha[3]+0.4743416490252568*alpha[2]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[6] = tensor_2x2v_p2_surfvy_quad_6(1, fSkin); 
  } else { 

    fUpwindQuad[6] = tensor_2x2v_p2_surfvy_quad_6(-1, fEdge); 
  } 
  if (0.711512473537885*alpha[17]-0.4242640687119285*alpha[14]+0.5303300858899104*alpha[13]-0.5303300858899104*alpha[11]+0.3162277660168379*alpha[8]-0.3952847075210473*alpha[7]-0.6363961030678926*alpha[6]-0.4743416490252568*alpha[3]+0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[7] = tensor_2x2v_p2_surfvy_quad_7(1, fSkin); 
  } else { 

    fUpwindQuad[7] = tensor_2x2v_p2_surfvy_quad_7(-1, fEdge); 
  } 
  if ((-0.5692099788303082*(alpha[18]+alpha[17]))-0.4242640687119285*(alpha[14]+alpha[13])+0.4242640687119281*alpha[12]+0.4242640687119285*alpha[11]-0.853814968245462*alpha[10]+0.3162277660168379*(alpha[8]+alpha[7])-0.6363961030678926*(alpha[6]+alpha[5])+0.6363961030678926*alpha[4]-0.4743416490252568*alpha[3]+0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[8] = tensor_2x2v_p2_surfvy_quad_8(1, fSkin); 
  } else { 

    fUpwindQuad[8] = tensor_2x2v_p2_surfvy_quad_8(-1, fEdge); 
  } 
  if ((-0.4242640687119281*alpha[12])-0.4242640687119285*alpha[11]+0.3162277660168379*(alpha[8]+alpha[7])+0.6363961030678926*alpha[4]-0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[9] = tensor_2x2v_p2_surfvy_quad_9(1, fSkin); 
  } else { 

    fUpwindQuad[9] = tensor_2x2v_p2_surfvy_quad_9(-1, fEdge); 
  } 
  if (0.5303300858899104*alpha[11]+0.3162277660168379*alpha[8]-0.3952847075210473*alpha[7]-0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[10] = tensor_2x2v_p2_surfvy_quad_10(1, fSkin); 
  } else { 

    fUpwindQuad[10] = tensor_2x2v_p2_surfvy_quad_10(-1, fEdge); 
  } 
  if (0.4242640687119281*alpha[12]-0.4242640687119285*alpha[11]+0.3162277660168379*(alpha[8]+alpha[7])-0.6363961030678926*alpha[4]-0.4743416490252568*alpha[2]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[11] = tensor_2x2v_p2_surfvy_quad_11(1, fSkin); 
  } else { 

    fUpwindQuad[11] = tensor_2x2v_p2_surfvy_quad_11(-1, fEdge); 
  } 
  if (0.5303300858899104*alpha[12]-0.3952847075210473*alpha[8]+0.3162277660168379*alpha[7]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[12] = tensor_2x2v_p2_surfvy_quad_12(1, fSkin); 
  } else { 

    fUpwindQuad[12] = tensor_2x2v_p2_surfvy_quad_12(-1, fEdge); 
  } 
  if (0.3535533905932737*alpha[0]-0.3952847075210473*(alpha[8]+alpha[7]) > 0) { 

    fUpwindQuad[13] = tensor_2x2v_p2_surfvy_quad_13(1, fSkin); 
  } else { 

    fUpwindQuad[13] = tensor_2x2v_p2_surfvy_quad_13(-1, fEdge); 
  } 
  if ((-0.5303300858899104*alpha[12])-0.3952847075210473*alpha[8]+0.3162277660168379*alpha[7]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[14] = tensor_2x2v_p2_surfvy_quad_14(1, fSkin); 
  } else { 

    fUpwindQuad[14] = tensor_2x2v_p2_surfvy_quad_14(-1, fEdge); 
  } 
  if ((-0.4242640687119281*alpha[12])+0.4242640687119285*alpha[11]+0.3162277660168379*(alpha[8]+alpha[7])-0.6363961030678926*alpha[4]+0.4743416490252568*alpha[2]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[15] = tensor_2x2v_p2_surfvy_quad_15(1, fSkin); 
  } else { 

    fUpwindQuad[15] = tensor_2x2v_p2_surfvy_quad_15(-1, fEdge); 
  } 
  if ((-0.5303300858899104*alpha[11])+0.3162277660168379*alpha[8]-0.3952847075210473*alpha[7]+0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[16] = tensor_2x2v_p2_surfvy_quad_16(1, fSkin); 
  } else { 

    fUpwindQuad[16] = tensor_2x2v_p2_surfvy_quad_16(-1, fEdge); 
  } 
  if (0.4242640687119281*alpha[12]+0.4242640687119285*alpha[11]+0.3162277660168379*(alpha[8]+alpha[7])+0.6363961030678926*alpha[4]+0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[17] = tensor_2x2v_p2_surfvy_quad_17(1, fSkin); 
  } else { 

    fUpwindQuad[17] = tensor_2x2v_p2_surfvy_quad_17(-1, fEdge); 
  } 
  if ((-0.5692099788303082*(alpha[18]+alpha[17]))+0.4242640687119285*(alpha[14]+alpha[13])-0.4242640687119281*alpha[12]-0.4242640687119285*alpha[11]+0.853814968245462*alpha[10]+0.3162277660168379*(alpha[8]+alpha[7])-0.6363961030678926*(alpha[6]+alpha[5])+0.6363961030678926*alpha[4]+0.4743416490252568*alpha[3]-0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[18] = tensor_2x2v_p2_surfvy_quad_18(1, fSkin); 
  } else { 

    fUpwindQuad[18] = tensor_2x2v_p2_surfvy_quad_18(-1, fEdge); 
  } 
  if (0.711512473537885*alpha[17]+0.4242640687119285*alpha[14]-0.5303300858899104*alpha[13]+0.5303300858899104*alpha[11]+0.3162277660168379*alpha[8]-0.3952847075210473*alpha[7]-0.6363961030678926*alpha[6]+0.4743416490252568*alpha[3]-0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[19] = tensor_2x2v_p2_surfvy_quad_19(1, fSkin); 
  } else { 

    fUpwindQuad[19] = tensor_2x2v_p2_surfvy_quad_19(-1, fEdge); 
  } 
  if (0.5692099788303082*alpha[18]-0.5692099788303082*alpha[17]+0.4242640687119285*(alpha[14]+alpha[13])+0.4242640687119281*alpha[12]-0.4242640687119285*alpha[11]-0.853814968245462*alpha[10]+0.3162277660168379*(alpha[8]+alpha[7])-0.6363961030678926*alpha[6]+0.6363961030678926*alpha[5]-0.6363961030678926*alpha[4]+0.4743416490252568*alpha[3]-0.4743416490252568*alpha[2]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[20] = tensor_2x2v_p2_surfvy_quad_20(1, fSkin); 
  } else { 

    fUpwindQuad[20] = tensor_2x2v_p2_surfvy_quad_20(-1, fEdge); 
  } 
  if (0.711512473537885*alpha[18]-0.5303300858899104*alpha[14]+0.4242640687119285*alpha[13]+0.5303300858899104*alpha[12]-0.3952847075210473*alpha[8]+0.3162277660168379*alpha[7]-0.6363961030678926*alpha[5]+0.4743416490252568*alpha[3]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[21] = tensor_2x2v_p2_surfvy_quad_21(1, fSkin); 
  } else { 

    fUpwindQuad[21] = tensor_2x2v_p2_surfvy_quad_21(-1, fEdge); 
  } 
  if ((-0.5303300858899104*(alpha[14]+alpha[13]))-0.3952847075210473*(alpha[8]+alpha[7])+0.4743416490252568*alpha[3]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[22] = tensor_2x2v_p2_surfvy_quad_22(1, fSkin); 
  } else { 

    fUpwindQuad[22] = tensor_2x2v_p2_surfvy_quad_22(-1, fEdge); 
  } 
  if ((-0.711512473537885*alpha[18])-0.5303300858899104*alpha[14]+0.4242640687119285*alpha[13]-0.5303300858899104*alpha[12]-0.3952847075210473*alpha[8]+0.3162277660168379*alpha[7]+0.6363961030678926*alpha[5]+0.4743416490252568*(alpha[3]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[23] = tensor_2x2v_p2_surfvy_quad_23(1, fSkin); 
  } else { 

    fUpwindQuad[23] = tensor_2x2v_p2_surfvy_quad_23(-1, fEdge); 
  } 
  if ((-0.5692099788303082*alpha[18])+0.5692099788303082*alpha[17]+0.4242640687119285*(alpha[14]+alpha[13])-0.4242640687119281*alpha[12]+0.4242640687119285*alpha[11]-0.853814968245462*alpha[10]+0.3162277660168379*(alpha[8]+alpha[7])+0.6363961030678926*alpha[6]-0.6363961030678926*(alpha[5]+alpha[4])+0.4743416490252568*(alpha[3]+alpha[2])-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[24] = tensor_2x2v_p2_surfvy_quad_24(1, fSkin); 
  } else { 

    fUpwindQuad[24] = tensor_2x2v_p2_surfvy_quad_24(-1, fEdge); 
  } 
  if ((-0.711512473537885*alpha[17])+0.4242640687119285*alpha[14]-0.5303300858899104*(alpha[13]+alpha[11])+0.3162277660168379*alpha[8]-0.3952847075210473*alpha[7]+0.6363961030678926*alpha[6]+0.4743416490252568*(alpha[3]+alpha[2])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[25] = tensor_2x2v_p2_surfvy_quad_25(1, fSkin); 
  } else { 

    fUpwindQuad[25] = tensor_2x2v_p2_surfvy_quad_25(-1, fEdge); 
  } 
  if (0.5692099788303082*(alpha[18]+alpha[17])+0.4242640687119285*(alpha[14]+alpha[13])+0.4242640687119281*alpha[12]+0.4242640687119285*alpha[11]+0.853814968245462*alpha[10]+0.3162277660168379*(alpha[8]+alpha[7])+0.6363961030678926*(alpha[6]+alpha[5]+alpha[4])+0.4743416490252568*(alpha[3]+alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[26] = tensor_2x2v_p2_surfvy_quad_26(1, fSkin); 
  } else { 

    fUpwindQuad[26] = tensor_2x2v_p2_surfvy_quad_26(-1, fEdge); 
  } 
