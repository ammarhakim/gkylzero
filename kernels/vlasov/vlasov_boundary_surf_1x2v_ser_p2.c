...
GKYL_CU_DH double vlasov_boundary_surfvx_1x2v_ser_p2(const double *w, const double *dxv, const double amax, const double *qmem, const int edge, const double *fEdge, const double *fSkin, double* GKYL_RESTRICT out) 
{ 
...
GKYL_CU_DH double vlasov_boundary_surfvy_1x2v_ser_p2(const double *w, const double *dxv, const double amax, const double *qmem, const int edge, const double *fEdge, const double *fSkin, double* GKYL_RESTRICT out) 
{ 
  // w:           Cell-center coordinates.
  // dxv[NDIM]:   Cell spacing.
  // amax:        amax in global lax flux.
  ...
  favg[0] = 1.58113883008419*(fSkin[9]+fEdge[9])+1.224744871391589*fSkin[3]-1.224744871391589*fEdge[3]+0.7071067811865475*(fSkin[0]+fEdge[0]); 
  favg[1] = 1.58113883008419*(fSkin[15]+fEdge[15])+1.224744871391589*fSkin[5]-1.224744871391589*fEdge[5]+0.7071067811865475*(fSkin[1]+fEdge[1]); 
  favg[2] = 1.58113883008419*(fSkin[16]+fEdge[16])+1.224744871391589*fSkin[6]-1.224744871391589*fEdge[6]+0.7071067811865475*(fSkin[2]+fEdge[2]); 
  favg[3] = 1.58113883008419*(fSkin[19]+fEdge[19])+1.224744871391589*fSkin[10]-1.224744871391589*fEdge[10]+0.7071067811865475*(fSkin[4]+fEdge[4]); 
  favg[4] = 1.224744871391589*fSkin[13]-1.224744871391589*fEdge[13]+0.7071067811865475*(fSkin[7]+fEdge[7]); 
  favg[5] = 1.224744871391589*fSkin[14]-1.224744871391589*fEdge[14]+0.7071067811865475*(fSkin[8]+fEdge[8]); 
  favg[6] = 1.224744871391589*fSkin[17]-1.224744871391589*fEdge[17]+0.7071067811865475*(fSkin[11]+fEdge[11]); 
  favg[7] = 1.224744871391589*fSkin[18]-1.224744871391589*fEdge[18]+0.7071067811865475*(fSkin[12]+fEdge[12]); 

  Ghat[0] = (0.7905694150420947*fSkin[9]-0.7905694150420947*fEdge[9]+0.6123724356957944*(fSkin[3]+fEdge[3])+0.3535533905932737*fSkin[0]-0.3535533905932737*fEdge[0])*amax+0.25*(alpha[6]*favg[6]+alpha[4]*favg[4]+alpha[3]*favg[3]+alpha[2]*favg[2]+alpha[1]*favg[1]+alpha[0]*favg[0]); 
  Ghat[1] = (0.7905694150420947*fSkin[15]-0.7905694150420947*fEdge[15]+0.6123724356957944*(fSkin[5]+fEdge[5])+0.3535533905932737*fSkin[1]-0.3535533905932737*fEdge[1])*amax+0.223606797749979*(alpha[3]*favg[6]+favg[3]*alpha[6]+alpha[1]*favg[4]+favg[1]*alpha[4])+0.25*(alpha[2]*favg[3]+favg[2]*alpha[3]+alpha[0]*favg[1]+favg[0]*alpha[1]); 
  Ghat[2] = (0.7905694150420947*fSkin[16]-0.7905694150420947*fEdge[16]+0.6123724356957944*(fSkin[6]+fEdge[6])+0.3535533905932737*fSkin[2]-0.3535533905932737*fEdge[2])*amax+0.223606797749979*alpha[3]*favg[7]+0.25*(alpha[4]*favg[6]+favg[4]*alpha[6])+0.223606797749979*alpha[2]*favg[5]+0.25*(alpha[1]*favg[3]+favg[1]*alpha[3]+alpha[0]*favg[2]+favg[0]*alpha[2]); 
  Ghat[3] = (0.7905694150420947*fSkin[19]-0.7905694150420947*fEdge[19]+0.6123724356957944*(fSkin[10]+fEdge[10])+0.3535533905932737*fSkin[4]-0.3535533905932737*fEdge[4])*amax+0.2*alpha[6]*favg[7]+0.223606797749979*(alpha[2]*favg[7]+alpha[1]*favg[6]+favg[1]*alpha[6]+alpha[3]*(favg[5]+favg[4])+favg[3]*alpha[4])+0.25*(alpha[0]*favg[3]+favg[0]*alpha[3]+alpha[1]*favg[2]+favg[1]*alpha[2]); 
  Ghat[4] = (0.6123724356957944*(fSkin[13]+fEdge[13])+0.3535533905932737*fSkin[7]-0.3535533905932737*fEdge[7])*amax+0.159719141249985*alpha[6]*favg[6]+0.25*(alpha[2]*favg[6]+favg[2]*alpha[6])+0.159719141249985*alpha[4]*favg[4]+0.25*(alpha[0]*favg[4]+favg[0]*alpha[4])+0.223606797749979*(alpha[3]*favg[3]+alpha[1]*favg[1]); 
  Ghat[5] = (0.6123724356957944*(fSkin[14]+fEdge[14])+0.3535533905932737*fSkin[8]-0.3535533905932737*fEdge[8])*amax+0.25*alpha[1]*favg[7]+0.223606797749979*alpha[6]*favg[6]+0.25*alpha[0]*favg[5]+0.223606797749979*(alpha[3]*favg[3]+alpha[2]*favg[2]); 
  Ghat[6] = (0.6123724356957944*(fSkin[17]+fEdge[17])+0.3535533905932737*fSkin[11]-0.3535533905932737*fEdge[11])*amax+0.2*alpha[3]*favg[7]+(0.159719141249985*alpha[4]+0.25*alpha[0])*favg[6]+(0.223606797749979*favg[5]+0.159719141249985*favg[4])*alpha[6]+0.25*(favg[0]*alpha[6]+alpha[2]*favg[4]+favg[2]*alpha[4])+0.223606797749979*(alpha[1]*favg[3]+favg[1]*alpha[3]); 
  Ghat[7] = (0.6123724356957944*(fSkin[18]+fEdge[18])+0.3535533905932737*fSkin[12]-0.3535533905932737*fEdge[12])*amax+(0.223606797749979*alpha[4]+0.25*alpha[0])*favg[7]+0.2*(alpha[3]*favg[6]+favg[3]*alpha[6])+0.25*alpha[1]*favg[5]+0.223606797749979*(alpha[2]*favg[3]+favg[2]*alpha[3]); 
