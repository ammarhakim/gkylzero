...
{ 
  ...
  if ((-0.4242640687119285*(alpha[13]+alpha[11]))+0.3162277660168379*alpha[7]+0.6363961030678926*(alpha[5]+alpha[4])-0.4743416490252568*(alpha[3]+alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[0] = tensor_1x3v_p2_surfvx_quad_0(1, fSkin); 
  } else { 

    fUpwindQuad[0] = tensor_1x3v_p2_surfvx_quad_0(-1, fEdge); 
  } 
  if (0.5303300858899104*(alpha[13]+alpha[11])-0.3952847075210473*alpha[7]-0.4743416490252568*(alpha[3]+alpha[2])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[1] = tensor_1x3v_p2_surfvx_quad_1(1, fSkin); 
  } else { 

    fUpwindQuad[1] = tensor_1x3v_p2_surfvx_quad_1(-1, fEdge); 
  } 
  if ((-0.4242640687119285*(alpha[13]+alpha[11]))+0.3162277660168379*alpha[7]-0.6363961030678926*(alpha[5]+alpha[4])-0.4743416490252568*(alpha[3]+alpha[2])+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[2] = tensor_1x3v_p2_surfvx_quad_2(1, fSkin); 
  } else { 

    fUpwindQuad[2] = tensor_1x3v_p2_surfvx_quad_2(-1, fEdge); 
  } 
  if ((-0.4242640687119285*alpha[13])+0.3162277660168379*alpha[7]+0.6363961030678926*alpha[5]-0.4743416490252568*(alpha[3]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[3] = tensor_1x3v_p2_surfvx_quad_3(1, fSkin); 
  } else { 

    fUpwindQuad[3] = tensor_1x3v_p2_surfvx_quad_3(-1, fEdge); 
  } 
  if (0.5303300858899104*alpha[13]-0.3952847075210473*alpha[7]-0.4743416490252568*alpha[3]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[4] = tensor_1x3v_p2_surfvx_quad_4(1, fSkin); 
  } else { 

    fUpwindQuad[4] = tensor_1x3v_p2_surfvx_quad_4(-1, fEdge); 
  } 
  if ((-0.4242640687119285*alpha[13])+0.3162277660168379*alpha[7]-0.6363961030678926*alpha[5]-0.4743416490252568*alpha[3]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[5] = tensor_1x3v_p2_surfvx_quad_5(1, fSkin); 
  } else { 

    fUpwindQuad[5] = tensor_1x3v_p2_surfvx_quad_5(-1, fEdge); 
  } 
  if ((-0.4242640687119285*alpha[13])+0.4242640687119285*alpha[11]+0.3162277660168379*alpha[7]+0.6363961030678926*alpha[5]-0.6363961030678926*alpha[4]-0.4743416490252568*alpha[3]+0.4743416490252568*alpha[2]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[6] = tensor_1x3v_p2_surfvx_quad_6(1, fSkin); 
  } else { 

    fUpwindQuad[6] = tensor_1x3v_p2_surfvx_quad_6(-1, fEdge); 
  } 
  if (0.5303300858899104*alpha[13]-0.5303300858899104*alpha[11]-0.3952847075210473*alpha[7]-0.4743416490252568*alpha[3]+0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[7] = tensor_1x3v_p2_surfvx_quad_7(1, fSkin); 
  } else { 

    fUpwindQuad[7] = tensor_1x3v_p2_surfvx_quad_7(-1, fEdge); 
  } 
  if ((-0.4242640687119285*alpha[13])+0.4242640687119285*alpha[11]+0.3162277660168379*alpha[7]-0.6363961030678926*alpha[5]+0.6363961030678926*alpha[4]-0.4743416490252568*alpha[3]+0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[8] = tensor_1x3v_p2_surfvx_quad_8(1, fSkin); 
  } else { 

    fUpwindQuad[8] = tensor_1x3v_p2_surfvx_quad_8(-1, fEdge); 
  } 
  if ((-0.4242640687119285*alpha[11])+0.3162277660168379*alpha[7]+0.6363961030678926*alpha[4]-0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[9] = tensor_1x3v_p2_surfvx_quad_9(1, fSkin); 
  } else { 

    fUpwindQuad[9] = tensor_1x3v_p2_surfvx_quad_9(-1, fEdge); 
  } 
  if (0.5303300858899104*alpha[11]-0.3952847075210473*alpha[7]-0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[10] = tensor_1x3v_p2_surfvx_quad_10(1, fSkin); 
  } else { 

    fUpwindQuad[10] = tensor_1x3v_p2_surfvx_quad_10(-1, fEdge); 
  } 
  if ((-0.4242640687119285*alpha[11])+0.3162277660168379*alpha[7]-0.6363961030678926*alpha[4]-0.4743416490252568*alpha[2]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[11] = tensor_1x3v_p2_surfvx_quad_11(1, fSkin); 
  } else { 

    fUpwindQuad[11] = tensor_1x3v_p2_surfvx_quad_11(-1, fEdge); 
  } 
  if (0.3162277660168379*alpha[7]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[12] = tensor_1x3v_p2_surfvx_quad_12(1, fSkin); 
  } else { 

    fUpwindQuad[12] = tensor_1x3v_p2_surfvx_quad_12(-1, fEdge); 
  } 
  if (0.3535533905932737*alpha[0]-0.3952847075210473*alpha[7] > 0) { 

    fUpwindQuad[13] = tensor_1x3v_p2_surfvx_quad_13(1, fSkin); 
  } else { 

    fUpwindQuad[13] = tensor_1x3v_p2_surfvx_quad_13(-1, fEdge); 
  } 
  if (0.3162277660168379*alpha[7]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[14] = tensor_1x3v_p2_surfvx_quad_14(1, fSkin); 
  } else { 

    fUpwindQuad[14] = tensor_1x3v_p2_surfvx_quad_14(-1, fEdge); 
  } 
  if (0.4242640687119285*alpha[11]+0.3162277660168379*alpha[7]-0.6363961030678926*alpha[4]+0.4743416490252568*alpha[2]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[15] = tensor_1x3v_p2_surfvx_quad_15(1, fSkin); 
  } else { 

    fUpwindQuad[15] = tensor_1x3v_p2_surfvx_quad_15(-1, fEdge); 
  } 
  if ((-0.5303300858899104*alpha[11])-0.3952847075210473*alpha[7]+0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[16] = tensor_1x3v_p2_surfvx_quad_16(1, fSkin); 
  } else { 

    fUpwindQuad[16] = tensor_1x3v_p2_surfvx_quad_16(-1, fEdge); 
  } 
  if (0.4242640687119285*alpha[11]+0.3162277660168379*alpha[7]+0.6363961030678926*alpha[4]+0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[17] = tensor_1x3v_p2_surfvx_quad_17(1, fSkin); 
  } else { 

    fUpwindQuad[17] = tensor_1x3v_p2_surfvx_quad_17(-1, fEdge); 
  } 
  if (0.4242640687119285*alpha[13]-0.4242640687119285*alpha[11]+0.3162277660168379*alpha[7]-0.6363961030678926*alpha[5]+0.6363961030678926*alpha[4]+0.4743416490252568*alpha[3]-0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[18] = tensor_1x3v_p2_surfvx_quad_18(1, fSkin); 
  } else { 

    fUpwindQuad[18] = tensor_1x3v_p2_surfvx_quad_18(-1, fEdge); 
  } 
  if ((-0.5303300858899104*alpha[13])+0.5303300858899104*alpha[11]-0.3952847075210473*alpha[7]+0.4743416490252568*alpha[3]-0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[19] = tensor_1x3v_p2_surfvx_quad_19(1, fSkin); 
  } else { 

    fUpwindQuad[19] = tensor_1x3v_p2_surfvx_quad_19(-1, fEdge); 
  } 
  if (0.4242640687119285*alpha[13]-0.4242640687119285*alpha[11]+0.3162277660168379*alpha[7]+0.6363961030678926*alpha[5]-0.6363961030678926*alpha[4]+0.4743416490252568*alpha[3]-0.4743416490252568*alpha[2]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[20] = tensor_1x3v_p2_surfvx_quad_20(1, fSkin); 
  } else { 

    fUpwindQuad[20] = tensor_1x3v_p2_surfvx_quad_20(-1, fEdge); 
  } 
  if (0.4242640687119285*alpha[13]+0.3162277660168379*alpha[7]-0.6363961030678926*alpha[5]+0.4743416490252568*alpha[3]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[21] = tensor_1x3v_p2_surfvx_quad_21(1, fSkin); 
  } else { 

    fUpwindQuad[21] = tensor_1x3v_p2_surfvx_quad_21(-1, fEdge); 
  } 
  if ((-0.5303300858899104*alpha[13])-0.3952847075210473*alpha[7]+0.4743416490252568*alpha[3]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[22] = tensor_1x3v_p2_surfvx_quad_22(1, fSkin); 
  } else { 

    fUpwindQuad[22] = tensor_1x3v_p2_surfvx_quad_22(-1, fEdge); 
  } 
  if (0.4242640687119285*alpha[13]+0.3162277660168379*alpha[7]+0.6363961030678926*alpha[5]+0.4743416490252568*(alpha[3]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[23] = tensor_1x3v_p2_surfvx_quad_23(1, fSkin); 
  } else { 

    fUpwindQuad[23] = tensor_1x3v_p2_surfvx_quad_23(-1, fEdge); 
  } 
  if (0.4242640687119285*(alpha[13]+alpha[11])+0.3162277660168379*alpha[7]-0.6363961030678926*(alpha[5]+alpha[4])+0.4743416490252568*(alpha[3]+alpha[2])-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[24] = tensor_1x3v_p2_surfvx_quad_24(1, fSkin); 
  } else { 

    fUpwindQuad[24] = tensor_1x3v_p2_surfvx_quad_24(-1, fEdge); 
  } 
  if ((-0.5303300858899104*(alpha[13]+alpha[11]))-0.3952847075210473*alpha[7]+0.4743416490252568*(alpha[3]+alpha[2])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[25] = tensor_1x3v_p2_surfvx_quad_25(1, fSkin); 
  } else { 

    fUpwindQuad[25] = tensor_1x3v_p2_surfvx_quad_25(-1, fEdge); 
  } 
  if (0.4242640687119285*(alpha[13]+alpha[11])+0.3162277660168379*alpha[7]+0.6363961030678926*(alpha[5]+alpha[4])+0.4743416490252568*(alpha[3]+alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[26] = tensor_1x3v_p2_surfvx_quad_26(1, fSkin); 
  } else { 

    fUpwindQuad[26] = tensor_1x3v_p2_surfvx_quad_26(-1, fEdge); 
  } 
