...
#include <gkyl_basis_tensor_1x3v_p2_surfvx_quad.h> 
...
{ 
  ...
  if (0.6363961030678926*(alpha[5]+alpha[4])-0.4743416490252568*(alpha[3]+alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad_l[0] = tensor_1x3v_p2_surfvx_quad_0(1, fl); 
    fUpwindQuad_r[0] = tensor_1x3v_p2_surfvx_quad_0(1, fc); 
  } else { 

    fUpwindQuad_l[0] = tensor_1x3v_p2_surfvx_quad_0(-1, fc); 
    fUpwindQuad_r[0] = tensor_1x3v_p2_surfvx_quad_0(-1, fr); 
  } 
  if (0.3535533905932737*alpha[0]-0.4743416490252568*(alpha[3]+alpha[2]) > 0) { 

    fUpwindQuad_l[1] = tensor_1x3v_p2_surfvx_quad_1(1, fl); 
    fUpwindQuad_r[1] = tensor_1x3v_p2_surfvx_quad_1(1, fc); 
  } else { 

    fUpwindQuad_l[1] = tensor_1x3v_p2_surfvx_quad_1(-1, fc); 
    fUpwindQuad_r[1] = tensor_1x3v_p2_surfvx_quad_1(-1, fr); 
  } 
  if ((-0.6363961030678926*(alpha[5]+alpha[4]))-0.4743416490252568*(alpha[3]+alpha[2])+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad_l[2] = tensor_1x3v_p2_surfvx_quad_2(1, fl); 
    fUpwindQuad_r[2] = tensor_1x3v_p2_surfvx_quad_2(1, fc); 
  } else { 

    fUpwindQuad_l[2] = tensor_1x3v_p2_surfvx_quad_2(-1, fc); 
    fUpwindQuad_r[2] = tensor_1x3v_p2_surfvx_quad_2(-1, fr); 
  } 
  if (0.6363961030678926*alpha[5]-0.4743416490252568*(alpha[3]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[3] = tensor_1x3v_p2_surfvx_quad_3(1, fl); 
    fUpwindQuad_r[3] = tensor_1x3v_p2_surfvx_quad_3(1, fc); 
  } else { 

    fUpwindQuad_l[3] = tensor_1x3v_p2_surfvx_quad_3(-1, fc); 
    fUpwindQuad_r[3] = tensor_1x3v_p2_surfvx_quad_3(-1, fr); 
  } 
  if (0.3535533905932737*alpha[0]-0.4743416490252568*alpha[3] > 0) { 

    fUpwindQuad_l[4] = tensor_1x3v_p2_surfvx_quad_4(1, fl); 
    fUpwindQuad_r[4] = tensor_1x3v_p2_surfvx_quad_4(1, fc); 
  } else { 

    fUpwindQuad_l[4] = tensor_1x3v_p2_surfvx_quad_4(-1, fc); 
    fUpwindQuad_r[4] = tensor_1x3v_p2_surfvx_quad_4(-1, fr); 
  } 
  if ((-0.6363961030678926*alpha[5])-0.4743416490252568*alpha[3]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[5] = tensor_1x3v_p2_surfvx_quad_5(1, fl); 
    fUpwindQuad_r[5] = tensor_1x3v_p2_surfvx_quad_5(1, fc); 
  } else { 

    fUpwindQuad_l[5] = tensor_1x3v_p2_surfvx_quad_5(-1, fc); 
    fUpwindQuad_r[5] = tensor_1x3v_p2_surfvx_quad_5(-1, fr); 
  } 
  if (0.6363961030678926*alpha[5]-0.6363961030678926*alpha[4]-0.4743416490252568*alpha[3]+0.4743416490252568*alpha[2]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad_l[6] = tensor_1x3v_p2_surfvx_quad_6(1, fl); 
    fUpwindQuad_r[6] = tensor_1x3v_p2_surfvx_quad_6(1, fc); 
  } else { 

    fUpwindQuad_l[6] = tensor_1x3v_p2_surfvx_quad_6(-1, fc); 
    fUpwindQuad_r[6] = tensor_1x3v_p2_surfvx_quad_6(-1, fr); 
  } 
  if ((-0.4743416490252568*alpha[3])+0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[7] = tensor_1x3v_p2_surfvx_quad_7(1, fl); 
    fUpwindQuad_r[7] = tensor_1x3v_p2_surfvx_quad_7(1, fc); 
  } else { 

    fUpwindQuad_l[7] = tensor_1x3v_p2_surfvx_quad_7(-1, fc); 
    fUpwindQuad_r[7] = tensor_1x3v_p2_surfvx_quad_7(-1, fr); 
  } 
  if ((-0.6363961030678926*alpha[5])+0.6363961030678926*alpha[4]-0.4743416490252568*alpha[3]+0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad_l[8] = tensor_1x3v_p2_surfvx_quad_8(1, fl); 
    fUpwindQuad_r[8] = tensor_1x3v_p2_surfvx_quad_8(1, fc); 
  } else { 

    fUpwindQuad_l[8] = tensor_1x3v_p2_surfvx_quad_8(-1, fc); 
    fUpwindQuad_r[8] = tensor_1x3v_p2_surfvx_quad_8(-1, fr); 
  } 
  if (0.6363961030678926*alpha[4]-0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[9] = tensor_1x3v_p2_surfvx_quad_9(1, fl); 
    fUpwindQuad_r[9] = tensor_1x3v_p2_surfvx_quad_9(1, fc); 
  } else { 

    fUpwindQuad_l[9] = tensor_1x3v_p2_surfvx_quad_9(-1, fc); 
    fUpwindQuad_r[9] = tensor_1x3v_p2_surfvx_quad_9(-1, fr); 
  } 
  if (0.3535533905932737*alpha[0]-0.4743416490252568*alpha[2] > 0) { 

    fUpwindQuad_l[10] = tensor_1x3v_p2_surfvx_quad_10(1, fl); 
    fUpwindQuad_r[10] = tensor_1x3v_p2_surfvx_quad_10(1, fc); 
  } else { 

    fUpwindQuad_l[10] = tensor_1x3v_p2_surfvx_quad_10(-1, fc); 
    fUpwindQuad_r[10] = tensor_1x3v_p2_surfvx_quad_10(-1, fr); 
  } 
  if ((-0.6363961030678926*alpha[4])-0.4743416490252568*alpha[2]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[11] = tensor_1x3v_p2_surfvx_quad_11(1, fl); 
    fUpwindQuad_r[11] = tensor_1x3v_p2_surfvx_quad_11(1, fc); 
  } else { 

    fUpwindQuad_l[11] = tensor_1x3v_p2_surfvx_quad_11(-1, fc); 
    fUpwindQuad_r[11] = tensor_1x3v_p2_surfvx_quad_11(-1, fr); 
  } 
  if (0.3535533905932737*alpha[0]-0.4743416490252568*alpha[1] > 0) { 

    fUpwindQuad_l[12] = tensor_1x3v_p2_surfvx_quad_12(1, fl); 
    fUpwindQuad_r[12] = tensor_1x3v_p2_surfvx_quad_12(1, fc); 
  } else { 

    fUpwindQuad_l[12] = tensor_1x3v_p2_surfvx_quad_12(-1, fc); 
    fUpwindQuad_r[12] = tensor_1x3v_p2_surfvx_quad_12(-1, fr); 
  } 
  if (0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[13] = tensor_1x3v_p2_surfvx_quad_13(1, fl); 
    fUpwindQuad_r[13] = tensor_1x3v_p2_surfvx_quad_13(1, fc); 
  } else { 

    fUpwindQuad_l[13] = tensor_1x3v_p2_surfvx_quad_13(-1, fc); 
    fUpwindQuad_r[13] = tensor_1x3v_p2_surfvx_quad_13(-1, fr); 
  } 
  if (0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[14] = tensor_1x3v_p2_surfvx_quad_14(1, fl); 
    fUpwindQuad_r[14] = tensor_1x3v_p2_surfvx_quad_14(1, fc); 
  } else { 

    fUpwindQuad_l[14] = tensor_1x3v_p2_surfvx_quad_14(-1, fc); 
    fUpwindQuad_r[14] = tensor_1x3v_p2_surfvx_quad_14(-1, fr); 
  } 
  if ((-0.6363961030678926*alpha[4])+0.4743416490252568*alpha[2]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[15] = tensor_1x3v_p2_surfvx_quad_15(1, fl); 
    fUpwindQuad_r[15] = tensor_1x3v_p2_surfvx_quad_15(1, fc); 
  } else { 

    fUpwindQuad_l[15] = tensor_1x3v_p2_surfvx_quad_15(-1, fc); 
    fUpwindQuad_r[15] = tensor_1x3v_p2_surfvx_quad_15(-1, fr); 
  } 
  if (0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[16] = tensor_1x3v_p2_surfvx_quad_16(1, fl); 
    fUpwindQuad_r[16] = tensor_1x3v_p2_surfvx_quad_16(1, fc); 
  } else { 

    fUpwindQuad_l[16] = tensor_1x3v_p2_surfvx_quad_16(-1, fc); 
    fUpwindQuad_r[16] = tensor_1x3v_p2_surfvx_quad_16(-1, fr); 
  } 
  if (0.6363961030678926*alpha[4]+0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[17] = tensor_1x3v_p2_surfvx_quad_17(1, fl); 
    fUpwindQuad_r[17] = tensor_1x3v_p2_surfvx_quad_17(1, fc); 
  } else { 

    fUpwindQuad_l[17] = tensor_1x3v_p2_surfvx_quad_17(-1, fc); 
    fUpwindQuad_r[17] = tensor_1x3v_p2_surfvx_quad_17(-1, fr); 
  } 
  if ((-0.6363961030678926*alpha[5])+0.6363961030678926*alpha[4]+0.4743416490252568*alpha[3]-0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[18] = tensor_1x3v_p2_surfvx_quad_18(1, fl); 
    fUpwindQuad_r[18] = tensor_1x3v_p2_surfvx_quad_18(1, fc); 
  } else { 

    fUpwindQuad_l[18] = tensor_1x3v_p2_surfvx_quad_18(-1, fc); 
    fUpwindQuad_r[18] = tensor_1x3v_p2_surfvx_quad_18(-1, fr); 
  } 
  if (0.4743416490252568*alpha[3]-0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[19] = tensor_1x3v_p2_surfvx_quad_19(1, fl); 
    fUpwindQuad_r[19] = tensor_1x3v_p2_surfvx_quad_19(1, fc); 
  } else { 

    fUpwindQuad_l[19] = tensor_1x3v_p2_surfvx_quad_19(-1, fc); 
    fUpwindQuad_r[19] = tensor_1x3v_p2_surfvx_quad_19(-1, fr); 
  } 
  if (0.6363961030678926*alpha[5]-0.6363961030678926*alpha[4]+0.4743416490252568*alpha[3]-0.4743416490252568*alpha[2]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[20] = tensor_1x3v_p2_surfvx_quad_20(1, fl); 
    fUpwindQuad_r[20] = tensor_1x3v_p2_surfvx_quad_20(1, fc); 
  } else { 

    fUpwindQuad_l[20] = tensor_1x3v_p2_surfvx_quad_20(-1, fc); 
    fUpwindQuad_r[20] = tensor_1x3v_p2_surfvx_quad_20(-1, fr); 
  } 
  if ((-0.6363961030678926*alpha[5])+0.4743416490252568*alpha[3]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[21] = tensor_1x3v_p2_surfvx_quad_21(1, fl); 
    fUpwindQuad_r[21] = tensor_1x3v_p2_surfvx_quad_21(1, fc); 
  } else { 

    fUpwindQuad_l[21] = tensor_1x3v_p2_surfvx_quad_21(-1, fc); 
    fUpwindQuad_r[21] = tensor_1x3v_p2_surfvx_quad_21(-1, fr); 
  } 
  if (0.4743416490252568*alpha[3]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[22] = tensor_1x3v_p2_surfvx_quad_22(1, fl); 
    fUpwindQuad_r[22] = tensor_1x3v_p2_surfvx_quad_22(1, fc); 
  } else { 

    fUpwindQuad_l[22] = tensor_1x3v_p2_surfvx_quad_22(-1, fc); 
    fUpwindQuad_r[22] = tensor_1x3v_p2_surfvx_quad_22(-1, fr); 
  } 
  if (0.6363961030678926*alpha[5]+0.4743416490252568*(alpha[3]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[23] = tensor_1x3v_p2_surfvx_quad_23(1, fl); 
    fUpwindQuad_r[23] = tensor_1x3v_p2_surfvx_quad_23(1, fc); 
  } else { 

    fUpwindQuad_l[23] = tensor_1x3v_p2_surfvx_quad_23(-1, fc); 
    fUpwindQuad_r[23] = tensor_1x3v_p2_surfvx_quad_23(-1, fr); 
  } 
  if ((-0.6363961030678926*(alpha[5]+alpha[4]))+0.4743416490252568*(alpha[3]+alpha[2])-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[24] = tensor_1x3v_p2_surfvx_quad_24(1, fl); 
    fUpwindQuad_r[24] = tensor_1x3v_p2_surfvx_quad_24(1, fc); 
  } else { 

    fUpwindQuad_l[24] = tensor_1x3v_p2_surfvx_quad_24(-1, fc); 
    fUpwindQuad_r[24] = tensor_1x3v_p2_surfvx_quad_24(-1, fr); 
  } 
  if (0.4743416490252568*(alpha[3]+alpha[2])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[25] = tensor_1x3v_p2_surfvx_quad_25(1, fl); 
    fUpwindQuad_r[25] = tensor_1x3v_p2_surfvx_quad_25(1, fc); 
  } else { 

    fUpwindQuad_l[25] = tensor_1x3v_p2_surfvx_quad_25(-1, fc); 
    fUpwindQuad_r[25] = tensor_1x3v_p2_surfvx_quad_25(-1, fr); 
  } 
  if (0.6363961030678926*(alpha[5]+alpha[4])+0.4743416490252568*(alpha[3]+alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad_l[26] = tensor_1x3v_p2_surfvx_quad_26(1, fl); 
    fUpwindQuad_r[26] = tensor_1x3v_p2_surfvx_quad_26(1, fc); 
  } else { 

    fUpwindQuad_l[26] = tensor_1x3v_p2_surfvx_quad_26(-1, fc); 
    fUpwindQuad_r[26] = tensor_1x3v_p2_surfvx_quad_26(-1, fr); 
  } 
  ...
  Ghat_l[1] += 0.3162277660168379*(alpha[5]*fUpwind_l[13]+alpha[4]*fUpwind_l[11])+0.3162277660168379*alpha[1]*fUpwind_l[7]+0.3535533905932737*(alpha[3]*fUpwind_l[5]+fUpwind_l[3]*alpha[5]+alpha[2]*fUpwind_l[4]+fUpwind_l[2]*alpha[4]+alpha[0]*fUpwind_l[1]+fUpwind_l[0]*alpha[1]); 
  Ghat_l[2] += 0.3162277660168379*alpha[4]*fUpwind_l[12]+0.3535533905932737*alpha[5]*fUpwind_l[10]+0.3162277660168379*alpha[2]*fUpwind_l[8]+0.3535533905932737*(alpha[3]*fUpwind_l[6]+alpha[1]*fUpwind_l[4]+fUpwind_l[1]*alpha[4]+alpha[0]*fUpwind_l[2]+fUpwind_l[0]*alpha[2]); 
  Ghat_l[3] += 0.3162277660168379*alpha[5]*fUpwind_l[15]+0.3535533905932737*alpha[4]*fUpwind_l[10]+0.3162277660168379*alpha[3]*fUpwind_l[9]+0.3535533905932737*(alpha[2]*fUpwind_l[6]+alpha[1]*fUpwind_l[5]+fUpwind_l[1]*alpha[5]+alpha[0]*fUpwind_l[3]+fUpwind_l[0]*alpha[3]); 
  Ghat_l[4] += 0.2828427124746191*alpha[4]*fUpwind_l[20]+0.3162277660168379*alpha[5]*fUpwind_l[17]+0.3162277660168379*(alpha[2]*fUpwind_l[12]+alpha[1]*fUpwind_l[11])+0.3535533905932737*alpha[3]*fUpwind_l[10]+0.3162277660168379*alpha[4]*(fUpwind_l[8]+fUpwind_l[7])+0.3535533905932737*(alpha[5]*fUpwind_l[6]+alpha[0]*fUpwind_l[4]+fUpwind_l[0]*alpha[4]+alpha[1]*fUpwind_l[2]+fUpwind_l[1]*alpha[2]); 
  Ghat_l[5] += 0.2828427124746191*alpha[5]*fUpwind_l[21]+0.3162277660168379*alpha[4]*fUpwind_l[17]+0.3162277660168379*(alpha[3]*fUpwind_l[15]+alpha[1]*fUpwind_l[13])+0.3535533905932737*alpha[2]*fUpwind_l[10]+0.3162277660168379*alpha[5]*(fUpwind_l[9]+fUpwind_l[7])+0.3535533905932737*(alpha[4]*fUpwind_l[6]+alpha[0]*fUpwind_l[5]+fUpwind_l[0]*alpha[5]+alpha[1]*fUpwind_l[3]+fUpwind_l[1]*alpha[3]); 
  Ghat_l[6] += 0.3162277660168379*(alpha[5]*fUpwind_l[19]+alpha[4]*fUpwind_l[18])+0.3162277660168379*(alpha[3]*fUpwind_l[16]+alpha[2]*fUpwind_l[14])+0.3535533905932737*(alpha[1]*fUpwind_l[10]+alpha[0]*fUpwind_l[6]+alpha[4]*fUpwind_l[5]+fUpwind_l[4]*alpha[5]+alpha[2]*fUpwind_l[3]+fUpwind_l[2]*alpha[3]); 
  Ghat_l[7] += 0.3535533905932737*(alpha[3]*fUpwind_l[13]+alpha[2]*fUpwind_l[11]+alpha[0]*fUpwind_l[7])+0.3162277660168379*(alpha[5]*fUpwind_l[5]+alpha[4]*fUpwind_l[4]+alpha[1]*fUpwind_l[1]); 
  Ghat_l[8] += 0.3535533905932737*(alpha[5]*fUpwind_l[18]+alpha[3]*fUpwind_l[14]+alpha[1]*fUpwind_l[12]+alpha[0]*fUpwind_l[8])+0.3162277660168379*(alpha[4]*fUpwind_l[4]+alpha[2]*fUpwind_l[2]); 
  Ghat_l[9] += 0.3535533905932737*(alpha[4]*fUpwind_l[19]+alpha[2]*fUpwind_l[16]+alpha[1]*fUpwind_l[15]+alpha[0]*fUpwind_l[9])+0.3162277660168379*(alpha[5]*fUpwind_l[5]+alpha[3]*fUpwind_l[3]); 
  Ghat_l[10] += 0.2828427124746191*(alpha[5]*fUpwind_l[24]+alpha[4]*fUpwind_l[23])+0.3162277660168379*(alpha[3]*fUpwind_l[19]+alpha[2]*fUpwind_l[18]+alpha[1]*fUpwind_l[17])+0.3162277660168379*(alpha[5]*fUpwind_l[16]+alpha[4]*(fUpwind_l[14]+fUpwind_l[13])+alpha[5]*fUpwind_l[11])+0.3535533905932737*(alpha[0]*fUpwind_l[10]+alpha[1]*fUpwind_l[6]+alpha[2]*fUpwind_l[5]+fUpwind_l[2]*alpha[5]+alpha[3]*fUpwind_l[4]+fUpwind_l[3]*alpha[4]); 
  Ghat_l[11] += 0.3162277660168379*alpha[2]*fUpwind_l[20]+0.3535533905932737*alpha[3]*fUpwind_l[17]+0.2828427124746191*alpha[4]*fUpwind_l[12]+0.3535533905932737*alpha[0]*fUpwind_l[11]+0.3162277660168379*alpha[5]*fUpwind_l[10]+0.3535533905932737*alpha[2]*fUpwind_l[7]+0.3162277660168379*(alpha[1]*fUpwind_l[4]+fUpwind_l[1]*alpha[4]); 
  Ghat_l[12] += 0.3162277660168379*(alpha[5]*fUpwind_l[23]+alpha[1]*fUpwind_l[20])+0.3535533905932737*(alpha[3]*fUpwind_l[18]+alpha[5]*fUpwind_l[14]+alpha[0]*fUpwind_l[12])+0.2828427124746191*alpha[4]*fUpwind_l[11]+0.3535533905932737*alpha[1]*fUpwind_l[8]+0.3162277660168379*(alpha[2]*fUpwind_l[4]+fUpwind_l[2]*alpha[4]); 
  Ghat_l[13] += 0.3162277660168379*alpha[3]*fUpwind_l[21]+0.3535533905932737*alpha[2]*fUpwind_l[17]+0.2828427124746191*alpha[5]*fUpwind_l[15]+0.3535533905932737*alpha[0]*fUpwind_l[13]+0.3162277660168379*alpha[4]*fUpwind_l[10]+0.3535533905932737*alpha[3]*fUpwind_l[7]+0.3162277660168379*(alpha[1]*fUpwind_l[5]+fUpwind_l[1]*alpha[5]); 
  Ghat_l[14] += 0.3162277660168379*(alpha[5]*fUpwind_l[25]+alpha[3]*fUpwind_l[22])+0.3535533905932737*(alpha[1]*fUpwind_l[18]+alpha[0]*fUpwind_l[14]+alpha[5]*fUpwind_l[12])+0.3162277660168379*alpha[4]*fUpwind_l[10]+0.3535533905932737*alpha[3]*fUpwind_l[8]+0.3162277660168379*alpha[2]*fUpwind_l[6]; 
  Ghat_l[15] += 0.3162277660168379*(alpha[4]*fUpwind_l[24]+alpha[1]*fUpwind_l[21])+0.3535533905932737*(alpha[2]*fUpwind_l[19]+alpha[4]*fUpwind_l[16]+alpha[0]*fUpwind_l[15])+0.2828427124746191*alpha[5]*fUpwind_l[13]+0.3535533905932737*alpha[1]*fUpwind_l[9]+0.3162277660168379*(alpha[3]*fUpwind_l[5]+fUpwind_l[3]*alpha[5]); 
  Ghat_l[16] += 0.3162277660168379*(alpha[4]*fUpwind_l[25]+alpha[2]*fUpwind_l[22])+0.3535533905932737*(alpha[1]*fUpwind_l[19]+alpha[0]*fUpwind_l[16]+alpha[4]*fUpwind_l[15])+0.3162277660168379*alpha[5]*fUpwind_l[10]+0.3535533905932737*alpha[2]*fUpwind_l[9]+0.3162277660168379*alpha[3]*fUpwind_l[6]; 
  Ghat_l[17] += 0.3162277660168379*(alpha[3]*fUpwind_l[24]+alpha[2]*fUpwind_l[23])+0.2828427124746191*(alpha[5]*fUpwind_l[19]+alpha[4]*fUpwind_l[18])+0.3535533905932737*(alpha[0]*fUpwind_l[17]+alpha[2]*fUpwind_l[13]+alpha[3]*fUpwind_l[11])+0.3162277660168379*(alpha[1]*fUpwind_l[10]+alpha[4]*fUpwind_l[5]+fUpwind_l[4]*alpha[5]); 
  Ghat_l[18] += 0.2828427124746191*alpha[5]*fUpwind_l[26]+0.3162277660168379*(alpha[3]*fUpwind_l[25]+alpha[1]*fUpwind_l[23]+alpha[5]*(fUpwind_l[22]+fUpwind_l[20]))+0.3535533905932737*alpha[0]*fUpwind_l[18]+0.2828427124746191*alpha[4]*fUpwind_l[17]+0.3535533905932737*(alpha[1]*fUpwind_l[14]+alpha[3]*fUpwind_l[12])+0.3162277660168379*alpha[2]*fUpwind_l[10]+0.3535533905932737*alpha[5]*fUpwind_l[8]+0.3162277660168379*alpha[4]*fUpwind_l[6]; 
  Ghat_l[19] += 0.2828427124746191*alpha[4]*fUpwind_l[26]+0.3162277660168379*(alpha[2]*fUpwind_l[25]+alpha[1]*fUpwind_l[24]+alpha[4]*(fUpwind_l[22]+fUpwind_l[21]))+0.3535533905932737*alpha[0]*fUpwind_l[19]+0.2828427124746191*alpha[5]*fUpwind_l[17]+0.3535533905932737*(alpha[1]*fUpwind_l[16]+alpha[2]*fUpwind_l[15])+0.3162277660168379*alpha[3]*fUpwind_l[10]+0.3535533905932737*alpha[4]*fUpwind_l[9]+0.3162277660168379*alpha[5]*fUpwind_l[6]; 
  Ghat_l[20] += 0.3535533905932737*(alpha[3]*fUpwind_l[23]+alpha[0]*fUpwind_l[20])+0.3162277660168379*alpha[5]*fUpwind_l[18]+0.3162277660168379*(alpha[1]*fUpwind_l[12]+alpha[2]*fUpwind_l[11])+0.2828427124746191*alpha[4]*fUpwind_l[4]; 
  Ghat_l[21] += 0.3535533905932737*(alpha[2]*fUpwind_l[24]+alpha[0]*fUpwind_l[21])+0.3162277660168379*alpha[4]*fUpwind_l[19]+0.3162277660168379*(alpha[1]*fUpwind_l[15]+alpha[3]*fUpwind_l[13])+0.2828427124746191*alpha[5]*fUpwind_l[5]; 
  Ghat_l[22] += 0.3535533905932737*(alpha[1]*fUpwind_l[25]+alpha[0]*fUpwind_l[22])+0.3162277660168379*(alpha[4]*fUpwind_l[19]+alpha[5]*fUpwind_l[18])+0.3162277660168379*(alpha[2]*fUpwind_l[16]+alpha[3]*fUpwind_l[14]); 
  Ghat_l[23] += 0.3162277660168379*alpha[3]*fUpwind_l[26]+0.2828427124746191*alpha[5]*fUpwind_l[25]+0.3535533905932737*(alpha[0]*fUpwind_l[23]+alpha[3]*fUpwind_l[20])+0.3162277660168379*(alpha[1]*fUpwind_l[18]+alpha[2]*fUpwind_l[17])+0.3162277660168379*alpha[5]*fUpwind_l[12]+0.2828427124746191*alpha[4]*fUpwind_l[10]; 
  Ghat_l[24] += 0.3162277660168379*alpha[2]*fUpwind_l[26]+0.2828427124746191*alpha[4]*fUpwind_l[25]+0.3535533905932737*(alpha[0]*fUpwind_l[24]+alpha[2]*fUpwind_l[21])+0.3162277660168379*(alpha[1]*fUpwind_l[19]+alpha[3]*fUpwind_l[17])+0.3162277660168379*alpha[4]*fUpwind_l[15]+0.2828427124746191*alpha[5]*fUpwind_l[10]; 
  Ghat_l[25] += 0.3162277660168379*alpha[1]*fUpwind_l[26]+0.3535533905932737*alpha[0]*fUpwind_l[25]+0.2828427124746191*(alpha[4]*fUpwind_l[24]+alpha[5]*fUpwind_l[23])+0.3535533905932737*alpha[1]*fUpwind_l[22]+0.3162277660168379*(alpha[2]*fUpwind_l[19]+alpha[3]*fUpwind_l[18])+0.3162277660168379*(alpha[4]*fUpwind_l[16]+alpha[5]*fUpwind_l[14]); 
  Ghat_l[26] += 0.3535533905932737*alpha[0]*fUpwind_l[26]+0.3162277660168379*(alpha[1]*fUpwind_l[25]+alpha[2]*fUpwind_l[24]+alpha[3]*fUpwind_l[23])+0.2828427124746191*(alpha[4]*fUpwind_l[19]+alpha[5]*fUpwind_l[18]); 
  ...
  Ghat_r[1] += 0.3162277660168379*(alpha[5]*fUpwind_r[13]+alpha[4]*fUpwind_r[11])+0.3162277660168379*alpha[1]*fUpwind_r[7]+0.3535533905932737*(alpha[3]*fUpwind_r[5]+fUpwind_r[3]*alpha[5]+alpha[2]*fUpwind_r[4]+fUpwind_r[2]*alpha[4]+alpha[0]*fUpwind_r[1]+fUpwind_r[0]*alpha[1]); 
  Ghat_r[2] += 0.3162277660168379*alpha[4]*fUpwind_r[12]+0.3535533905932737*alpha[5]*fUpwind_r[10]+0.3162277660168379*alpha[2]*fUpwind_r[8]+0.3535533905932737*(alpha[3]*fUpwind_r[6]+alpha[1]*fUpwind_r[4]+fUpwind_r[1]*alpha[4]+alpha[0]*fUpwind_r[2]+fUpwind_r[0]*alpha[2]); 
  Ghat_r[3] += 0.3162277660168379*alpha[5]*fUpwind_r[15]+0.3535533905932737*alpha[4]*fUpwind_r[10]+0.3162277660168379*alpha[3]*fUpwind_r[9]+0.3535533905932737*(alpha[2]*fUpwind_r[6]+alpha[1]*fUpwind_r[5]+fUpwind_r[1]*alpha[5]+alpha[0]*fUpwind_r[3]+fUpwind_r[0]*alpha[3]); 
  Ghat_r[4] += 0.2828427124746191*alpha[4]*fUpwind_r[20]+0.3162277660168379*alpha[5]*fUpwind_r[17]+0.3162277660168379*(alpha[2]*fUpwind_r[12]+alpha[1]*fUpwind_r[11])+0.3535533905932737*alpha[3]*fUpwind_r[10]+0.3162277660168379*alpha[4]*(fUpwind_r[8]+fUpwind_r[7])+0.3535533905932737*(alpha[5]*fUpwind_r[6]+alpha[0]*fUpwind_r[4]+fUpwind_r[0]*alpha[4]+alpha[1]*fUpwind_r[2]+fUpwind_r[1]*alpha[2]); 
  Ghat_r[5] += 0.2828427124746191*alpha[5]*fUpwind_r[21]+0.3162277660168379*alpha[4]*fUpwind_r[17]+0.3162277660168379*(alpha[3]*fUpwind_r[15]+alpha[1]*fUpwind_r[13])+0.3535533905932737*alpha[2]*fUpwind_r[10]+0.3162277660168379*alpha[5]*(fUpwind_r[9]+fUpwind_r[7])+0.3535533905932737*(alpha[4]*fUpwind_r[6]+alpha[0]*fUpwind_r[5]+fUpwind_r[0]*alpha[5]+alpha[1]*fUpwind_r[3]+fUpwind_r[1]*alpha[3]); 
  Ghat_r[6] += 0.3162277660168379*(alpha[5]*fUpwind_r[19]+alpha[4]*fUpwind_r[18])+0.3162277660168379*(alpha[3]*fUpwind_r[16]+alpha[2]*fUpwind_r[14])+0.3535533905932737*(alpha[1]*fUpwind_r[10]+alpha[0]*fUpwind_r[6]+alpha[4]*fUpwind_r[5]+fUpwind_r[4]*alpha[5]+alpha[2]*fUpwind_r[3]+fUpwind_r[2]*alpha[3]); 
  Ghat_r[7] += 0.3535533905932737*(alpha[3]*fUpwind_r[13]+alpha[2]*fUpwind_r[11]+alpha[0]*fUpwind_r[7])+0.3162277660168379*(alpha[5]*fUpwind_r[5]+alpha[4]*fUpwind_r[4]+alpha[1]*fUpwind_r[1]); 
  Ghat_r[8] += 0.3535533905932737*(alpha[5]*fUpwind_r[18]+alpha[3]*fUpwind_r[14]+alpha[1]*fUpwind_r[12]+alpha[0]*fUpwind_r[8])+0.3162277660168379*(alpha[4]*fUpwind_r[4]+alpha[2]*fUpwind_r[2]); 
  Ghat_r[9] += 0.3535533905932737*(alpha[4]*fUpwind_r[19]+alpha[2]*fUpwind_r[16]+alpha[1]*fUpwind_r[15]+alpha[0]*fUpwind_r[9])+0.3162277660168379*(alpha[5]*fUpwind_r[5]+alpha[3]*fUpwind_r[3]); 
  Ghat_r[10] += 0.2828427124746191*(alpha[5]*fUpwind_r[24]+alpha[4]*fUpwind_r[23])+0.3162277660168379*(alpha[3]*fUpwind_r[19]+alpha[2]*fUpwind_r[18]+alpha[1]*fUpwind_r[17])+0.3162277660168379*(alpha[5]*fUpwind_r[16]+alpha[4]*(fUpwind_r[14]+fUpwind_r[13])+alpha[5]*fUpwind_r[11])+0.3535533905932737*(alpha[0]*fUpwind_r[10]+alpha[1]*fUpwind_r[6]+alpha[2]*fUpwind_r[5]+fUpwind_r[2]*alpha[5]+alpha[3]*fUpwind_r[4]+fUpwind_r[3]*alpha[4]); 
  Ghat_r[11] += 0.3162277660168379*alpha[2]*fUpwind_r[20]+0.3535533905932737*alpha[3]*fUpwind_r[17]+0.2828427124746191*alpha[4]*fUpwind_r[12]+0.3535533905932737*alpha[0]*fUpwind_r[11]+0.3162277660168379*alpha[5]*fUpwind_r[10]+0.3535533905932737*alpha[2]*fUpwind_r[7]+0.3162277660168379*(alpha[1]*fUpwind_r[4]+fUpwind_r[1]*alpha[4]); 
  Ghat_r[12] += 0.3162277660168379*(alpha[5]*fUpwind_r[23]+alpha[1]*fUpwind_r[20])+0.3535533905932737*(alpha[3]*fUpwind_r[18]+alpha[5]*fUpwind_r[14]+alpha[0]*fUpwind_r[12])+0.2828427124746191*alpha[4]*fUpwind_r[11]+0.3535533905932737*alpha[1]*fUpwind_r[8]+0.3162277660168379*(alpha[2]*fUpwind_r[4]+fUpwind_r[2]*alpha[4]); 
  Ghat_r[13] += 0.3162277660168379*alpha[3]*fUpwind_r[21]+0.3535533905932737*alpha[2]*fUpwind_r[17]+0.2828427124746191*alpha[5]*fUpwind_r[15]+0.3535533905932737*alpha[0]*fUpwind_r[13]+0.3162277660168379*alpha[4]*fUpwind_r[10]+0.3535533905932737*alpha[3]*fUpwind_r[7]+0.3162277660168379*(alpha[1]*fUpwind_r[5]+fUpwind_r[1]*alpha[5]); 
  Ghat_r[14] += 0.3162277660168379*(alpha[5]*fUpwind_r[25]+alpha[3]*fUpwind_r[22])+0.3535533905932737*(alpha[1]*fUpwind_r[18]+alpha[0]*fUpwind_r[14]+alpha[5]*fUpwind_r[12])+0.3162277660168379*alpha[4]*fUpwind_r[10]+0.3535533905932737*alpha[3]*fUpwind_r[8]+0.3162277660168379*alpha[2]*fUpwind_r[6]; 
  Ghat_r[15] += 0.3162277660168379*(alpha[4]*fUpwind_r[24]+alpha[1]*fUpwind_r[21])+0.3535533905932737*(alpha[2]*fUpwind_r[19]+alpha[4]*fUpwind_r[16]+alpha[0]*fUpwind_r[15])+0.2828427124746191*alpha[5]*fUpwind_r[13]+0.3535533905932737*alpha[1]*fUpwind_r[9]+0.3162277660168379*(alpha[3]*fUpwind_r[5]+fUpwind_r[3]*alpha[5]); 
  Ghat_r[16] += 0.3162277660168379*(alpha[4]*fUpwind_r[25]+alpha[2]*fUpwind_r[22])+0.3535533905932737*(alpha[1]*fUpwind_r[19]+alpha[0]*fUpwind_r[16]+alpha[4]*fUpwind_r[15])+0.3162277660168379*alpha[5]*fUpwind_r[10]+0.3535533905932737*alpha[2]*fUpwind_r[9]+0.3162277660168379*alpha[3]*fUpwind_r[6]; 
  Ghat_r[17] += 0.3162277660168379*(alpha[3]*fUpwind_r[24]+alpha[2]*fUpwind_r[23])+0.2828427124746191*(alpha[5]*fUpwind_r[19]+alpha[4]*fUpwind_r[18])+0.3535533905932737*(alpha[0]*fUpwind_r[17]+alpha[2]*fUpwind_r[13]+alpha[3]*fUpwind_r[11])+0.3162277660168379*(alpha[1]*fUpwind_r[10]+alpha[4]*fUpwind_r[5]+fUpwind_r[4]*alpha[5]); 
  Ghat_r[18] += 0.2828427124746191*alpha[5]*fUpwind_r[26]+0.3162277660168379*(alpha[3]*fUpwind_r[25]+alpha[1]*fUpwind_r[23]+alpha[5]*(fUpwind_r[22]+fUpwind_r[20]))+0.3535533905932737*alpha[0]*fUpwind_r[18]+0.2828427124746191*alpha[4]*fUpwind_r[17]+0.3535533905932737*(alpha[1]*fUpwind_r[14]+alpha[3]*fUpwind_r[12])+0.3162277660168379*alpha[2]*fUpwind_r[10]+0.3535533905932737*alpha[5]*fUpwind_r[8]+0.3162277660168379*alpha[4]*fUpwind_r[6]; 
  Ghat_r[19] += 0.2828427124746191*alpha[4]*fUpwind_r[26]+0.3162277660168379*(alpha[2]*fUpwind_r[25]+alpha[1]*fUpwind_r[24]+alpha[4]*(fUpwind_r[22]+fUpwind_r[21]))+0.3535533905932737*alpha[0]*fUpwind_r[19]+0.2828427124746191*alpha[5]*fUpwind_r[17]+0.3535533905932737*(alpha[1]*fUpwind_r[16]+alpha[2]*fUpwind_r[15])+0.3162277660168379*alpha[3]*fUpwind_r[10]+0.3535533905932737*alpha[4]*fUpwind_r[9]+0.3162277660168379*alpha[5]*fUpwind_r[6]; 
  Ghat_r[20] += 0.3535533905932737*(alpha[3]*fUpwind_r[23]+alpha[0]*fUpwind_r[20])+0.3162277660168379*alpha[5]*fUpwind_r[18]+0.3162277660168379*(alpha[1]*fUpwind_r[12]+alpha[2]*fUpwind_r[11])+0.2828427124746191*alpha[4]*fUpwind_r[4]; 
  Ghat_r[21] += 0.3535533905932737*(alpha[2]*fUpwind_r[24]+alpha[0]*fUpwind_r[21])+0.3162277660168379*alpha[4]*fUpwind_r[19]+0.3162277660168379*(alpha[1]*fUpwind_r[15]+alpha[3]*fUpwind_r[13])+0.2828427124746191*alpha[5]*fUpwind_r[5]; 
  Ghat_r[22] += 0.3535533905932737*(alpha[1]*fUpwind_r[25]+alpha[0]*fUpwind_r[22])+0.3162277660168379*(alpha[4]*fUpwind_r[19]+alpha[5]*fUpwind_r[18])+0.3162277660168379*(alpha[2]*fUpwind_r[16]+alpha[3]*fUpwind_r[14]); 
  Ghat_r[23] += 0.3162277660168379*alpha[3]*fUpwind_r[26]+0.2828427124746191*alpha[5]*fUpwind_r[25]+0.3535533905932737*(alpha[0]*fUpwind_r[23]+alpha[3]*fUpwind_r[20])+0.3162277660168379*(alpha[1]*fUpwind_r[18]+alpha[2]*fUpwind_r[17])+0.3162277660168379*alpha[5]*fUpwind_r[12]+0.2828427124746191*alpha[4]*fUpwind_r[10]; 
  Ghat_r[24] += 0.3162277660168379*alpha[2]*fUpwind_r[26]+0.2828427124746191*alpha[4]*fUpwind_r[25]+0.3535533905932737*(alpha[0]*fUpwind_r[24]+alpha[2]*fUpwind_r[21])+0.3162277660168379*(alpha[1]*fUpwind_r[19]+alpha[3]*fUpwind_r[17])+0.3162277660168379*alpha[4]*fUpwind_r[15]+0.2828427124746191*alpha[5]*fUpwind_r[10]; 
  Ghat_r[25] += 0.3162277660168379*alpha[1]*fUpwind_r[26]+0.3535533905932737*alpha[0]*fUpwind_r[25]+0.2828427124746191*(alpha[4]*fUpwind_r[24]+alpha[5]*fUpwind_r[23])+0.3535533905932737*alpha[1]*fUpwind_r[22]+0.3162277660168379*(alpha[2]*fUpwind_r[19]+alpha[3]*fUpwind_r[18])+0.3162277660168379*(alpha[4]*fUpwind_r[16]+alpha[5]*fUpwind_r[14]); 
  Ghat_r[26] += 0.3535533905932737*alpha[0]*fUpwind_r[26]+0.3162277660168379*(alpha[1]*fUpwind_r[25]+alpha[2]*fUpwind_r[24]+alpha[3]*fUpwind_r[23])+0.2828427124746191*(alpha[4]*fUpwind_r[19]+alpha[5]*fUpwind_r[18]); 
