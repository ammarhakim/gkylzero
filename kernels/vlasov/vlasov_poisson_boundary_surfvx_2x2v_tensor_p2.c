...
GKYL_CU_DH void vlasov_poisson_boundary_surfvx_2x2v_tensor_p2(const double *w, const double *dxv, const double *fac_phi, const double *vecA, const int edge, const double *fEdge, const double *fSkin, double* GKYL_RESTRICT out) 
{ 
  ...
  if ((-0.4242640687119281*alpha[12])+0.3162277660168379*alpha[8]+0.6363961030678926*alpha[4]-0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[0] = tensor_2x2v_p2_surfvx_quad_0(1, fSkin); 
  } else { 

    fUpwindQuad[0] = tensor_2x2v_p2_surfvx_quad_0(-1, fEdge); 
  } 
  if (0.3162277660168379*alpha[8]-0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[1] = tensor_2x2v_p2_surfvx_quad_1(1, fSkin); 
  } else { 

    fUpwindQuad[1] = tensor_2x2v_p2_surfvx_quad_1(-1, fEdge); 
  } 
  if (0.4242640687119281*alpha[12]+0.3162277660168379*alpha[8]-0.6363961030678926*alpha[4]-0.4743416490252568*alpha[2]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 
    ...
    fUpwindQuad[2] = tensor_2x2v_p2_surfvx_quad_2(1, fSkin); 
  } else { 

    fUpwindQuad[2] = tensor_2x2v_p2_surfvx_quad_2(-1, fEdge); 
  } 
  if (0.5303300858899104*alpha[12]-0.3952847075210473*alpha[8]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[3] = tensor_2x2v_p2_surfvx_quad_3(1, fSkin); 
  } else { 

    fUpwindQuad[3] = tensor_2x2v_p2_surfvx_quad_3(-1, fEdge); 
  } 
  if (0.3535533905932737*alpha[0]-0.3952847075210473*alpha[8] > 0) { 

    fUpwindQuad[4] = tensor_2x2v_p2_surfvx_quad_4(1, fSkin); 
  } else { 

    fUpwindQuad[4] = tensor_2x2v_p2_surfvx_quad_4(-1, fEdge); 
  } 
  if ((-0.5303300858899104*alpha[12])-0.3952847075210473*alpha[8]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[5] = tensor_2x2v_p2_surfvx_quad_5(1, fSkin); 
  } else { 

    fUpwindQuad[5] = tensor_2x2v_p2_surfvx_quad_5(-1, fEdge); 
  } 
  if ((-0.4242640687119281*alpha[12])+0.3162277660168379*alpha[8]-0.6363961030678926*alpha[4]+0.4743416490252568*alpha[2]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[6] = tensor_2x2v_p2_surfvx_quad_6(1, fSkin); 
  } else { 

    fUpwindQuad[6] = tensor_2x2v_p2_surfvx_quad_6(-1, fEdge); 
  } 
  if (0.3162277660168379*alpha[8]+0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[7] = tensor_2x2v_p2_surfvx_quad_7(1, fSkin); 
  } else { 

    fUpwindQuad[7] = tensor_2x2v_p2_surfvx_quad_7(-1, fEdge); 
  } 
  if (0.4242640687119281*alpha[12]+0.3162277660168379*alpha[8]+0.6363961030678926*alpha[4]+0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[8] = tensor_2x2v_p2_surfvx_quad_8(1, fSkin); 
  } else { 

    fUpwindQuad[8] = tensor_2x2v_p2_surfvx_quad_8(-1, fEdge); 
  } 
  if ((-0.4242640687119281*alpha[12])+0.3162277660168379*alpha[8]+0.6363961030678926*alpha[4]-0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[9] = tensor_2x2v_p2_surfvx_quad_9(1, fSkin); 
  } else { 

    fUpwindQuad[9] = tensor_2x2v_p2_surfvx_quad_9(-1, fEdge); 
  } 
  if (0.3162277660168379*alpha[8]-0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[10] = tensor_2x2v_p2_surfvx_quad_10(1, fSkin); 
  } else { 

    fUpwindQuad[10] = tensor_2x2v_p2_surfvx_quad_10(-1, fEdge); 
  } 
  if (0.4242640687119281*alpha[12]+0.3162277660168379*alpha[8]-0.6363961030678926*alpha[4]-0.4743416490252568*alpha[2]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[11] = tensor_2x2v_p2_surfvx_quad_11(1, fSkin); 
  } else { 

    fUpwindQuad[11] = tensor_2x2v_p2_surfvx_quad_11(-1, fEdge); 
  } 
  if (0.5303300858899104*alpha[12]-0.3952847075210473*alpha[8]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[12] = tensor_2x2v_p2_surfvx_quad_12(1, fSkin); 
  } else { 

    fUpwindQuad[12] = tensor_2x2v_p2_surfvx_quad_12(-1, fEdge); 
  } 
  if (0.3535533905932737*alpha[0]-0.3952847075210473*alpha[8] > 0) { 

    fUpwindQuad[13] = tensor_2x2v_p2_surfvx_quad_13(1, fSkin); 
  } else { 

    fUpwindQuad[13] = tensor_2x2v_p2_surfvx_quad_13(-1, fEdge); 
  } 
  if ((-0.5303300858899104*alpha[12])-0.3952847075210473*alpha[8]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[14] = tensor_2x2v_p2_surfvx_quad_14(1, fSkin); 
  } else { 

    fUpwindQuad[14] = tensor_2x2v_p2_surfvx_quad_14(-1, fEdge); 
  } 
  if ((-0.4242640687119281*alpha[12])+0.3162277660168379*alpha[8]-0.6363961030678926*alpha[4]+0.4743416490252568*alpha[2]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[15] = tensor_2x2v_p2_surfvx_quad_15(1, fSkin); 
  } else { 

    fUpwindQuad[15] = tensor_2x2v_p2_surfvx_quad_15(-1, fEdge); 
  } 
  if (0.3162277660168379*alpha[8]+0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[16] = tensor_2x2v_p2_surfvx_quad_16(1, fSkin); 
  } else { 

    fUpwindQuad[16] = tensor_2x2v_p2_surfvx_quad_16(-1, fEdge); 
  } 
  if (0.4242640687119281*alpha[12]+0.3162277660168379*alpha[8]+0.6363961030678926*alpha[4]+0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[17] = tensor_2x2v_p2_surfvx_quad_17(1, fSkin); 
  } else { 

    fUpwindQuad[17] = tensor_2x2v_p2_surfvx_quad_17(-1, fEdge); 
  } 
  if ((-0.4242640687119281*alpha[12])+0.3162277660168379*alpha[8]+0.6363961030678926*alpha[4]-0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[18] = tensor_2x2v_p2_surfvx_quad_18(1, fSkin); 
  } else { 

    fUpwindQuad[18] = tensor_2x2v_p2_surfvx_quad_18(-1, fEdge); 
  } 
  if (0.3162277660168379*alpha[8]-0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[19] = tensor_2x2v_p2_surfvx_quad_19(1, fSkin); 
  } else { 

    fUpwindQuad[19] = tensor_2x2v_p2_surfvx_quad_19(-1, fEdge); 
  } 
  if (0.4242640687119281*alpha[12]+0.3162277660168379*alpha[8]-0.6363961030678926*alpha[4]-0.4743416490252568*alpha[2]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[20] = tensor_2x2v_p2_surfvx_quad_20(1, fSkin); 
  } else { 

    fUpwindQuad[20] = tensor_2x2v_p2_surfvx_quad_20(-1, fEdge); 
  } 
  if (0.5303300858899104*alpha[12]-0.3952847075210473*alpha[8]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[21] = tensor_2x2v_p2_surfvx_quad_21(1, fSkin); 
  } else { 

    fUpwindQuad[21] = tensor_2x2v_p2_surfvx_quad_21(-1, fEdge); 
  } 
  if (0.3535533905932737*alpha[0]-0.3952847075210473*alpha[8] > 0) { 

    fUpwindQuad[22] = tensor_2x2v_p2_surfvx_quad_22(1, fSkin); 
  } else { 

    fUpwindQuad[22] = tensor_2x2v_p2_surfvx_quad_22(-1, fEdge); 
  } 
  if ((-0.5303300858899104*alpha[12])-0.3952847075210473*alpha[8]+0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[23] = tensor_2x2v_p2_surfvx_quad_23(1, fSkin); 
  } else { 

    fUpwindQuad[23] = tensor_2x2v_p2_surfvx_quad_23(-1, fEdge); 
  } 
  if ((-0.4242640687119281*alpha[12])+0.3162277660168379*alpha[8]-0.6363961030678926*alpha[4]+0.4743416490252568*alpha[2]-0.4743416490252568*alpha[1]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[24] = tensor_2x2v_p2_surfvx_quad_24(1, fSkin); 
  } else { 

    fUpwindQuad[24] = tensor_2x2v_p2_surfvx_quad_24(-1, fEdge); 
  } 
  if (0.3162277660168379*alpha[8]+0.4743416490252568*alpha[2]+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[25] = tensor_2x2v_p2_surfvx_quad_25(1, fSkin); 
  } else { 

    fUpwindQuad[25] = tensor_2x2v_p2_surfvx_quad_25(-1, fEdge); 
  } 
  if (0.4242640687119281*alpha[12]+0.3162277660168379*alpha[8]+0.6363961030678926*alpha[4]+0.4743416490252568*(alpha[2]+alpha[1])+0.3535533905932737*alpha[0] > 0) { 

    fUpwindQuad[26] = tensor_2x2v_p2_surfvx_quad_26(1, fSkin); 
  } else { 

    fUpwindQuad[26] = tensor_2x2v_p2_surfvx_quad_26(-1, fEdge); 
  } 
  ...
  Ghat[0] += 0.3535533905932737*(alpha[12]*fUpwind[12]+alpha[8]*fUpwind[8]+alpha[4]*fUpwind[4]+alpha[2]*fUpwind[2]+alpha[1]*fUpwind[1]+alpha[0]*fUpwind[0]); 
  Ghat[1] += 0.3162277660168379*alpha[12]*fUpwind[20]+0.3535533905932737*(alpha[8]*fUpwind[12]+fUpwind[8]*alpha[12])+0.3162277660168379*alpha[4]*fUpwind[11]+0.3162277660168379*alpha[1]*fUpwind[7]+0.3535533905932737*(alpha[2]*fUpwind[4]+fUpwind[2]*alpha[4]+alpha[0]*fUpwind[1]+fUpwind[0]*alpha[1]); 
  Ghat[2] += 0.3162277660168379*(alpha[4]*fUpwind[12]+fUpwind[4]*alpha[12])+0.3162277660168379*(alpha[2]*fUpwind[8]+fUpwind[2]*alpha[8])+0.3535533905932737*(alpha[1]*fUpwind[4]+fUpwind[1]*alpha[4]+alpha[0]*fUpwind[2]+fUpwind[0]*alpha[2]); 
  Ghat[3] += 0.3535533905932737*(alpha[12]*fUpwind[18]+alpha[8]*fUpwind[14]+alpha[4]*fUpwind[10]+alpha[2]*fUpwind[6]+alpha[1]*fUpwind[5]+alpha[0]*fUpwind[3]); 
  Ghat[4] += 0.2828427124746191*alpha[4]*fUpwind[20]+0.3162277660168379*alpha[2]*fUpwind[12]+0.2828427124746191*fUpwind[11]*alpha[12]+0.3162277660168379*(fUpwind[2]*alpha[12]+alpha[1]*fUpwind[11])+0.3162277660168379*(alpha[4]*fUpwind[8]+fUpwind[4]*alpha[8]+alpha[4]*fUpwind[7])+0.3535533905932737*(alpha[0]*fUpwind[4]+fUpwind[0]*alpha[4]+alpha[1]*fUpwind[2]+fUpwind[1]*alpha[2]); 
  Ghat[5] += 0.3162277660168379*alpha[12]*fUpwind[23]+0.3535533905932737*alpha[8]*fUpwind[18]+0.3162277660168379*alpha[4]*fUpwind[17]+0.3535533905932737*alpha[12]*fUpwind[14]+0.3162277660168379*alpha[1]*fUpwind[13]+0.3535533905932737*(alpha[2]*fUpwind[10]+alpha[4]*fUpwind[6]+alpha[0]*fUpwind[5]+alpha[1]*fUpwind[3]); 
  Ghat[6] += 0.3162277660168379*alpha[4]*fUpwind[18]+0.3162277660168379*alpha[2]*fUpwind[14]+fUpwind[10]*(0.3162277660168379*alpha[12]+0.3535533905932737*alpha[1])+0.3162277660168379*fUpwind[6]*alpha[8]+0.3535533905932737*(alpha[0]*fUpwind[6]+alpha[4]*fUpwind[5]+alpha[2]*fUpwind[3]); 
  Ghat[7] += 0.3535533905932737*alpha[8]*fUpwind[20]+0.3162277660168379*alpha[12]*fUpwind[12]+0.3535533905932737*(alpha[2]*fUpwind[11]+alpha[0]*fUpwind[7])+0.3162277660168379*(alpha[4]*fUpwind[4]+alpha[1]*fUpwind[1]); 
  Ghat[8] += 0.2258769757263128*alpha[12]*fUpwind[12]+0.3535533905932737*(alpha[1]*fUpwind[12]+fUpwind[1]*alpha[12])+0.2258769757263128*alpha[8]*fUpwind[8]+0.3535533905932737*(alpha[0]*fUpwind[8]+fUpwind[0]*alpha[8])+0.3162277660168379*(alpha[4]*fUpwind[4]+alpha[2]*fUpwind[2]); 
  Ghat[9] += 0.3535533905932737*(alpha[12]*fUpwind[25]+alpha[8]*fUpwind[22]+alpha[4]*fUpwind[19]+alpha[2]*fUpwind[16]+alpha[1]*fUpwind[15]+alpha[0]*fUpwind[9]); 
  Ghat[10] += 0.2828427124746191*alpha[4]*fUpwind[23]+0.3162277660168379*alpha[2]*fUpwind[18]+(0.282842712474619*alpha[12]+0.3162277660168379*alpha[1])*fUpwind[17]+0.3162277660168379*(alpha[4]*(fUpwind[14]+fUpwind[13])+fUpwind[6]*alpha[12])+0.3162277660168379*alpha[8]*fUpwind[10]+0.3535533905932737*(alpha[0]*fUpwind[10]+alpha[1]*fUpwind[6]+alpha[2]*fUpwind[5]+fUpwind[3]*alpha[4]); 
  Ghat[11] += 0.3162277660168379*alpha[2]*fUpwind[20]+0.2828427124746191*(alpha[4]*fUpwind[12]+fUpwind[4]*alpha[12])+0.3162277660168379*alpha[8]*fUpwind[11]+0.3535533905932737*(alpha[0]*fUpwind[11]+alpha[2]*fUpwind[7])+0.3162277660168379*(alpha[1]*fUpwind[4]+fUpwind[1]*alpha[4]); 
  Ghat[12] += (0.2020305089104422*alpha[12]+0.3162277660168379*alpha[1])*fUpwind[20]+(0.2258769757263128*alpha[8]+0.3535533905932737*alpha[0])*fUpwind[12]+(0.2258769757263128*fUpwind[8]+0.3162277660168379*fUpwind[7]+0.3535533905932737*fUpwind[0])*alpha[12]+0.2828427124746191*alpha[4]*fUpwind[11]+0.3535533905932737*(alpha[1]*fUpwind[8]+fUpwind[1]*alpha[8])+0.3162277660168379*(alpha[2]*fUpwind[4]+fUpwind[2]*alpha[4]); 
  Ghat[13] += 0.3535533905932737*alpha[8]*fUpwind[23]+0.3162277660168379*alpha[12]*fUpwind[18]+0.3535533905932737*(alpha[2]*fUpwind[17]+alpha[0]*fUpwind[13])+0.3162277660168379*(alpha[4]*fUpwind[10]+alpha[1]*fUpwind[5]); 
  Ghat[14] += (0.2258769757263128*alpha[12]+0.3535533905932737*alpha[1])*fUpwind[18]+0.2258769757263128*alpha[8]*fUpwind[14]+0.3535533905932737*(alpha[0]*fUpwind[14]+fUpwind[5]*alpha[12])+0.3162277660168379*alpha[4]*fUpwind[10]+0.3535533905932737*fUpwind[3]*alpha[8]+0.3162277660168379*alpha[2]*fUpwind[6]; 
  Ghat[15] += 0.3162277660168379*alpha[12]*fUpwind[26]+0.3535533905932737*alpha[8]*fUpwind[25]+0.3162277660168379*alpha[4]*fUpwind[24]+0.3535533905932737*alpha[12]*fUpwind[22]+0.3162277660168379*alpha[1]*fUpwind[21]+0.3535533905932737*(alpha[2]*fUpwind[19]+alpha[4]*fUpwind[16]+alpha[0]*fUpwind[15]+alpha[1]*fUpwind[9]); 
  Ghat[16] += 0.3162277660168379*(alpha[4]*fUpwind[25]+alpha[2]*fUpwind[22])+(0.3162277660168379*alpha[12]+0.3535533905932737*alpha[1])*fUpwind[19]+0.3162277660168379*alpha[8]*fUpwind[16]+0.3535533905932737*(alpha[0]*fUpwind[16]+alpha[4]*fUpwind[15]+alpha[2]*fUpwind[9]); 
  Ghat[17] += 0.3162277660168379*alpha[2]*fUpwind[23]+0.2828427124746191*alpha[4]*fUpwind[18]+0.3162277660168379*alpha[8]*fUpwind[17]+0.3535533905932737*(alpha[0]*fUpwind[17]+alpha[2]*fUpwind[13])+0.282842712474619*fUpwind[10]*alpha[12]+0.3162277660168379*(alpha[1]*fUpwind[10]+alpha[4]*fUpwind[5]); 
  Ghat[18] += (0.2020305089104422*alpha[12]+0.3162277660168379*alpha[1])*fUpwind[23]+(0.2258769757263128*alpha[8]+0.3535533905932737*alpha[0])*fUpwind[18]+0.2828427124746191*alpha[4]*fUpwind[17]+(0.2258769757263128*alpha[12]+0.3535533905932737*alpha[1])*fUpwind[14]+alpha[12]*(0.3162277660168379*fUpwind[13]+0.3535533905932737*fUpwind[3])+0.3162277660168379*alpha[2]*fUpwind[10]+0.3535533905932737*fUpwind[5]*alpha[8]+0.3162277660168379*alpha[4]*fUpwind[6]; 
  Ghat[19] += 0.2828427124746191*alpha[4]*fUpwind[26]+0.3162277660168379*alpha[2]*fUpwind[25]+0.282842712474619*alpha[12]*fUpwind[24]+0.3162277660168379*(alpha[1]*fUpwind[24]+alpha[4]*(fUpwind[22]+fUpwind[21]))+(0.3162277660168379*alpha[8]+0.3535533905932737*alpha[0])*fUpwind[19]+0.3162277660168379*alpha[12]*fUpwind[16]+0.3535533905932737*(alpha[1]*fUpwind[16]+alpha[2]*fUpwind[15]+alpha[4]*fUpwind[9]); 
  Ghat[20] += (0.2258769757263128*alpha[8]+0.3535533905932737*alpha[0])*fUpwind[20]+0.2020305089104422*alpha[12]*fUpwind[12]+0.3162277660168379*(alpha[1]*fUpwind[12]+fUpwind[1]*alpha[12]+alpha[2]*fUpwind[11])+0.3535533905932737*fUpwind[7]*alpha[8]+0.2828427124746191*alpha[4]*fUpwind[4]; 
  Ghat[21] += 0.3535533905932737*alpha[8]*fUpwind[26]+0.3162277660168379*alpha[12]*fUpwind[25]+0.3535533905932737*(alpha[2]*fUpwind[24]+alpha[0]*fUpwind[21])+0.3162277660168379*alpha[4]*fUpwind[19]+0.3162277660168379*alpha[1]*fUpwind[15]; 
  Ghat[22] += (0.2258769757263128*alpha[12]+0.3535533905932737*alpha[1])*fUpwind[25]+(0.2258769757263128*alpha[8]+0.3535533905932737*alpha[0])*fUpwind[22]+0.3162277660168379*alpha[4]*fUpwind[19]+0.3162277660168379*alpha[2]*fUpwind[16]+0.3535533905932737*(alpha[12]*fUpwind[15]+alpha[8]*fUpwind[9]); 
  Ghat[23] += (0.2258769757263128*alpha[8]+0.3535533905932737*alpha[0])*fUpwind[23]+0.2020305089104422*alpha[12]*fUpwind[18]+0.3162277660168379*(alpha[1]*fUpwind[18]+alpha[2]*fUpwind[17])+0.3535533905932737*alpha[8]*fUpwind[13]+0.3162277660168379*fUpwind[5]*alpha[12]+0.2828427124746191*alpha[4]*fUpwind[10]; 
  Ghat[24] += 0.3162277660168379*alpha[2]*fUpwind[26]+0.2828427124746191*alpha[4]*fUpwind[25]+0.3162277660168379*alpha[8]*fUpwind[24]+0.3535533905932737*(alpha[0]*fUpwind[24]+alpha[2]*fUpwind[21])+(0.282842712474619*alpha[12]+0.3162277660168379*alpha[1])*fUpwind[19]+0.3162277660168379*alpha[4]*fUpwind[15]; 
  Ghat[25] += (0.2020305089104422*alpha[12]+0.3162277660168379*alpha[1])*fUpwind[26]+(0.2258769757263128*alpha[8]+0.3535533905932737*alpha[0])*fUpwind[25]+0.2828427124746191*alpha[4]*fUpwind[24]+(0.2258769757263128*alpha[12]+0.3535533905932737*alpha[1])*fUpwind[22]+0.3162277660168379*alpha[12]*fUpwind[21]+0.3162277660168379*alpha[2]*fUpwind[19]+0.3162277660168379*alpha[4]*fUpwind[16]+0.3535533905932737*(alpha[8]*fUpwind[15]+fUpwind[9]*alpha[12]); 
  Ghat[26] += (0.2258769757263128*alpha[8]+0.3535533905932737*alpha[0])*fUpwind[26]+0.2020305089104422*alpha[12]*fUpwind[25]+0.3162277660168379*(alpha[1]*fUpwind[25]+alpha[2]*fUpwind[24])+0.3535533905932737*alpha[8]*fUpwind[21]+0.2828427124746191*alpha[4]*fUpwind[19]+0.3162277660168379*alpha[12]*fUpwind[15]; 
