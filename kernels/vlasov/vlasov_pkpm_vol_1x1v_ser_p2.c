...
GKYL_CU_DH double vlasov_pkpm_vol_1x1v_ser_p2(const double *w, const double *dxv, const double *u_i, const double *p_ij, const double *bvar, const double *f, double* GKYL_RESTRICT out) 
{ 
  ...
  alpha_mid += fabs(0.25*alpha_cdim[0]-0.2795084971874737*alpha_cdim[4]); 

  alpha_vdim[0] = dv1par*((3.872983346207417*(bxbz[1]*uz[2]+bxby[1]*uy[2]+bxbx[1]*ux[2])+1.732050807568877*(bxbz[0]*uz[1]+bxby[0]*uy[1]+bxbx[0]*ux[1]))*wvpar+3.872983346207417*(bz[1]*Pxz[2]+by[1]*Pxy[2]+bx[1]*Pxx[2])+1.732050807568877*(bz[0]*Pxz[1]+by[0]*Pxy[1]+bx[0]*Pxx[1])); 
  alpha_vdim[1] = dv1par*(((3.464101615137754*bxbz[2]+3.872983346207417*bxbz[0])*uz[2]+(3.464101615137754*bxby[2]+3.872983346207417*bxby[0])*uy[2]+(3.464101615137754*bxbx[2]+3.872983346207417*bxbx[0])*ux[2]+1.732050807568877*(bxbz[1]*uz[1]+bxby[1]*uy[1]+bxbx[1]*ux[1]))*wvpar+3.464101615137754*(Pxz[2]*bz[2]+Pxy[2]*by[2]+Pxx[2]*bx[2])+3.872983346207417*(bz[0]*Pxz[2]+by[0]*Pxy[2]+bx[0]*Pxx[2])+1.732050807568877*(Pxz[1]*bz[1]+Pxy[1]*by[1]+Pxx[1]*bx[1])); 
  alpha_vdim[2] = (1.118033988749895*(bxbz[1]*uz[2]+bxby[1]*uy[2]+bxbx[1]*ux[2])+0.5*(bxbz[0]*uz[1]+bxby[0]*uy[1]+bxbx[0]*ux[1]))*dv1par*dvpar; 
  alpha_vdim[3] = ((bxbz[2]+1.118033988749895*bxbz[0])*uz[2]+(bxby[2]+1.118033988749895*bxby[0])*uy[2]+(bxbx[2]+1.118033988749895*bxbx[0])*ux[2]+0.5*(bxbz[1]*uz[1]+bxby[1]*uy[1]+bxbx[1]*ux[1]))*dv1par*dvpar; 
  alpha_vdim[4] = dv1par*(3.464101615137754*(bxbz[1]*uz[2]+bxby[1]*uy[2]+bxbx[1]*ux[2])*wvpar+1.732050807568877*((uz[1]*bxbz[2]+uy[1]*bxby[2]+ux[1]*bxbx[2])*wvpar+Pxz[1]*bz[2]+Pxy[1]*by[2]+Pxx[1]*bx[2])+3.464101615137754*(bz[1]*Pxz[2]+by[1]*Pxy[2]+bx[1]*Pxx[2])); 
  alpha_vdim[6] = (bxbz[1]*uz[2]+bxby[1]*uy[2]+bxbx[1]*ux[2]+0.5*(uz[1]*bxbz[2]+uy[1]*bxby[2]+ux[1]*bxbx[2]))*dv1par*dvpar; 
